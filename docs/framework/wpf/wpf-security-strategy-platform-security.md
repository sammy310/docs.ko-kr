...
#### <a name="gs-compilation"></a>/GS 컴파일  
 [!INCLUDE[TLA2#tla_winxpsp2](../../../includes/tla2sharptla-winxpsp2-md.md)] 모두를 포함 하 여 많은 코어 시스템 라이브러리를 다시 컴파일하여 보호를 제공 합니다 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 와 같은 종속성은 [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)], 버퍼 오버런 완화에 도움을 합니다. 이 작업을 위해 /GS 매개 변수와 C/C++ 명령줄 컴파일러를 함께 사용합니다. 버퍼 오버런을 명시적으로 피해야 하지만 /GS 컴파일은 실수 또는 악의적으로 만들어진 잠재적인 취약성에 대한 심층 방어의 예를 제공합니다.  
 ...
 높은 수준에서 /GS 컴파일러 플래그는 로컬 문자열 버퍼가 있는 함수의 반환 주소를 보호하기 위해 특수 보안 쿠키를 삽입하여 잠재적인 버퍼 오버런으로부터 보호합니다. 함수가 반환된 후 보안 쿠키를 이전 값과 비교합니다. 값이 변경된 경우 버퍼 오버런이 발생했을 수 있으며 프로세스가 오류 상태로 중지됩니다. 프로세스를 중지하면 잠재적인 악성 코드의 실행이 방지됩니다. 참조 [/GS (버퍼 보안 검사)](/cpp/build/reference/gs-buffer-security-check) 대 한 자세한 내용은 합니다.  
  
 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 아직 다른 방어 계층을 추가 하는 /GS 플래그로 컴파일됩니다 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 응용 프로그램입니다.  
  
#### <a name="microsoft-windows-update-enhancements"></a>Microsoft Windows 업데이트 향상 기능  
 [!INCLUDE[TLA#tla_win_update](../../../includes/tlasharptla-win-update-md.md)] 도 향상 되었습니다 [!INCLUDE[TLA2#tla_winxpsp2](../../../includes/tla2sharptla-winxpsp2-md.md)] 업데이트 다운로드 및 설치에 대 한 간편 하 게 합니다. 이러한 변경 내용은 특히 보안 업데이트와 관련해서 시스템을 최신 상태로 유지할 수 있게 하여 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 고객의 보안을 훨씬 향상시킵니다.  
  
<a name="Windows_Vista"></a>   
### <a name="windows-vista"></a>Windows Vista  
 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 사용자에 게 [!INCLUDE[TLA#tla_longhorn](../../../includes/tlasharptla-longhorn-md.md)] "최소 권한 사용자 액세스", 코드 무결성 검사를 포함 하 여 운영 체제의 추가 보안 향상 기능에서 혜택 및 권한 격리 됩니다.  
...
#### <a name="code-integrity-checks"></a>코드 무결성 검사  
 [!INCLUDE[TLA#tla_longhorn](../../../includes/tlasharptla-longhorn-md.md)] 악성 코드가 시스템 파일이 나 커널에 로드 및 실행 시간에 삽입 되지 않도록 하기 위해 하위 코드 무결성 검사를 통합 합니다. 이는 시스템 파일 보호 수준을 벗어납니다.  
  
<a name="Limited_Rights_Process_for_Browser_Hosted_Applications"></a>   
### <a name="limited-rights-process-for-browser-hosted-applications"></a>브라우저에서 호스트된 애플리케이션에 대한 제한된 권한 프로세스  
 브라우저에서 호스트된 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 응용 프로그램은 인터넷 영역 샌드박스 내에서 실행됩니다. [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 와 통합 [!INCLUDE[TLA#tla_ie](../../../includes/tlasharptla-ie-md.md)] 추가 지원으로이 보호를 확장 합니다.  
...
#### <a name="internet-explorer-6-service-pack-2-and-internet-explorer-7-for-xp"></a>Internet Explorer 6 서비스 팩 2 및 XP용 Internet Explorer 7  
 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 에 대 한 프로세스 권한을 제한 하 여 운영 체제 보안을 활용 [!INCLUDE[TLA#tla_winfxwebapp#plural](../../../includes/tlasharptla-winfxwebappsharpplural-md.md)] 추가로 보호 합니다. 브라우저에서 호스트된 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 응용 프로그램을 시작하기 전에 운영 체제는 프로세스 토큰에서 불필요한 권한을 제거하는 호스트 프로세스를 만듭니다. 제거되는 권한의 몇 가지 예로 사용자 컴퓨터 종료, 드라이버 로드 및 컴퓨터의 모든 파일에 대한 읽기 권한 등이 포함됩니다.  
...
### <a name="validation-and-verification"></a>유효성 검사 및 검증  
 어셈블리 격리와 무결성을 제공하기 위해 [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)]은 유효성 검사 프로세스를 사용합니다. [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)] 유효성 검사는 어셈블리 외부에서 가리키는 주소에 대 한 Portable Executable (PE) 파일 형식으로 유효성을 검사 하 여 어셈블리가 격리 되었는지 확인 합니다. [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)] 유효성 검사에는 어셈블리에 포함 된 메타 데이터의 무결성을 확인 합니다.  
...
-   무분별한 메모리 액세스  
 ...
 검증할 수 있는 코드의 장점은 주요 이유 이유 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] .NET Framework를 기반 합니다. 검증할 수 있는 코드를 사용하면 가능한 취약성이 악용될 가능성이 훨씬 줄어듭니다.  
...
### <a name="code-access-security"></a>코드 액세스 보안  
 ...
 실행을 시작할 때 [!INCLUDE[TLA2#tla_cas](../../../includes/tla2sharptla-cas-md.md)]에서 관리되는 응용 프로그램에 부여하는 권한 집합을 권한 집합이라고 하며 응용 프로그램에서 제공하는 증거로 결정됩니다. [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] 응용 프로그램의 경우 제공되는 증거는 응용 프로그램이 시작되는 위치 또는 영역입니다. [!INCLUDE[TLA2#tla_cas](../../../includes/tla2sharptla-cas-md.md)] 다음 영역을 식별합니다.  
