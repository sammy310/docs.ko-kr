...
다양 한 요소의 모양이 UI에서 동일한 형식의 모든 요소에 대 한 자주 동일합니다. UI를 사용 하 여 [스타일](../controls/styling-and-templating.md) 을 여러 요소 간에 모양을 다시 사용할 수 있습니다. 스타일의 재사용 가능성 XAML 만들기 및 관리를 간소화할 수 있습니다. 이 섹션에서는 이전 단계에서 정의된 요소별 특성을 스타일로 바꿉니다.
...
2. "Name" 및 "Department" 콘텐츠 바인딩 <xref:System.Windows.Controls.Label> 요소를 사용 하는 적절 한 데이터 원본 속성입니다. 데이터 바인딩에 대 한 자세한 내용은 참조 하세요. [데이터 바인딩 개요](../data/data-binding-overview.md)합니다.
