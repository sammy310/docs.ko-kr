...
<a name="introduction"></a>이 개요는 WPF의 탐색 토폴로지에 대한 소개를 제공합니다. 세 가지 일반적인 탐색 토폴로지를 샘플과 함께 차례로 설명합니다.  
  
> [!NOTE]
> 이 항목을 읽기 전에 페이지 함수를 사용하여 WPF에서 구조화 된 탐색의 개념에 대해 잘 알고 있어야합니다. 이 두 항목에 대한 자세한 내용은 [구조화 된 탐색 개요를](structured-navigation-overview.md)참조하십시오.  
...
## <a name="navigation-topologies"></a>탐색 토폴로지  
 WPF에서 탐색은 일반적으로 클릭할 때 다른<xref:System.Windows.Documents.Hyperlink>페이지로 이동하는 하이퍼링크() 페이지()<xref:System.Windows.Controls.Page>로 구성됩니다. 탐색되는 페이지는 균일한 리소스 식별자(URI)로 [식별됩니다(WPF의 Pack URI](pack-uris-in-wpf.md)참조). 페이지, 하이퍼링크 및 균일한 리소스 식별자(URI)를 보여 주는 다음 간단한 예제를 고려하십시오.  
  
 [!code-xaml[NavigationTopologiesOverviewSnippets#Page1](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationTopologiesOverviewSnippets/CS/Page1.xaml#page1)]  
  
 [!code-xaml[NavigationTopologiesOverviewSnippets#Page2](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationTopologiesOverviewSnippets/CS/Page2.xaml#page2)]  
  
 이러한 페이지는 페이지 사이를 탐색하는 방법에 따라 구조가 결정되는 *탐색 토폴로지에* 정렬됩니다. 이 특정 탐색 토폴로지는 간단한 시나리오에 적합하지만 탐색은 더 복잡한 토폴로지를 필요로 하며 그중 일부는 애플리케이션이 실행 중일 때만 정의할 수 있습니다.  
  
 이 항목에서는 *고정 선형,* *고정 계층및* *동적으로 생성된*세 가지 일반적인 탐색 토폴로지에 대해 다룹니다. 각 탐색 토폴로지는 다음 그림에 [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] 표시된 것과 같은 샘플을 사용하여 시연됩니다.  
...
## <a name="navigation-over-a-fixed-linear-topology"></a>고정 선형 토폴로지 탐색  
 고정 선형 토폴로지는 고정된 시퀀스로 탐색되는 하나 이상의 마법사 페이지가 있는 마법사의 구조와 유사합니다. 다음 그림은 고정선형 토폴로지가 있는 마법사의 상위 수준 구조 및 흐름을 보여 주며 있습니다.  
...
- 호출 페이지에서 마법사를 초기화하고 첫 번째 마법사 페이지로 이동하는 시작 프로그램 페이지로 이동합니다. 호출 페이지는 첫 [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]번째 <xref:System.Windows.Navigation.PageFunction%601>마법사 페이지를 직접 호출할 수 있기 때문에 런처 페이지(-less)는 필요하지 않습니다. 그러나 시작 페이지를 사용하면 특히 초기화가 복잡한 경우 마법사 초기화를 단순화할 수 있습니다.  
...
## <a name="dynamic-navigation-over-a-fixed-hierarchical-topology"></a>고정 계층적 토폴로지 동적 탐색  
 일부 응용 프로그램에서는 다음 그림과 같이 페이지를 두 개 이상의 다른 페이지로 탐색할 수 있습니다.
  
 ![여러 페이지로 이동할 수 있는 페이지를 보여 주는 다이어그램입니다.](./media/navigation-topologies-overview/navigation-topology-multiple-pages.png)  
  
 이 구조는 고정 계층적 토폴로지로 알려져 있으며 계층 구조가 통과되는 시퀀스는 종종 애플리케이션이나 사용자에 의해 런타임 시 결정됩니다. 런타임 시 두 개 이상의 다른 페이지를 탐색할 수 있는 계층 구조의 각 페이지는 탐색할 페이지를 결정하는 데 필요한 데이터를 수집합니다. 다음 그림은 이전 그림을 기반으로 가능한 몇 가지 탐색 시퀀스 중 하나를 보여 줍니다.  
...
## <a name="navigation-over-a-dynamically-generated-topology"></a>동적으로 생성된 토폴로지 탐색  
 일부 애플리케이션에서 두 개 이상의 페이지를 탐색하는 시퀀스는 런타임 시 사용자, 애플리케이션 또는 외부 데이터에 의해서만 결정될 수 있습니다. 다음 그림은 결정되지 않은 탐색 시퀀스가 있는 페이지 집합을 보여 줍니다.  
