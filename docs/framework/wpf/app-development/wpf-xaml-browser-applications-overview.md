...
[!INCLUDE[TLA#tla_xbap#plural](../../../../includes/tlasharptla-xbapsharpplural-md.md)] 웹 응용 프로그램과 리치 클라이언트 응용 프로그램의 기능을 결합 합니다. XBAP는 웹 애플리케이션처럼 웹 서버에 배포할 수 있으며 Internet Explorer 또는 Firefox에서 시작할 수 있습니다. Xbap는 리치 클라이언트 응용 프로그램과 마찬가지로 WPF의 기능 활용을 걸릴 수 있습니다. XBAP를 개발하는 것은 리치 클라이언트 개발과도 비슷합니다. 이 항목에서는 XBAP 개발에 대한 간단하고 고급 수준의 소개를 제공하며 XBAP 개발이 표준 리치 클라이언트 개발과 다른 점을 설명합니다.  
...
### <a name="clearing-cached-xbaps"></a>캐시된 XBAP 지우기  
 ...
 이러한 상황에서 사용 하 여 캐시 된 버전을 제거할 수 있습니다 합니다 **Mage** 명령 프롬프트에서 명령 (Visual Studio 또는 Windows SDK와 함께 설치 됨). 다음 명령은 애플리케이션 캐시를 지웁니다.  
...
> [!CAUTION]
>  Internet Explorer에서 액티브 콘텐츠가 실행되도록 하면 컴퓨터가 위험에 처할 수 있습니다. Internet Explorer 보안 설정을 변경 하지 않을 경우에 서버에서 HTML 페이지를 시작 하 고 Visual Studio 디버거를 프로세스에 연결할 수 있습니다.  
...
- 입력은 WPF에서 별도 스레드에서 <xref:System.Windows.Controls.WebBrowser>을 키보드 입력을 가로챌 수 없으며 IME 상태가 공유 되지 않습니다.  
...
- <xref:System.Windows.Interop.HwndHost.MessageHook> 때문에 발생 하지 않습니다 <xref:System.Windows.Interop.HwndHost> 서브 클래스를 다른 스레드 또는 프로세스에서 실행 되는 창을 수 없습니다.  
