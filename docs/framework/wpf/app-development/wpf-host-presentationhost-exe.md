...
|-event \<eventname>|이 이름을 사용 하 여 이벤트를 열고 Presentationhost.exe이 초기화 되 고 WPF 콘텐츠를 호스팅할 준비가 되 면 신호를 보낼 수 있습니다. 이벤트가 아직 만들어지지 않은 등의 이유로 인해 이벤트를 여는 데 오류가 발생하면 PresentationHost.exe가 종료됩니다.|  
