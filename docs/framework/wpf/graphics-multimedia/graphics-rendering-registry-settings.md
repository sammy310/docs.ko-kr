...
## <a name="what-are-xpdm-and-wddm"></a>XPDM 및 WDDM이란?  
 일부 그래픽 렌더링 레지스트리 설정은 비디오 카드가 XPDM 드라이버를 사용하는지 또는 WDDM 드라이버를 사용하는지에 따라 기본값이 다릅니다. XPDM은 마이크로 소프트 윈도우 XP 디스플레이 드라이버 모델이며 WDDM은 윈도우 디스플레이 드라이버 모델입니다. WDDM은 윈도우 비스타와 윈도우 7을 실행하는 컴퓨터에서 사용할 수 있습니다. XPDM은 윈도우 비스타, 마이크로 소프트 윈도우 XP, 마이크로 소프트 윈도우 서버 2003을 실행하는 컴퓨터에서 사용할 수 있습니다. WDDM에 대한 자세한 내용은 [Windows 디스플레이 드라이버 모델(WDDM) 디자인 가이드를](/windows-hardware/drivers/display/windows-vista-display-driver-model-design-guide)참조하십시오.  
...
|**참조 래스터라이저 옵션 사용**|[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]에서 참조 래스터라이저를 사용해야 하는지 지정합니다.|  
 ...
 이러한 설정은 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 레지스트리 설정을 참조하는 방법을 아는 외부 구성 유틸리티에서 액세스할 수 있습니다. 이러한 설정은 Windows 레지스트리 편집기에서 직접 값에 액세스하여 만들거나 수정할 수도 있습니다.  
 ...
 **최대 다중 샘플 값을** 사용하면 3D 콘텐츠의 최대 항앨리어싱 량을 조정할 수 있습니다. 이 수준을 사용하여 Windows Vista에서 3D 안티앨리어링을 사용하지 않도록 설정합니다.  
  
 **최대 다중 샘플 값**은 0~16 사이의 DWORD 값입니다. 값이 0이면 3D 콘텐츠의 다중 샘플 안티앨리어싱을 사용하지 않도록 설정하고 16의 값은 비디오 카드에서 지원하는 경우 최대 16배의 다중 샘플 안티앨리어싱을 사용하려고 시도합니다. XPDM 드라이버를 사용하는 컴퓨터에서 이 레지스트리 키 값을 설정하면 응용 프로그램이 많은 양의 추가 비디오 메모리를 사용하고 3D 렌더링 의 성능을 저하시키며 렌더링 오류 및 안정성이 발생할 수 있습니다. 문제.  
...
|`HKEY_CURRENT_USER\SOFTWARE\Microsoft\Avalon.Graphics\RequiredVideoDriverDate`|String|  
 ...
 2004년 11월, Microsoft는 새로운 버전의 드라이버 테스트 지침을 발표했습니다. 이 날짜 이후에 작성된 드라이버는 더 나은 안정성을 제공합니다. 기본적으로 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]는 이러한 드라이버에 대해 하드웨어 가속 파이프라인을 사용하며, 이 날짜 이전에 게시된 XPDM 드라이버용 소프트웨어 렌더링으로 대체됩니다.  
 ...
 **사용 참조 래스터라이저 옵션을** 사용하면 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 디버깅을 위해 시뮬레이션된 하드웨어 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 렌더링 모드로 강제 변환할 수 있습니다.  
