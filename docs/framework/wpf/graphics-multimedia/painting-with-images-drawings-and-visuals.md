...
## <a name="paint-an-area-with-a-visual"></a>시각적 표시로 영역 그리기  
 가장 융통성과 모든 브러시 중 강력한 합니다 <xref:System.Windows.Media.VisualBrush> 영역을 그립니다는 <xref:System.Windows.Media.Visual>합니다. <xref:System.Windows.Media.Visual> 많은 유용한 그래픽 구성 요소의 상위 요소로 사용 되는 하위 수준 그래픽 형식입니다. 예를 들어 합니다 <xref:System.Windows.Window>, <xref:System.Windows.FrameworkElement>, 및 <xref:System.Windows.Controls.Control> 클래스는 모든 유형의 <xref:System.Windows.Media.Visual> 개체입니다. 사용 하는 <xref:System.Windows.Media.VisualBrush>, 거의 모든 영역을 그릴 수 있습니다 [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] 그래픽 개체입니다.  
  
> [!NOTE]
>  하지만 <xref:System.Windows.Media.VisualBrush> 의 형식인 <xref:System.Windows.Freezable> 개체를 고정할 수 없습니다 (읽기 전용으로 만들) 때 해당 <xref:System.Windows.Media.VisualBrush.Visual%2A> 속성 이외의 다른 값으로 설정 됩니다 `null`합니다.  
...
## <a name="tilebrush-features"></a>TileBrush 기능  
 <xref:System.Windows.Media.ImageBrush>를 <xref:System.Windows.Media.DrawingBrush>, 및 <xref:System.Windows.Media.VisualBrush> 종류가 <xref:System.Windows.Media.TileBrush> 개체입니다. <xref:System.Windows.Media.TileBrush> 개체는 다양 한 이미지, 그림 또는 시각적 개체를 사용 하 여 영역을 그리는 방법에 대 한 제어를 사용 하 여 제공 합니다. 예를 들어 늘어난 이미지만으로 영역을 그리기보다, 패턴을 만드는 일련의 이미지 타일을 사용하여 영역을 그릴 수 있습니다.  
