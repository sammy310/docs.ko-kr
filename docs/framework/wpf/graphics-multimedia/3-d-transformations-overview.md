...
## <a name="transforming-models"></a>모델 변환  
 ...
 각 모델 개체에는 모델을 이동 하거나, 방향을 변경 하거나, 크기를 조정할 수 있는 <xref:System.Windows.Media.Media3D.Model3D.Transform%2A> 속성이 있습니다. 변환을 적용할 때는 변환에 의해 지정된 벡터 또는 값이 무엇이든 실제적으로 모델의 모든 점을 오프셋합니다. 즉, 모델이 정의된 좌표 공간("모델 공간")은 변환하지만 장면 전체("월드 공간")의 좌표계에서 모델의 기하 도형을 구성하는 값은 변경하지 않습니다.  
  
## <a name="translation-transformations"></a>좌표 이동 변환  
 3 차원 변환은 추상 기본 클래스 <xref:System.Windows.Media.Media3D.Transform3D>에서 상속 됩니다. 여기에는 <xref:System.Windows.Media.Media3D.TranslateTransform3D>, <xref:System.Windows.Media.Media3D.ScaleTransform3D>및 <xref:System.Windows.Media.Media3D.RotateTransform3D>상관 변환 클래스가 포함 됩니다. 또한 [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] 3-d 시스템은 보다 간결한 행렬 작업에서 동일한 변환을 지정할 수 있는 <xref:System.Windows.Media.Media3D.MatrixTransform3D> 클래스를 제공 합니다.  
  
 <xref:System.Windows.Media.Media3D.TranslateTransform3D> <xref:System.Windows.Media.Media3D.TranslateTransform3D.OffsetX%2A>, <xref:System.Windows.Media.Media3D.TranslateTransform3D.OffsetY%2A>및 <xref:System.Windows.Media.Media3D.TranslateTransform3D.OffsetZ%2A> 속성을 사용 하 여 Model3D의 모든 요소를 지정 하는 오프셋 벡터의 방향으로 이동 합니다. 예를 들어 큐브의 꼭짓점 하나가 (2,2,2)에 있고 오프셋 벡터가 (0,1.6,1)인 경우 해당 꼭짓점 (2,2,2)가 (2,3.6,3)으로 이동합니다. 모델 공간에서 큐브의 꼭짓점은 계속 (2,2,2)이지만 이제 모델 공간의 (2,2,2)가 월드 공간의 (2,3.6,3)이 되도록 모델 공간과 월드 공간의 관계가 변경되었습니다.  
  
 ![좌표 이동 그림](./media/transforms-translate.png "변환-변환")  
오프셋을 사용한 좌표 이동  
...
## <a name="scale-transformations"></a>배율 변환  
 <xref:System.Windows.Media.Media3D.ScaleTransform3D> 중심점에 대 한 참조를 사용 하 여 지정 된 배율 벡터에 따라 모델의 배율을 변경 합니다. X, Y 및 Z축에서 모델의 배율을 같은 값으로 조정하는 균일한 배율을 지정하면 모델의 크기를 비례적으로 변경할 수 있습니다. 예를 들어 변환의 <xref:System.Windows.Media.ScaleTransform.ScaleX%2A>, <xref:System.Windows.Media.ScaleTransform.ScaleY%2A>및 <xref:System.Windows.Media.Media3D.ScaleTransform3D.ScaleZ%2A> 속성을 0.5로 설정 하면 모델 크기는 동일한 속성을 2로 설정 하면 세 축 모두에서 크기가 2 배가 됩니다.  
  
 ![Uniform ScaleTransform3D](./media/threecubes-uniformscale-1.png "threecubes_uniformscale_1")  
ScaleVector 예제  
  
 균일하지 않은 배율 변환(X, Y 및 Z 값이 모두 같지 않은 배율 변환)을 지정하면 다른 모델에 영향을 주지 않으면서 모델을 1차원 또는 2차원에서 늘리거나 줄일 수 있습니다. 예를 들어 <xref:System.Windows.Media.ScaleTransform.ScaleX%2A>를 1로 설정 하 <xref:System.Windows.Media.Media3D.ScaleTransform3D.ScaleZ%2A> 고 <xref:System.Windows.Media.ScaleTransform.ScaleY%2A>를 2로 설정 하면 변환 된 모델의 높이가 두 배가 되지만 X 및 Z 축을 따라 변경 되지 않은 상태로 유지 됩니다.  
...
ScaleCenter 예제  
 ...
 "현재 위치의" 모델 크기를 조정 하려면 ScaleTransform3D's <xref:System.Windows.Media.ScaleTransform.CenterX%2A>, <xref:System.Windows.Media.ScaleTransform.CenterY%2A>및 <xref:System.Windows.Media.Media3D.ScaleTransform3D.CenterZ%2A> 속성을 설정 하 여 모델의 중심을 지정 합니다. 이렇게 하면 그래픽 시스템에서 모델 공간의 크기를 조정 하 고 지정 된 <xref:System.Windows.Media.Media3D.Point3D>중심으로 변환 합니다. 반대로 모델을 원점을 기준으로 빌드한 경우 다른 중심점을 지정하면 모델이 원점에서 먼 쪽으로 좌표 이동합니다.  
  
## <a name="rotation-transformations"></a>회전 변환  
 3차원에서 모델을 여러 방법으로 회전할 수 있습니다. 일반적인 회전 변환에서는 축과 해당 축을 기준으로 하는 회전 각도를 지정합니다. <xref:System.Windows.Media.Media3D.RotateTransform3D> 클래스를 사용 하면 <xref:System.Windows.Media.Media3D.RotateTransform3D.Rotation%2A> 속성으로 <xref:System.Windows.Media.Media3D.Rotation3D>를 정의할 수 있습니다. 그런 다음 Rotation3D의 <xref:System.Windows.Media.Media3D.AxisAngleRotation3D.Axis%2A> 및 <xref:System.Windows.Media.Media3D.AxisAngleRotation3D.Angle%2A> 속성 (이 경우 <xref:System.Windows.Media.Media3D.AxisAngleRotation3D>)을 지정 하 여 변환을 정의 합니다. 다음 예제에서는 모델을 Y축을 기준으로 60도 회전시킵니다.  
 ...
 축-각도 회전은 RotateTransform3D의 <xref:System.Windows.Media.Media3D.RotateTransform3D.CenterX%2A>, <xref:System.Windows.Media.Media3D.RotateTransform3D.CenterY%2A>및 <xref:System.Windows.Media.Media3D.RotateTransform3D.CenterZ%2A> 속성에 값이 지정 되지 않은 경우 원본에 대 한 회전을 가정 합니다. 배율 조정과 마찬가지로 회전도 모델의 전체 좌표 공간을 변환한다는 점에 유의합니다. 모델이 원점을 기준으로 만들어지지 않았거나 이전에 모델을 좌표 이동한 경우 회전시키면 현재 위치에서 회전되는 것이 아니라 원점을 기준으로 "피벗"할 수 있습니다.  
...
## <a name="animating-transformations"></a>변환에 애니메이션 효과 주기  
 ...
 개체를 현재 위치에서 회전시킬 뿐 아니라 진동 동작을 적용하여 볼 개체의 더 많은 부분을 노출하려 한다고 가정합니다. 모델에 RotateTransform3D를 적용하고 한 벡터에서 다른 벡터로 회전 축에 애니메이션 효과를 주도록 선택할 수 있습니다. 다음 코드 예제에서는 RotateTransform3D가 <xref:System.Windows.Media.TransformGroup>를 사용 하 여 모델에 적용 되는 여러 변환 중 하나인 것으로 가정 하 여 변환 Rotation3D의 Axis 속성에 <xref:System.Windows.Media.Animation.Vector3DAnimation>를 적용 하는 방법을 보여 줍니다.  
 ...
 다른 변환 속성을 대상으로 비슷한 구문을 사용하여 개체를 이동하거나 배율 조정할 수 있습니다.  예를 들어 크기 조정 변환의 ScaleCenter 속성에 <xref:System.Windows.Media.Animation.Point3DAnimation>를 적용 하 여 모델에서 해당 셰이프를 부드럽게 왜곡할 수 있습니다.  
  
 앞의 예제에서는 <xref:System.Windows.Media.Media3D.GeometryModel3D>의 속성을 변환 하지만 장면에서 다른 모델의 속성을 변환할 수도 있습니다.  예를 들어 Light 개체에 적용된 변환에 애니메이션 효과를 주면 이동하는 명암 효과를 만들어 모델의 모양을 인상 깊게 변경할 수 있습니다.  
