...
- 형식의 합니다 *StartDate* 속성이 <xref:System.DateTime>를 밀리초 단위로 시간을 포함 하는 날짜를 반환 하는 합니다. 이 애플리케이션에는 더 짧은 날짜 문자열이 표시되도록 사용자 지정 변환기가 사용되었습니다. [데이터 변환](#data_conversion) 섹션에서는 변환기를 살펴봅니다.  
 ...
 제공 하는 유효성 검사 논리는이 이미지에 표시 되지 합니다 *시작 날짜* <xref:System.Windows.Controls.TextBox>합니다. 사용자가 잘못 된 입력 날짜 (잘못 된 서식 지정 또는 과거 날짜)를 사용 하 여 알림이 <xref:System.Windows.Controls.ToolTip> 및 옆에 빨간색 느낌표가 <xref:System.Windows.Controls.TextBox>합니다. [데이터 유효성 검사](#data_validation) 섹션에서는 유효성 검사 논리를 만드는 방법을 설명합니다.  
...
- 그림에는 지정되지 않았지만 바인딩 소스 개체는 사용자 지정 [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] 개체로 제한되지 않습니다. [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 데이터 바인딩은 [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] 개체 및 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 형식의 데이터를 지원합니다. 바인딩 소스 수를 제공 하기 위해 몇 가지 예는 <xref:System.Windows.UIElement>, 목록 개체를 [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] ADO.NET 데이터 또는 웹 서비스와 연결 된 개체 또는 포함 된 XmlNode에 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 데이터. 자세한 내용은 [바인딩 소스 개요](binding-sources-overview.md)를 참조하세요.  
 ...
 바인딩을 설정 하려면 사용 된 <xref:System.Windows.Data.Binding> 개체입니다. 이 항목의 나머지 부분에 다양 한 관련 된 개념 및 일부 속성 및 사용에 설명 합니다 <xref:System.Windows.Data.Binding> 개체입니다.  
  
<a name="direction_of_data_flow"></a>   
### <a name="direction-of-the-data-flow"></a>데이터 흐름 방향  
 앞서 언급 했 듯이 및 위의 그림에서 화살표에 표시 된 대로 바인딩의 데이터 흐름 이동할 수 바인딩 대상에서 바인딩 소스 (사용자의 값을 편집 하는 경우이 원본 값이 변경 되는 예를 들어 한 <xref:System.Windows.Controls.TextBox>) 및/또는 바인딩 소스에서 바인딩 대상 (예를 들어 프로그램 <xref:System.Windows.Controls.TextBox> 콘텐츠 바인딩 소스에서 변경 내용으로 업데이트) 바인딩 소스 적절 한 알림을 제공 하는 경우.  
...
### <a name="what-triggers-source-updates"></a>소스 업데이트를 트리거하는 항목  
 바인딩 <xref:System.Windows.Data.BindingMode.TwoWay> 또는 <xref:System.Windows.Data.BindingMode.OneWayToSource> 대상 속성의 변경 내용을 수신 대기 하 고 소스에 다시 전파 합니다. 이를 소스 업데이트라고 합니다. 예를 들어 TextBox의 텍스트를 편집하여 기본 소스 값을 변경할 수 있습니다. 데이터 흐름의 방향 값을 기준으로 결정 됩니다 마지막 섹션에서 설명한 대로는 <xref:System.Windows.Data.Binding.Mode%2A> 바인딩의 속성입니다.  
 ...
 비슷합니다는 <xref:System.Windows.Data.Binding.Mode%2A> 속성을 종속성 속성에 서로 다른 기본 <xref:System.Windows.Data.Binding.UpdateSourceTrigger%2A> 값입니다. 대부분의 종속성 속성에 대한 기본값이 <xref:System.Windows.Data.UpdateSourceTrigger.PropertyChanged>인 반면 <xref:System.Windows.Controls.TextBox.Text%2A> 속성의 기본값은 <xref:System.Windows.Data.UpdateSourceTrigger.LostFocus>입니다. 즉, 때마다 소스 업데이트가 수행에 대 한 세밀 하 게 되는 대상 속성이 변경 <xref:System.Windows.Controls.CheckBox>es 및 기타 간단한 컨트롤입니다. 하지만 텍스트 필드의 경우 키 입력이 있을 때마다 업데이트하면 성능이 저하될 수 있고 새 값으로 커밋하기 전에 사용자가 백스페이스 키를 누르고 입력 오류를 수정할 수 있는 기회가 사라집니다. 이유는 바로 합니다 <xref:System.Windows.Controls.TextBox.Text%2A> 속성이 기본값인 <xref:System.Windows.Data.UpdateSourceTrigger.LostFocus> 대신 <xref:System.Windows.Data.UpdateSourceTrigger.PropertyChanged>합니다.  
  
 참조 된 <xref:System.Windows.Data.Binding.UpdateSourceTrigger%2A> 기본값을 찾는 방법에 대 한 정보에 대 한 속성 페이지 <xref:System.Windows.Data.Binding.UpdateSourceTrigger%2A> 종속성 속성의 값입니다.  
  
 다음 표에서 각각에 대 한 예제 시나리오를 제공 <xref:System.Windows.Data.Binding.UpdateSourceTrigger%2A> 를 사용 하 여 값을 <xref:System.Windows.Controls.TextBox> 예를 들어:  
  
|UpdateSourceTrigger 값|소스 값이 업데이트될 때|TextBox의 예제 시나리오|  
|-------------------------------|----------------------------------------|----------------------------------|  
|LostFocus (에 대 한 기본 <xref:System.Windows.Controls.TextBox.Text%2A?displayProperty=nameWithType>)|TextBox 컨트롤이 포커스를 잃을 때|<xref:System.Windows.Controls.TextBox> 유효성 검사 논리를 사용 하 여 연결 된 (데이터 유효성 검사 섹션 참조)|  
...
## <a name="creating-a-binding"></a>바인딩 만들기  
 ...
 이전 섹션에 설명 된 개념 중 일부를 만들며, 사용 하 여 바인딩을 설정 합니다 <xref:System.Windows.Data.Binding> 개체 및 각 바인딩에 일반적으로 네 가지 구성 요소: 대상, 대상 속성, 바인딩 소스 및 경로 사용 하 여 원본 값에 바인딩. 이 섹션에서는 바인딩을 설정하는 방법을 설명합니다.  
...
### <a name="specifying-the-binding-source"></a>바인딩 소스 지정  
 이전 예제에서 설정 하 여 바인딩 소스 지정은 <xref:System.Windows.FrameworkElement.DataContext%2A> 속성에는 <xref:System.Windows.Controls.DockPanel> 요소. <xref:System.Windows.Controls.Button> 다음 상속 합니다 <xref:System.Windows.FrameworkElement.DataContext%2A> 에서 값을 <xref:System.Windows.Controls.DockPanel>, 부모 요소는. 다시 말하지만, 바인딩 소스 개체는 바인딩의 네 가지 필수 구성 요소 중 하나입니다. 따라서 바인딩 소스 개체를 지정하지 않으면 바인딩은 아무 작업도 하지 않습니다.  
...
### <a name="specifying-the-path-to-the-value"></a>값 경로 지정  
 사용할 바인딩 소스가 개체 이면의 <xref:System.Windows.Data.Binding.Path%2A> 바인딩에 사용할 값을 지정 하는 속성입니다. 바인딩하는 경우 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 사용 데이터는 <xref:System.Windows.Data.Binding.XPath%2A> 속성 값을 지정 합니다. 일부 경우에 사용 하 여 적용할 수 있습니다 합니다 <xref:System.Windows.Data.Binding.Path%2A> 경우에 데이터 속성 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)]합니다. 예를 들어 XPath 쿼리의) (결과로 반환된 된 XmlNode의 Name 속성에 액세스 하려는 경우 사용 해야 합니다 <xref:System.Windows.Data.Binding.Path%2A> 외에 속성을 <xref:System.Windows.Data.Binding.XPath%2A> 속성입니다.  
  
 구문 정보 및 예제에 대 한 참조를 <xref:System.Windows.Data.Binding.Path%2A> 및 <xref:System.Windows.Data.Binding.XPath%2A> 속성 페이지.  
...
### <a name="binding-and-bindingexpression"></a>Binding 및 BindingExpression  
 다른 기능 및 데이터 바인딩의 사용 방법을 살펴보기 전에 소개도 유용 하다는 <xref:System.Windows.Data.BindingExpression> 클래스입니다. 이전 섹션에서 설명한 것 처럼 합니다 <xref:System.Windows.Data.Binding> 클래스는 바인딩의; 선언에 대 한 높은 수준의 클래스는 <xref:System.Windows.Data.Binding> 클래스는 바인딩의 특징을 지정할 수 있는 여러 속성을 제공 합니다. 관련된 클래스인 <xref:System.Windows.Data.BindingExpression>는 원본과 대상 간의 연결을 유지 관리 하는 기본 개체입니다. 바인딩에는 여러 바인딩 식에서 공유될 수 있는 모든 정보가 포함됩니다. A <xref:System.Windows.Data.BindingExpression> 공유할 수 없는 인스턴스 식의 모든 인스턴스 정보를 포함 하는 <xref:System.Windows.Data.Binding>합니다.  
...
## <a name="data-conversion"></a>데이터 변환  
 이전 예제에서 단추는 빨간색 때문에 해당 <xref:System.Windows.Controls.Control.Background%2A> 속성은 값이 "Red"를 사용 하 여 문자열 속성에 바인딩됩니다. 형식 변환기가 있기 때문에 작동 합니다 <xref:System.Windows.Media.Brush> 문자열 값을 변환할 대상 형식은 <xref:System.Windows.Media.Brush>합니다.  
...
## <a name="binding-to-collections"></a>컬렉션에 바인딩  
 ...
 바인딩 소스는 속성에 데이터가 포함된 단일 개체로 처리되거나 종종 함께 그룹화되는 다형 개체의 데이터 컬렉션(예: 데이터베이스에 대한 쿼리의 결과)으로 처리될 수 있습니다. 지금까지 단일 개체에 대한 바인딩만 설명했지만 일반적인 시나리오는 데이터 컬렉션에 대한 바인딩입니다. 일반적인 시나리오를 사용 하는 예를 들어를 <xref:System.Windows.Controls.ItemsControl> 와 같은 <xref:System.Windows.Controls.ListBox>, <xref:System.Windows.Controls.ListView>, 또는 <xref:System.Windows.Controls.TreeView> 에 표시 된 응용 프로그램에서 같은 데이터 컬렉션을 표시 하는 [데이터 바인딩이란?](#what_is_data_binding) 섹션.  
...
[!code-xaml[DataBindingLab#Master2](~/samples/snippets/csharp/VS_Snippets_Wpf/DataBindingLab/CSharp/MainWindow.xaml#master2)]  
 ...
 다음 표에서 뷰 데이터 형식을 기본 컬렉션 뷰로 또는 만들어집니다 <xref:System.Windows.Data.CollectionViewSource> 원본 컬렉션 형식을 기반으로 합니다.  
...
#### <a name="master-detail-binding-scenario"></a>마스터-세부 바인딩 시나리오  
 현재 항목의 개념은 컬렉션에서 항목을 탐색하는 경우뿐 아니라 마스터-세부 바인딩 시나리오에도 유용합니다. 다시 [데이터 바인딩이란?](#what_is_data_binding) 섹션의 애플리케이션 [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]를 살펴보겠습니다. 응용 프로그램 내에서 선택 합니다 <xref:System.Windows.Controls.ListBox> 에 표시 되는 콘텐츠를 결정는 <xref:System.Windows.Controls.ContentControl>. 다른 방식으로 적용할 때를 <xref:System.Windows.Controls.ListBox> 항목이 선택 되는 <xref:System.Windows.Controls.ContentControl> 선택한 항목의 세부 정보를 표시 합니다.  
...
6. 바인딩 엔진은 모든 사용자 지정 되어 있는지 확인 <xref:System.Windows.Controls.ValidationRule> 개체는 정의 된 <xref:System.Windows.Controls.ValidationRule.ValidationStep%2A> 로 설정 된 <xref:System.Windows.Controls.ValidationStep.CommittedValue> 에 대 한 <xref:System.Windows.Data.Binding>를 호출 하는 경우에 <xref:System.Windows.Controls.ValidationRule.Validate%2A> 각 메서드 <xref:System.Windows.Controls.ValidationRule> 있는 <xref:System.Windows.Controls.ValidationRule.ValidationStep%2A> 로 설정 <xref:System.Windows.Controls.ValidationStep.CommittedValue> 모두 통과 될 때까지 또는 그 중 하나는 오류에 실행 될 때까지 합니다.  
 ...
 유효한 값을 전송 (대상에서 소스로 또는 소스에서 대상) 어느 방향으로든 지웁니다 참고도는 <xref:System.Windows.Controls.Validation.Errors%2A?displayProperty=nameWithType> 연결 된 속성입니다.  
  
 바인딩 중 하나에 있는 경우는 <xref:System.Windows.Controls.ExceptionValidationRule> 연결 또는 <xref:System.Windows.Data.Binding.ValidatesOnExceptions%2A> 속성이로 설정 되어 `true` 바인딩 엔진이 소스를 설정 하는 경우 예외가 throw 됩니다, 바인딩 엔진 확인 하는 경우 및는 <xref:System.Windows.Data.Binding.UpdateSourceExceptionFilter%2A>합니다. 사용 하는 옵션이 있습니다를 <xref:System.Windows.Data.Binding.UpdateSourceExceptionFilter%2A> 예외 처리에 대 한 사용자 지정 처리기를 제공 하는 콜백 합니다. 경우는 <xref:System.Windows.Data.Binding.UpdateSourceExceptionFilter%2A> 에 지정 되어 있지는 <xref:System.Windows.Data.Binding>, 바인딩 엔진에서 만드는 <xref:System.Windows.Controls.ValidationError> 예외를 사용 하 여에 추가 <xref:System.Windows.Controls.Validation.Errors%2A?displayProperty=nameWithType> 바인딩된 요소의 컬렉션입니다.  
