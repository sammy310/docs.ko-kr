...
이 예제에서는를 [!INCLUDE[TLA#tla_xml](../../../../includes/tlasharptla-xml-md.md)] <xref:System.Windows.Data.XmlDataProvider>사용 하 여 데이터에 바인딩하는 방법을 보여 줍니다.  
 ...
 를 사용 하는 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 경우응용프로그램에서데이터바인딩을통해액세스할수있는기본데이터는모든노드의트리가될수있습니다.<xref:System.Windows.Data.XmlDataProvider> 즉,는 <xref:System.Windows.Data.XmlDataProvider> [!INCLUDE[TLA#tla_xml](../../../../includes/tlasharptla-xml-md.md)] 노드 트리를 바인딩 소스로 사용 하는 편리한 방법을 제공 합니다.  
...
> [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 데이터의 루트 노드에는 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 네임스페이스를 빈 문자열로 설정하는 **xmlns** 특성이 있습니다. [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 페이지에 인라인인 데이터 고립 영역에 XPath 쿼리를 적용하기 위한 요구 사항입니다. 이 인라인 사례 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]에서 데이터 아일랜드는 <xref:System.Windows> 네임 스페이스를 상속 합니다. 이 때문에 XPath 쿼리를 <xref:System.Windows> 네임 스페이스로 정규화 하지 않고 쿼리를 잘못 전달 하기 위해 네임 스페이스를 비워 두어야 합니다.  
 ...
 이 예제 <xref:System.Windows.Data.Binding.XPath%2A> 에서는의 <xref:System.Windows.Controls.TextBlock> 바인딩 <xref:System.Windows.DataTemplate> 가 "*Title*"로 설정 되어 있기 때문에 책 제목이 표시 됩니다. *ISBN*과 같은 특성의 값을 표시 하려면 해당 <xref:System.Windows.Data.Binding.XPath%2A> 값을 "`@ISBN`"로 설정 합니다.  
  
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]의 **XPath** 속성은 XmlNode.SelectNodes 메서드를 통해 처리됩니다. **XPath** 쿼리를 수정하여 다른 결과를 얻을 수 있습니다. 다음은 이전 예제에서 바인딩된 <xref:System.Windows.Data.Binding.XPath%2A> <xref:System.Windows.Controls.ListBox> 의 쿼리에 대 한 몇 가지 예입니다.  
 ...
 **XPath** 쿼리를 실행 하면 <xref:System.Xml.XmlNode> 또는 XmlNodes 목록을 반환 합니다. <xref:System.Xml.XmlNode>는 clr (공용 언어 런타임) 개체입니다. 즉, <xref:System.Windows.Data.Binding.Path%2A> 속성을 사용 하 여 clr (공용 언어 런타임) 속성에 바인딩할 수 있습니다. 이전 예를 다시 살펴보겠습니다. 예제의 나머지 부분이 동일 하 게 유지 되 고 다음에 대 한 <xref:System.Windows.Controls.TextBlock> 바인딩을 변경 하는 경우 <xref:System.Windows.Controls.ListBox>에서 반환 된 XmlNodes의 이름을 볼 수 있습니다. 이 경우 반환된 모든 노드의 이름은 "*Book*"입니다.  
 ...
 데이터가 원격 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 파일에 있는 경우 다음과 같이 <xref:System.Windows.Data.XmlDataProvider.Source%2A> 특성에 적절 한를 할당 하 여 데이터에 대 한 [!INCLUDE[TLA2#tla_url](../../../../includes/tla2sharptla-url-md.md)] 액세스를 정의 합니다. [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)]  
