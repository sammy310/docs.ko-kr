...
이 항목에서는 <xref:System.Windows.Window> 컨트롤의 스타일 및 템플릿에 대해 설명 합니다. 기본 <xref:System.Windows.Controls.ControlTemplate>를 수정 하 여 컨트롤에 고유한 모양을 제공할 수 있습니다. 자세한 내용은 [ControlTemplate을 만들어 기존 컨트롤의 모양 사용자 지정](customizing-the-appearance-of-an-existing-control.md)을 참조하세요.  
  
## <a name="window-parts"></a>창 파트  
 <xref:System.Windows.Window> 컨트롤에는 명명 된 파트가 없습니다.  
...
## <a name="window-controltemplate-example"></a>Window ControlTemplate 예제  
 다음 예제에서는 <xref:System.Windows.Window> 컨트롤에 대 한 <xref:System.Windows.Controls.ControlTemplate>를 정의 하는 방법을 보여 줍니다.  
