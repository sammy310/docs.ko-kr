...
이 항목에서는 <xref:System.Windows.Navigation.NavigationWindow> 컨트롤의 스타일 및 템플릿에 대해 설명 합니다. 기본 <xref:System.Windows.Controls.ControlTemplate>를 수정 하 여 컨트롤에 고유한 모양을 제공할 수 있습니다. 자세한 내용은 [ControlTemplate을 만들어 기존 컨트롤의 모양 사용자 지정](customizing-the-appearance-of-an-existing-control.md)을 참조하세요.  
  
## <a name="navigationwindow-parts"></a>NavigationWindow 파트  
 다음 표에서는 <xref:System.Windows.Navigation.NavigationWindow> 컨트롤의 명명 된 파트를 나열 합니다.  
  
|파트|Type|설명|  
|-|-|-|  
|PART_NavWinCP|<xref:System.Windows.Controls.ContentPresenter>|콘텐츠의 영역입니다.|  
...
## <a name="navigationwindow-controltemplate-example"></a>NavigationWindow ControlTemplate 예제  
 이 예제에서는 기본적으로 <xref:System.Windows.Navigation.NavigationWindow> <xref:System.Windows.Controls.ControlTemplate>에 정의 된 모든 요소를 포함 하지만 특정 값은 예제로 간주 되어야 합니다.  
