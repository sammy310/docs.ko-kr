...
이 항목에서는 글로벌 시장을 대상으로 응용 프로그램을 작성할 때 알아야 하는 문제를 소개 [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] 합니다. 전역화 프로그래밍 요소는 네임 스페이스의 .NET에 정의 되어 있습니다 <xref:System.Globalization> .

<a name="xaml_globalization"></a>
## <a name="xaml-globalization"></a>XAML 전역화
 Extensible Application Markup Language (XAML)은 XML을 기반으로 하며 XML 사양에 정의 된 전역화 지원을 활용 합니다. 다음 섹션에서는 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 알고 있어야 하는 몇 가지 기능에 대해 설명 합니다.
...
### <a name="encoding"></a>Encoding
 에서 지원 되는 인코딩은 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] ASCII, 유니코드 utf-16 및 u t f-8입니다. Encoding 문은 문서의 시작 부분에 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 있습니다. 인코딩 특성이 없으며 바이트 순서가 없으면 기본적으로 구문 분석기가 UTF-8로 설정됩니다. UTF-8과 UTF-16이 기본 인코딩입니다. UTF-7은 지원되지 않습니다. 다음 예제에서는 파일에 UTF-8 인코딩을 지정 하는 방법을 보여 줍니다 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] .
...
### <a name="language-attribute"></a>언어 특성
 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)][xml: lang](../../../desktop-wpf/xaml-services/xml-language-handling.md) 을 사용 하 여 요소의 language 특성을 나타냅니다.  클래스를 활용 하려면 <xref:System.Globalization.CultureInfo> language 특성 값이에서 미리 정의 된 문화권 이름 중 하나 여야 <xref:System.Globalization.CultureInfo> 합니다. [xml:lang](../../../desktop-wpf/xaml-services/xml-language-handling.md)은 요소 트리에서 상속 가능하며(반드시 종속성 속성 상속성때문이 아니라 XML 규칙에 따름), 명시적으로 할당되지 않은 경우 기본값은 비어 있습니다.
...
### <a name="unicode"></a>유니코드(Unicode)
 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 서로게이트를 비롯 한 모든 유니코드 기능을 지원 합니다. 문자 집합을 유니코드에 매핑할 수 있으면 지원 됩니다. 예를 들어, GB18030에서는 중국어, 일본어 및 한국어(CFK) 확장 A와 B에 매핑되는 일부 문자와 서로게이트 쌍을 소개하므로 완전히 지원됩니다. [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]응용 프로그램에서는를 사용 <xref:System.Globalization.StringInfo> 하 여 서로게이트 쌍 또는 결합 문자가 있는지 여부를 몰라도 문자열을 조작할 수 있습니다.

<a name="design_intl_ui_with_xaml"></a>
## <a name="designing-an-international-user-interface-with-xaml"></a>XAML로 국가별 사용자 인터페이스 설계
 이 섹션에서는 [!INCLUDE[TLA#tla_ui](../../../../includes/tlasharptla-ui-md.md)] 응용 프로그램을 작성할 때 고려해 야 하는 기능에 대해 설명 합니다.

<a name="intl_text"></a>
### <a name="international-text"></a>국가별 텍스트
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 에는 지원 되는 모든 Microsoft .NET Framework 쓰기 시스템에 대 한 기본 제공 처리 기능이 포함 되어 있습니다.
...
### <a name="international-layout"></a>국가별 레이아웃
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]에서는 가로, 양방향 및 세로 레이아웃을 지원하는 매우 편리한 방법을 제공합니다. 프레젠테이션 프레임 워크에서 <xref:System.Windows.FrameworkElement.FlowDirection%2A> 속성은 레이아웃을 정의 하는 데 사용할 수 있습니다. 흐름 방향 패턴은 다음과 같습니다.
...
### <a name="building-localizable-applications"></a>지역화 가능 애플리케이션 빌드
 지역화는를 다른 문화권에 맞게 조정 하는 것을 의미 [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] 합니다. 응용 프로그램을 지역화 가능 하 게 만들려면 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 개발자가 지역화 가능한 리소스를 모두 리소스 어셈블리에 빌드해야 합니다. 리소스 어셈블리는 다른 언어로 지역화 되며 코드 숨김이 리소스 관리 API를 사용 하 여 로드 합니다. 응용 프로그램에 필요한 파일 중 하나는 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 프로젝트 파일 (proj)입니다. 애플리케이션에서 사용하는 모든 리소스는 프로젝트 파일에 포함되어야 합니다. .csprop 파일의 다음 예에서는 이 작업을 수행하는 방법을 보여줍니다.
...
```
 ...
 응용 프로그램에서 리소스를 사용 하려면를 인스턴스화하고 <xref:System.Resources.ResourceManager> 사용 하려는 리소스를 로드 합니다. 다음 예제에서는 이 작업을 수행하는 방법을 보여 줍니다.
...
## <a name="using-clickonce-with-localized-applications"></a>지역화된 애플리케이션에서 ClickOnce 사용
 ...
 이 문제는 중립 언어 대체 특성을 설정하여 해결합니다. 애플리케이션 개발자가 선택적으로 기본 어셈블리에서 리소스를 제거하고 특정 문화권에 해당하는 위성 어셈블리에서 해당 리소스를 찾을 수 있게 지정할 수 있습니다. 이 프로세스를 제어 하려면를 사용 <xref:System.Resources.NeutralResourcesLanguageAttribute> 합니다. 클래스의 생성자에 <xref:System.Resources.NeutralResourcesLanguageAttribute> 는 두 개의 시그니처가 있습니다. 하나는 <xref:System.Resources.UltimateResourceFallbackLocation> 에서 <xref:System.Resources.ResourceManager> 대체 리소스 (주 어셈블리 또는 위성 어셈블리)를 추출할 위치를 지정 하기 위해 매개 변수를 사용 합니다. 다음 예제에서는 이 특성을 사용하는 방법을 보여 줍니다. 최종 대체 (fallback) 위치에 대 한 코드는에서 <xref:System.Resources.ResourceManager> 현재 실행 중인 어셈블리의 디렉터리에 있는 "de" 하위 디렉터리에 있는 리소스를 찾도록 합니다.
