...
WPF에서 연결 된 속성 일반적인 없는 [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] 손쉬운 get/set 액세스에 대 한 "래퍼" 메서드가 있습니다. 연결된 속성은 속성이 설정된 인스턴스의 [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] 네임스페이스의 일부가 아니어도 되기 때문입니다. 그러나 XAML 프로세서는 XAML을 구문 분석할 때 이러한 값을 설정할 수 있어야 합니다. 사용 된 효과적인 연결 된 속성을 지원 하려면 연결된 된 속성의 소유자 형식 메서드를 구현 해야 전용된 접근자 형태로 **Get_PropertyName_** 하 고 **Set_PropertyName_** 합니다. 이 전용 접근자 메서드는 코드로 연결된 속성을 가져오거나 설정하는 데도 유용합니다. 코드의 관점에서 연결된 속성은 속성 접근자 대신 메서드 접근자가 있는 지원 필드와 유사하며, 지원 필드는 구체적으로 정의될 필요 없이 모든 개체에 있을 수 있습니다.
...
에 대 한 서명 합니다 **Get_PropertyName_** 접근자 이어야 합니다.
...
에 대 한 서명 합니다 **Set_PropertyName_** 접근자 이어야 합니다.
