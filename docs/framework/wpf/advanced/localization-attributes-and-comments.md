...
[!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)]지역화 주석은 XAML 소스 코드 내부의 속성으로, 개발자가 지역화에 대한 규칙과 힌트를 제공하기 위해 제공합니다. [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] 지역화 주석에는 두 가지 정보 집합(지역화 가능성 특성 및 자유 형식 지역화 주석)이 포함됩니다. 지역화 가능성 특성은 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 지역화 API가 사용하여 지역화할 리소스를 나타냅니다. 자유 형식 주석은 애플리케이션 작성자가 포함하려고 하는 정보입니다.  

<a name="Localizer_Comments_"></a>
## <a name="localization-comments"></a>지역화 주석  
 태그 응용 프로그램 작성자가 텍스트 길이, 글꼴 패밀리 또는 글꼴 크기에 대한 제약 조건과 같은 XAML의 특정 요소에 대한 요구 사항이 있는 경우 XAML 코드의 주석을 사용하여 이 정보를 지역 화자에게 전달할 수 있습니다. 주석을 소스 코드에 추가하는 절차는 다음과 같습니다.  
  
1. 응용 프로그램 개발자는 XAML 소스 코드에 지역화 주석을 추가합니다.  
...
4. 로컬로 사용할 수 있는 리소스가 BAML에서 추출되면 지역화 가능 특성은 BAML 지역화 API에서 적용됩니다.  
  
5. 자유 형식 주석만 포함하는 지역화 주석 파일은 나중에 지역화 프로세스에 통합됩니다.  
  
 다음 예제에서는 XAML 파일에 지역화 주석을 추가하는 방법을 보여 주며 있습니다.  
...
- **범주**. 지역화 담당자 도구에서 값을 수정할 수 있어야 하는지 여부를 지정합니다. <xref:System.Windows.LocalizabilityAttribute.Category%2A>을 참조하세요.  
  
- **Readability** 지역화 담당자 도구에서 값을 읽고 표시해야 하는지 여부를 지정합니다. <xref:System.Windows.LocalizabilityAttribute.Readability%2A>을 참조하세요.  
  
- **Modifiability** 지역화 담당자 도구에서 값 수정을 허용하는지 여부를 지정합니다. <xref:System.Windows.LocalizabilityAttribute.Modifiability%2A>을 참조하세요.  
