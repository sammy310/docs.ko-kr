...
- <xref:System.Windows.Media.DrawingGroup>다른 도면을 그립니다. 다른 그리기를 단일 합성 그리기로 결합하려면 그리기 그룹을 사용합니다.  
 ...
 오브젝트는 <xref:System.Windows.Media.GeometryDrawing> 형상 컨텐더를 렌더링하는 데 사용됩니다. 에서 파생되는 클래스 및 구체적인 클래스(예: <xref:System.Windows.Media.PathGeometry> <xref:System.Windows.Media.CombinedGeometry>및 <xref:System.Windows.Media.EllipseGeometry>)는 2D 그래픽을 렌더링하고 적중 테스트 및 클리핑 지원을 제공하는 수단을 제공합니다. <xref:System.Windows.Media.Geometry> 기하 도형 개체를 사용하면 컨트롤 영역을 정의하거나 이미지에 적용할 클립 영역을 정의하는 등의 작업을 수행할 수 있습니다. 기하 도형 개체는 사각형 및 원과 같은 단순 영역 또는 둘 이상의 기하 도형 개체에서 만들어진 복합 영역이 될 수 있습니다. 보다 복잡한 기하학적 영역은 <xref:System.Windows.Media.PathSegment>에서 와 같은 <xref:System.Windows.Media.ArcSegment> <xref:System.Windows.Media.BezierSegment>파생 객체를 <xref:System.Windows.Media.QuadraticBezierSegment>결합하여 작성할 수 있습니다.  
  
 표면상으로는 <xref:System.Windows.Media.Geometry> 클래스와 <xref:System.Windows.Shapes.Shape> 클래스가 비슷합니다. 둘 다 2D 그래픽 렌더링에 사용되며 둘 다 예를 들어 에서 <xref:System.Windows.Media.EllipseGeometry> <xref:System.Windows.Shapes.Ellipse>파생되는 유사한 콘크리트 클래스를 가지고 있습니다. 그러나 두 클래스 집합 사이에는 중요한 차이점이 있습니다. 하나는 <xref:System.Windows.Media.Geometry> 클래스자체를 그릴 수 있는 기능과 <xref:System.Windows.Shapes.Shape> 같은 클래스의 기능이 부족합니다. 기하 도형 개체를 그리려면 DrawingContext, Drawing 또는 Path(Path가 Shape라는 것에 주의)와 같은 또 다른 클래스를 사용하여 그리기 작업을 수행해야 합니다. 모양 오브젝트에는 이러한 속성이 포함되어 있는 반면, 형상 오브젝트를 그리는 클래스에는 채우기, 획 및 획 두께와 같은 렌더링 속성이 있습니다. 이러한 차이를 생각하는 한 가지 방법은 형상 객체가 원과 같은 영역을 정의하는 반면 셰이프 오브젝트는 영역을 정의하고 해당 영역이 채워지고 윤곽을 그리는 방법을 정의하고 레이아웃 시스템에 참여한다는 것입니다.  
...
<a name="Images"></a>
## <a name="images"></a>이미지  
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]이미징은 이전 버전의 Windows에서 이미징 기능을 크게 향상시고 있습니다. 비트맵을 표시하거나 공통 컨트롤에서 이미지를 사용하는 것과 같은 이미징 기능은 주로 Microsoft Windows 그래픽 장치 인터페이스(GDI) 또는 Microsoft Windows GDI+ 응용 프로그램 프로그래밍 인터페이스(API)에서 처리되었습니다. 이러한 API는 기준 이미징 기능을 제공하지만 코덱 확장성 및 고충실도 이미지 지원과 같은 기능이 부족했습니다. WPF 이미징 API는 GDI 및 GDI+의 단점을 극복하고 응용 프로그램 내에서 이미지를 표시하고 사용할 수 있는 새로운 API 집합을 제공하도록 재설계되었습니다.  
...
### <a name="bitmapscalingmode"></a>BitmapScalingMode  
 비트맵의 배율을 애니메이션할 때 기본 고품질 이미지 리샘플링 알고리즘은 때때로 프레임 속도 저하를 유발하기에 충분한 시스템 리소스를 소비하여 애니메이션이 더듬어질 수 있습니다. 개체의 <xref:System.Windows.Media.RenderOptions.BitmapScalingMode%2A> 속성을 <xref:System.Windows.Media.BitmapScalingMode.LowQuality>로 설정하면 비트맵의 크기를 조정할 때 더 부드러운 애니메이션을 만들 수 있습니다. <xref:System.Windows.Media.RenderOptions> <xref:System.Windows.Media.BitmapScalingMode.LowQuality>모드는 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 렌더링 엔진이 이미지를 처리할 때 품질에 최적화된 알고리즘에서 속도에 최적화된 알고리즘으로 전환하도록 지시합니다.  
...
### <a name="cachinghint"></a>CachingHint  
 ...
 개체의 <xref:System.Windows.Media.RenderOptions.CachingHint%2A> 속성을 <xref:System.Windows.Media.CachingHint.Cache>로 설정하면 타일된 브러시 개체의 캐시된 버전을 사용하여 성능을 높일 수 있습니다. <xref:System.Windows.Media.RenderOptions>  
