...
- [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]콘텐츠에 C# 대 한 실행 파일을 만들고[!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)]콘텐츠를 포함 하 C++ 는 DLL을 참조 하도록 합니다.
...
2. /Cli를 사용 하 여 C++Windows 응용 프로그램 구현 기존의 관리 되지 않는 C++ 응용 프로그램으로 시작 하는 경우에는 일반적으로`/clr`컴파일러 플래그를 포함 하도록 프로젝트 설정을 변경 하 여 관리 코드를 호출할 수 있습니다 (`/clr`컴파일을 지 원하는 데 필요할 수 있는 모든 범위). 는이 항목에서 설명 하지 않음).
...
6. [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 콘텐츠 개체에 대한 참조를 보유하는 정적 필드가 포함된 관리 클래스를 구현합니다. 이 클래스를 사용 하면 [!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)] 코드에서 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 콘텐츠 개체에 대 한 참조를 가져올 수 있지만, <xref:System.Windows.Interop.HwndSource> 실수로 가비지 수집 되지 않도록 하는 것이 더 중요 합니다.
...
2. <xref:System.Windows.Interop.HwndHost>에서 파생 하 여 [!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)] 콘텐츠를 포함 하는 개체를 만듭니다.
...
<xref:System.Windows.Interop.HwndHost>를 특수 컨트롤로 간주할 수 있습니다. 기술적으로 <xref:System.Windows.Interop.HwndHost>는 <xref:System.Windows.Controls.Control> 파생 클래스가 아니라 <xref:System.Windows.FrameworkElement> 파생 클래스 이지만 상호 운용성을 위해 컨트롤로 간주할 수 있습니다. <xref:System.Windows.Interop.HwndHost>는 호스트 된 콘텐츠의 기본 [!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)] 특성을 추상화 하 여 나머지 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 호스트 된 콘텐츠를 다른 컨트롤 형식 개체 (입력을 렌더링 하 고 처리 해야 함)로 간주 합니다. <xref:System.Windows.Interop.HwndHost>은 일반적으로 다른 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] <xref:System.Windows.FrameworkElement>처럼 동작 하지만, 기본 Hwnd 지원 될 수 있는 항목의 제한에 따라 출력 (그리기 및 그래픽) 및 입력 (마우스 및 키보드)에 대 한 몇 가지 중요 한 차이점이 있습니다.

#### <a name="notable-differences-in-output-behavior"></a>출력 동작의 주요 차이점

- <xref:System.Windows.Interop.HwndHost> 기본 클래스인 <xref:System.Windows.FrameworkElement>에는 UI에 대 한 변경을 의미 하는 몇 가지 속성이 있습니다. 여기에는 해당 요소 내의 요소 레이아웃을 부모로 변경 하는 <xref:System.Windows.FrameworkElement.FlowDirection%2A?displayProperty=nameWithType>와 같은 속성이 포함 됩니다. 그러나 이러한 속성의 대부분은 [!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)]에 대응되는 속성이 있더라도 해당 속성에 매핑되지 않습니다. 이러한 속성 및 해당 의미는 렌더링 기술에 따라 많이 달라지므로 매핑이 실용적이지 않게 됩니다. 따라서 <xref:System.Windows.Interop.HwndHost>에서 <xref:System.Windows.FrameworkElement.FlowDirection%2A>와 같은 속성을 설정 해도 아무런 효과가 없습니다.
...
- 일반적으로 입력 장치는 <xref:System.Windows.Interop.HwndHost> 호스트 된 [!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)] 영역 내에서 범위가 지정 되는 반면 입력 이벤트는 [!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)]으로 직접 이동 합니다.
...
<xref:System.Windows.Interop.IKeyboardInputSink> 및 <xref:System.Windows.Interop.IKeyboardInputSite> 인터페이스를 사용 하 여 혼합 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 및 [!INCLUDE[TLA2#tla_win32](../../../../includes/tla2sharptla-win32-md.md)] 응용 프로그램에 대 한 원활한 키보드 환경을 만들 수 있습니다.
