...
ms.locfileid: "59140857"
---
# <a name="attached-events-overview"></a>연결된 이벤트 개요
[!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] 언어 구성 요소를 호출 하는 이벤트 유형 정의 *연결 된 이벤트*합니다. 연결된 이벤트의 개념을 사용하면 실제로 정의하거나 이벤트를 상속하는 요소가 아닌 임의의 요소에 특정 이벤트의 핸들러를 추가할 수 있습니다. 이 경우 이벤트를 잠재적으로 발생시키는 개체나 인스턴스를 처리하는 대상이 정의하지 않으며 또는 그렇지 않으면 이벤트를 "소유"합니다.  
...
## <a name="how-wpf-implements-attached-events"></a>WPF가 연결된 이벤트를 구현하는 방법  
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)], 연결 된 이벤트를 지 원하는 한 <xref:System.Windows.RoutedEvent> 필드 및 되며 발생 후 트리를 통해 라우팅됩니다. 일반적으로 연결된 이벤트(이벤트를 발생하는 개체)의 소스는 시스템이나 서비스 소스이며, 이벤트를 발생하는 코드를 실행하는 개체는 직접적인 요소 트리 파트가 아닙니다.  
...
-   메서드 **추가*EventName*처리기** 두 매개 변수를 사용 합니다. 첫 번째 매개 변수는 이벤트를 식별 해야 하며 식별 된 이벤트에 있는 이름과 일치 해야 합니다 ***EventName*** 메서드 이름에서입니다. 두 번째 매개 변수는 추가할 처리기입니다. 메서드는 이어야 합니다 `public` 및 `static`, 반환 값이 없습니다.  
  
-   메서드 **제거할*EventName*처리기** 두 매개 변수를 사용 합니다. 첫 번째 매개 변수는 이벤트를 식별 해야 하며 식별 된 이벤트에 있는 이름과 일치 해야 합니다 ***EventName*** 메서드 이름에서입니다. 두 번째 매개 변수는 제거할 처리기입니다. 메서드는 이어야 합니다 `public` 및 `static`, 반환 값이 없습니다.  
  
 합니다 **추가*EventName*처리기** 접근자 메서드를 용이 하 게는 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 특성은 요소에서 선언 하는 이벤트 처리기를 연결 된 경우를 처리 합니다. **추가*EventName*처리기** 하 고 **제거*EventName*처리기** 메서드 또한 코드에 대 한 이벤트 처리기 저장소에 대 한 액세스를 사용 합니다 연결 된 이벤트입니다.  
 ...
 **추가*EventName*처리기** 에 대 한 구현을 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 호출 하는 연결 된 이벤트 구성는 <xref:System.Windows.UIElement.AddHandler%2A> 라우트된 이벤트 및 처리기를 인수로 사용 하 여 합니다.  
  
 이 구현 전략 및 라우트된 이벤트 시스템 일반적 연결 된 이벤트에 대 한 처리를 제한 하거나 <xref:System.Windows.UIElement> 파생 클래스 또는 <xref:System.Windows.ContentElement> 파생 클래스에서 클래스에만 있으므로 <xref:System.Windows.UIElement.AddHandler%2A> 구현 합니다.  
...
## <a name="raising-a-wpf-attached-event"></a>WPF 연결된 이벤트 발생  
 일반적으로 기존 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 정의된 연결된 이벤트를 코드에서 발생시킬 필요가 없습니다. 이러한 이벤트는 일반 "서비스" 개념적 모델을 따르고 서비스 클래스와 같은 <xref:System.Windows.Input.InputManager> 이벤트를 발생 시키는 담당 합니다.  
