...
## <a name="mapping-to-custom-classes-and-assemblies"></a>사용자 지정 클래스 및 어셈블리 매핑  
 ...
 참조 된 CLR 네임 스페이스의 일부 또는 모두를 포함 하는 어셈블리를 `assembly=` 합니다. 이 값은 일반적으로 경로가 아니라 어셈블리의 이름이며 .dll 또는 .exe와 같은 확장명을 포함하지 않습니다. 해당 어셈블리의 경로는 매핑하려는 XAML을 포함하는 프로젝트 파일에서 프로젝트 참조로 설정해야 합니다. 버전 관리 및 강력한 이름 서명을 통합 하기 위해 `assembly` 값은 단순 문자열 이름이 아니라 <xref:System.Reflection.AssemblyName>에 의해 정의 된 문자열일 수 있습니다.  
...
## <a name="designer-namespaces-and-other-prefixes-from-xaml-templates"></a>XAML 템플릿의 디자이너 네임스페이스 및 기타 접두사  
 ...
 Visual Studio 용 WPF 디자이너는 일반적으로 `d:`접두사에 매핑되는 디자이너 네임 스페이스를 사용 합니다. WPF 용 최신 프로젝트 템플릿은 Visual Studio 용 WPF 디자이너와 기타 디자인 환경의 XAML 교환을 지원 하기 위해이 XAML 네임 스페이스를 미리 매핑할 수 있습니다. 이 디자인 XAML 네임스페이스는 디자이너에서 XAML 기반 UI를 왕복하는 동안 디자인 상태를 계속 유지하는 데 사용됩니다. 또한 디자이너에서 런타임 데이터 소스를 사용할 수 있게 하는 `d:IsDataSource` 등의 기능에도 사용됩니다.  
...
## <a name="wpf-and-assembly-loading"></a>WPF 및 어셈블리 로딩  
 WPF에 대 한 XAML 스키마 컨텍스트는 WPF 응용 프로그램 모델과 통합 되며이는 CLR에서 정의한 <xref:System.AppDomain>개념을 사용 합니다. 다음 시퀀스에서는 XAML 스키마 컨텍스트가 <xref:System.AppDomain> 및 기타 요소의 WPF 사용에 따라 어셈블리를 로드 하거나 런타임 또는 디자인 타임에 형식을 찾는 방법을 해석 하는 방법을 설명 합니다.  
  
1. 가장 최근에 로드 된 어셈블리에서 시작 하 여 이름의 모든 측면과 일치 하는 이미 로드 된 어셈블리를 검색 하 여 <xref:System.AppDomain>을 반복 합니다.  
  
2. 이름이 정규화 된 경우 정규화 된 이름에 대 한 <xref:System.Reflection.Assembly.Load%28System.String%29?displayProperty=nameWithType>를 호출 합니다.  
...
5. 이름이 정규화 되지 않은 경우 <xref:System.Reflection.Assembly.LoadWithPartialName%2A?displayProperty=nameWithType>를 호출 합니다.  
  
 느슨한 XAML에서는 로드할 소스 어셈블리가 없으므로 3단계가 사용되지 않습니다.  
  
 WPF 용 컴파일된 XAML (XamlBuildTask를 통해 생성)은 <xref:System.AppDomain>에서 이미 로드 된 어셈블리를 사용 하지 않습니다 (1 단계). 또한 XamlBuildTask 출력에서 이름이 정규화되지 않아서는 안되므로 5단계가 적용되지 않습니다.  
