...
## <a name="remarks"></a>설명  
 는 `TemplateBinding` 템플릿 시나리오에 대 한 [바인딩의](binding-markup-extension.md) 최적화 된 형태 이며로 `Binding` 생성 된와 `{Binding RelativeSource={RelativeSource TemplatedParent}, Mode=OneWay}`유사 합니다. `TemplateBinding`은 속성이 기본적으로 양방향 바인딩과 관련되었더라도 항상 단방향 바인딩입니다. 관련된 두 속성은 모두 종속성 속성이어야 합니다. 템플릿 부모로 양방향 바인딩을 구현 하려면 다음 바인딩 문을 대신 `{Binding RelativeSource={RelativeSource TemplatedParent}, Mode=TwoWay, Path=MyDependencyProperty}`사용 합니다.
  
 [RelativeSource](relativesource-markupextension.md) 는 템플릿 내에서 상대 속성 바인딩을 수행 하기 위해 또는 대신 `TemplateBinding` 또는와 함께 사용 되는 또 다른 태그 확장입니다.  
  
 컨트롤 템플릿에 대 한 개념은 여기에서 다루지 않습니다. 자세한 내용은 [컨트롤 스타일 및 템플릿](../controls/control-styles-and-templates.md)을 참조 하세요.  
...
```  
 ...
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] XAML 프로세서 구현에서이 태그 확장에 대 한 처리는 <xref:System.Windows.TemplateBindingExtension> 클래스에 의해 정의 됩니다.  
