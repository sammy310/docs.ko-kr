...
### <a name="nested-pumping"></a>중첩 펌핑
 ...
 이 경우 <xref:System.Windows.Threading.Dispatcher.PushFrame%2A>은 <xref:System.Windows.MessageBox.Show%2A?displayProperty=nameWithType>호출 시 프로그램 컨텍스트를 유지 관리 하 고 새 메시지 루프를 시작 하 여 백그라운드 창을 다시 그리고 메시지 상자 창에 대 한 입력을 처리 합니다. 사용자가 확인을 클릭 하 여 팝업 창을 지우면 중첩 루프가 종료 되 고 <xref:System.Windows.MessageBox.Show%2A>호출 후에 제어가 다시 시작 됩니다.
