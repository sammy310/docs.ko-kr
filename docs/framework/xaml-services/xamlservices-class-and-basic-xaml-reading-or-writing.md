...
<xref:System.Xaml.XamlServices> XAML 노드 스트림에 대 한 특정 액세스 하지 않아도 되는 XAML 시나리오를 해결 하기 위해 사용할 수 있는.NET Framework XAML 서비스에서 제공 하는 클래스 또는 해당 노드에서 가져온 XAML 형식 시스템 정보입니다. <xref:System.Xaml.XamlServices> API는 다음과 같이 요약할 수 있습니다: `Load` 또는 `Parse` XAML 로드 경로 지원 하도록 `Save` 는 XAML 저장 경로 지원 하도록 및 `Transform` 로드 경로 조인 하는 기술을 제공 하 여 경로 저장 합니다. `Transform` 다른 한 XAML 스키마의 변경에 사용할 수 있습니다. 이 항목에서는 이러한 각 API 분류를 요약하고 특정 메서드 오버로드 간의 차이점을 설명합니다.  
...
## <a name="parse"></a>Parse  
 <xref:System.Xaml.XamlServices.Parse%2A> 비슷합니다 `Load` 로드 경로 XAML 노드 스트림의 XAML 입력에서 만든 API 이기 때문에 있습니다. 그러나 이 경우 XAML 입력은 로드할 모든 XAML이 포함된 문자열로 직접 제공됩니다. <xref:System.Xaml.XamlServices.Parse%2A> 프레임 워크 시나리오 보다 응용 프로그램 시나리오에 보다 적절 한 간단한 방법입니다. 자세한 내용은 <xref:System.Xaml.XamlServices.Parse%2A>을 참조하세요. <xref:System.Xaml.XamlServices.Parse%2A> 가 래핑된 <xref:System.Xaml.XamlServices.Load%28System.Xml.XmlReader%29> 호출을 <xref:System.IO.StringReader> 내부적으로 합니다.  
...
## <a name="transform"></a>변환  
 <xref:System.Xaml.XamlServices.Transform%2A> 변환 또는 로드 경로 및 저장을 연결 하 여 XAML을 변환 경로 단일 작업으로 합니다.  <xref:System.Xaml.XamlReader> 및 <xref:System.Xaml.XamlWriter>에 대해 다른 스키마 컨텍스트 또는 다른 지원 형식 시스템을 사용할 수 있으며, 이에 따라 결과 XAML이 변환되는 방식이 달라집니다. 광범위한 변환 작업에 효율적입니다.  
