...
|[\<compiler>요소인](compiler-element.md)|언어 공급자에 대한 컴파일러 구성 특성을 지정합니다.|  
...
## <a name="remarks"></a>설명  
 ...
 `<providerOption>`.NET Framework 2.0 machine.config 또는 루트 web.config 파일에 요소를 추가 하 여 버전 사양을 전역적으로 지정할 수 있습니다. Machine.config 파일에서 기본 컴파일러 버전을 3.5로 업데이트 하는 경우 `<providerOption>` 응용 프로그램 구성 파일의 요소를 사용 하 여 응용 프로그램 별로 다시 2.0로 변경할 수 있습니다.  
