...
---
# <a name="filter-element-for-add-for-listeners-for-trace"></a>\<추적 >에 \< \< 대한수신기>에대한>추가\<의 필터 > 요소
추적의 `Listeners` 컬렉션에 있는 수신기에 필터를 추가 합니다.  
...
## <a name="remarks"></a>설명  
 요소 `<filter>` 는 `<add>` [sharedlisteners >에 정의 된 수신기의 이름 뿐만 아니라 수신기의 형식을 지정 하는 추적 수신기의 요소에 \<](sharedlisteners-element.md)포함 되어야 합니다. [ \<> Sharedlisteners](sharedlisteners-element.md)에 정의 되어 있는 경우 해당 수신기의 필터를 해당 요소에 정의 해야 합니다.  
  
 이 요소는 컴퓨터 구성 파일 (machine.config) 및 응용 프로그램 구성 파일에서 사용할 수 있습니다.  
  
## <a name="example"></a>예제  
 다음 예제에서는 필터 이벤트 `<filter>` 수준을로 `Error`지정 하 여 요소를 사용 하 여 추적 `console` 에 대 `Listeners` 한 컬렉션의 수신기에 필터를 추가 하는 방법을 보여 줍니다.  
