...
별도의 공유 메모리 크기에 대해 레지스트리 키 HKEY_LOCAL_MACHINE \SYSTEM\CurrentControlSet\Services\\ *\<범주 이름 >* \PERFORMANCE의 DWORD FileMappingSize 값이 먼저 참조 된 다음 구성 파일의 전역 공유 메모리에 지정 된 값이 나옵니다. FileMappingSize 값이 없는 경우 별도의 공유 메모리 크기가 구성 파일에서 4 개의 4 (1/4) 전역 설정으로 설정 됩니다.
