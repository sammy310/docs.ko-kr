...
## <a name="remarks"></a>주의  
 **\<assert >** 요소의 두 특성은 모두 선택 사항입니다. 메시지를 쓸 파일을 지정 하지 않고 메시지 상자를 사용 하지 않도록 설정 하거나 메시지 상자를 활성화 한 상태로 메시지를 쓸 파일을 지정할 수 있습니다.  
  
## <a name="example"></a>예제  
 다음 예제에서는 Debug를 호출할 때 메시지 상자를 표시 하지 않도록 설정 하는 방법을 보여 줍니다 **. Assert** 및 `c:\log.txt`에 메시지를 씁니다.  
