...
---
# <a name="specifiedpickupdirectory-element-network-settings"></a>\<specified픽업디렉터> 요소(네트워크 설정)
SMTP(단순 메일 전송 프로토콜) 서버에 대한 로컬 디렉터리를 구성합니다.  
...
|---------------|-----------------|  
|`pickupDirectoryLocation`|응용 프로그램이 SMTP 서버에서 나중에 처리하기 위해 전자 메일을 저장하는 디렉터리입니다.|  
...
## <a name="example"></a>예제  
 다음 예제는 c:\maildrop을 메일 픽업 디렉터리로 지정합니다.  
