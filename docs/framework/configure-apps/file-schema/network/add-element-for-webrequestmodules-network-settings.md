...
|`type`|<xref:System.Type.FullName%2A> 속성으로 표시 되는 정규화 된 형식 이름 및이 웹 요청 모듈을 구현 하는 쉼표로 구분 된 어셈블리 이름 (<xref:System.Reflection.Assembly.FullName%2A> 속성으로 표시 됨)입니다.|  
...
## <a name="remarks"></a>주의  
 ...
 `prefix` 특성에 대 한 값은 유효한 URI의 선행 문자 여야 합니다. 예를 들어 `http` 또는 `http://www.contoso.com`로 이름을 지정할 수 있습니다.
