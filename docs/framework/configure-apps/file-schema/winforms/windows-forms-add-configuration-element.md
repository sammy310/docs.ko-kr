...
### <a name="key-attribute-names-and-associated-values"></a>특성 이름 및 연결 된 값 `key`
...
| "CheckedListBox" | "true"&#124;"false" | <xref:System.Windows.Forms.CheckedListBox> 컨트롤이 .NET Framework 4.7에 도입 된 크기 조정 및 레이아웃 향상 기능을 사용 하는지 여부를 나타냅니다. 크기 조정 및 레이아웃 개선을 옵트아웃 (opt in) 하려면 "true"로 설정 합니다. 그렇지 않으면 "false"입니다. |
| "DataGridView. DisableHighDpiImprovements 된 기능" | "true"&#124;"false" | <xref:System.Windows.Forms.DataGridView> 컨트롤 크기 조정 및 레이아웃 기능이 .NET Framework 4.7에 도입 되었는지 여부를 나타냅니다. DPI 인식에서 옵트아웃 (opt out) 하려면 "true"로 설정 합니다. 그렇지 않으면 "false"입니다. |
...
| "DisableSinglePassControlScaling" | "true"&#124;"false" | <xref:System.Windows.Forms.Form> 단일 패스에서 크기가 조정 되는지 여부를 나타냅니다. 단일 패스 크기 조정을 사용 하지 않도록 설정 하려면 "true"로 설정 합니다. 그렇지 않으면 false입니다. 자세한 내용은 [설명](#remarks) 의 "단일 패스 크기 조정" 섹션을 참조 하세요. |
| "MonthCalendar. DisableSinglePassControlScaling" | "true"&#124;"false" | <xref:System.Windows.Forms.MonthCalendar> 컨트롤이 단일 패스에서 크기가 조정 되는지 여부를 나타냅니다. 단일 패스 크기 조정을 사용 하지 않도록 설정 하려면 "true"로 설정 합니다. 그렇지 않으면 false입니다. 자세한 내용은 [설명](#remarks) 의 "단일 패스 크기 조정" 섹션을 참조 하세요. |
| "Toolstrip. DisableHighDpiImprovements" | "true"&#124;"false" | <xref:System.Windows.Forms.ToolStrip> 컨트롤이 .NET Framework 4.7에 도입 된 크기 조정 및 레이아웃 향상 기능을 사용 하는지 여부를 나타냅니다. DPI 인식에서 옵트아웃 (opt out) 하려면 "true"로 설정 합니다. 그렇지 않으면 "false"입니다. |
...
`<System.Windows.Forms.ApplicationConfigurationSection>` 요소를 사용 하면 각각 특정 구성 설정을 정의 하는 하나 이상의 자식 `<add>` 요소를 추가할 수 있습니다.
...
높은 DPI 인식은 옵트인 (opt in) 기능입니다. 기본적으로 `DpiAwareness`의 값은 `false`입니다. 응용 프로그램 구성 파일에서이 키의 값을 `PerMonitorV2`로 설정 하 여 DPI 인식에 대 한 Windows Forms 지원을 옵트인 (opt in) 할 수 있습니다. DPI 인식을 사용 하는 경우 모든 개별 DPI 기능도 사용할 수 있습니다. 여기에는 다음이 포함됩니다.

- `DisableDpiChangedMessageHandling` 키로 제어 되는 DPI 변경 메시지입니다.

- `EnableWindowsFormsHighDpiAutoResizing` 키로 제어 되는 동적 DPI 지원

- 단일 패스 컨트롤 크기 조정-개별 <xref:System.Windows.Forms.Form> 컨트롤에 대 한 `Form.DisableSinglePassControlScaling`, 고정 된 컨트롤의 경우 `AnchorLayout.DisableSinglePassControlScaling` 키, <xref:System.Windows.Forms.MonthCalendar> 컨트롤의 `MonthCalendar.DisableSinglePassControlScaling` 키에 의해 제어 됩니다.

- <xref:System.Windows.Forms.CheckedListBox> 컨트롤의 `CheckListBox.DisableHighDpiImprovements` 키, <xref:System.Windows.Forms.DataGridView> 컨트롤의 `DataGridView.DisableHighDpiImprovements` 키, `Toolstrip.DisableHighDpiImprovements` 컨트롤의 <xref:System.Windows.Forms.ToolStrip> 키로 제어 되는 높은 DPI 크기 조정 및 레이아웃 개선

`PerMonitorV2` `DpiAwareness` 설정 하 여 제공 하는 단일 기본 옵트인 설정은 일반적으로 새 Windows Forms 응용 프로그램에 적합 합니다. 그러나 해당 하는 키를 응용 프로그램 구성 파일에 추가 하 여 개별 높은 DPI의 향상 된 기능을 옵트아웃 (opt out) 할 수 있습니다. 예를 들어 동적 DPI 지원을 제외 하 고 모든 새 DPI 기능을 활용 하려면 응용 프로그램 구성 파일에 다음을 추가 합니다.
