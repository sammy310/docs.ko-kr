...
`principalPermissionMode` 특성은 보호된 메서드의 사용 권한을 부여할 때 사용할 사용자 그룹을 지정합니다. 기본값은 `UseWindowsGroups`이며 리소스에 액세스하려는 ID에 대해 "Administrators" 또는 "Users"와 같은 Windows 그룹을 검색하도록 지정합니다. 다음 코드와 같이 \<system.web > 요소 아래에 구성 된 사용자 지정 역할 공급자를 사용 하도록 `UseAspNetRoles`를 지정할 수도 있습니다.
...
다음 코드는 `principalPermissionMode` 특성에 사용 되는 `roleProviderName`를 보여 줍니다.
