...
|deadLetterQueue|사용할 배달 못 한 편지 큐의 형식을 지정합니다.<br /><br /> 유효한 값은 다음과 같습니다.<br /><br /> 사용자 지정 합니다. 사용자 지정 배달 못 한 편지 큐입니다.<br />-None. 배달 못 한 편지 큐는 사용할 수 있습니다.<br />-시스템: 시스템 배달 못 한 편지 큐를 사용 합니다.<br /><br /> 이 특성은 DeadLetterQueue 형식입니다.|  
...
|exactlyOnce|이 바인딩이 처리한 메시지를 정확히 한 번만 받을지 여부를 지정하는 부울 값입니다. 기본값은 `true`입니다.<br /><br /> 메시지는 보증과 함께 또는 보증 없이 보낼 수 있습니다. 보증을 사용하면 응용 프로그램에서는 보낸 메시지가 수신 메시지 큐에 도착했는지 확인할 수 있으며, 메시지가 도착하지 않았을 경우에는 배달 못 한 편지 큐를 읽어 이를 확인할 수 있습니다.<br /><br /> `exactlyOnce`을로 설정 하면 `true`, msmq는 한 번만 보낸된 메시지가 수신 메시지 큐에 배달 하 고 메시지가 배달 못 한 편지 큐로 보내집니다 배달이 실패 한 경우를 나타냅니다.<br /><br /> `exactlyOnce`를 `true`로 설정하여 보내는 메시지는 트랜잭션 큐로만 보내야 합니다.|  
...
|maxImmediateRetries|응용 프로그램 큐에서 읽은 메시지에 즉시 재시도의 최대 수를 지정 하는 정수 시도 합니다. 기본값은 5입니다.<br /><br /> 메시지에 대한 최대 즉시 재시도 횟수만큼 시도했으나 응용 프로그램에서 메시지를 배달하지 못한 경우 그 메시지는 나중에 재시도하기 위해 재시도 큐로 보내집니다. 재시도 주기를 지정하지 않은 경우 해당 메시지가 포이즌 메시지 큐로 보내지거나 발신자에게 부정 승인이 다시 보내집니다.|  
...
|rejectAfterLastRetry|최대 재시도 횟수만큼 시도한 후에 배달하지 못한 메시지에 대해 수행할 작업을 지정하는 부울 값입니다.<br /><br /> `true` 의미는 부정 승인을 보낸 사람에 게 반환 되 고 메시지가 삭제 됩니다. `false` 메시지는 포이즌 메시지 큐에 전송 되는 것을 의미 합니다. 기본값은 `false`입니다.<br /><br /> 값이 `false`이면 수신 응용 프로그램에서는 포이즌 메시지 큐를 읽어 포이즌 메시지(배달하지 못한 메시지)를 처리할 수 있습니다.<br /><br /> MSMQ 3.0에서는 부정 승인을 발신자에게 반환할 수 없으므로 이 특성이 무시됩니다.|  
...
|serializationFormat|MSMQ 메시지의 일부로 보내지는 개체를 serialize하는 데 사용되는 포맷터를 지정합니다. 유효한 값은 다음과 같습니다.<br /><br /> -ActiveX: ActiveX 포맷터는 COM 개체를 직렬화 할 때 사용 됩니다.<br />-이진:  개체를 이진 패킷으로 serialize합니다.<br />-   ByteArray:  개체를 바이트 배열로 serialize합니다.<br />-Stream:  개체를 스트림으로 serialize합니다.<br />-   Xml:  개체를 XML 패킷으로 serialize합니다. 기본값은 XML입니다.<br /><br /> 이 특성은 <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat> 형식입니다.|  
