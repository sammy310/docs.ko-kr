...
|[\<webHttpEndpoint>](webhttpendpoint.md)|[\<webHttpBinding>](webhttpbinding.md)동작을 자동으로 추가 하는 고정 바인딩을 사용 하 여 표준 끝점을 정의 [\<webHttp>](webhttp.md) 합니다. REST 서비스를 작성할 때는 이 엔드포인트를 사용합니다.|  
|[\<webScriptEndpoint>](webscriptendpoint.md)|[\<webHttpBinding>](webhttpbinding.md)동작을 자동으로 추가 하는 고정 바인딩을 사용 하 여 표준 끝점을 정의 [\<enableWebScript>](enablewebscript.md) 합니다. ASP.NET AJAX 애플리케이션에서 호출되는 서비스를 기록할 때 이 엔드포인트를 사용합니다.|  
