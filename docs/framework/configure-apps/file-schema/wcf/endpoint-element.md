...
|name|선택적 특성입니다. 서비스 끝점의 이름을 지정하는 문자열입니다. 기본값은 바인딩 이름과 계약 설명 이름을 연결한 값입니다. 서비스에 엔드포인트가 여러 개일 수 있으므로 엔드포인트의 `name` 특성은 서비스 이름과 구분됩니다.|  
...
|요소|설명|  
|-------------|-----------------|  
|[\<service>](../../../../../docs/framework/configure-apps/file-schema/wcf/service.md)|클라이언트가 연결할 수 있는 엔드포인트의 목록을 정의하는 구성 섹션입니다.|  
