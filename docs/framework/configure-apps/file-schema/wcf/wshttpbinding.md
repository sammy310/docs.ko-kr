...
## <a name="remarks"></a>설명  
 는와 `WSHttpBinding` 유사 `BasicHttpBinding` 하지만 더 많은 웹 서비스 기능을 제공 합니다. BasicHttpBinding과 마찬가지로 HTTP 전송을 사용하며 메시지 보안을 제공하지만 다른 점은 트랜잭션, 신뢰할 수 있는 메시징, WS-Addressing도 가능하다는 것입니다. 이러한 기능은 기본적으로 또는 단일 제어 설정을 통해 활성화됩니다.  
