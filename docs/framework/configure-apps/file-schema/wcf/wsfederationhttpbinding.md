...
[\<security>](security-of-wsfederationhttpbinding.md)가 기본값인로 설정 <xref:System.ServiceModel.WSFederationHttpSecurityMode.Message> 된 경우

는 `wsFederationHttpBinding` 의 메시지 보안 설정에 대 한 세부 정보를 제어 합니다 [\<message>](message-element-of-wsfederationhttpbinding.md) . 바인딩이 생성 된 [\<security>](security-of-wsfederationhttpbinding.md) 후에는 바인딩에서 사용 하는 보안을 변경할 수 없기 때문에 요소는 get 액세스만 제공 합니다.

`wsFederationHttpBinding`또한은 개인 정보 알림이 있는 URI를 설정 하 고 검색 하는 privacyNoticeAt 특성도 제공 합니다.
