...
|privacyNoticeVersion|현재 개인 정보 알림의 버전을 지정 하는 정수입니다.|
...
페더레이션은 인증 및 권한 부여를 위해 여러 시스템에서 id를 공유 하는 기능입니다. 이러한 id는 사용자 또는 컴퓨터를 나타낼 수 있습니다. 페더레이션된 HTTP는 SOAP 보안과 혼합 모드 보안을 지원 하지만 전송 보안을 사용 하 여 독점적으로 지원 하지는 않습니다. 이 바인딩은 WS-FEDERATION 프로토콜에 대 한 WCF (Windows Communication Foundation) 지원을 제공 합니다. 이 바인딩으로 구성 된 서비스는 HTTP 전송을 사용 해야 합니다.

바인딩은 바인딩 요소의 스택으로 구성 됩니다. 가 기본값인 로 설정된 경우

`wsFederationHttpBinding`에 있는 바인딩 요소의 스택은`wsHttpBinding`

[\<보안 >](security-of-wsfederationhttpbinding.md) <xref:System.ServiceModel.WSFederationHttpSecurityMode.Message>기본값인로 설정 됩니다.

`wsFederationHttpBinding` [\<메시지 >](message-element-of-wsfederationhttpbinding.md)에서 메시지 보안 설정의 세부 정보를 제어 합니다. [\<security >](security-of-wsfederationhttpbinding.md) 요소는 바인딩이 생성 된 후 바인딩에서 사용 하는 보안을 변경할 수 없기 때문에 get 액세스만 제공 합니다.
...
이 바인딩에 대 한 자세한 내용은 [방법: WSFederationHttpBinding 만들기](../../../wcf/feature-details/how-to-create-a-wsfederationhttpbinding.md)를 참조 하세요.
