...
ms.locfileid: "61768895"
---
# <a name="message-of-ws2007httpbinding"></a>\<메시지 >의 \<ws2007HttpBinding >
...
|`UserName`|서비스에서 `UserName` 자격 증명을 사용하여 클라이언트를 인증하도록 요구할 수 있습니다. WCF에서는 암호 다이제스트를 보내거나 암호를 사용 하 고 메시지 보안에 이러한 키를 사용 하 여 키를 파생 없습니다. 따라서 WCF에서는 전송에 사용 하는 경우 보안을 `UserName` 자격 증명입니다. 이 자격 증명 모드에서는 상호 운용이 가능한 교환 또는 `negotiateServiceCredential` 특성을 기반으로 하는 상호 운용이 불가능한 협상이 수행됩니다.|  
