...
|지속성|큐의 메시지가 지속적인지 또는 일시적인지를 나타내는 부울 값입니다. 지속적 메시지는 큐 관리자가 중단되더라도 남아 있지만, 일시적 메시지는 손실됩니다. 애플리케이션에서 더 낮은 대기 시간을 요구하며 어느 정도의 메시지 손실을 허용할 수 있다면 일시적 메시지가 유용합니다. `exactlyOnce` 특성을 `true`로 설정하면 메시지는 지속적이어야 합니다. 기본값은 `true`입니다.|  
...
|receiveContextEnabled|큐에서 메시지 처리를 위한 수신 컨텍스트를 사용하는지 여부를 지정하는 부울입니다. 이 값을로 설정 하면 `true` 서비스는 큐에서 메시지를 "피킹" 하 여 처리를 시작할 수 있으며 오류가 발생 하 고 예외가 throw 되 면 큐에 남아 있습니다. 서비스 "잠글" 수도 있습니다 다시 시간에서 나중에 처리 하기 위해 메시지입니다. ReceiveContext는 처리 된 메시지를 "완료" 하 여 큐에서 제거할 수 있는 메커니즘을 제공 합니다. 더 이상 네트워크를 통해 큐에 메시지를 읽고 다시 쓰지 않으며 처리 중에 개별 메시지가 다른 서비스 인스턴스 간에 바운스 되지 않습니다.|  
