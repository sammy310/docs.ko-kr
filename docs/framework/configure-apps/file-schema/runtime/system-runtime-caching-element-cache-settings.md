...
> [!NOTE]
>  출력 캐싱 기능 및 형식에는 <xref:System.Runtime.Caching> 네임 스페이스는.NET Framework 4의에서 새로운 기능입니다.  
  
## <a name="example"></a>예제

다음 예제는 <xref:System.Runtime.Caching.MemoryCache> 클래스를 기반으로 캐시를 구성하는 방법을 보여 줍니다. 또한 메모리 캐시를 위한 `namedCaches` 항목의 인스턴스를 구성하는 방법을 보여 줍니다. 캐시의 이름을 설정 하 여 기본 캐시 항목 이름으로 설정 되는 `name` 특성을 "Default"입니다.  
