...
|[\<GCHeapAffinitizeMask >](gcheapaffinitizemask-element.md)|가비지 수집 힙과 개별 프로세서 간의 선호도를 정의 합니다.|
|[\<GCHeapCount >](gcheapcount-element.md)|서버 가비지 수집에 사용할 힙/스레드 수를 지정 합니다.|
|[\<GCLOHThreshold >](gclohthreshold-element.md)|가비지 수집기가 큰 개체 힙에 개체를 배치 하 게 하는 임계값 크기를 지정 합니다.|
|[\<GCNoAffinitize >](gcnoaffinitize-element.md)|Cpu를 사용 하 여 서버 가비지 수집 스레드를 선호도 여부를 지정 합니다.|
...
구성 파일의 [\<runtime >](runtime-element.md) 섹션에 있는 자식 요소는 응용 프로그램이 실행 되는 방법을 구성 하기 위해 공용 언어 런타임에서 사용 됩니다. 예를 들어 [\<gcServer >](gcserver-element.md) 요소는 가비지 수집기가 워크스테이션 가비지 컬렉션을 사용 하는지 아니면 서버 가비지 컬렉션을 사용 하는지 결정 하 고, [\<UseRandomizedStringHashAlgorithm >](userandomizedstringhashalgorithm-element.md) 요소는 공용 언어 런타임에서 응용 프로그램별 또는 응용 프로그램당 도메인 별로 문자열의 해시 코드를 계산할지 여부를 결정 하 고, `AppContextSwitchOverrides` 요소를 사용 하 여 라이브러리 사용자가 라이브러리에서 제공 하는 변경 된 기능을 옵트인 또는 옵트아웃 (opt out)

[\<runtime >](runtime-element.md) 섹션의 요소는 응용 프로그램 시작 시 공용 언어 런타임에 의해 자동으로 읽힙니다. <xref:System.AppDomainSetup.ConfigurationFile%2A?displayProperty=nameWithType> 속성에 이름을 제공 하 여 기본이 아닌 응용 프로그램 도메인에 대 한 구성 파일을 정의할 수도 있습니다. 응용 프로그램 도메인을 로드 하면 해당 설정이 자동으로 읽힙니다. 응용 프로그램 구성 파일의 [\<런타임 >](runtime-element.md) 섹션에서 설정을 직접 읽어야 하는 경우는 거의 없습니다.
