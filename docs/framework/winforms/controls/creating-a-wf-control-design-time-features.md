...
이 연습에서는 사용자 지정 컨트롤에 대 한 사용자 지정 디자이너를 만드는 방법을 보여 줍니다. `MarqueeControl` 이라는`MarqueeControlRootDesigner`형식과 연결 된 디자이너 클래스를 구현 합니다.

이 `MarqueeControl` 형식은 애니메이션 광원 및 깜박이는 텍스트를 사용 하 여 극장 움직이는 텍스트와 비슷한 표시를 구현 합니다.
...
- 속성을 숨기고 필터링 하기 위한 사용자 지정 디자이너 만들기
...
작업이 완료 되 면 사용자 지정 컨트롤은 다음과 같이 표시 됩니다.
...
다음 단계는 컨트롤 라이브러리 프로젝트를 만드는 것입니다. 새 사용자 지정 컨트롤과 해당 하는 사용자 지정 디자이너를 만듭니다.
...
2. **솔루션 탐색기**를 사용 하 여 선택한 언어에 따라 이름이 "UserControl1.cs" 또는 "UserControl1" 인 원본 파일을 삭제 하 여 프로젝트의 기본 컨트롤을 삭제 합니다. 자세한 내용은 [방법: 항목](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2010/0ebzhwsk(v=vs.100))을 제거, 삭제 및 제외 합니다.

3. 프로젝트에 새 <xref:System.Windows.Forms.UserControl> 항목을 추가 합니다. `MarqueeControlLibrary` 새 소스 파일에 "MarqueeControl"의 기본 이름을 지정 합니다.

4. **솔루션 탐색기**를 사용 하 여 `MarqueeControlLibrary` 프로젝트에 새 폴더를 만듭니다. 자세한 내용은 [방법: 새 프로젝트 항목](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2010/w0572c5b(v=vs.100))을 추가 합니다. 새 폴더의 이름을 "Design"으로 합니다.

5. **디자인** 폴더를 마우스 오른쪽 단추로 클릭 하 고 새 클래스를 추가 합니다. 원본 파일에 "MarqueeControlRootDesigner"의 기본 이름을 지정 합니다.

6. 이 참조를 `MarqueeControlLibrary` 프로젝트에 추가 하려면 system.web 어셈블리의 형식을 사용 해야 합니다.

    > [!NOTE]
    > System.web 어셈블리를 사용 하려면 프로젝트가 .NET Framework 클라이언트 프로필이 아닌 .NET Framework의 전체 버전을 대상으로 해야 합니다. 대상 프레임 워크 [를 변경 하려면 방법: 한 버전의 .NET Framework를 대상으로 지정](/visualstudio/ide/how-to-target-a-version-of-the-dotnet-framework).
...
## <a name="defining-a-custom-control-and-its-custom-designer"></a>사용자 지정 컨트롤 및 사용자 지정 디자이너 정의
...
4. 클래스에서 상속 하도록 `MarqueeControlRootDesigner` 의 선언을 변경 합니다. <xref:System.Windows.Forms.Design.DocumentDesigner> **도구 상자**를 사용 하여디자이너상호작용을지정하려면를적용합니다.<xref:System.ComponentModel.ToolboxItemFilterAttribute>
     ...
     **참고** `MarqueeControlRootDesigner` 클래스에 대 한 정의가 "MarqueeControlLibrary" 라는 네임 스페이스에 포함 되었습니다. 이 선언은 디자인 관련 형식에 대해 예약 된 특수 네임 스페이스에 디자이너를 배치 합니다.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#530](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/marqueecontrolrootdesigner.cs#530)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#530](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/marqueecontrolrootdesigner.vb#530)]

5. `MarqueeControlRootDesigner` 클래스에 대 한 생성자를 정의 합니다. 생성자 본문에 문을 삽입 합니다. <xref:System.Diagnostics.Trace.WriteLine%2A> 이는 디버깅 목적으로 유용 합니다.
...
## <a name="creating-an-instance-of-your-custom-control"></a>사용자 지정 컨트롤의 인스턴스 만들기
 컨트롤의 사용자 지정 디자인 타임 동작을 관찰 하기 위해 `MarqueeControlTest` 프로젝트의 폼에 컨트롤의 인스턴스를 추가 합니다.
...
1. 클래스에서 상속 하도록 `DemoMarqueeControl` 의 선언을 변경 합니다. `MarqueeControl`

2. 프로젝트를 빌드합니다.

3. Windows Forms 디자이너에서 `Form1`을 엽니다.

4. **도구 상자** 에서 **MarqueeControlTest 구성 요소** 탭을 찾아 엽니다. 를 `DemoMarqueeControl` **도구 상자** 에서 폼으로 끌어옵니다.

5. 프로젝트를 빌드합니다.

## <a name="setting-up-the-project-for-design-time-debugging"></a>디자인 타임 디버깅을 위한 프로젝트 설정
...
3. **시작 작업** 섹션에서 **시작 외부 프로그램**을 선택 합니다. Visual studio의 개별 인스턴스를 디버깅 하 게 될 것 이기 때문에 visual studio![IDE를 찾아보려면 줄임표 (visual](./media/visual-studio-ellipsis-button.png)studio 속성 창의 줄임표 단추 (...) 단추를 클릭 합니다. 실행 파일의 이름은 devenv.exe입니다 .를 기본 위치에 설치한 경우 해당 경로는%programfiles%\Microsoft Visual Studio 9.0 \ Common7\IDE\devenv.exe.

4. 확인을 클릭 하 여 대화 상자를 닫습니다.

5. 프로젝트를 `MarqueeControlLibrary` 마우스 오른쪽 단추로 클릭 하 고 "시작 프로젝트로 설정"을 선택 하 여이 디버깅 구성을 사용 하도록 설정 합니다.
...
1. **코드 편집기** 에서 <xref:System.Diagnostics.Trace.WriteLine%2A> 소스 파일을 열고 문에 중단점을 설정 합니다. `MarqueeControlRootDesigner`

2. F5 키를 눌러 디버깅 세션을 시작 합니다. Visual Studio의 새 인스턴스가 만들어집니다.
...
4. 디자이너에서 `DemoMarqueeControl` 를 엽니다. Visual Studio의 디버깅 인스턴스는 포커스를 획득 하 고 중단점에서 실행을 중지 합니다. F5 키를 눌러 디버깅 세션을 계속 합니다.

이 시점에서 모든 항목은 사용자 지정 컨트롤 및 이와 관련 된 사용자 지정 디자이너를 개발 하 고 디버그할 수 있도록 준비 되어 있습니다. 이 연습의 나머지 부분에서는 컨트롤 및 디자이너의 기능 구현에 대 한 세부 정보에 중점을 두고 있습니다.
...
11. 속성 접근자를 구현 합니다. 클라이언트 `LightColor` 에 및 `DarkColor`의 두 가지 속성을 노출 합니다. <xref:System.ComponentModel.CategoryAttribute.Category%2A> 및<xref:System.ComponentModel.BrowsableAttribute.Browsable%2A> 특성은 이러한 속성에 적용 되므로 "움직이는 텍스트" 라는 속성 창의 사용자 지정 섹션에 속성이 표시 됩니다.
...
4. 속성 창에서 이벤트 단추를 클릭 합니다. <xref:System.ComponentModel.BackgroundWorker.DoWork> 및<xref:System.ComponentModel.BackgroundWorker.ProgressChanged> 이벤트에 대 한 처리기를 연결 합니다.
...
## <a name="creating-a-custom-designer-to-shadow-and-filter-properties"></a>속성을 숨기고 필터링 하기 위한 사용자 지정 디자이너 만들기
...
이러한 메서드를 사용 하 여 구성 요소의 공용 인터페이스를 변경 하는 경우 다음 규칙을 따라야 합니다.
...
2. `MarqueeBorderDesigner` **코드 편집기**에서 소스 파일을 엽니다. 파일의 맨 위에 다음 네임 스페이스를 가져옵니다.
...
## <a name="handling-component-changes"></a>구성 요소 변경 내용 처리
 클래스 `MarqueeControlRootDesigner` 는 `MarqueeControl` 인스턴스에 대 한 사용자 지정 디자인 타임 환경을 제공 합니다. 대부분의 디자인 타임 기능은 <xref:System.Windows.Forms.Design.DocumentDesigner> 클래스에서 상속 됩니다. 코드는 구성 요소 변경을 처리 하 고 디자이너 동사를 추가 하는 두 가지 특정 사용자 지정을 구현 합니다.
...
## <a name="adding-designer-verbs-to-your-custom-designer"></a>사용자 지정 디자이너에 디자이너 동사 추가
...
디자이너에 두 개의 디자이너 동사를 추가 합니다. **테스트를 실행** 하 고 **테스트를 중지**합니다. 이러한 동사를 사용 하 여 디자인 타임 `MarqueeControl` 에의 런타임 동작을 볼 수 있습니다. 이러한 동사는에 추가 `MarqueeControlRootDesigner`됩니다.
...
사용자에 대 한 사용자 지정 디자인 타임 환경을 만들 때 속성 창와 사용자 지정 상호 작용을 만드는 것이 좋습니다. 를 <xref:System.Drawing.Design.UITypeEditor>만들어이를 수행할 수 있습니다. 자세한 내용은 [방법: UI 형식 편집기](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2013/fd3kt7d5(v=vs.120))를 만듭니다.
...
## <a name="creating-a-view-control-for-your-custom-uitypeeditor"></a>사용자 지정 UITypeEditor에 대 한 뷰 컨트롤 만들기

1. 속성 `MarqueeLightShape` 은 두 가지 유형의 조명 모양 및를 `Square` `Circle`지원 합니다. 속성 창에서 이러한 값을 그래픽으로 표시 하는 용도로만 사용 되는 사용자 지정 컨트롤을 만듭니다. 에서이 사용자 지정 컨트롤을 사용 <xref:System.Drawing.Design.UITypeEditor> 하 여 속성 창와 상호 작용할 수 있습니다.
...
1. 및`squarePanel` <xref:System.Windows.Forms.Control.Click> 컨트롤에대한이벤트처리기`circlePanel` 를 구현 합니다. 이러한 메서드는 <xref:System.Windows.Forms.Design.IWindowsFormsEditorService.CloseDropDown%2A> 를 호출 하 여 <xref:System.Drawing.Design.UITypeEditor> 사용자 지정 편집 세션을 종료 합니다.

    [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#390](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/lightshapeselectioncontrol.cs#390)]
    [!code-vb[System.Windows.Forms.Design.DocumentDesigner#390](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/lightshapeselectioncontrol.vb#390)]

2. <xref:System.Windows.Forms.Design.IWindowsFormsEditorService> 라는`editorService`인스턴스 변수를 선언 합니다.
...
1. `MarqueeLightShape` 라는`lightShapeValue`인스턴스 변수를 선언 합니다.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#330](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/lightshapeselectioncontrol.cs#330)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#330](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/lightshapeselectioncontrol.vb#330)]

2. `squarePanel` <xref:System.Windows.Forms.Control.Click> 생성자에서 <xref:System.Windows.Forms.Control.Click> 이벤트 처리기를 및`circlePanel` 컨트롤의 이벤트에 연결 합니다. `LightShapeSelectionControl` 또한 디자인 환경에서 `MarqueeLightShape` `lightShapeValue` 필드에 값을 할당 하는 생성자 오버 로드를 정의 합니다.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#340](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/lightshapeselectioncontrol.cs#340)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#340](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/lightshapeselectioncontrol.vb#340)]

3. <xref:System.ComponentModel.Component.Dispose%2A> 메서드에서 <xref:System.Windows.Forms.Control.Click> 이벤트 처리기를 분리 합니다.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#350](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/lightshapeselectioncontrol.cs#350)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#350](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/lightshapeselectioncontrol.vb#350)]

4. **솔루션 탐색기**에서 **모든 파일 표시** 단추를 클릭합니다. LightShapeSelectionControl.Designer.cs 또는 LightShapeSelectionControl 파일을 열고 <xref:System.ComponentModel.Component.Dispose%2A> 메서드의 기본 정의를 제거 합니다.
...
6. <xref:System.Windows.Forms.Control.OnPaint%2A> 메서드를 재정의합니다. 이 구현에서는 채워진 사각형 및 원을 그립니다. 또한 한 도형 둘레에 테두리를 그리면 선택한 값이 강조 표시 됩니다.
...
1. 프로젝트를 `MarqueeControlTest` 시작 프로젝트로 설정 하 고 실행 합니다. 를 표시 `DemoMarqueeControl`하는 양식이 표시 됩니다. **시작** 단추를 클릭 하 여 애니메이션을 시작 합니다. 깜박이는 텍스트가 표시 되 고 테두리 주위에서 조명이 이동 합니다.
...
- 라이선스를 `MarqueeControl`사용 합니다. 자세한 내용은 [방법: 라이선스 구성 요소 및](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2013/fe8b1eh9(v=vs.120))컨트롤.
