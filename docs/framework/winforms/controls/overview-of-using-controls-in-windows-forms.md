...
이 항목에서는 Windows Forms 응용 프로그램의 필수 요소를 설명하고 Windows Forms 응용 프로그램에서 컨트롤을 사용하고 이벤트를 처리하는 간단한 예제를 제공합니다.  
  
## <a name="simple-windows-forms-applications"></a>간단한 윈도우 양식 응용 프로그램  
 최소한 Windows Forms 응용 프로그램은 다음과 같은 요소로 구성됩니다.  
  
- 에서 파생되는 하나 <xref:System.Windows.Forms.Form?displayProperty=nameWithType>이상의 클래스.  
  
- `static` (Visual`shared` <xref:System.Windows.Forms.Application.Run%2A> Basic) 메서드를 호출하고 인스턴스를 <xref:System.Windows.Forms.Form> 전달하는 `Main` 메서드입니다. 메서드는 <xref:System.Windows.Forms.Application.Run%2A> 운영 체제에서 응용 프로그램에 메시지를 처리합니다.  
  
 다음 코드 예제에서는 Windows Forms 응용 프로그램의 필수 요소를 보여 준다.  
...
## <a name="using-controls-in-a-windows-forms-application"></a>Windows 양식 응용 프로그램에서 컨트롤 사용  
 다음 코드 예제에서는 Windows Forms 응용 프로그램에서 컨트롤을 사용하고 이벤트를 처리하는 방법을 보여 주는 간단한 응용 프로그램을 보여 줍니다. 이 예제는 양식에 있는 세 개의 단추로 구성됩니다. 각 버튼을 클릭하면 배경색이 변경됩니다.  
