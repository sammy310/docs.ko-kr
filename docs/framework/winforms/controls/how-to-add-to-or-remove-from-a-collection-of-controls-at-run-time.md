...
3. 부모 컨트롤의 `Controls` 컬렉션에 컨트롤을 추가합니다.  
     ...
     다음 코드 예제에서는 컨트롤의 인스턴스를 만드는 방법을 보여 줍니다 <xref:System.Windows.Forms.Button> . 컨트롤에 폼이 필요 하 <xref:System.Windows.Forms.Panel> 고, 생성 되는 단추에 대 한 이벤트 처리 메서드가 `NewPanelButton_Click` 이미 있습니다.  
...
1. 이벤트에서 이벤트 처리기를 제거합니다. Visual Basic에서 [RemoveHandler 문](../../../visual-basic/language-reference/statements/removehandler-statement.md) 키워드를 사용 합니다. c #에서 [-= 연산자](../../../csharp/language-reference/operators/subtraction-operator.md)를 사용 합니다.  
  
2. `Remove` 메서드를 사용하여 패널의 `Controls` 컬렉션에서 원하는 컨트롤을 삭제합니다.  
  
3. 메서드를 호출 <xref:System.Windows.Forms.Control.Dispose%2A> 하 여 컨트롤에서 사용 하는 모든 리소스를 해제 합니다.  
