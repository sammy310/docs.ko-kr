...
- 속성을 변경 하 여 컨트롤의 시각적 표시에 영향을 주는 경우 `set` 접근자에서 컨트롤이 <xref:System.Windows.Forms.Control>에서 상속 하는 <xref:System.Windows.Forms.Control.Invalidate%2A> 메서드를 호출 합니다. 그런 다음 <xref:System.Windows.Forms.Control.Invalidate%2A>는 컨트롤을 다시 그리는 <xref:System.Windows.Forms.Control.OnPaint%2A> 메서드를 호출 합니다. <xref:System.Windows.Forms.Control.Invalidate%2A>를 여러 번 호출 하면 효율성을 위해 <xref:System.Windows.Forms.Control.OnPaint%2A>에 대 한 단일 호출이 발생 합니다.  
...
```  
 ...
 다음 코드 조각은 형식 변환기 및 UI 형식 편집기를 속성 `Value`과 연결합니다. 이 경우 `Value`는 정수 이며 기본 형식 변환기를 갖지만, <xref:System.ComponentModel.TypeConverterAttribute> 특성은 디자이너에서이를 백분율로 표시할 수 있도록 하는 사용자 지정 형식 변환기 (`FlashTrackBarValueConverter`)를 적용 합니다. UI 형식 편집기인 `FlashTrackBarValueEditor`은 백분율을 시각적으로 표시할 수 있습니다. 이 예에서는 <xref:System.ComponentModel.TypeConverterAttribute> 또는 <xref:System.ComponentModel.EditorAttribute> 특성으로 지정 된 형식 변환기 또는 편집기가 기본 변환기를 재정의 하는 방법도 보여 줍니다.  
