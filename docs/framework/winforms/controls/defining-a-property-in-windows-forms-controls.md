...
- 속성을 변경하면 컨트롤의 시각적 표시에 영향을 <xref:System.Windows.Forms.Control.Invalidate%2A> 주는 경우 컨트롤에서 <xref:System.Windows.Forms.Control>메서드(컨트롤에서 상속)를 호출합니다. `set` <xref:System.Windows.Forms.Control.Invalidate%2A>차례로 컨트롤을 <xref:System.Windows.Forms.Control.OnPaint%2A> 다시 그리는 메서드를 호출합니다. 효율성을 위해 <xref:System.Windows.Forms.Control.Invalidate%2A> 단일 호출을 <xref:System.Windows.Forms.Control.OnPaint%2A> 초래하는 여러 호출.  
...
```  
 ...
 다음 코드 조각은 형식 변환기 및 UI 형식 편집기를 속성 `Value`과 연결합니다. 이 경우 `Value` 정수이며 기본 형식 변환기가 있지만 특성에 <xref:System.ComponentModel.TypeConverterAttribute> 는 디자이너가 백분율로 표시할 수 있는 사용자 지정 형식 변환기()가`FlashTrackBarValueConverter`적용됩니다. UI 형식 편집기인 `FlashTrackBarValueEditor`은 백분율을 시각적으로 표시할 수 있습니다. 이 예제에서는 <xref:System.ComponentModel.TypeConverterAttribute> 또는 <xref:System.ComponentModel.EditorAttribute> 특성에 의해 지정된 형식 변환기 또는 편집기가 기본 변환기를 재정의한다는 것을 보여 주기도 합니다.  
