...
<xref:System.Windows.Forms.DataGridView>를 사용하여 데이터 소스의 데이터를 표시할 때 데이터 소스의 스키마에 있는 열이 표시하려는 순서대로 나타나지 않는 경우가 있습니다. <xref:System.Windows.Forms.DataGridViewColumn.DisplayIndex%2A> 클래스의 <xref:System.Windows.Forms.DataGridViewColumn> 속성을 사용하여 열의 표시 순서를 변경할 수 있습니다.  
...
- Northwind 샘플 데이터베이스의 <xref:System.Windows.Forms.DataGridView> 테이블과 같은 표시된 열 이름을 포함하는 테이블에 바인딩된 `customersDataGridView`라는 `Customers` 컨트롤  
