...
Windows Forms 텍스트 상자는 사용자의 입력을 가져오거나 텍스트를 표시 하는 데 사용 됩니다. `TextBox` 컨트롤은 읽기 전용으로 만들 수도 있지만 편집 가능한 텍스트에 일반적으로 사용 됩니다. 텍스트 상자는 여러 줄을 표시 하 고 텍스트를 컨트롤의 크기로 래핑하고 기본 서식 지정을 추가할 수 있습니다. `TextBox` 컨트롤은 컨트롤에 표시 되거나 입력 되는 텍스트에 대 한 단일 형식을 허용 합니다.  
