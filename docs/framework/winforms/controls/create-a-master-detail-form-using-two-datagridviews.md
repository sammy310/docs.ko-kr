...
다음 코드 예제에서는 두 개의 <xref:System.Windows.Forms.DataGridView> 구성 요소에 바인딩된 두 개의 <xref:System.Windows.Forms.BindingSource> 컨트롤을 사용하여 마스터/세부 폼을 만듭니다. 데이터 소스는 SQL Server Northwind 샘플 데이터베이스의 <xref:System.Data.DataSet> 및 `Customers` 테이블과 `Orders` 열을 통해 두 테이블을 연결하는 <xref:System.Data.DataRelation>을 포함하는 `CustomerID`니다.  
  
 하나의 <xref:System.Windows.Forms.BindingSource>가 데이터 집합의 부모 `Customers` 테이블에 바인딩됩니다. 이 데이터는 마스터 <xref:System.Windows.Forms.DataGridView> 컨트롤에 표시됩니다. 다른 <xref:System.Windows.Forms.BindingSource>는 첫 번째 데이터 커넥터에 바인딩됩니다. 두 번째 <xref:System.Windows.Forms.BindingSource.DataMember%2A>의 <xref:System.Windows.Forms.BindingSource> 속성은 <xref:System.Data.DataRelation> 이름으로 설정됩니다. 이렇게 하면 연결된 세부 <xref:System.Windows.Forms.DataGridView> 컨트롤에 마스터 `Orders` 컨트롤의 현재 행에 해당하는 자식 <xref:System.Windows.Forms.DataGridView> 테이블의 행이 표시됩니다.  
  
 이 코드 예제에 대 한 전체 설명은 [연습: 두 개의 Windows Forms DataGridView 컨트롤을 사용 하 여 마스터/세부 폼 만들기](creating-a-master-detail-form-using-two-datagridviews.md)를 참조 하세요.  
