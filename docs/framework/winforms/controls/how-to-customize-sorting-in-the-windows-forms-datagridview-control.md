...
## <a name="custom-sorting-using-the-icomparer-interface"></a>IComparer 인터페이스를 사용한 사용자 지정 정렬  
 다음 코드 예제에서는 `Sort(IComparer)` 인터페이스 구현을 받아서 다중 열 정렬을 수행하는 <xref:System.Windows.Forms.DataGridView.Sort%2A> 메서드의 <xref:System.Collections.IComparer> 오버로드를 사용한 사용자 지정 정렬을 보여 줍니다.  
