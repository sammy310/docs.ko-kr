...
Windows Forms `NotifyIcon` 구성 요소에는 백그라운드에서 실행 되는 프로세스에 대 한 작업 표시줄의 상태 알림 영역에 아이콘이 표시 되 고 그렇지 않으면 사용자 인터페이스가 표시 되지 않습니다. 예를 들어 작업 표시줄의 상태 알림 영역에서 아이콘을 클릭하여 액세스할 수 있는 바이러스 방지 프로그램이 있습니다.  
  
## <a name="in-this-section"></a>섹션 내용  
 [NotifyIcon 구성 요소 개요](notifyicon-component-overview-windows-forms.md)  
 사용자 인터페이스가 없는 백그라운드에서 실행 중인 프로세스에 대 한 아이콘을 사용자가 볼 수 있도록 하는 `NotifyIcon` 구성 요소의 일반적인 개념을 소개 합니다.  
  
 [방법: Windows Forms NotifyIcon 구성 요소를 사용하여 작업 표시줄에 애플리케이션 아이콘 추가](app-icons-to-the-taskbar-with-wf-notifyicon.md)  
 `NotifyIcon` 구성 요소에 표시 되는 아이콘을 설정 하는 방법에 대해 설명 합니다.  
 ...
 `NotifyIcon` 구성 요소에 바로 가기 메뉴를 추가 하는 방법에 대해 설명 합니다.  
