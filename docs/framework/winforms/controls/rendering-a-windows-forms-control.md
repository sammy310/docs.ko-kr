...
## <a name="gdi-basics"></a>GDI + 기본 사항  
 <xref:System.Drawing.Graphics> 클래스는 메서드 원, 삼각형, 원호 및 타원과 같은 다양 한 도형을 그리는 데 뿐만 아니라 텍스트를 표시 하기 위한 메서드를 제공 합니다. <xref:System.Drawing?displayProperty=nameWithType> 네임 스페이스 및 해당 하위 네임 스페이스 (원, 사각형, 원호 및 기타) 셰이프, 색, 글꼴, 브러시, 등과 같은 그래픽 요소를 캡슐화 하는 클래스를 포함 합니다. 에 대 한 자세한 내용은 [!INCLUDE[ndptecgdi](../../../../includes/ndptecgdi-md.md)]를 참조 하세요 [관리 되는 그래픽 클래스를 사용 하 여](../advanced/using-managed-graphics-classes.md)입니다. 필수 [!INCLUDE[ndptecgdi](../../../../includes/ndptecgdi-md.md)] 에 나와 [방법: 진행률을 보여 주는 Windows Forms 컨트롤 만들기](how-to-create-a-windows-forms-control-that-shows-progress.md)합니다.  
