...
## <a name="example"></a>예제  
 다음 코드 예제를 구현 하는 방법에 설명 합니다 <xref:System.ComponentModel.IListSource> 인터페이스입니다. 이라는 구성 요소 `EmployeeListSource` 노출를 <xref:System.Collections.IList> 구현 하 여 데이터 바인딩에 <xref:System.ComponentModel.IListSource.GetList%2A> 메서드.  
