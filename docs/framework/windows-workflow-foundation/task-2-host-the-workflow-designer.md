...
이 항목에서는 Windows Presentation Foundation (WPF) 응용 프로그램 [!INCLUDE[wfd1](../../../includes/wfd1-md.md)] 에서 인스턴스를 호스트 하는 절차에 대해 설명 합니다.

프로시저는 디자이너가 포함 된 **Grid** 컨트롤을 구성 하 고, 기본 <xref:System.Activities.Presentation.WorkflowDesigner> <xref:System.Activities.Statements.Sequence> 작업을 포함 하는의 인스턴스를 프로그래밍 방식으로 만들고, 디자이너 메타 데이터를 등록 하 여 모든 작업에 대 한 디자이너 지원을 제공 합니다. 기본 제공 작업 및는 [!INCLUDE[wfd2](../../../includes/wfd2-md.md)] WPF 응용 프로그램에서를 호스팅합니다.

### <a name="to-host-the-workflow-designer"></a>워크플로 디자이너를 호스트하려면

1. 작업 1에서 [만든 HostingApplication 프로젝트를 엽니다. 새 Windows Presentation Foundation 응용 프로그램](task-1-create-a-new-wpf-app.md)을 만듭니다.
...
4. **속성** 창 `ColumnDefinitions` 에서 속성 옆에 있는 줄임표 ( **...** )를 클릭 하 여 **컬렉션 편집기** 대화 상자를 엽니다.

5. **컬렉션 편집기** 대화 상자에서 **추가** 단추를 세 번 클릭 하 여 레이아웃에 세 개의 열을 삽입 합니다. 첫 번째 열에는 **도구 상자가**포함 되 고, 두 번째 열 [!INCLUDE[wfd2](../../../includes/wfd2-md.md)]은를 호스팅하고, 세 번째 열은 속성 검사자에 사용 됩니다.
...
8. **솔루션 탐색기**에서 mainwindow.xaml를 마우스 오른쪽 단추로 클릭 하 고 **코드 보기**를 선택 합니다. 다음 단계에 따라 코드를 수정합니다.
    ...
    3. 다음 `AddDesigner` 메서드를 `MainWindow` 클래스에 추가합니다. 구현은의 <xref:System.Activities.Presentation.WorkflowDesigner>인스턴스를 만들고 여기에 <xref:System.Activities.Statements.Sequence> 활동을 추가한 다음 grid1 **Grid**의 중간 열에 배치 합니다.
    ...
    4. 디자이너 메타데이터를 등록하여 모든 기본 제공 활동에 대한 디자이너 지원을 추가합니다. 그러면 도구 상자의 활동을 <xref:System.Activities.Statements.Sequence>의 원래 [!INCLUDE[wfd2](../../../includes/wfd2-md.md)] 활동으로 끌어 놓을 수 있습니다. 이렇게 하려면 `RegisterMetadata` 메서드를 `MainWindow` 클래스에 추가합니다.
        ...
        활동 디자이너를 [등록 하는 방법에 대 한 자세한 내용은 방법: 사용자 지정 활동 디자이너](how-to-create-a-custom-activity-designer.md)를 만듭니다.
...
10. [작업 3: 도구 상자 및 PropertyGrid 창을](task-3-create-the-toolbox-and-propertygrid-panes.md) 만들어 다시 호스트 된 워크플로 디자이너에 **도구 상자** 와 **PropertyGrid** 지원을 추가 하는 방법을 알아봅니다.
