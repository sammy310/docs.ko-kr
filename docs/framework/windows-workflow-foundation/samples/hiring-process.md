...
이 샘플에서는 워크플로 서비스로 호스트되는 두 개의 워크플로와 메시징 활동을 사용하여 비즈니스 프로세스를 구현하는 방법을 보여 줍니다. 이 워크플로는 Contoso, Inc라는 가상 회사의 IT 인프라 중 일부입니다.  
 ...
 `HiringRequest`로 구현된 <xref:System.Activities.Statements.Flowchart> 워크플로 프로세스는 조직에 있는 여러 관리자의 권한 부여를 요구합니다. 이 목표를 달성하기 위해 워크플로는 조직의 다른 기존 서비스(이 경우 받은 편지함 서비스 및 일반 WCF(일반 Windows 통신 재단) 서비스로 구현된 조직 데이터 서비스를 사용합니다.  
...
3. 요청자의 관리자가 승인한 후 부서 소유자는 요청을 승인해야 합니다.  
...
|OrgService|조직 정보(직원, 직무, 직무 종류 및 부서)를 노출하는 서비스입니다. 이 서비스는 ERP(Enterprise Resource Plan)의 회사 조직 모듈로 생각하면 됩니다.<br /><br /> 이 프로젝트는 WCF(Windows 통신 재단) 서비스를 노출하는 콘솔 응용 프로그램으로 구현됩니다.|  
...
|메시징 활동|순서도는 다음 두 가지 방식으로 메시징 활동을 사용합니다.<br /><br /> - 사용자로부터 정보를 얻기 위해 (각 승인 단계에서 결정 및 관련 정보를 수신하기 위해).<br />- 다른 기존 서비스와 상호 작용합니다 (받은 편지함 서비스 및 조직 데이터 서비스, 서비스 참조를 통해 사용).|HiringRequestService|  
|콘텐츠 기반 상관 관계|승인 메시지는 채용 요청의 ID 속성과 상관 관계가 있습니다.<br /><br /> - 프로세스가 시작되면 상관 관계 핸들이 요청의 ID로 초기화됩니다.<br />- 들어오는 승인 메시지는 ID와 상관 관계가 있습니다(각 승인 메시지의 첫 번째 매개 변수는 요청의 ID입니다).|HiringRequestService/ResumeRequestService|  
|사용자 지정 활동(선언적 및 코드 기반)|이 샘플에는 다음과 같은 몇 가지 사용자 지정 활동이 있습니다.<br /><br /> -   `SaveActionTracking`: 이 활동은 사용자 <xref:System.Activities.Tracking.TrackingRecord> 지정(사용)을 <xref:System.Activities.NativeActivityContext.Track%2A>내보올 수 있습니다. 이 활동은 <xref:System.Activities.NativeActivity>를 확장하는 명령적 코드를 사용하여 작성되었습니다.<br />-   `GetEmployeesByPositionTypes`: 이 활동은 위치 유형 아이디 목록을 수신하고 Contoso에 해당 위치가 있는 사람들의 목록을 반환합니다. 이 활동은 활동 디자이너를 사용하여 선언적으로 작성되었습니다.<br />-   `SaveHiringRequestInfo`: 이 활동은 `HiringRequest` (사용)의 `HiringRequestRepository.Save`정보를 저장합니다. 이 활동은 <xref:System.Activities.CodeActivity>를 확장하는 명령적 코드를 사용하여 작성되었습니다.|HiringRequestService|  
...
|병렬 활동|-   <xref:System.Activities.Statements.ParallelForEach%601>CEO 및 HR 관리자의 받은 편지함에 병렬로 등록하는 데 사용됩니다(두 명의 HR 관리자 승인 단계 대기 중).<br />-   <xref:System.Activities.Statements.Parallel>완료 및 거부 된 단계에서 일부 정리 작업을 수행하는 데 사용됩니다.|HiringRequestService|  
...
|워크플로 서비스|`ResumeRequestService`는 워크플로 서비스를 사용하여 구현됩니다. 워크플로 정의 및 서비스 정보는 ResumeRequestService.xamlx에 포함되어 있습니다. 서비스는 지속성과 추적을 사용하도록 구성됩니다.|ResumeRequestService|  
...
|ASP.NET [!INCLUDE[wf1](../../../../includes/wf1-md.md)] 응용 프로그램에서 사용.|워크플로는 두 개의 ASP.NET 응용 프로그램에서 액세스됩니다.|InternalClient/CareersWebSite|  
...
1. Visual Studio에 대한 개발자 명령 프롬프트를 엽니다.  
...
2. **솔루션 탐색기에서** 솔루션을 마우스 오른쪽 단추로 클릭하고 **속성을**선택합니다.  
  
3. 옵션을 선택 **여러 시작 프로젝트** 및 설정 **채용웹사이트,** **내부 클라이언트,** 고용 **Start** **요청 서비스**및 **시작 ResumeRequestService.** **ContosoHR,** **받은 편지함 서비스**및 **조직 서비스를** 없음으로 둡니다.  
...
2. 솔루션에서 **InternalClient를** 마우스 오른쪽 단추로 클릭한 다음 **브라우저에서 보기를**선택합니다. `InternalClient`의 기본 페이지가 표시됩니다. 서비스가 실행되고 있는지 확인한 다음 링크를 클릭합니다.  
  
3. **채용 요청** 모듈이 표시됩니다. 여기에서 자세히 설명된 시나리오를 따르면 됩니다.  
...
6. 솔루션에서 **CareersWeb** 사이트를 마우스 오른쪽 단추로 클릭하고 **브라우저에서 보기를**선택합니다.  
  
7. 솔루션에서 `InternalClient` **InternalClient를** 마우스 오른쪽 단추로 클릭하고 **브라우저에서 보기를**선택하여 다시 이동합니다.  
  
8. 받은 편지함 상단 메뉴에서 **채용 공고** 링크를 클릭하여 **채용 공고** 섹션으로 이동합니다. 여기에서 자세히 설명된 시나리오를 따르면 됩니다.  
...
2. 생성 된 후, 요청은 마이클의 받은 편지함에 나타납니다 (당신이 요청을 볼 수없는 경우 **새로 고침을** 클릭) 피터 브렘의 승인을 기다리고, 누가 마이클의 관리자입니다.  
  
3. 베드로는 미가엘의 요청에 따라 행동하기를 원한다. Peter는 직무에 C# 사용 경력이 3년이 아닌 5년 경력이 필요하다고 판단하고 자신의 의견을 다시 보내 검토하도록 합니다.  
  
4. Michael은 매니저가 받은 편지함에서 메시지를 보고 행동하기를 원합니다. 마이클은 위치 요청의 역사를보고 베드로와 동의합니다. Michael은 C# 경력 5년이 필요하다는 내용으로 설명을 수정하고 수정 내용을 수락합니다.  
  
5. 베드로는 미가엘의 수정된 요청에 따라 행동하고 그것을 받아들인다. 이제 엔지니어링 부서 책임자인 Tsvi Reiter가 이 요청을 승인해야 합니다.  
...
8. 요청이 Michael의 받은 편지함에서 제거되고 SDET 를 고용하는 프로세스가 시작되었습니다.  
  
### <a name="start-resume-request"></a>이력서 요청 시작  
  
1. 이제 사용자가 지원할 수 있는 외부 웹 사이트에 게시되기를 기다리고 **있습니다(채용 공고** 링크를 클릭하는 것을 볼 수 있음). 현재 직무를 종료하고 게시하는 HR 담당자가 직무에 대한 처리를 담당합니다.  
  
2. HR은 60분의 시간(실제 생활에서며칠 또는 몇 주일 수 있음)을 설정하여 이 작업 직책을 편집하려고 **합니다(편집** 링크를 클릭하여). 시간 제한을 통해 지정한 시간에 따라 외부 웹 사이트에서 직무를 중단할 수 있습니다.  
  
3. 편집된 작업 위치를 저장하면 **이력서 받기** 탭에 나타납니다(웹 페이지를 새로 고쳐 새 작업 위치를 확인함).  
  
### <a name="collecting-resumes"></a>이력서 접수  
  
1. 외부 웹 사이트에 직무가 표시되어야 합니다. 구직하려는 지원자는 이 직무에 지원하고 이력서를 제출할 수 있습니다.  
  
2. 채용 공고 목록 서비스로 돌아가면 지금까지 수집된 "이력서 보기"를 할 수 있습니다.  
...
2. 솔루션이 제대로 빌드되지 않으면 다음 내용을 확인합니다.  
    ...
    - 에 대한 `ContosoHR` 참조가 또는 `CareersWebSite` 프로젝트에서 누락되지 않습니다. `InternalClient`  
...
3. 솔루션이 제대로 실행되지 않으면 다음 내용을 확인합니다.  
    ...
    2. 서비스 참조가 업데이트되었는지 여부  
        ...
        2. **Contoso를** 마우스 오른쪽 단추로 클릭하고 **웹/서비스 참조 업데이트를**선택합니다.  
  
        3. 비주얼 스튜디오에서 CTRL+SHIFT+B를 눌러 솔루션을 다시 빌드합니다.  
