...
구성 된 워크플로 생성 하는 다음 예제에서는 <xref:System.Activities.Statements.Sequence> 포함 하는 자식 활동이 포함 된 활동을 <xref:System.Activities.Statements.TransactionScope> 활동. 
  <xref:System.Activities.Statements.TransactionScope.Body%2A>의 <xref:System.Activities.Statements.TransactionScope> 활동은 <xref:System.Activities.Statements.TransactionScope> 활동에 의해 초기화된 트랜잭션에서 실행됩니다.  
...
사용에 대 한 자세한 내용은 참조 하십시오 <xref:System.ServiceModel.Activities.TransactedReceiveScope>를 참조 하세요 [트랜잭션을 워크플로 서비스 내부 및 외부로 흐르는](../wcf/feature-details/flowing-transactions-into-and-out-of-workflow-services.md)합니다.  
