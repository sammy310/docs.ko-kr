...
WF(Windows 워크플로우 Foundation)의 보정은 후속 오류가 발생할 때 이전에 완료된 작업을 취소하거나 보정할 수 있는 메커니즘입니다(응용 프로그램에서 정의한 논리에 따라). 이 단원에서는 워크플로에서 보정을 사용하는 방법에 대해 설명합니다.  
...
> [!NOTE]
> 이 항목에서는 워크플로의 보정에 대해 설명합니다. 워크플로의 트랜잭션에 대한 자세한 [Transactions](workflow-transactions.md) 내용은 <xref:System.Activities.Statements.TransactionScope>트랜잭션 및 을 참조하십시오. 트랜잭션에 대한 자세한 <xref:System.Transactions?displayProperty=nameWithType> 내용은 <xref:System.Transactions.Transaction?displayProperty=nameWithType>및 을 참조하십시오.  
 ...
 **예약비행: 티켓은 예약되어 있습니다.**  
**관리자 승인: 관리자 승인을 받았습니다.** 
 **구매비행: 티켓을 구매합니다.** 
 **상태: 닫힘으로 워크플로가 성공적으로 완료되었습니다.**
> [!NOTE]
...
> [!NOTE]
> a가 <xref:System.Activities.Statements.CompensableActivity> *확인되면*활동에 대한 보상을 더 이상 호출할 수 없습니다. 확인 프로세스에 대해서는 이 단원의 뒷부분에서 설명합니다.  
 ...
 **예약비행: 티켓은 예약되어 있습니다.**  
**시뮬레이션된 오류 조건: 응용 프로그램예외 를 throw합니다.** 
 **워크플로 처리되지 않은 예외:**
**System.Application예외: 워크플로에서 시뮬레이션된 오류 조건입니다.** 
 **취소비행: 항공권이 취소됩니다.** 
 **상태: 취소됨으로 워크플로가 성공적으로 완료되었습니다.**
...
```  
 ...
 워크플로가 호출되면 <xref:System.Activities.WorkflowApplication.OnUnhandledException%2A>에서 호스트 애플리케이션이 시뮬레이션된 오류 조건 예외를 처리하고 워크플로가 취소되고 <xref:System.Activities.Statements.CompensableActivity>의 취소 논리가 호출됩니다. 이 예제에서는 보정 논리와 취소 논리가 다른 목표를 가지고 있습니다. <xref:System.Activities.Statements.CompensableActivity.Body%2A>가 성공적으로 완료되면 신용 카드가 청구되고 비행기가 예약되었으므로 보정은 두 단계 모두 실행 취소해야 합니다. (이 예에서는 항공편을 취소하면 신용 카드 요금이 자동으로 취소됩니다.) 그러나 <xref:System.Activities.Statements.CompensableActivity> 취소된 경우 이는 완료되지 <xref:System.Activities.Statements.CompensableActivity.Body%2A> 않았음을 의미하므로 논리는 <xref:System.Activities.Statements.CompensableActivity.CancellationHandler%2A> 취소를 가장 잘 처리하는 방법을 결정할 수 있어야 합니다. 이 예제에서 <xref:System.Activities.Statements.CompensableActivity.CancellationHandler%2A>가 신용 카드 청구를 취소하지만 `ReserveFlight`가 <xref:System.Activities.Statements.CompensableActivity.Body%2A>에서 마지막 활동이었으므로 비행기를 취소하려고 하지 않습니다. `ReserveFlight`가 <xref:System.Activities.Statements.CompensableActivity.Body%2A>에서 마지막 활동이었으므로 성공적으로 완료되었다면 <xref:System.Activities.Statements.CompensableActivity.Body%2A>가 완료되었으므로 가능한 취소도 없습니다.  
 ...
 **상태: 취소됨으로 워크플로가 성공적으로 완료되었습니다.**  취소에 대한 자세한 내용은 [취소](modeling-cancellation-behavior-in-workflows.md)를 참조하십시오.  
