...
## <a name="troubleshooting-a-wf-using-wf-tracking"></a>WF 추적을 사용한 WF 문제 해결  
 Windows Workflow Foundation 활동을 처리 하는 동안 오류를 감지 하려면 오류가 발생 한 <xref:System.Activities.Tracking.ActivityStateRecord>를 쿼리 하는 추적 프로필을 사용 하 여 추적을 사용 하도록 설정할 수 있습니다. 해당 쿼리는 다음 코드로 지정됩니다.  
...
```  
 ...
 오류가 전파되어 오류 처리기(예: <xref:System.Activities.Statements.TryCatch> 활동) 내에서 처리되면 <xref:System.Activities.Tracking.FaultPropagationRecord>를 사용하여 오류를 검색할 수 있습니다. <xref:System.Activities.Tracking.FaultPropagationRecord>는 오류의 소스 활동과 오류 처리기의 이름을 나타냅니다. <xref:System.Activities.Tracking.FaultPropagationRecord> 오류에 대 한 예외 스택의 형태로 오류 세부 정보를 포함 합니다. <xref:System.Activities.Tracking.FaultPropagationRecord>를 구독 하는 쿼리는 다음 예제에 나와 있습니다.  
...
```  
 ...
 워크플로 인스턴스가 처리 되지 않은 예외를 발견 하면 Windows Workflow Foundation 추적을 사용 하도록 설정한 경우 <xref:System.Activities.Tracking.WorkflowInstanceUnhandledExceptionRecord> 개체를 내보냅니다.  
