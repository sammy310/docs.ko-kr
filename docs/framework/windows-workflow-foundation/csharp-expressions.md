...
.NET Framework 4.5부터 C# 식이 WINDOWS WORKFLOW FOUNDATION (WF)에서 지원 됩니다. .NET Framework C# 4.5를 대상으로 하는 Visual Studio 2012에서 만든 새 C# 워크플로 프로젝트는 식을 사용 하 고 Visual Basic 워크플로 프로젝트는 Visual Basic 식을 사용 합니다. Visual Basic 식을 사용 하는 기존 .NET Framework 4 워크플로 프로젝트는 프로젝트 언어 및 지원 되는 것과 관계 없이 [!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)] 마이그레이션할 수 있습니다. 이 항목에서는 [!INCLUDE[wf1](../../../includes/wf1-md.md)]의 C# 식에 대해 간략하게 설명합니다.

## <a name="using-c-expressions-in-workflows"></a>워크플로에서 C# 식 사용

- [워크플로 디자이너 C# 에서 식 사용](csharp-expressions.md#WFDesigner)

  - [이전 버전과의 호환성](csharp-expressions.md#BackwardCompat)

- [코드 C# 워크플로에서 식 사용](csharp-expressions.md#CodeWorkflows)

- [XAML C# 워크플로에서 식 사용](csharp-expressions.md#XamlWorkflows)

  - [컴파일된 Xaml](csharp-expressions.md#CompiledXaml)

  - [느슨한 Xaml](csharp-expressions.md#LooseXaml)

- [.XAMLX C# workflow services에서 식 사용](csharp-expressions.md#WFServices)

### <a name="WFDesigner"></a>워크플로 디자이너 C# 에서 식 사용

.NET Framework 4.5부터 C# 식이 WINDOWS WORKFLOW FOUNDATION (WF)에서 지원 됩니다. C#.NET Framework 4.5 C# 를 대상으로 하는 Visual Studio 2012에서 만든 워크플로 프로젝트 Visual Basic 워크플로 프로젝트는 Visual Basic 식을 사용 합니다. 원하는 C# 식을 지정 하려면  **C# 식 입력**상자에 입력 합니다. 이 레이블은 디자이너에서 활동이 선택된 경우 속성 창에 표시되거나 워크플로 디자이너의 활동에 표시됩니다. 다음 예제에서는 `WriteLine`의 `Sequence` 내에 두 개의 `NoPersistScope` 활동이 포함되어 있습니다.

![자동으로 만들어진 시퀀스 활동을 보여 주는 스크린샷](./media/csharp-expressions/auto-surround-sequence-activity.png)

> [!NOTE]
> C#식은 Visual Studio 에서만 지원 되며 다시 호스트 된 워크플로 디자이너에서는 지원 되지 않습니다. 다시 호스트 된 디자이너에서 지원 되는 새로운 WF45 기능에 대 한 자세한 내용은 [rehosted 워크플로 디자이너의 새로운 Workflow Foundation 4.5 기능 지원](wf-features-in-the-rehosted-workflow-designer.md)을 참조 하세요.

#### <a name="BackwardCompat"></a>이전 버전과의 호환성

[!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)]로 마이그레이션된 기존 .NET Framework 4 C# 워크플로 프로젝트의 Visual Basic 식이 지원 됩니다. 워크플로 디자이너에서 Visual Basic 식을 볼 때 Visual Basic 식이 유효한 C# 구문이 아니면 기존 Visual Basic 식의 텍스트는 **XAML에 설정 된 값**으로 바뀝니다. Visual Basic 식이 유효한 C# 구문인 경우에는 해당 식이 표시됩니다. Visual Basic 식을 C#으로 업데이트하려면 워크플로 디자이너에서 식을 편집하고 해당하는 C#식을 지정하면 됩니다. Visual Basic 식을 반드시 C#으로 업데이트할 필요는 없지만, Workflow Designer에서 식을 업데이트한 후에는 식이 C#으로 변환되며 이를 Visual Basic으로 되돌릴 수는 없습니다.

### <a name="CodeWorkflows"></a>코드 C# 워크플로에서 식 사용
...
> [!NOTE]
> C# 식이 컴파일되지 않은 경우 다음과 유사한 메시지를 사용 하 여 워크플로를 호출할 때 <xref:System.NotSupportedException> throw 됩니다. `Expression Activity type 'CSharpValue`1 '를 실행 하려면 컴파일이 필요 합니다.  워크플로가 컴파일 되었는지 확인 하세요. '
...
### <a name="XamlWorkflows"></a>XAML C# 워크플로에서 식 사용
...
C# 식이 컴파일되지 않은 경우 `Open` 작업이 성공하지만 워크플로 호출에는 실패합니다. 다음 `CompileExpressions` 메서드는 이전에 [코드 워크플로의 식 사용 C# ](csharp-expressions.md#CodeWorkflows) 섹션의 메서드와 동일 합니다.
