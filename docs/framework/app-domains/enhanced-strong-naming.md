...
- 키는 지속적으로 공격을 받으며 개선된 기술과 하드웨어가 퍼블릭기가 퍼블릭 키로부터 유추되기 쉽도록 합니다. 공격을 방지하기 위해 더 큰 키가 필요합니다. .NET Framework 4.5 이전 .NET Framework 버전은 크기 키(기본 크기는 1024비트)로 서명하는 기능을 제공하지만, 새 키로 어셈블리에 서명하면 어셈블리의 이전 ID를 참조하는 모든 이진 파일이 중단됩니다. 따라서 호환성을 유지하려는 경우 서명 키의 크기를 업그레이드하기가 매우 어렵습니다.  
