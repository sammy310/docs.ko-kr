...
5. **서버** / **데이터베이스 탐색기** Northwind 아래에서 **테이블** 을 확장 하 고 테이블을 `Customers` 개체 관계형 디자이너 (O/R 디자이너)로 끕니다.
