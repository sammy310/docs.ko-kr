...
#### <a name="advertiseserviceaction"></a>AdvertiseServiceAction  
 합니다 [AdvertiseServiceAction 메서드](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh859971(v=vs.103)) 지정된 된 리소스를 사용할 수 있는 작업을 확인 하기 위해 호출 됩니다. 이 메서드는 항상 사용할 수 있는 것은 아닌 동작에 대해서만 호출됩니다. 요청 중인 리소스 상태 또는 서비스의 상태를 기반으로 하는 OData 응답에 동작을 포함해야 하는지 여부를 확인할 때 사용됩니다. 이를 확인하는 방법은 전적으로 사용자의 재량에 따라 결정됩니다. 가용성을 계산하는 비용이 높거나 현재 리소스가 피드에 있는 경우 확인을 건너뛰고 동작을 알려도 됩니다. 반환 중인 현재 리소스가 피드의 일부인 경우 `inFeed` 매개 변수가 `true`로 설정됩니다.  
  
#### <a name="createinvokable"></a>CreateInvokable  
 [CreateInvokable](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh859940(v=vs.103)) 만들려면 호출 되는 [IDataServiceInvokable](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh859893(v=vs.103)) 동작의 동작을 구현 하는 코드를 캡슐화 하는 대리자를 포함 하는 합니다. 이렇게 합니다 [IDataServiceInvokable](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh859893(v=vs.103)) 인스턴스를 만들지만 동작을 호출 하지 않습니다. WCF Data Service 동작은 의도하지 않은 결과를 포함할 수 있고 이러한 변경 내용을 디스크에 저장하도록 업데이트 공급자와 함께 사용해야 합니다. 합니다 [IDataServiceInvokable.Invoke](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh859924(v=vs.103)) 메서드는 업데이트 공급자의 savechanges ()에서 메서드를 호출 합니다.  
  
#### <a name="getserviceactions"></a>GetServiceActions  
 이 메서드가 반환의 컬렉션인 [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) 모든 WCF 데이터 서비스를 노출 하는 작업을 나타내는 인스턴스. [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) 작업 이름, 해당 매개 변수 및 반환 형식과 같은 정보를 포함 하는 액션의 메타 데이터 표현입니다.  
  
#### <a name="getserviceactionsbybindingparametertype"></a>GetServiceActionsByBindingParameterType  
 이 메서드는 모든 컬렉션을 반환 [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) 인스턴스를 지정 된 바인딩 매개 변수 형식으로 바인딩할 수 있습니다. 즉, 모든 [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103))지정된 된 리소스 형식 (바인딩 매개 변수 형식이 라고도 함)에서 작동할 수 있는 합니다. 이 서비스는 해당 리소스에 대해 호출할 수 있는 작업에 대 한 정보를 포함 하기 위해 리소스를 반환 하는 경우 사용 됩니다. 이 메서드는 파생된 형식이 아닌 정확한 바인딩 매개 변수 형식을 바인딩할 수 있는 동작만 반환해야 합니다. 이 메서드는 발생하는 형식 당 요청마다 한 번만 호출되며 결과는 WCF Data Services에서 캐시합니다.  
  
#### <a name="tryresolveserviceaction"></a>TryResolveServiceAction  
 이 메서드는 지정 된 검색 [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) 반환 `true` 경우를 [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) 를 찾을 수 있습니다. 경우 찾을 수는 [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) 반환 되는 `serviceAction` `out` 매개 변수.  
