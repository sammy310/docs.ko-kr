...
Visual Studio 2015를 사용 하 여 ASP.NET 응용 프로그램 또는 ASP.NET 웹 사이트와 WCF 데이터 서비스를 개발 하는 경우 개발 중에 데이터 서비스를 실행 하는 웹 서버를 선택할을 수 있습니다. 다음 웹 서버 테스트 하 고 로컬 컴퓨터의 데이터 서비스를 디버깅할 수 있도록 Visual Studio와 통합 합니다.

1. **로컬 IIS 서버**

     ASP.NET 응용 프로그램 또는 인터넷 정보 서비스 (IIS)에서 실행 되는 ASP.NET 웹 사이트는 데이터 서비스를 만든 경우에 개발 하 고 로컬 컴퓨터에 IIS를 사용 하 여 데이터 서비스를 테스트 하는 것이 좋습니다. IIS에서 데이터 서비스를 실행하면 디버깅하는 동안 HTTP 요청을 쉽게 추적할 수 있습니다. 또한 IIS에서 데이터 서비스에 필요한 파일, 데이터베이스 및 기타 리소스에 액세스하는 데 필요한 권한을 미리 결정할 수도 있습니다. 데이터 서비스에서 IIS를 실행 하려면 있습니다 해야 하면 IIS 및 Windows Communication Foundation (WCF)이 설치 되 고 올바르게 구성 하 고 파일 시스템 및 데이터베이스에서 IIS 계정에 대 한 액세스를 부여 합니다. 자세한 내용은 [방법: IIS에서 실행 되는 WCF 데이터 서비스 개발](../../../../docs/framework/data/wcf/how-to-develop-a-wcf-data-service-running-on-iis.md)합니다.
...
2. **Visual Studio 개발 서버**
     ...
     Visual Studio ASP.NET 프로젝트에 대 한 기본 웹 서버는 Visual Studio 개발 서버를 기본 제공 웹 서버를 포함 합니다. 이 웹 서버는 개발 중에 로컬 컴퓨터의 ASP.NET 프로젝트를 실행 하도록 설계 되었습니다. 합니다 [WCF Data Services 퀵 스타트](../../../../docs/framework/data/wcf/quickstart-wcf-data-services.md) Visual Studio 개발 서버에서 실행 되는 데이터 서비스를 만드는 방법을 보여 줍니다.
...
- 데이터 서비스는 일반적으로 ASP.NET 응용 프로그램 프로젝트를 개발 하지만 Visual Studio에서 ASP.NET 웹 사이트 프로젝트로 데이터 서비스를 만들 수도 수 있습니다. 두 가지 프로젝트 유형의 차이점에 대 한 정보를 참조 하세요 [웹 응용 프로그램 프로젝트와 Visual Studio에서 웹 사이트 프로젝트 비교](https://docs.microsoft.com/previous-versions/aspnet/dd547590(v=vs.110))합니다.

- 사용 하 여 데이터 서비스를 만들 때를 **새 항목 추가** 대화 상자 데이터 서비스 Visual Studio에서 IIS에서 ASP.NET에 의해 호스팅됩니다. ASP.NET 및 IIS에 데이터 서비스의 기본 호스트 상태인 동안 다른 호스팅 옵션은 지원 됩니다. 자세한 내용은 [데이터 서비스 호스팅](../../../../docs/framework/data/wcf/hosting-the-data-service-wcf-data-services.md)합니다.
...
- **IIS에서 호스트되는 웹 서버**
    ...
    데이터 서비스를 ASP.NET 프로젝트로 개발 하는 경우 표준 ASP.NET 배포 프로세스를 사용 하 여 IIS 웹 서버에 배포할 수 있습니다.  Visual Studio는 asp.net, 배포 하는 데이터 서비스를 호스팅하는 ASP.NET 프로젝트의 종류에 따라 다음과 같은 배포 기술을 제공 합니다.
  ...
  - **ASP.NET 웹 사이트를 위한 배포 기술**
    ...
    - [연습: XCOPY를 사용 하 여 ASP.NET 웹 응용 프로그램 배포](https://docs.microsoft.com/previous-versions/aspnet/f735abw9(v=vs.100))
     ...
     ASP.NET 응용 프로그램에 대 한 배포 옵션에 대 한 자세한 내용은 참조 하세요. [Visual Studio 및 ASP.NET 웹 배포 개요](https://docs.microsoft.com/previous-versions/aspnet/dd394698(v=vs.110))합니다.
