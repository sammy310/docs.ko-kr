...
이 항목에서는 WCF 데이터 서비스 클라이언트를 사용할 때 LINQ 쿼리가 구성되고 실행되는 방식과 LINQ를 사용하여 OData(개방형 데이터 프로토콜)를 구현하는 데이터 서비스를 쿼리하는 제한 사항에 대한 정보를 제공합니다. OData 기반 데이터 서비스에 대한 쿼리 작성 및 실행에 대한 자세한 내용은 [데이터 서비스 쿼리 를](querying-the-data-service-wcf-data-services.md)참조하십시오.  
  
## <a name="composing-linq-queries"></a>LINQ 쿼리 작성  
 LINQ 쿼리를 사용하면 <xref:System.Collections.Generic.IEnumerable%601>을 구현하는 개체의 집합에 대한 쿼리를 작성할 수 있습니다. Visual Studio의 **서비스 참조 추가** 대화 상자와 DataSvcUtil.exe 도구는 피드에서 <xref:System.Data.Services.Client.DataServiceContext>반환된 엔터티를 나타내는 개체뿐만 아니라 에서 상속하는 엔터티 컨테이너 클래스로 OData 서비스의 표현을 생성하는 데 사용됩니다. 또한 이러한 도구는 서비스에서 피드로 노출되는 모음의 엔터티 컨테이너 클래스에 대한 속성도 생성합니다. 데이터 서비스를 캡슐화하는 클래스의 해당 속성은 <xref:System.Data.Services.Client.DataServiceQuery%601>을 반환합니다. <xref:System.Data.Services.Client.DataServiceQuery%601> 클래스가 LINQ로 정의된 <xref:System.Linq.IQueryable%601> 인터페이스를 구현하기 때문에 데이터 서비스를 통해 노출되는 피드에 대해 LINQ 쿼리를 작성할 수 있으며, 이 쿼리는 클라이언트 라이브러리에 의해 실행 시 데이터 서비스로 보내지는 쿼리 요청 URI로 변환됩니다.  
  
> [!IMPORTANT]
> LINQ 구문에서 표현할 수 있는 쿼리 집합은 OData 데이터 서비스에서 사용하는 URI 구문에서 활성화된 쿼리 집합보다 광범위합니다. 쿼리를 대상 데이터 서비스의 URI에 매핑할 수 없으면 <xref:System.NotSupportedException>이 발생합니다. 자세한 내용은 이 항목의 [지원되지 않는 LINQ 메서드를](linq-considerations-wcf-data-services.md#unsupportedMethods) 참조하십시오.  
...
[!code-vb[Astoria Northwind Client#AddQueryOptionsLinqSpecific](../../../../samples/snippets/visualbasic/VS_Snippets_Misc/astoria_northwind_client/vb/source.vb#addqueryoptionslinqspecific)]
 ...
 이 LINQ 쿼리는 Northwind 기반 [빠른 시작](quickstart-wcf-data-services.md) 데이터 서비스에 대해 실행되는 다음 쿼리 URI로 변환됩니다.  
  
```http
http://localhost:12345/Northwind.svc/Orders?Orderby=ShippedDate&?filter=Freight gt 30  
```  
  
 LINQ에 대한 자세한 내용은 [언어 통합 쿼리(LINQ) - C#](../../../csharp/programming-guide/concepts/linq/index.md) 또는 [언어 통합 쿼리(LINQ) - Visual Basic](../../../visual-basic/programming-guide/concepts/linq/index.md)을 참조하십시오.  
...
[!code-vb[Astoria Northwind Client#AddQueryOptionsLinqExpressionSpecific](../../../../samples/snippets/visualbasic/VS_Snippets_Misc/astoria_northwind_client/vb/source.vb#addqueryoptionslinqexpressionspecific)]
 ...
 WCF 데이터 서비스 클라이언트는 두 종류의 구성된 쿼리를 쿼리 URI로 변환할 수 있으며 쿼리 식에 쿼리 메서드를 더하여 LINQ 쿼리를 확장할 수 있습니다. 쿼리 식 또는 <xref:System.Data.Services.Client.DataServiceQuery%601>에 메서드 구문을 추가하여 LINQ 쿼리를 작성할 때 메서드가 호출되는 순서로 연산이 쿼리 URI에 추가됩니다. 이 동작은 <xref:System.Data.Services.Client.DataServiceQuery%601.AddQueryOption%2A> 메서드를 호출하여 쿼리 URI에 각 쿼리 옵션을 추가하는 것과 같습니다.  
  
## <a name="executing-linq-queries"></a>LINQ 쿼리 실행  
 <xref:System.Linq.Enumerable.First%60%601%28System.Collections.Generic.IEnumerable%7B%60%600%7D%29> 또는 <xref:System.Linq.Enumerable.Single%60%601%28System.Collections.Generic.IEnumerable%7B%60%600%7D%29>과 같은 쿼리에 추가되는 특정 LINQ 쿼리 메서드로 인해 쿼리가 실행됩니다. 또한 쿼리는 `foreach` 루프 중이나 쿼리가 `List` 컬렉션에 할당될 때와 같이 결과가 명시적으로 열거될 때에도 실행됩니다. 자세한 내용은 [데이터 서비스 쿼리 를](querying-the-data-service-wcf-data-services.md)참조하십시오.  
  
 클라이언트는 LINQ 쿼리를 두 부분으로 실행합니다. 가능한 경우 쿼리의 LINQ 식이 클라이언트에서 먼저 계산된 다음 URI 기반 쿼리가 생성되고 서비스의 데이터와 비교하여 평가되도록 데이터 서비스로 보내집니다. 자세한 내용은 [데이터 서비스 쿼리에서](querying-the-data-service-wcf-data-services.md) [클라이언트 및 서버 실행](querying-the-data-service-wcf-data-services.md#executingQueries) 섹션을 참조하십시오.  
  
 LINQ 쿼리를 OData 호환 쿼리 URI에서 번역할 수 없는 경우 실행을 시도할 때 예외가 발생 합니다. 자세한 내용은 [데이터 서비스 쿼리 를](querying-the-data-service-wcf-data-services.md)참조하십시오.  
  
## <a name="linq-query-examples"></a>LINQ 쿼리 예제  
 다음 섹션의 예제에서는 OData 서비스에 대해 실행할 수 있는 LINQ 쿼리의 종류를 보여 줍니다.  
...
### <a name="expand"></a>Expand  
 OData 데이터 서비스를 쿼리할 때 쿼리대상 엔터티와 관련된 엔터티에 반환된 피드가 포함되도록 요청할 수 있습니다. <xref:System.Data.Services.Client.DataServiceQuery%601.Expand%2A> 메서드는 LINQ 쿼리의 대상인 엔터티 집합에 대한 <xref:System.Data.Services.Client.DataServiceQuery%601>에서 호출되며, 관련 엔터티 집합 이름은 `path` 매개 변수로 지정됩니다. 자세한 내용은 [지연된 콘텐츠 로드를](loading-deferred-content-wcf-data-services.md)참조하십시오.  
...
## <a name="unsupported-linq-methods"></a>지원되는 LINQ 메서드  
 다음 표에는 지원되지 않는 LINQ 메서드의 클래스가 포함되어 있으며 OData 서비스에 대해 실행된 쿼리에 포함될 수 없습니다.  
...
|<xref:System.DateTime>회원<sup>1</sup>|지원되는 OData 함수|  
|-------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
...
|<xref:System.DateTime.Year>|`int year(DateTime p0)`|  
 ...
 <sup>1개</sup> Visual Basic의 <xref:Microsoft.VisualBasic.DateAndTime?displayProperty=nameWithType> 동등한 날짜 <xref:Microsoft.VisualBasic.DateAndTime.DatePart%2A> 및 시간 속성과 메서드도 지원됩니다.  
  
|<xref:System.Math> 멤버|지원되는 OData 함수|  
|---------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
...
|<xref:System.Linq.Expressions.Expression> 멤버|지원되는 OData 함수|  
|---------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
