...
## <a name="dynamic-compilation"></a>동적 컴파일
 액세스 시 동적으로 컴파일되는 ASP.NET 환경에서 <xref:System.Transactions>를 가져오고 사용하려면 <xref:System.Transactions> 어셈블리에 대한 참조를 구성 파일에 넣어야 합니다. 특히 `compilation/assemblies` 기본 루트 **Web.config** 구성 파일 또는 특정 웹 응용 프로그램의 구성 파일의 섹션 아래에 참조를 추가 해야 합니다. 다음 예에 이러한 부하 분산 방식이 나와 있습니다.
