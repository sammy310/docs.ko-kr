...
- **AllowPartiallyTrustedCallers** (APTCA)는 단일 응용 프로그램 도메인 내의 트랜잭션을 사용 하는 응용 프로그램에 대 한 합니다.  
...
### <a name="aptca-partial-trust"></a>APTCA(부분 신뢰)  
 합니다 <xref:System.Transactions> 으로 표시 되어 있으므로 부분적으로 신뢰할 수 있는 코드에서 어셈블리를 호출할 수 있습니다 합니다 **AllowPartiallyTrustedCallers** 특성 (APTCA). 이 특성은 기본적으로 암시적를 제거 <xref:System.Security.Permissions.SecurityAction.LinkDemand> 에 대 한 합니다 **FullTrust** 된 권한 집합입니다 그렇지 않은 경우 각 형식의 공개적으로 액세스할 수 있는 각 메서드에 자동으로 배치 합니다. 그러나 일부 형식과 멤버에는 더 강력한 권한이 필요합니다.  
...
### <a name="fulltrust-link-demands"></a>FullTrust 링크 요청  
 ...
 지속적인 인 리스트 먼 트 및 복구에 대 한 자세한 내용은 참조 하세요. 합니다 [트랜잭션에 참가 요소로 등록 리소스](../../../../docs/framework/data/transactions/enlisting-resources-as-participants-in-a-transaction.md) 하 고 [복구 수행](../../../../docs/framework/data/transactions/performing-recovery.md) 항목입니다.  
  
 COM+와의 레거시 interop 작업을 수행하는 응용 프로그램에도 완전 신뢰가 있어야 합니다.  
  
 다음은 형식의 목록 및 부분적으로에서 호출할 수 없는 멤버는 신뢰할 수 있는 코드와 데코레이팅되기 때문에 합니다 **FullTrust** 선언적 보안 특성:  
...
- <xref:System.Transactions.TransactionScope.%23ctor%28System.Transactions.TransactionScopeOption%2CSystem.Transactions.TransactionOptions%2CSystem.Transactions.EnterpriseServicesInteropOption%29>  
 ...
 직접 실행 호출자가 보유 하는 데 필요한만 합니다 **FullTrust** 권한 위의 형식이 나 메서드를 사용 하도록 설정 합니다.
