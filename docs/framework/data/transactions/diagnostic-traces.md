...
|추적 수준|Description|  
|-----------------|-----------------|  
|위험|다음과 같은 심각한 오류가 발생했습니다.<br /><br /> - 사용자 기능에 즉각적인 손실을 일으킬 수 있는 오류입니다.<br />- 관리자가 기능 손실을 방지하기 위해 조치를 취해야 하는 이벤트입니다.<br />- 코드가 중단됩니다.<br />- 이 추적 수준은 다른 중요한 추적을 해석하기에 충분한 컨텍스트를 제공할 수도 있습니다. 이를 통해 심각한 오류를 발생시키는 작업 시퀀스를 식별할 수 있습니다.|  
|Error|사용자 기능이 손실될 수 있는 오류(예: 잘못된 구성 또는 네트워크 동작)가 발생했습니다.|  
|Warning|이후에 오류 또는 치명적인 오류를 발생시킬 수 있는 조건이 있습니다(예: 할당 실패 또는 한도 접근). 사용자 코드 오류(예: 트랜잭션 중단, 시간 초과, 인증 실패)의 정상적인 처리 중에 경고가 생성될 수도 있습니다.|  
...
## <a name="trace-codes"></a>추적 코드  
 다음 표에서는 <xref:System.Transactions> 인프라에서 생성되는 추적 코드를 보여 줍니다. 테이블에는 추적 코드 식별자, 추적에 <xref:System.Diagnostics.EventTypeFilter.EventType%2A> 대한 열거 수준 및 추적에 대한 **TraceRecord에** 포함된 추가 데이터가 포함됩니다. 또한 추적의 해당 추적 수준도 **TraceRecord**에 저장됩니다.  
...
|TransactionScopeCreated|정보|TransactionScopeResult. 다음 중 하나일 수 있습니다.<br /><br /> - 새 트랜잭션.<br />- 트랜잭션이 통과되었습니다.<br />- 종속 트랜잭션이 통과되었습니다.<br />- 현재 트랜잭션을 사용.<br />- 거래 없음.<br /><br /> 새 현재 TransactionTraceId|  
|TransactionScopeDisposed|정보|범위의 "예상된" 현재 트랜잭션의 트랜잭션추적Id입니다.|  
|TransactionScopeIncomplete|Warning|범위의 "예상된" 현재 트랜잭션의 트랜잭션추적Id입니다.|  
|TransactionScopeNestedIncorrectly|Warning|범위의 "예상된" 현재 트랜잭션의 트랜잭션추적Id입니다.|  
|TransactionScopeCurrentTransactionChanged|Warning|이전의 현재 TransactionTraceId, 기타 TransactionTraceId|  
|TransactionScopeTimeout|Warning|범위의 "예상된" 현재 트랜잭션의 트랜잭션추적Id입니다.|  
...
|ConfiguredDefaultTimeoutAdjusted|Warning|추가 데이터가 없습니다.|  
|TransactionTimeout|Warning|시간 초과되는 트랜잭션의 TransactionTraceId입니다.|  
