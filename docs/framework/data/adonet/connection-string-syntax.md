...
## <a name="windows-authentication"></a>Windows 인증  
 Windows 인증을 사용 하는 것이 좋습니다 (라고도 *통합 보안*) 지원 되는 데이터 원본에 연결 합니다. 연결 문자열에 사용되는 구문은 공급자별로 다릅니다. 다음 표에서는 .NET Framework 데이터 공급자에서 사용되는 Windows 인증 구문을 보여 줍니다.  
...
> [!IMPORTANT]
>  에 대 한 기본 설정 된 `Persist Security Info` 키워드는 `false`합니다. 이 키워드를 `true` 또는 `yes`로 설정하면 연결이 열린 다음 연결에서 사용자 ID 및 암호와 같은 보안 관련 정보를 얻을 수 있습니다. 유지할 `Persist Security Info` 로 `false` 신뢰할 수 없는 소스에 중요 한 연결 문자열 정보에 액세스할 수 없는지 확인 합니다.  

### <a name="windows-authentication-with-sqlclient"></a>SqlClient 사용 하 여 Windows 인증 
 연결할 때 Windows 인증을 사용 구문의 다음 폼 중 각 합니다 **AdventureWorks** 로컬 서버에서 데이터베이스입니다.  
...
### <a name="sql-server-authentication-with-sqlclient"></a>SqlClient 사용 하 여 SQL Server 인증   
...
Azure SQL Data Warehouse 또는 Azure SQL Database에 연결 하 고 형식으로 로그인을 제공 하는 경우 `user@servername`, 있는지 확인 합니다 `servername` 로그인에는 값에 대해 제공 된 값과 일치 `Server=`합니다.
...
### <a name="connect-to-a-named-instance-of-sql-server"></a>SQL Server의 명명 된 인스턴스에 연결 합니다.
SQL Server의 명명 된 인스턴스에 연결 하려면 사용 합니다 *서버 이름 \ 인스턴스 이름* 구문입니다.  
...
### <a name="type-system-version-changes"></a>Type System Version 변경 내용  
 합니다 `Type System Version` 키워드를 <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> SQL Server 형식의 클라이언트측 표현을 지정 합니다. <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 키워드에 대한 자세한 내용은 `Type System Version`을 참조하세요.  
...
## <a name="connecting-and-attaching-to-sql-server-express-user-instances"></a>SQL Server Express 사용자 인스턴스에 연결 및 추가  
 ...
 사용자 인스턴스 사용에 대 한 자세한 내용은 참조 하세요. [SQL Server Express 사용자 인스턴스](../../../../docs/framework/data/adonet/sql/sql-server-express-user-instances.md)합니다.  
  
## <a name="using-trustservercertificate"></a>TrustServerCertificate 사용  
 `TrustServerCertificate` 키워드는 유효한 인증서를 사용 하 여 SQL Server 인스턴스에 연결 하는 경우에 유효 합니다. `TrustServerCertificate`이 `true`로 설정된 경우 전송 계층에서는 SSL을 사용하여 채널을 암호화하고 인증서 체인을 무시하여 신뢰의 유효성을 확인합니다.  
...
### <a name="enabling-encryption"></a>암호화 설정  
 인증서를 프로 비전 된 서버의 경우 암호화를 사용 하도록 설정 합니다 **프로토콜 암호화 강제** 및 **서버 인증서 신뢰** 옵션은 SQL Server 구성 관리자에서 설정 되어야 합니다. 이 경우 확인 가능한 인증서가 서버에 제공되지 않은 경우 암호화에서 유효성 검사 없이 자체 서명한 서버 인증서를 사용합니다.  
  
 응용 프로그램 설정에서 SQL Server에 구성된 보안 수준을 낮출 수는 없지만 선택적으로 높일 수는 있습니다. 응용 프로그램을 설정 하 여 암호화를 요청할 수 있습니다는 `TrustServerCertificate` 하 고 `Encrypt` 키워드 `true`, 발생 하는 암호화 서버 인증서가 프로 비전 된 경우에 보장 및 **프로토콜 암호화 강제**  클라이언트에 대해 구성 되지 않았습니다. 그러나 `TrustServerCertificate`이 클라이언트 구성에서 설정되지 않은 경우 서버 인증서를 제공해야 합니다.  
