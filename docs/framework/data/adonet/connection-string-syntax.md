...
## <a name="windows-authentication"></a>Windows 인증  
 Windows 인증을 지원하는 데이터 원본에 연결하려면 Windows *인증(통합 보안이라고도*함)을 사용하는 것이 좋습니다. 연결 문자열에 사용되는 구문은 공급자별로 다릅니다. 다음 표에서는 .NET Framework 데이터 공급자에서 사용되는 Windows 인증 구문을 보여 줍니다.  
...
> [!IMPORTANT]
> 키워드의 `Persist Security Info` 기본 설정은 `false`. 이 키워드를 `true` 또는 `yes`로 설정하면 연결이 열린 다음 연결에서 사용자 ID 및 암호와 같은 보안 관련 정보를 얻을 수 있습니다. 신뢰할 `Persist Security Info` 수 `false` 없는 원본이 중요한 연결 문자열 정보에 액세스할 수 없도록 설정합니다.  

### <a name="windows-authentication-with-sqlclient"></a>SqlClient를 가진 윈도우 인증
 다음 각 구문은 Windows 인증을 사용하여 로컬 서버의 **AdventureWorks** 데이터베이스에 연결합니다.  
...
### <a name="sql-server-authentication-with-sqlclient"></a>SQLClient를 가진 SQL 서버 인증
...
Azure SQL Database 또는 Azure SQL Data 웨어하우스에 연결하고 형식에 `user@servername`로그인을 `servername` 제공하는 경우 로그인의 값이 `Server=`에 제공된 값과 일치하는지 확인합니다.
...
SQL Server의 명명된 인스턴스에 연결하려면 *서버 이름\인스턴스 이름* 구문을 사용합니다.  
...
### <a name="type-system-version-changes"></a>Type System Version 변경 내용  
 의 `Type System Version` <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 키워드는 SQL Server 형식의 클라이언트 측 표현을 지정합니다. <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 키워드에 대한 자세한 내용은 `Type System Version`을 참조하세요.  
...
## <a name="connecting-and-attaching-to-sql-server-express-user-instances"></a>SQL Server Express 사용자 인스턴스에 연결 및 추가  
 ...
 사용자 인스턴스 작업에 대한 자세한 내용은 [SQL Server Express 사용자 인스턴스](./sql/sql-server-express-user-instances.md)를 참조하십시오.  
  
## <a name="using-trustservercertificate"></a>TrustServerCertificate 사용  
 `TrustServerCertificate` 키워드는 유효한 인증서를 사용하여 SQL Server 인스턴스에 연결할 때만 유효합니다. `TrustServerCertificate`이 `true`로 설정된 경우 전송 계층에서는 SSL을 사용하여 채널을 암호화하고 인증서 체인을 무시하여 신뢰의 유효성을 확인합니다.  
...
### <a name="enabling-encryption"></a>암호화 설정  
 인증서가 서버에 프로비전되지 않은 경우 암호화를 사용하려면 강제 **프로토콜 암호화** 및 신뢰 **서버 인증서** 옵션을 SQL Server 구성 관리자에 설정해야 합니다. 이 경우 확인할 수 있는 인증서가 서버에 제공되지 않으면 유효성 검사 없이 자체 서명된 서버 인증서가 암호화에 사용됩니다.  
  
 애플리케이션 설정에서 SQL Server에 구성된 보안 수준을 낮출 수는 없지만 선택적으로 높일 수는 있습니다. 응용 프로그램은 서버 인증서가 `TrustServerCertificate` `Encrypt` 프로비전되지 `true`않았고 강제 **프로토콜 암호화가** 클라이언트에 대해 구성되지 않은 경우에도 암호화가 수행되도록 보장하기 위해 및 키워드를 설정하여 암호화를 요청할 수 있습니다. 그러나 `TrustServerCertificate`이 클라이언트 구성에서 설정되지 않은 경우 서버 인증서를 제공해야 합니다.  
...
|yes|예|무시됨|무시됨|암호화는 확인 가능한 서버 인증서가 있는 경우에만 발생합니다. 그렇지 않으면 연결 시도가 실패합니다.|  
|yes|yes|아니요(기본값)|무시됨|항상 암호화가 수행되지만 자체 서명된 서버 인증서가 사용될 수 있습니다.|  
|yes|yes|yes|아니요(기본값)|암호화는 확인 가능한 서버 인증서가 있는 경우에만 발생합니다. 그렇지 않으면 연결 시도가 실패합니다.|  
...
> [!IMPORTANT]
> 유니버설 데이터 링크(UDL) 파일에서 **OleDbConnection에** 대한 연결 정보를 제공할 수 있습니다. 그러나 그렇게하지 않도록해야합니다. UDL 파일은 암호화되지 않으므로 연결 문자열 정보를 일반 텍스트로 노출시킵니다. UDL 파일은 애플리케이션에 대해 외부 파일 기반 리소스이므로 .NET Framework를 사용하여 보호할 수 없습니다. UDL 파일은 **SqlClient**.  
