...
## <a name="creating-dataview-from-a-query-with-sorting-information"></a>정렬 정보가 있는 쿼리에서 DataView 만들기  
 ...
 <xref:System.Data.DataView>의 인덱스는 <xref:System.Data.DataView>가 만들어지거나 정렬 또는 필터링 정보가 수정될 때 작성됩니다. 를 만든 LINQ to DataSet 쿼리에서 정렬 기준을 제공 하 <xref:System.Data.DataView> 고 나중에 정렬 정보를 수정 하지 않는 것이 가장 좋은 성능을 얻을 수 있습니다. 자세한 내용은 [DataView 성능](dataview-performance.md)을 참조 하세요.  
...
## <a name="using-the-string-based-sort-property"></a>문자열 기반 정렬 속성 사용  
 의 문자열 기반 정렬 기능은 <xref:System.Data.DataView> 여전히 LINQ to DataSet와 함께 작동 합니다. LINQ to DataSet 쿼리에서를 만든 후에는 <xref:System.Data.DataView> 속성을 사용 하 여에 대 한 <xref:System.Data.DataView.Sort%2A> 정렬을 설정할 수 있습니다 <xref:System.Data.DataView> .  
 ...
 문자열 기반 필터링에 대 한 자세한 내용은 <xref:System.Data.DataView.Sort%2A> [데이터 정렬 및 필터링](./dataset-datatable-dataview/sorting-and-filtering-data.md)을 참조 하세요.  
...
- [데이터 정렬](/previous-versions/visualstudio/visual-studio-2013/bb546145(v=vs.120))
