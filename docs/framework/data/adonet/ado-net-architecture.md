...
## <a name="adonet-components"></a>ADO.NET 구성 요소  
 데이터에 액세스 하 고 조작 하는 ADO.NET의 두 가지 주요 구성 요소는 .NET Framework 데이터 공급자와 <xref:System.Data.DataSet>입니다.  
...
### <a name="choosing-a-datareader-or-a-dataset"></a>DataReader 또는 DataSet 선택  
 응용 프로그램에서 `DataReader` ( [DataReader를 사용 하 여 데이터 검색](retrieving-data-using-a-datareader.md)참조) 또는 `DataSet` (데이터 [집합, Datatable 및 dataviews](./dataset-datatable-dataview/index.md)참조)를 사용할지 여부를 결정 하는 경우 응용 프로그램에 필요한 기능 형식을 고려 합니다. `DataSet`을 사용하면 다음을 수행할 수 있습니다.  
...
## <a name="wcf-data-services"></a>WCF 데이터 서비스  
