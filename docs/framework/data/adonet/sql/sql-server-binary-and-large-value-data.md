...
SQL Server에서는 `max`, `varchar` 및 `nvarchar` 데이터 형식의 스토리지 용량을 확장하는 `varbinary` 지정자를 지원합니다. `varchar(max)`를 `nvarchar(max)`, 및 `varbinary(max)` 이라고 *큰 값 데이터 형식*합니다. 큰 값 데이터 형식을 사용하면 데이터를 최대 2^31-1바이트까지 저장할 수 있습니다.  
