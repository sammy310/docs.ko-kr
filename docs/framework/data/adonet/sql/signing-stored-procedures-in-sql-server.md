...
# <a name="signing-stored-procedures-in-sql-server"></a>SQL Server에서 저장 프로시저에 서명
 ...
 인증서나 비대칭 키를 사용하여 저장 프로시저에 서명할 수 있습니다. 이 기능은 권한을 소유권 체인을 통해 상속할 수 없거나 동적 SQL과 같이 소유권 체인이 끊어진 시나리오를 위해 디자인되었습니다. 그런 다음 인증서에 액세스 해야 하는 저장된 프로시저 개체에 사용자 권한 부여 인증서에 매핑된 사용자를 만들 수 있습니다.  

 동일한 인증서에 매핑된 로그인을 만들려면 및 그런 다음 해당 로그인에 필요한 서버 수준 사용 권한을 부여 하거나 수도 로그인 하나 이상의 고정된 서버 역할을 추가 합니다. 사용을 방지 하도록 설계 된이 `TRUSTWORTHY` 데이터베이스는 높은 권한 수준이 필요한 시나리오에 대 한 설정입니다.  
  
 저장된 프로시저를 실행 하면 SQL Server 인증서 사용자 및/또는 로그인의 권한을 호출자와 결합 합니다. 달리는 `EXECUTE AS` 절은 프로시저의 실행 컨텍스트를 변경 되지 않습니다. 로그인과 사용자 이름을 반환하는 기본 제공 함수는 인증서 사용자 이름이 아니라 호출자의 이름을 반환합니다.  
  
## <a name="creating-certificates"></a>인증서 만들기  
 인증서 또는 비대칭 키의 저장된 프로시저 코드에서 execute와 함께 암호화 된 해시로 구성 된 데이터 다이제스트를 사용 하 여 저장된 프로시저에 서명할 때-개인 키를 사용 하 여 만든는 사용자입니다. 런타임에 데이터 다이제스트는 공개 키로 암호 해독되어 저장 프로시저의 해시 값과 비교됩니다. Execute 변경-으로 디지털 서명은 더 이상 일치 하도록 사용자에 해시 값을 무효화 합니다. 저장된 프로시저를 수정 삭제 서명을 완전히 방지 하는 저장된 프로시저 코드 변경에서 개인 키에 대 한 액세스 권한이 있는 사람입니다. 두 경우 모두 다시 서명 해야 프로시저 코드 또는 execute를 변경할 때마다-사용자로 합니다.  
...
|리소스|설명|  
|--------------|-----------------|  
|[모듈 서명](https://go.microsoft.com/fwlink/?LinkId=98590) SQL Server 온라인 설명서의|모듈 서명을 설명하고 샘플 시나리오 및 관련 Transact-SQL 항목 링크를 제공합니다.|  
