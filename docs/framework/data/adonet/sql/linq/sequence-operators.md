...
---
# <a name="sequence-operators"></a><span data-ttu-id="069d8-102">시퀀스 연산자</span><span class="sxs-lookup"><span data-stu-id="069d8-102">Sequence Operators</span></span>
<span data-ttu-id="069d8-103">일반적으로 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]에서는 다음과 같은 하나 이상의 품질을 갖는 시퀀스 연산자를 지원하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="069d8-103">Generally speaking, [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] does not support sequence operators that have one or more of the following qualities:</span></span>  
  
- <span data-ttu-id="069d8-104">인덱스 매개 변수로 람다를 사용하는 연산자</span><span class="sxs-lookup"><span data-stu-id="069d8-104">Take a lambda with an index parameter.</span></span>  
  
- <span data-ttu-id="069d8-105"><xref:System.Linq.Queryable.TakeWhile%2A>과 같이 연속적인 행의 속성을 사용하는 시퀀스 연산자</span><span class="sxs-lookup"><span data-stu-id="069d8-105">Rely on the properties of sequential rows, such as <xref:System.Linq.Queryable.TakeWhile%2A>.</span></span>  
  
- <span data-ttu-id="069d8-106"><xref:System.Collections.Generic.IComparer%601>과 같이 임의의 CLR 구현을 사용하는 연산자</span><span class="sxs-lookup"><span data-stu-id="069d8-106">Rely on an arbitrary CLR implementation, such as <xref:System.Collections.Generic.IComparer%601>.</span></span>  
  
|<span data-ttu-id="069d8-107">지원되지 않는 예제</span><span class="sxs-lookup"><span data-stu-id="069d8-107">Examples of Unsupported</span></span>|  
|-----------------------------|  
...
## <a name="differences-from-net"></a><span data-ttu-id="069d8-108">.NET과의 차이점</span><span class="sxs-lookup"><span data-stu-id="069d8-108">Differences from .NET</span></span>  
 <span data-ttu-id="069d8-109">`Average`에 대해 지원되는 모든 시퀀스 연산자는 CLR(공용 언어 런타임) 예외에서 예상대로 동작합니다.</span><span class="sxs-lookup"><span data-stu-id="069d8-109">All supported sequence operators work as expected in the common language runtime (CLR) except for `Average`.</span></span> <span data-ttu-id="069d8-110">CLR에서 `Average`가 항상 `Average` 또는 <xref:System.Double>을 반환하는 것에 반해 <xref:System.Decimal>에서는 평균화하는 형식과 동일한 형식의 값을 반환합니다.</span><span class="sxs-lookup"><span data-stu-id="069d8-110">`Average` returns a value of the same type as the type being averaged, whereas in the CLR `Average` always returns either a <xref:System.Double> or a <xref:System.Decimal>.</span></span> <span data-ttu-id="069d8-111">소스 인수를 명시적으로 double/decimal로 캐스팅하거나 선택기를 double/decimal로 캐스팅하면 결과 SQL에서는 이러한 변환을 갖게 되고 예상한대로 결과가 나타납니다.</span><span class="sxs-lookup"><span data-stu-id="069d8-111">If the source argument is explicitly cast to double / decimal or the selector casts to double / decimal, the resulting SQL will also have such a conversion and the result will be as expected.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="069d8-112">참고자료</span><span class="sxs-lookup"><span data-stu-id="069d8-112">See also</span></span>

- [<span data-ttu-id="069d8-113">데이터 형식 및 함수</span><span class="sxs-lookup"><span data-stu-id="069d8-113">Data Types and Functions</span></span>](../../../../../../docs/framework/data/adonet/sql/linq/data-types-and-functions.md)
