...
이 섹션에서는 LINQ 에서 엔터티 쿼리로 알려진 문제에 대한 정보를 제공합니다.  
...
## <a name="ordering-information-lost"></a>순서 정보 손실  
 열을 익명 유형으로 투영하면 SQL Server 2005 데이터베이스에 대해 "80"의 호환성 수준으로 설정된 일부 쿼리에서 정렬 정보가 손실됩니다.  이런 현상은 다음 예제와 같이 정렬 순서 목록에 있는 열 이름이 선택기의 열 이름과 일치할 때 발생합니다.  
...
## <a name="unsigned-integers-not-supported"></a>부호 없는 정수 지원되지 않음  
 ENTITY Framework가 서명되지 않은 정수를 지원하지 않으므로 LINQ to Entity 쿼리에서 서명되지 않은 정수 유형을 지정하는 것은 지원되지 않습니다. 서명되지 않은 정수를 지정하면 <xref:System.ArgumentException> 다음 예제와 같이 쿼리 식 변환 중에 예외가 throw됩니다. 다음 예제에서는 ID 48000인 주문을 쿼리합니다.  
