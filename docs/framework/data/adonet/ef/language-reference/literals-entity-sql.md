...
## <a name="datetime"></a>DateTime  
 ...
 날짜 부분의 형식은 다음과 같아야 합니다. `YYYY` - `MM` - `DD` 여기서 `YYYY` 은 0001과 9999 사이의 4 자리 연도 값이 고,은 `MM` 1에서 12 사이의 월 이며,는 `DD` 지정 된 월에 유효한 일 값입니다 `MM` .  
...
## <a name="guid"></a>GUID  
 ...
 `GUID` 리터럴은 GUID(Globally Unique Identifier)를 나타냅니다. 이 시퀀스는 키워드에 의해 구성 된 시퀀스로, `GUID` *레지스트리* 형식 (8-4-4-4-12) 형식으로 된 16 진수 숫자 (작은따옴표로 묶여 있음)입니다. 16진수는 대/소문자를 구분하지 않습니다.  
