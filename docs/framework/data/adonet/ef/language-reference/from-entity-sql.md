...
## <a name="from-clause-items"></a>FROM 절 항목  
 각 `FROM` 절 항목은 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 쿼리의 소스 컬렉션을 참조합니다. [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 다음 클래스를 지 원하는 `FROM` 절 항목: 간단한 `FROM` 절 항목 `JOIN FROM` 절 항목 및 `APPLY FROM` 절 항목입니다. 각 `FROM` 절 항목에 대해서는 다음 단원에서 자세히 설명합니다.  
...
### <a name="join-from-clause-item"></a>JOIN FROM 절 항목  
 `JOIN FROM` 절 항목은 두 `FROM` 절 항목 간의 조인을 나타냅니다. [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 에서는 조인, 내부 조인, 왼쪽 및 오른쪽 우선 외부 조인 및 full outer join에 cross입니다. 이러한 모든 조인은 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)]에서와 유사한 방식으로 지원됩니다. [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)]과 마찬가지로 `FROM`에 관련된 두 `JOIN` 절 항목은 서로 독립적이어야 합니다. 즉, 상호 관련될 수 없습니다. 이러한 경우 `CROSS APPLY` 또는 `OUTER APPLY`를 사용할 수 있습니다.  
...
### <a name="apply-clause-item"></a>APPLY 절 항목  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 에서는 두 가지 종류의 `APPLY`: `CROSS APPLY` 고 `OUTER APPLY`입니다.  
...
> [!NOTE]
>  `CROSS` 및 `OUTER APPLY` 연산자에 도입 된 [!INCLUDE[ssVersion2005](../../../../../../includes/ssversion2005-md.md)]합니다. 경우에 따라 쿼리 파이프라인에서 `CROSS APPLY` 및/또는 `OUTER APPLY` 연산자가 포함된 Transact-SQL을 생성할 수 있습니다. 때문에 SQL Server의 버전을 비롯 한 일부 백엔드 공급자 이전의 [!INCLUDE[ssVersion2005](../../../../../../includes/ssversion2005-md.md)]이러한 연산자를 지원 하지 않습니다, 해당 백엔드 공급자에서 쿼리를 실행할 수 없습니다.  
>   
...
## <a name="semantics"></a>의미 체계  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 각 간단한 별칭 소개 `FROM` 절 항목 범위에 있습니다. 예를 들어, 다음 FROM 절 조각에서 범위에 생성된 이름은 c, d, e입니다.  
