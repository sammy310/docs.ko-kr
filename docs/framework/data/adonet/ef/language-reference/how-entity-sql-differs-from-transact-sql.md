...
이 항목에서는 차이점을 설명 합니다. [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 및 TRANSACT-SQL입니다.  
...
## <a name="support-for-expressions"></a>식 지원  
 TRANSACT-SQL 하위 쿼리 (테이블) 및 식 (행 및 열)에 있습니다.  
  
 컬렉션 및 중첩된 컬렉션을 지원 하기 위해 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 모든 식을 만듭니다. [!INCLUDE[esql](../../../../../../includes/esql-md.md)] TRANSACT-SQL 보다 더 구성 가능-어디서 나 모든 식을 사용할 수 있습니다. 쿼리 식은 항상 프로젝션된 형식의 컬렉션을 생성하며, 컬렉션 식이 허용된 곳이라면 어디서든 쿼리 식을 사용할 수 있습니다. 지원 되지 않는 TRANSACT-SQL 식에 대 한 자세한 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]를 참조 하세요 [지원 되지 않는 식](../../../../../../docs/framework/data/adonet/ef/language-reference/unsupported-expressions-entity-sql.md)합니다.  
...
## <a name="uniform-treatment-of-subqueries"></a>일관된 하위 쿼리 처리  
 TRANSACT-SQL 테이블 강조 지정 되 면 하위 쿼리를 문맥상으로 해석을 수행 합니다. 예를 들어, `from` 절의 하위 쿼리는 multiset(테이블)로 간주됩니다. 하지만 동일한 하위 쿼리가 `select` 절에서 사용되면 스칼라 하위 쿼리로 간주됩니다. 왼쪽에서 사용 되는 하위 쿼리 마찬가지로 `in` 연산자는 스칼라 하위 쿼리를 multiset 하위 쿼리로 오른쪽에 있는 것으로 예상 되지만 되도록 것으로 간주 됩니다.  
...
### <a name="avoiding-implicit-coercions-for-subqueries"></a>하위 쿼리에 대한 암시적 강제 변환 방지  
 일관된 하위 쿼리 처리의 부작용 중 하나는 하위 쿼리를 스칼라 값으로 암시적으로 변환하는 것입니다. 특히, TRANSACT-SQL (단일 필드)이 있는 행의 multiset은 암시적으로 변환 데이터 형식의 필드는 스칼라 값으로.  
...
## <a name="select-value-avoiding-the-implicit-row-wrapper"></a>Select Value: 암시적 행 래퍼 방지  
 TRANSACT-SQL 하위 절은 select 절의 항목 주위에 행 래퍼를 암시적으로 만듭니다. 이는 스칼라 또는 개체의 컬렉션을 만들 수 없음을 의미합니다. TRANSACT-SQL 단일 필드를 갖는 rowtype과 동일한 데이터 형식의 singleton 값 사이 암시적 강제 변환을 허용합니다.  
...
## <a name="left-correlation-and-aliasing"></a>왼쪽 상관 관계 및 별칭 지정  
 Transact-sql 식에서 지정된 된 범위 (과 같은 단일 절 `select` 또는 `from`) 동일한 범위에서 앞에서 정의한 식을 참조할 수 없습니다. 일부 SQL 언어에서는 (Transact SQL) 않는 제한 된 형태로 지원에 `from` 절.  
 ...
 또한 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]에서는 `group by` 절을 사용하는 쿼리에 추가적인 제한을 적용합니다. 이러한 쿼리에서 `select` 절 및 `having` 절의 식은 오직 별칭을 통해서만 `group by` 키를 참조할 수 있습니다. 다음 구문은 TRANSACT-SQL 에서만 아닌 유효한 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]:  
...
## <a name="referencing-columns-properties-of-tables-collections"></a>테이블(컬렉션)의 열(속성) 참조  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]의 모든 열 참조는 테이블 별칭으로 정규화해야 합니다. 다음 구문은 (가정 `a` 은 테이블의 유효한 열 `T`) Transact sql에서 되지 않습니다 유효 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]합니다.  
...
## <a name="navigation-through-objects"></a>개체 탐색  
 TRANSACT-SQL을 사용 하는 "." 표기법 (행)의 열 참조에 대 한 테이블입니다. [!INCLUDE[esql](../../../../../../includes/esql-md.md)]에서는 프로그래밍 언어에서 차용한 이 표기를 확장하여 개체의 속성 탐색을 지원합니다.  
...
## <a name="no-support-for-"></a>*를 지원하지 않음  
 TRANSACT-SQL 지원 정규화 되지 않은 * 전체 행을 지정 하 고 정규화 된 별칭으로 구문을 \* 구문 (t.\*) 해당 테이블의 필드에 대 한 바로 가기로 합니다. 또한 Transact SQL를 특수 개수에 대 한 사용 (\*) null을 포함 하는 집계 합니다.  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]에서는 * 구문을 지원하지 않습니다. 폼의 TRANSACT-SQL 쿼리 `select * from T` 하 고 `select T1.* from T1, T2...` 표현할 수 있습니다 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 으로 `select value t from T as t` 및 `select value t1 from T1 as t1, T2 as t2...`, 각각. 또한 이러한 구문은 상속(값 대체성)을 처리하지만, `select *` 변형은 선언된 형식의 최상위 속성으로 제한됩니다.  
...
## <a name="order-by-clause-usage"></a>ORDER BY 절 사용법  
 TRANSACT-SQL ORDER BY 절을 맨 위의 SELECT에만 지정할 수 있습니다... FROM . WHERE 블록에서만 지정할 수 있습니다. [!INCLUDE[esql](../../../../../../includes/esql-md.md)]에서는 중첩된 ORDER BY 식을 사용할 수 있으며, 이는 쿼리 내 임의의 위치에 올 수 있습니다. 그러나 중첩 쿼리 내의 순서는 유지되지 않습니다.  
...
## <a name="transact-sql-functionality-not-available-in-entity-sql"></a>Entity SQL에서 사용할 수 없는 Transact-SQL 기능  
 다음 TRANSACT-SQL 기능에서 사용할 수 없는 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]합니다.  
 ...
 명령형 프로그래밍 비교  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] TRANSACT-SQL과 달리 명령형 프로그래밍을 지원 하지 않습니다를 제공합니다. 대신 프로그래밍 언어를 사용합니다.  
 ...
 기본 제공 함수, 연산자  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 지원-TRANSACT-SQL의 하위 집합 기본 제공 함수 및 연산자입니다. 이러한 연산자와 함수는 주요 저장소 공급자에서 주로 지원합니다. [!INCLUDE[esql](../../../../../../includes/esql-md.md)]에서는 공급자 매니페스트에 선언된 저장소별 함수를 사용합니다. 또한 합니다 [!INCLUDE[adonet_ef](../../../../../../includes/adonet-ef-md.md)] 기본 제공 선언할 수 있으며 기존 사용자 정의 함수에 대 한 저장소 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 사용 하도록 합니다.  
  
 참고  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]에서는 쿼리 참고의 메커니즘을 제공하지 않습니다.  
  
 쿼리 결과 일괄 처리  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)]에서는 쿼리 결과 일괄 처리를 지원하지 않습니다. 예를 들어, 다음은 올바른 TRANSACT-SQL (일괄 처리로 전송 됨)입니다.  
