...
`relationship-type` 개념 스키마 정의 언어 (CSDL) 파일에서 관계의 형식 이름입니다. 합니다 `relationship-type` 으로 한정 됩니다 \<네임 스페이스 >.\< 관계 유형 이름 >.
...
관계는 엔터티 데이터 모델 (EDM)에서 고급 구문입니다. 둘 이상의 엔터티 형식 간에 관계를 설정할 수 있으며 사용자는 한쪽(엔터티)에서 다른 쪽으로 관계를 탐색할 수 있습니다. `from` 및 `to` 는 관계 내의 이름 확인에 모호성이 없는 경우에 한해 조건부로 사용 가능합니다.
