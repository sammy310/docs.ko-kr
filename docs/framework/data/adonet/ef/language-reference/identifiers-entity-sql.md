...
- 별칭이 지정되지 않은 쿼리 식이 단순 식별자 또는 따옴표 붙은 식별자인 경우, 해당 식별자가 별칭으로 사용됩니다. 예를 들어 `ROW(a, [b])`는 `ROW(a AS a, [b] AS [b])`가 됩니다.  
  
- 쿼리 식이 더 복잡한 식이지만 이 식의 마지막 구성 요소가 단순 식별자인 경우, 해당 식별자가 별칭으로 사용됩니다. 예를 들어 `ROW(a.a1, b.[b1])`는 `ROW(a.a1 AS a1, b.[b1] AS [b1])`가 됩니다.  
