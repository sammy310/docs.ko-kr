...
이 단원에서는 데이터 저장소 공급자가 데이터 저장소의 형식 및 함수를 지원하는 방법에 대해 설명합니다.  
 ...
 공급자가 지원하는 형식은 기본 데이터베이스에서 직접 또는 간접적으로 지원됩니다. 이러한 형식은 반드시 정확한 저장소 형식이 아니라 공급자가 Entity Framework를 지원하는 데 사용하는 형식입니다. 공급자/저장소 형식은 EDM(엔터티 데이터 모델) 측면에서 설명됩니다.  
  
 데이터 저장소에서 지원되는 함수의 매개 변수 및 반환 형식은 EDM 측면에서 지정됩니다.  
  
## <a name="requirements"></a>요구 사항  
 Entity Framework 및 데이터 저장소는 데이터 손실이나 잘림 없이 알려진 형식의 데이터를 앞뒤로 전달할 수 있어야 합니다.  
  
 공급자 매니페스트는 데이터 저장소에 대한 연결을 열 필요 없이 디자인 타임에 도구를 통해 로드할 수 있어야 합니다.  
  
 Entity Framework는 대/소문자를 중요하지만 기본 데이터 저장소가 아닐 수 있습니다. EDM 아티팩트(예: 식별자 및 형식 이름)가 정의되고 매니페스트에 사용되는 경우 Entity Framework 대/소문자 구분을 사용해야 합니다. 대/소문자를 구분할 수 있는 데이터 저장소 요소가 공급자 매니페스트에 나타나는 경우 해당 대/소문자 구분이 공급자 매니페스트에서 유지 관리되어야 합니다.  
  
 엔터티 프레임워크에는 모든 데이터 공급자에 대한 공급자 매니페스트가 필요합니다. Entity Framework를 사용하여 공급자 매니페스트가 없는 공급자를 사용하려고 하면 오류가 발생합니다.  
  
 다음 표는 공급자 상호 작용을 통해 예외가 발생할 때 Entity Framework에서 throw하는 예외의 종류를 설명합니다.  
...
### <a name="writing-a-provider-with-symmetric-type-mapping"></a>대칭 형식 매핑을 사용하여 공급자 작성  
 각 저장소 형식이 매핑 방향에 관계없이 단일 EDM 유형에 매핑되는 Entity Framework에 대한 공급자를 작성할 수 있습니다. EDM 형식에 해당하는 가장 단순한 매핑이 있는 공급자 형식의 경우 형식 시스템이 단순하거나 EDM 형식과 일치하기 때문에 대칭 솔루션을 사용할 수 있습니다.  
...
### <a name="writing-a-provider-with-asymmetric-type-mapping"></a>비대칭 형식 매핑을 사용하여 공급자 작성  
 엔터티 프레임워크에 대한 데이터 저장소 공급자를 작성할 때 일부 형식에 대한 EDM-공급자 형식 매핑은 공급자-EDM 형식 매핑과 다를 수 있습니다. 예를 들어, 바인딩되지 않은 EDM PrimitiveTypeKind.String은 공급자에서 nvarchar(4000)에 매핑될 수 있지만 nvarchar(4000)은 EDM PrimitiveTypeKind.String(MaxLength=4000)에 매핑됩니다.  
...
#### <a name="using-a-data-store-connection"></a>데이터 저장소 연결 사용  
 데이터 저장소 연결을 사용할 수 <xref:System.Data.Common.DbProviderServices.GetProviderManifestToken%2A?displayProperty=nameWithType> 있는 경우 메서드에 전달 <xref:System.Data.Common.DbProviderServices.GetProviderManifest%2A> 되는 토큰을 반환 하는 호출 합니다. <xref:System.Data.Common.DbProviderManifest> 이 메서드는 공급자의 구현에 `GetDbProviderManifestToken`위임합니다.  
...
#### <a name="using-a-provider-manifest-token"></a>공급자 매니페스트 토큰 사용  
 오프라인 시나리오의 경우 토큰이 SSDL 표현에서 선택됩니다. SSDL을 사용하면 공급자매니페스트토큰을 지정할 수 있습니다(자세한 내용은 [SSDL(스키마 요소)참조).](/ef/ef6/modeling/designer/advanced/edmx/ssdl-spec#schema-element-ssdl) 예를 들어, 연결을 열 수 없는 경우 SSDL에는 매니페스트에 대한 정보를 지정하는 공급자 매니페스트 토큰이 있습니다.  
...
|특성 이름|데이터 형식|필수|기본값|Description|  
|--------------------|---------------|--------------|-------------------|-----------------|  
|속성|String|yes|해당 없음|공급자별 데이터 형식 이름|  
|PrimitiveTypeKind|PrimitiveTypeKind|yes|해당 없음|EDM 형식 이름|  
...
|특성 이름|데이터 형식|필수|기본값|Description|  
|--------------------|---------------|--------------|-------------------|-----------------|  
|속성|String|yes|해당 없음|함수의 식별자/이름입니다.|  
|ReturnType|String|예|Void|함수의 EDM 반환 형식입니다.|  
|집계|부울|예|False|함수가 집계 함수인 경우 True입니다.|  
|BuiltIn|부울|예|True|함수가 데이터 저장소에 기본 제공되는 경우 True입니다.|  
|StoreFunctionName|String|예|\<Name>|데이터 저장소의 함수 이름입니다.  함수 이름의 리디렉션 수준을 허용합니다.|  
|NiladicFunction|부울|예|False|함수가 매개 변수를 필요로 하지 않고 매개 변수 없이 호출되는 경우 True입니다.|  
|ParameterType<br /><br /> 의미 체계|ParameterSemantics|예|AllowImplicit<br /><br /> 변환|쿼리 파이프라인에서 매개 변수 형식 대체를 처리할 방법에 대한 선택 항목:<br /><br /> - 정확한 일치만<br />- 허용암시프로모션<br />- 허용암시변환|  
...
|특성 이름|데이터 형식|필수|기본값|Description|  
|--------------------|---------------|--------------|-------------------|-----------------|  
|속성|String|yes|해당 없음|매개 변수의 식별자/이름입니다.|  
|Type|String|yes|해당 없음|매개 변수의 EDM 형식입니다.|  
|Mode|매개 변수<br /><br /> Direction|yes|해당 없음|매개 변수의 방향:<br /><br /> - 에서<br />- 아웃<br />- 인아웃|  
