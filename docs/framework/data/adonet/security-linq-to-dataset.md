...
## <a name="external-input"></a>외부 입력  
 응용 프로그램은 사용자나 다른 외부 에이전트로부터 외부 입력을 받아 그에 기반한 동작을 수행하는 경우가 많습니다.  LINQ to DataSet의 경우 응용 프로그램을 특정 방식으로 외부 입력 또는 사용 하 여 외부 쿼리에서 입력을 기반으로 쿼리를 작성 될 수 있습니다. LINQ to DataSet 쿼리 everywhere는 리터럴을 사용할 매개 변수를 수락 합니다. 응용 프로그램 개발자는 외부 에이전트에서 쿼리로 직접 리터럴을 삽입하는 대신 매개 변수가 있는 쿼리를 사용해야 합니다.  
