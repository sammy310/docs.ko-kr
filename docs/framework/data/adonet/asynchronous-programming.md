...
.NET Framework 4.5 이전에는 다음 메서드와 연결 속성을 사용 하 여 SqlClient를 사용한 비동기 프로그래밍이 완료 되었습니다 `Asynchronous Processing=true` .
...
> [!TIP]
> .NET Framework 4.5부터 이러한 레거시 메서드는 연결 문자열에 더 이상 필요 하지 않습니다 `Asynchronous Processing=true` .
...
> [!TIP]
> 새 비동기 메서드는 `Asynchronous Processing=true` 연결 문자열에 필요 하지 않습니다.
