...
다음 다이어그램에서는 세 가지 엔터티 형식 `Book`, `Publisher` 및 `Author`가 포함된 개념적 모델을 보여 줍니다. 탐색 속성 `Publisher` 및 `Authors`는 서적 엔터티 형식에 정의 되어 있습니다. 탐색 속성 `Books`는 Publisher 엔터티 형식과 `Author` 엔터티 형식에 모두 정의됩니다.
...
XML 특성은 탐색 속성을 정의 하는 데 필요한 정보를 전달 하는 데 사용 됩니다. 속성의 이름을 포함 하는 `Name` 특성에는 탐색 하는 연결의 이름을 포함 하 `Relationship`, 연결의 끝을 포함 하는 `FromRole` 및 `ToRole` 포함 됩니다.
