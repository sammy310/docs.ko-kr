...
- Oracle에서는 테이블의 열에 대해 새 값을 생성하기 위해 시퀀스를 만들지만 시퀀스와 테이블 또는 열은 서로 독립적입니다. Oracle 시퀀스는 테이블이나 저장 프로시저와 같은 개체입니다.  
 ...
 SQL Server 및 ADO.NET 사용하여 자동 증분 열을 만드는 자세한 내용은 [ID 또는 자동 번호 값 검색](retrieving-identity-or-autonumber-values.md) 및 자동 [증분 열 만들기를](./dataset-datatable-dataview/creating-autoincrement-columns.md)참조하십시오.  
