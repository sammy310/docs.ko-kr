...
포함 된 매니페스트 파일을 사용 하 여 구축 해야 응용 프로그램 보호 된 시스템 UI에 액세스 합니다 `uiAccess` 특성을 `requestedExecutionLevel` 태그를 다음과 같이:
...
`uiAccess` 기본적으로 "false" 즉, 특성을 생략 하거나 어셈블리에 대 한 매니페스트가 없을 경우 응용 프로그램은 보호 된 UI에 액세스할 수 없습니다.
