...
## <a name="servers-and-clients"></a>서버 및 클라이언트  
 ...
 UI 자동화 공급자 Active Accessibility 클라이언트로 정보를 제공할 수 있습니다 하 고 Active Accessibility 서버는 UI 자동화 클라이언트 응용 프로그램에 대 한 정보를 제공할 수 있습니다. 그러나 Active Accessibility에서 많은 정보를 노출 하지 않으므로 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)], 두 모델이 완전히 호환 되지는 않습니다.  
  
<a name="UI_Elements_compare"></a>   
## <a name="ui-elements"></a>UI 요소  
 Active Accessibility 표시 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 요소 하나로 `IAccessible` 인터페이스 또는 자식 식별자입니다. 두 `IAccessible` 포인터를 비교하여 동일한 요소를 참조하는지 확인하기는 어렵습니다.  
...
## <a name="tree-views-and-navigation"></a>트리 뷰 및 탐색  
 ...
 Active Accessibility 최종 사용자에 게 관련 되지 않은 여러 자동화 요소가 트리에 노출 됩니다. 클라이언트 애플리케이션은 모든 요소를 확인하여 의미 있는 요소를 파악해야 합니다.  
 ...
 Active Accessibility에, 요소 간 탐색은 공간 (예: 화면의 왼쪽에 있는 요소로 이동), (예를 들어 다음 메뉴 항목 또는 대화 상자 내에서 탭 순서에서 다음 항목으로 이동), 논리 또는 계층적 ( 예를 들어, 이동 또는 자식 컨테이너의 첫 번째 자식 부모의). 계층적 탐색의 경우, 자식 요소가 `IAccessible`을 구현하는 개체가 아닐 수도 있기 때문에 복잡한 방법입니다.  
...
## <a name="roles-and-control-types"></a>역할 및 컨트롤 형식  
 ...
 다음 표에서 Active Accessibility 역할 매핑의 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 형식을 제어 합니다.  
...
|`get_accHelpTopic`|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]||  
 ...
 다음 표에 나와 있는 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] Active Accessibility 상태 상수에 해당 하는 속성입니다.  
...
|STATE_SYSTEM_UNAVAILABLE|<xref:System.Windows.Automation.AutomationElement.IsEnabledProperty>|Y|  
 ...
 대부분의 Active Accessibility에서 구현 되지 상태 서버를 제어 하거나 없는 해당 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]합니다.  
...
|STATE_SYSTEM_EXTSELECTABLE|Active Accessibility 서버에서 구현 되 지도 않고 익숙하지도|  
|STATE_SYSTEM_MARQUEED|Active Accessibility 서버에서 구현 되 지도 않고 익숙하지도|  
|STATE_SYSTEM_SELFVOICING|Active Accessibility 서버에서 구현 되 지도 않고 익숙하지도|  
|STATE_SYSTEM_TRAVERSED|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]|  
|STATE_SYSTEM_ALERT_HIGH|Active Accessibility 서버에서 구현 되 지도 않고 익숙하지도|  
|STATE_SYSTEM_ALERT_MEDIUM|Active Accessibility 서버에서 구현 되 지도 않고 익숙하지도|  
|STATE_SYSTEM_ALERT_LOW|Active Accessibility 서버에서 구현 되 지도 않고 익숙하지도|  
|STATE_SYSTEM_FLOATING|Active Accessibility 서버에서 구현 되 지도 않고 익숙하지도|  
...
## <a name="events"></a>이벤트  
 이벤트 메커니즘은 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)], Active Accessibility에는 달리에 의존 하지 않는 Windows 이벤트 라우팅 (창 핸들과 밀접 하 게 연관 됨)는 클라이언트 응용 프로그램 후크를 설정할 필요가 없습니다. 이벤트에 대한 구독은 특정 이벤트뿐만 아니라 트리의 특정 부분에 대해서도 미세하게 조정할 수 있습니다. 공급자는 이벤트가 수신 대기하고 있는 사항을 추적하여 이벤트 발생을 미세 조정할 수 있습니다.  
  
 요소가 이벤트 콜백에 직접 전달되므로 클라이언트는 이벤트를 발생하는 요소를 쉽게 검색할 수 있습니다. 클라이언트가 이벤트를 구독할 때 캐시 요청이 활성화되어 있으면 요소의 속성이 자동으로 프리페치됩니다.  
  
 다음 표에서 Active Accessibility WinEvents 보여줍니다 및 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 이벤트입니다.  
...
|EVENT_OBJECT_REORDER|Active Accessibility에 사용 되는 일관 되지 않습니다. 직접적인 해당 이벤트가 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]에 정의되지 않았습니다.|  
