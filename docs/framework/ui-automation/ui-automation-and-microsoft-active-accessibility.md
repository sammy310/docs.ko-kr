...
> 이 설명서는 <xref:System.Windows.Automation> 네임스페이스에 정의된 관리되는 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 클래스를 사용하려는 .NET Framework 개발자를 위한 것입니다. [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]에 대한 최신 정보는 [Windows 자동화 API: UI 자동화](/windows/win32/winauto/entry-uiauto-win32)를 참조하세요.  
 ...
 Microsoft 활성 접근성은 응용 프로그램에 액세스할 수 있도록 하기 위한 이전 솔루션이었습니다. [!INCLUDE[TLA#tla_uiautomation](../../../includes/tlasharptla-uiautomation-md.md)]는 Microsoft Windows용 의 새로운 내게 필요한 옵션 모델이며 보조 기술 제품 및 자동화된 테스트 도구의 요구 사항을 해결하기 위한 것입니다. [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]활성 접근성에 비해 많은 개선 사항이 제공됩니다.  
  
 이 항목에는 이러한 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 기능의 주요 기능이 포함되어 있으며 이러한 기능이 활성 접근성과 어떻게 다른지 설명합니다.  
  
<a name="Programming_Languages_compare"></a>
## <a name="programming-languages"></a>프로그래밍 언어  
<활성 접근성은 이중 인터페이스를 지원하는 구성 요소 개체 모델(COM)을 기반으로 하므로 C/C++, Microsoft Visual Basic 6.0 및 스크립팅 언어에서 프로그래밍할 수 있습니다. [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]표준 컨트롤에 대한 클라이언트 측 공급자 라이브러리 포함)은 관리 코드로 작성되며 UI 자동화 클라이언트 응용 프로그램은 C# 또는 Visual Basic .NET을 사용하여 가장 쉽게 프로그래밍됩니다. 인터페이스 구현인 UI 자동화 공급자는 관리되는 코드 또는 C/C++로 작성할 수 있습니다.  
  
<a name="Support_in_Windows_Presentation_Foundation_"></a>
## <a name="support-in-windows-presentation-foundation"></a>Windows Presentation Foundation에서의 지원  
 WPF(Windows 프레젠테이션 파운데이션)는 사용자 인터페이스를 만들기 위한 새로운 모델입니다. WPF 요소에는 활성 내게 필요한 옵션에 대한 기본 지원이 포함되어 있지 않습니다. 그러나 활성 내게 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]필요한 옵션 클라이언트에 대한 브리징 지원을 포함하는 지원을 수행합니다. 전용으로 작성된 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 클라이언트만 텍스트에 대한 풍부한 지원과 같은 WPF의 내게 필요한 옵션 기능을 최대한 활용할 수 있습니다.  
  
<a name="Servers_and_Clients_compare"></a>
## <a name="servers-and-clients"></a>서버 및 클라이언트  
 활성 내게 필요한 옵션에서 서버와 클라이언트는 주로 서버의 `IAccessible`구현을 통해 직접 통신합니다.  
 ...
 UI 자동화 공급자는 활성 내게 필요한 옵션 클라이언트에 정보를 제공할 수 있으며 활성 내게 필요한 옵션 서버는 UI 자동화 클라이언트 응용 프로그램에 정보를 제공할 수 있습니다. 그러나 Active Accessibility는 많은 정보를 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]노출하지 않으므로 두 모델이 완전히 호환되지 않습니다.  
  
<a name="UI_Elements_compare"></a>
## <a name="ui-elements"></a>UI 요소  
 활성 접근성은 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] `IAccessible` 요소를 인터페이스 또는 자식 식별자로 제공합니다. 두 `IAccessible` 포인터를 비교하여 동일한 요소를 참조하는지 확인하기는 어렵습니다.  
...
## <a name="tree-views-and-navigation"></a>트리 뷰 및 탐색  
 ...
 활성 내게 필요한 옵션에서 최종 사용자와 관련이 없는 많은 자동화 요소가 트리에 노출됩니다. 클라이언트 애플리케이션은 모든 요소를 확인하여 의미 있는 요소를 파악해야 합니다.  
 ...
 활성 접근성에서 요소 간의 탐색은 공간(예: 화면 왼쪽에 있는 요소로 이동), 논리적(예: 다음 메뉴 항목으로 이동하거나 대화 상자 내의 탭 순서의 다음 항목) 또는 계층적 ( 예를 들어, 컨테이너에서 첫 번째 자식또는 자식에서 상위로 이동). 계층적 탐색의 경우, 자식 요소가 `IAccessible`을 구현하는 개체가 아닐 수도 있기 때문에 복잡한 방법입니다.  
  
 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]에서, 모든 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 요소는 동일한 기본 기능을 지원하는 <xref:System.Windows.Automation.AutomationElement> 개체입니다. 공급자의 관점에서 .) 상속된 <xref:System.Windows.Automation.Provider.IRawElementProviderSimple>인터페이스를 구현하는 개체입니다. 탐색은 주로 계층적입니다: 부모에서 자식, 그리고 한 형제에서 다음 형제로. 형제 간 탐색에는 탭 순서를 따를 수 있으므로 논리적 요소가 있습니다. 클래스를 사용하여 트리의 필터링된 뷰를 사용하여 모든 시작 지점에서 <xref:System.Windows.Automation.TreeWalker> 탐색할 수 있습니다. <xref:System.Windows.Automation.AutomationElement.FindFirst%2A> 및 <xref:System.Windows.Automation.AutomationElement.FindAll%2A>을 사용하여 특정 자식 또는 하위 항목을 탐색할 수도 있습니다. 예를 들어, 지정된 컨트롤 패턴을 지원하는 대화 상자 내에서 모든 요소를 쉽게 검색할 수 있습니다.  
  
 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 탐색은 활성 접근성보다 더 일관적입니다. 드롭다운 목록 및 팝업 창과 같은 일부 요소는 활성 내게 필요한 옵션 트리에 두 번 표시되며 탐색결과 예기치 않은 결과가 발생할 수 있습니다. 철근 컨트롤에 대한 활성 접근성을 제대로 구현하는 것은 실제로 불가능합니다. [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 을 사용하면 부모 재지정 및 위치 변경이 가능하므로, 창 소유권에 의해 계층 구조가 적용되더라도 트리에서 요소를 원하는 위치에 배치할 수 있습니다.  
  
<a name="Roles_and_Control_Types"></a>
## <a name="roles-and-control-types"></a>역할 및 컨트롤 형식  
 Active Accessibility는 `accRole` 속성`IAccessible::get_actRole`() )을 사용하여 ROLE_SYSTEM_SLIDER 또는 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)]ROLE_SYSTEM_MENUITEM 같은 요소의 역할에 대한 설명을 검색합니다. 요소의 역할을 통해 사용 가능한 기능을 알 수 있습니다. `IAccessible::accSelect` 및 `IAccessible::accDoDefaultAction`과 같은 고정 메서드를 사용하여 컨트롤과 상호 작용할 수 있습니다. 클라이언트 애플리케이션과 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 간의 상호 작용은 `IAccessible`을 통해서 수행할 수 있는 작업으로 제한됩니다.  
 ...
 다음 표에서는 형식을 제어하는 활성 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 접근성 역할의 매핑을 보여 주었습니다.  
  
|활성 접근성 역할|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 컨트롤 형식|  
|----------------------------------------------------------------------|----------------------------------------------------------------------------------------|  
...
|활성 내게 필요한 옵션 속성 접근자|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 속성 ID|설명|  
|-----------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|-------------|  
...
|`get_accHelpTopic`|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]||  
 ...
 다음 표에서는 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 활성 내게 필요한 옵션 상태 상수에 해당하는 속성을 보여 주며, 이에 해당합니다.  
  
|활성 접근성 상태|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 속성|상태 변경 트리거 여부|  
|-----------------------------------------------------------------------|------------------------------------------------------------------------------------|----------------------------|  
...
|STATE_SYSTEM_SELECTABLE|<xref:System.Windows.Automation.SelectionItemPattern>가 지원됨|N|  
...
|STATE_SYSTEM_UNAVAILABLE|<xref:System.Windows.Automation.AutomationElement.IsEnabledProperty>|Y|  
 ...
 다음 상태는 대부분의 Active Accessibility 제어 서버에 의해 구현되지 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]않았거나 에 해당하지 않습니다.  
...
|STATE_SYSTEM_EXTSELECTABLE|활성 내게 필요한 옵션 서버에서 광범위하게 구현되지 않음|  
|STATE_SYSTEM_MARQUEED|활성 내게 필요한 옵션 서버에서 광범위하게 구현되지 않음|  
|STATE_SYSTEM_SELFVOICING|활성 내게 필요한 옵션 서버에서 광범위하게 구현되지 않음|  
|STATE_SYSTEM_TRAVERSED|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]|  
|STATE_SYSTEM_ALERT_HIGH|활성 내게 필요한 옵션 서버에서 광범위하게 구현되지 않음|  
|STATE_SYSTEM_ALERT_MEDIUM|활성 내게 필요한 옵션 서버에서 광범위하게 구현되지 않음|  
|STATE_SYSTEM_ALERT_LOW|활성 내게 필요한 옵션 서버에서 광범위하게 구현되지 않음|  
|STATE_SYSTEM_FLOATING|활성 내게 필요한 옵션 서버에서 광범위하게 구현되지 않음|  
...
|STATE_SYSTEM_PRESSED|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]|  
 ...
 속성 식별자의 전체 목록은 [UI 자동화 속성 개요를](ui-automation-properties-overview.md)참조하십시오. [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]  
  
<a name="uiautomation_events_compare"></a>
## <a name="events"></a>이벤트  
 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]의 이벤트 메커니즘은 활성 내게 필요한 옵션과 달리 Windows 이벤트 라우팅(창 핸들과 밀접하게 연결되어 있음)에 의존하지 않으며 클라이언트 응용 프로그램에서 후크를 설정할 필요가 없습니다. 이벤트에 대한 구독은 특정 이벤트뿐만 아니라 트리의 특정 부분에 대해서도 미세하게 조정할 수 있습니다. 공급자는 이벤트가 수신 대기하고 있는 사항을 추적하여 이벤트 발생을 미세 조정할 수 있습니다.  
  
 요소가 이벤트 콜백에 직접 전달되므로 클라이언트는 이벤트를 발생하는 요소를 쉽게 검색할 수 있습니다. 클라이언트가 이벤트를 구독할 때 캐시 요청이 활성화되어 있으면 요소의 속성이 자동으로 프리페치됩니다.  
  
 다음 표에서는 활성 접근성 WinEvents [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 및 이벤트의 대응을 보여 주며,  
...
|EVENT_OBJECT_REORDER|활성 접근성에서 일관되게 사용되지 않습니다. 직접적인 해당 이벤트가 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]에 정의되지 않았습니다.|  
