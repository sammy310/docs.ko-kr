...
|<xref:System.Windows.Automation.TextPattern.RangeFromChild%2A> 여기서 <xref:System.Windows.Automation.AutomationElement> 이전에 반환 되는 개체 `GetChildren` 메서드.|나타내는 범위를 반환 합니다. "https://www.microsoft.com" 입니다.|  
...
|<xref:System.Windows.Automation.Text.TextPatternRange.GetEnclosingElement%2A>|텍스트 범위를 포함하는 가장 안쪽의 <xref:System.Windows.Automation.AutomationElement> 를 반환합니다. 이 예제에서는 텍스트 공급자 자체를 나타내는 <xref:System.Windows.Automation.AutomationElement> 입니다.|  
|<xref:System.Windows.Automation.Text.TextPatternRange.Move%2A> (TextUnit.Word, 1) 매개 변수입니다.|하이퍼링크의 텍스트가 개별 단어로 이루어져 있으므로 텍스트 범위를 "http"로 이동합니다. 이 경우 하이퍼링크는 단일 개체로 처리되지 않습니다.<br /><br /> {0} [http]} URL 텍스트에 포함 됩니다.|  
...
|<xref:System.Windows.Automation.TextPattern.RangeFromChild%2A> 여기서 <xref:System.Windows.Automation.AutomationElement> 이전에 반환 되는 개체 <xref:System.Windows.Automation.Text.TextPatternRange.GetChildren%2A> 메서드.|나타내는 중복 제거 범위를 반환 합니다. "![포함 된 이미지 예제](../../../docs/framework/ui-automation/media/uia-textpattern-embedded-objects-overview-imageexample.PNG "UIA_TextPattern_Embedded_Objects_Overview_ImageExample")"입니다.|  
...
|<xref:System.Windows.Automation.Text.TextPatternRange.GetEnclosingElement%2A>|텍스트 범위를 포함하는 가장 안쪽의 <xref:System.Windows.Automation.AutomationElement> 를 반환합니다. 이 예제에서는 텍스트 공급자 자체를 나타내는 <xref:System.Windows.Automation.AutomationElement> 입니다.|  
|<xref:System.Windows.Automation.Text.TextPatternRange.Move%2A> (TextUnit.Word, 1) 매개 변수입니다.|텍스트 범위를 "is "로 이동합니다. 텍스트 기반의 포함된 개체만 텍스트 스트림의 일부로 간주되므로, 이 예제의 이미지는 이동 또는 해당 반환 값(이 경우 1)에 영향을 주지 않습니다.|  
...
|호출되는 메서드|결과|  
|-------------------|------------|  
|<xref:System.Windows.Automation.GridPattern.GetItem%2A> 매개 변수 (0, 0)를 사용 하 여|테이블 셀의 내용을 나타내는 <xref:System.Windows.Automation.AutomationElement>를 반환합니다. 이 예제에서 요소는 텍스트 컨트롤입니다.|  
|<xref:System.Windows.Automation.TextPattern.RangeFromChild%2A> 여기서 <xref:System.Windows.Automation.AutomationElement> 이전에 반환 되는 개체 `GetItem` 메서드.|이미지를 해당 하는 범위를 반환 ![포함 된 이미지 예제](../../../docs/framework/ui-automation/media/uia-textpattern-embedded-objects-overview-imageexample.PNG "UIA_TextPattern_Embedded_Objects_Overview_ImageExample")합니다.|  
|<xref:System.Windows.Automation.Text.TextPatternRange.GetEnclosingElement%2A> 이전에 반환 되는 개체에 대 한 `RangeFromChild` 메서드.|테이블 셀을 나타내는 <xref:System.Windows.Automation.AutomationElement> 를 반환합니다. 이 예제에서 요소는 TableItemPattern을 지원하는 텍스트 컨트롤입니다.|  
|<xref:System.Windows.Automation.Text.TextPatternRange.GetEnclosingElement%2A> 이전에 반환 되는 개체에 대 한 `GetEnclosingElement` 메서드.|테이블을 나타내는 <xref:System.Windows.Automation.AutomationElement> 를 반환합니다.|  
|<xref:System.Windows.Automation.Text.TextPatternRange.GetEnclosingElement%2A> 이전에 반환 되는 개체에 대 한 `GetEnclosingElement` 메서드.|텍스트 공급자 자체를 나타내는 <xref:System.Windows.Automation.AutomationElement> 를 반환합니다.|  
  
 **예제 2 - 셀의 텍스트 콘텐츠 가져오기**  
  
|호출되는 메서드|결과|  
|-------------------|------------|  
|<xref:System.Windows.Automation.GridPattern.GetItem%2A> 사용 하 여 매개 변수 (1, 1)입니다.|테이블 셀의 내용을 나타내는 <xref:System.Windows.Automation.AutomationElement>를 반환합니다. 이 예제에서 요소는 텍스트 컨트롤입니다.|  
|<xref:System.Windows.Automation.TextPattern.RangeFromChild%2A> 여기서 <xref:System.Windows.Automation.AutomationElement> 이전에 반환 되는 개체 `GetItem` 메서드.|"Y"를 반환합니다.|  
