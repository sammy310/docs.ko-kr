...
- **표시 속성** 대화 상자의 **화면 해상도** 슬라이더 또는 Microsoft Word의 **색 선택** 선택 컨트롤 (아래 그림 참조)과 같이 <xref:System.Windows.Automation.Provider.IRawElementProviderFragmentRoot>를 구현 하는 자식 컨트롤을 관리 하는 단일 선택 컨트롤은 <xref:System.Windows.Automation.Provider.ISelectionProvider>를 구현 해야 합니다. 해당 자식은 <xref:System.Windows.Automation.Provider.IRawElementProviderFragment>와 <xref:System.Windows.Automation.Provider.ISelectionItemProvider>를 모두 구현 해야 합니다.  
