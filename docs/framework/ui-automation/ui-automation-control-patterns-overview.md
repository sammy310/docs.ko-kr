...
## <a name="ui-automation-providers-and-clients"></a>UI 자동화 공급자 및 클라이언트  
 ...
 클라이언트는 <xref:System.Windows.Automation.AutomationElement> <xref:System.Windows.Automation.AutomationElement.GetCurrentPropertyValue%2A?displayProperty=nameWithType> 메서드(예: <xref:System.Windows.Automation.AutomationElement.GetCachedPropertyValue%2A?displayProperty=nameWithType>또는) 또는 공통 언어 런타임(CLR) [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 접근자를 사용하여 패턴의 속성에 액세스합니다. 각 컨트롤 패턴 클래스에는 해당 컨트롤 <xref:System.Windows.Automation.InvokePattern.Pattern?displayProperty=nameWithType> <xref:System.Windows.Automation.SelectionPattern.Pattern?displayProperty=nameWithType>패턴을 식별하는 필드 멤버(예: 또는)가 <xref:System.Windows.Automation.AutomationElement.GetCachedPattern%2A> 있으며 <xref:System.Windows.Automation.AutomationElement.GetCurrentPattern%2A> <xref:System.Windows.Automation.AutomationElement>에 대한 해당 패턴을 검색하거나 매개 변수로 전달할 수 있습니다.  
...
|<xref:System.Windows.Automation.GridPattern>|<xref:System.Windows.Automation.Provider.IGridProvider>|지정된 셀로 이동 및 크기 조정과 같은 표 기능을 지원하는 컨트롤에 사용됩니다. 예를 들어, 윈도우 탐색기에서 큰 아이콘보기 또는 마이크로 소프트 워드의 헤더가없는 간단한 테이블.|  
|<xref:System.Windows.Automation.GridItemPattern>|<xref:System.Windows.Automation.Provider.IGridItemProvider>|표 내에서 셀이 있는 컨트롤에 사용됩니다. 개별 셀은 GridItem 패턴을 지원해야 합니다. 예를 들어 Microsoft Windows 탐색기 세부 정보 보기의 각 셀입니다.|  
...
|<xref:System.Windows.Automation.TablePattern>|<xref:System.Windows.Automation.Provider.ITableProvider>|헤더 정보 및 표가 있는 컨트롤에 사용됩니다. 예를 들어, 마이크로 소프트 엑셀 워크 시트.|  
...
|<xref:System.Windows.Automation.WindowPattern>|<xref:System.Windows.Automation.Provider.IWindowProvider>|Windows와 관련된 정보를 Microsoft Windows 운영 체제에 기본 개념으로 노출합니다. 창인 컨트롤의 예로는 최상위 응용 프로그램 창(Microsoft Word, Microsoft Windows 탐색기 등), 여러 문서 인터페이스(MDI) 자식 창 및 대화 상자가 있습니다.|  
