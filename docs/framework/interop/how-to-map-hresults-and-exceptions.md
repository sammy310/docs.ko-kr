...
```
 ...
 다음 표에서는 HRESULT와 .NET의 비교 가능한 예외 클래스 간의 일반적인 매핑을 제공합니다. 명시적 매핑이 없는 HRESULT 값은 `COMException`에 매핑됩니다. 전체 최신 매핑은 [dotnet/runtime 리포지토리](https://github.com/dotnet/runtime/blob/master/src/coreclr/vm/rexcep.h)에서 찾을 수 있습니다.
...
|`Message`|`IErrorInfo->GetDescription`에서 반환된 문자열.|
