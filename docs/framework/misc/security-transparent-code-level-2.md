...
- 메서드에 대한 규칙: 파생 메서드는 기본 메서드의 접근성을 변경할 수 없습니다. 기본 동작의 경우 주석으로 처리되지 않은 모든 파생 메서드는 `Transparent`입니다. 재정의된 메서드가 `SecurityCritical`로 명시적으로 주석을 달지 않으면 중요한 형식의 파생 항목으로 인해 예외가 throw됩니다.
