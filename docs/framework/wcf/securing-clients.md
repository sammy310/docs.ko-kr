...
### <a name="determining-the-client-credential-type"></a>클라이언트 자격 증명 형식 결정  
 Svcutil.exe 도구가 생성 한 구성 파일이 있는 경우 [\<bindings >](../configure-apps/file-schema/wcf/bindings.md) 섹션을 검토 하 여 필요한 클라이언트 자격 증명 형식을 확인 합니다. 이 단원에는 보안 요구 사항을 지정하는 바인딩 요소가 나와 있습니다. 특히 각 바인딩의 \<security > 요소를 검사 합니다. 해당 요소에는 `mode` 특성이 포함되어 있으며, 이 특성은 세 가지 가능한 값 즉, `Message`, `Transport` 또는 `TransportWithMessageCredential` 중 하나로 설정할 수 있습니다. 특성 값은 모드를 결정하고 모드는 자식 요소 중 중요한 요소를 결정합니다.  
...
- 클라이언트 코드에서 자격 증명 프로그래밍(`SetCertificate` 메서드 사용).  
 ...
 클라이언트에 대 한 구성 파일의 [\<behaviors >](../configure-apps/file-schema/wcf/behaviors.md) 섹션을 추가 하 고 아래와 같이 `clientCredentials` 요소를 사용 합니다.  
  
#### <a name="setting-a-clientcredentials-value-in-code"></a>코드에서 \<clientCredentials > 값 설정  
 코드에서 [\<clientCredentials >](../configure-apps/file-schema/wcf/clientcredentials.md) 값을 설정 하려면 <xref:System.ServiceModel.ClientBase%601> 클래스의 <xref:System.ServiceModel.ClientBase%601.ClientCredentials%2A> 속성에 액세스 해야 합니다. 다음 표에 설명한 것처럼 속성은 다양한 자격 증명 형식에 액세스할 수 있는 <xref:System.ServiceModel.Description.ClientCredentials> 개체를 반환합니다.  
...
#### <a name="setting-a-clientcredentials-value-in-configuration"></a>구성에서 \<clientCredentials > 값 설정  
 자격 증명 값은 [\<clientCredentials >](../configure-apps/file-schema/wcf/clientcredentials.md) 요소의 자식 요소로 끝점 동작을 사용 하 여 지정 됩니다. 사용되는 요소는 클라이언트 자격 증명 형식에 따라 다릅니다. 예를 들어 다음 예제에서는 <[ \<clientCertificate >](../configure-apps/file-schema/wcf/clientcertificate-of-clientcredentials-element.md)를 사용 하 여 x.509 인증서를 설정 하는 구성을 보여 줍니다.  
...
```  
 ...
 구성에서 클라이언트 자격 증명을 설정 하려면 구성 파일에 [\<endpointBehaviors >](../configure-apps/file-schema/wcf/endpointbehaviors.md) 요소를 추가 합니다. 또한 다음 예제와 같이 [\<client > 요소의 \<endpoint >](../configure-apps/file-schema/wcf/endpoint-of-client.md) `behaviorConfiguration` 특성을 사용 하 여 추가 된 동작 요소를 서비스의 끝점에 연결 해야 합니다. `behaviorConfiguration` 특성 값은 동작 `name` 특성 값과 일치해야 합니다.  
