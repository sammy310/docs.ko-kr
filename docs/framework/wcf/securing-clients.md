...
## <a name="validating-security"></a>보안 유효성 검사  
 ...
 그러나 잘 모르는 소스에서 클라이언트 및 메타데이터를 다운로드하려는 경우 코드가 사용하는 보안 방법의 유효성을 확인해야 합니다. 예를 들어, 서비스가 최소한의 수준에서 기밀성 및 무결성을 요구하지 않는 한 단순히 개인 정보 또는 재무 정보를 서비스에 보내는 클라이언트를 만들지 않아야 합니다. 귀하는 그러한 정보가 그들에게 표시되기 때문에 그러한 정보를 공개하고자하는 범위내에서 서비스 소유자를 신뢰해야합니다.  
...
1. 서비스에 필요한 *클라이언트 자격 증명 유형을* 결정합니다. 이 방법은 두 개의 메서드 중 하나에 의해 수행됩니다. 첫째로, 서비스 작성자의 설명서가 있는 경우 서비스에서 필요한 클라이언트 자격 증명 형식(있는 경우)을 지정해야 합니다. 둘째로, Svcutil.exe 도구로 생성된 구성 파일만 있는 경우 개별 바인딩을 검사하여 필요한 자격 증명 형식을 결정할 수 있습니다.  
  
2. 실제 클라이언트 자격 증명을 지정합니다. 실제 클라이언트 자격 증명을 형식과 구별하기 위해 *클라이언트 자격 증명 값이라고* 합니다. 예를 들어, 클라이언트 자격 증명 형식이 인증서를 지정할 경우 서비스가 신뢰하는 인증 기관에서 발급한 X.509 인증서를 제공해야 합니다.  
  
### <a name="determining-the-client-credential-type"></a>클라이언트 자격 증명 형식 결정  
 Svcutil.exe 도구가 생성된 구성 파일이 있는 경우 [ \<바인딩>](../configure-apps/file-schema/wcf/bindings.md) 섹션을 검사하여 필요한 클라이언트 자격 증명 유형을 확인합니다. 이 단원에는 보안 요구 사항을 지정하는 바인딩 요소가 나와 있습니다. 특히 각 \<바인딩의 보안> 요소를 검사합니다. 해당 요소에는 `mode` 특성이 포함되어 있으며, 이 특성은 세 가지 가능한 값 즉, `Message`, `Transport` 또는 `TransportWithMessageCredential` 중 하나로 설정할 수 있습니다. 특성 값은 모드를 결정하고 모드는 자식 요소 중 중요한 요소를 결정합니다.  
...
- 클라이언트 코드에서 자격 증명 프로그래밍(`SetCertificate` 메서드 사용).  
 ...
 클라이언트에 [ \<](../configure-apps/file-schema/wcf/behaviors.md) 대한 구성 파일의 비헤이비어>섹션을 `clientCredentials` 추가하고 요소를 사용합니다(아래 표시).  
  
#### <a name="setting-a-clientcredentials-value-in-code"></a>코드에서 \<클라이언트 자격 증명> 값 설정  
 코드에서 [ \<clientCredentials>](../configure-apps/file-schema/wcf/clientcredentials.md) 값을 설정하려면 <xref:System.ServiceModel.ClientBase%601.ClientCredentials%2A> <xref:System.ServiceModel.ClientBase%601> 클래스의 속성에 액세스해야 합니다. 다음 표에 설명한 것처럼 속성은 다양한 자격 증명 형식에 액세스할 수 있는 <xref:System.ServiceModel.Description.ClientCredentials> 개체를 반환합니다.  
  
|ClientCredential 속성|Description|메모|  
|-------------------------------|-----------------|-----------|  
...
#### <a name="setting-a-clientcredentials-value-in-configuration"></a>클라이언트 \<자격 증명> 구성에서 값 설정  
 자격 증명 값은 [ \<끝점](../configure-apps/file-schema/wcf/clientcredentials.md) 동작을 사용하여 클라이언트 자격 증명>요소의 자식 요소로 지정됩니다. 사용되는 요소는 클라이언트 자격 증명 형식에 따라 다릅니다. 예를 들어 다음 예제에서는 <[ \<clientCertificate>](../configure-apps/file-schema/wcf/clientcertificate-of-clientcredentials-element.md)사용하여 X.509 인증서를 설정하는 구성을 보여 주며 있습니다.  
...
```  
 ...
 구성에서 클라이언트 자격 증명을 설정하려면 [ \<구성 파일에 끝점>](../configure-apps/file-schema/wcf/endpointbehaviors.md) 요소를 추가합니다. 또한 추가된 동작 요소는 다음 예제와 같이 클라이언트 `behaviorConfiguration`>요소의 [ \<끝점 \<> 특성을](../configure-apps/file-schema/wcf/endpoint-of-client.md) 사용하여 서비스의 끝점에 연결되어야 합니다. `behaviorConfiguration` 특성 값은 동작 `name` 특성 값과 일치해야 합니다.  
...
> 예를 들어 사용자 이름 및 암호 또는 Windows 사용자 및 암호 값과 같은 일부 클라이언트 자격 증명 값은 애플리케이션 구성 파일을 사용하여 설정할 수 없습니다. 이러한 자격 증명 값은 코드에서만 지정할 수 있습니다.  
 ...
 클라이언트 자격 증명 설정에 대한 자세한 내용은 클라이언트 [자격 증명 값 지정 방법을](how-to-specify-client-credential-values.md)참조하십시오.  
