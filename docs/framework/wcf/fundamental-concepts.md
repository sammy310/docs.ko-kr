...
### <a name="messaging-and-endpoints"></a>메시징 및 엔드포인트  
 ...
 모델을 구별 *클라이언트*, 통신을 시작 하는 응용 프로그램인 및 *services*, 클라이언트가 통신 하는 응답할 때까지 대기 하는 응용 프로그램인 통신 합니다. 단일 응용 프로그램이 클라이언트 및 서비스 모두로 작동할 수 있습니다. 예제를 보려면 [이중 서비스](../../../docs/framework/wcf/feature-details/duplex-services.md) 하 고 [피어-투-피어 네트워킹](../../../docs/framework/wcf/feature-details/peer-to-peer-networking.md)합니다.  
  
 메시지는 엔드포인트 간에 전송됩니다. *끝점* 메시지 발신 이나 수신이 있는 위치 (또는 둘 다)은 및 메시지 교환에 필요한 모든 정보를 정의 합니다. 서비스는 하나 이상의 응용 프로그램 엔드포인트(및 0 이상의 인프라 엔드포인트)를 노출하고 클라이언트는 서비스 엔드포인트 중 하나와 호환되는 엔드포인트를 만듭니다.  
  
 *끝점* 메시지를 보낼 위치, 어떻게 이러한 보내야 및 메시지 모양을 표준 기반 방식에서에 대해 설명 합니다. 서비스 클라이언트 생성 적절 한 WCF 클라이언트 및 통신을 처리할 수 있는 메타 데이터로이 정보를 노출할 수 있습니다 *스택*합니다.  
  
### <a name="communication-protocols"></a>통신 프로토콜  
 통신 스택의 요소는 하나가 필요 합니다 *전송 프로토콜*합니다. HTTP 및 TCP와 같은 일반적인 전송을 사용하여 인트라넷 및 인터넷을 통해 메시지를 전송할 수 있습니다. 기타 전송으로는 피어 네트워킹 메시에서 메시지 큐 응용 프로그램과 노드의 통신을 지원하는 전송이 있습니다. WCF의 기본 제공 확장 점을 사용 하 여 추가 전송 메커니즘을 추가할 수 있습니다.  
...
## <a name="wcf-terms"></a>WCF 용어  
 ...
 응용 프로그램 엔드포인트(application endpoint)  
...
```  
 ...
 호스팅(hosting)  
 일부 프로세스의 경우 서비스를 호스트해야 합니다. A *호스트* 서비스의 수명을 제어 하는 응용 프로그램입니다. 서비스는 기존 호스팅 프로세스에 의해 자체 호스트되거나 관리될 수 있습니다.  
