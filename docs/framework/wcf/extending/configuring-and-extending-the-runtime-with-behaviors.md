...
3. 구성을 확장하는 사용자 지정 <xref:System.ServiceModel.Configuration.BehaviorExtensionElement> 구현. 이를 통해 애플리케이션 구성 파일에서 서비스 동작을 사용할 수 있습니다.  
 ...
 WCF의 서비스 동작 예제에는 <xref:System.ServiceModel.ServiceBehaviorAttribute> 특성, <xref:System.ServiceModel.Description.ServiceThrottlingBehavior> 및 동작이 포함 됩니다 <xref:System.ServiceModel.Description.ServiceMetadataBehavior> .  
...
#### <a name="contract-behaviors"></a>계약 동작  
 ...
 • 계약 인터페이스. 이 경우 모든 끝점에서 해당 형식의 모든 계약에 동작이 적용 되며 WCF는 속성의 값을 무시 <xref:System.ServiceModel.Description.IContractBehaviorAttribute.TargetContract%2A?displayProperty=nameWithType> 합니다.  
  
 • 서비스 클래스. 이 경우 동작은 속성의 값 인 계약의 끝점에만 적용 됩니다 <xref:System.ServiceModel.Description.IContractBehaviorAttribute.TargetContract%2A> .  
  
 • 콜백 클래스. 이 경우 이중 클라이언트의 끝점에 동작이 적용 되 고 WCF는 속성의 값을 무시 합니다 <xref:System.ServiceModel.Description.IContractBehaviorAttribute.TargetContract%2A> .  
...
#### <a name="operation-behaviors"></a>작업 동작  
 ...
 작업 동작을 작업에 추가하는 데는 다음과 같은 두 가지 메커니즘이 있습니다. 첫 번째 메커니즘에서는 작업을 모델링하는 메서드에서 사용될 사용자 지정 특성을 만듭니다. 작업이 또는에 추가 되 면 <xref:System.ServiceModel.ServiceHost> <xref:System.ServiceModel.ChannelFactory> WCF  <xref:System.ServiceModel.Description.IOperationBehavior> <xref:System.ServiceModel.Description.OperationDescription> 는 해당 작업에 대해 만들어진의 동작 컬렉션에 모든 특성을 추가 합니다.  
  
 두 번째 메커니즘에서는 생성된 <xref:System.ServiceModel.Description.OperationDescription>의 동작 컬렉션에 동작을 직접 추가합니다.  
  
 WCF의 작업 동작의 예로는 <xref:System.ServiceModel.OperationBehaviorAttribute> 및가 <xref:System.ServiceModel.TransactionFlowAttribute> 있습니다.  
...
> [!NOTE]
> 또한 회사 애플리케이션 정책이 적용되도록 구성을 지원하는 동작은 machine.config 구성 파일에 삽입하고 해당 항목을 잠금으로써 사용할 수 있습니다. 설명 및 예제는 [방법: 엔터프라이즈에서 끝점 잠그기](how-to-lock-down-endpoints-in-the-enterprise.md)를 참조 하세요.  
