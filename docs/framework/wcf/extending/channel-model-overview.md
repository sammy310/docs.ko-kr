...
## <a name="channel-stack"></a>채널 스택  
 ...
 첫 번째 유사성: 두 경우 모두 스택의 각 계층 아래 계층 및 해당 추상화 바로 위 계층에만 노출 하는 세계의 일부 추상화를 제공 합니다. 각 계층에서는 바로 아래 계층에 대한 추상화만 사용합니다. 또한 두 경우 모두, 두 스택이 통신할 때 각 계층은 상대 스택의 해당 계층과 통신합니다. 예를 들어 IP 계층은 IP 계층끼리, TCP 계층은 TCP 계층끼리 통신합니다.  
  
 그렇다면 차이점: 채널 스택은 메시지 배달 방법, 즉, 전송 뿐만 아니라 메시지 또는 새로운 기능 등의 기타 기능을 추상화를 제공 하도록 설계 되었습니다 TCP 스택은 실제 네트워크의 추상화를 제공 하도록 설계를 하는 동안 프로토콜이는 전송은 해당 되지만 그 이상은 포함 하 여 통신 하는 데 사용 됩니다. 예를 들어 신뢰할 수 있는 세션 바인딩 요소는 채널 스택의 일부지만 전송 아래에 있거나 전송 자체는 아닙니다. 이러한 추상화는 스택의 맨 아래 채널에게 기본 전송 프로토콜을 채널 스택 아키텍처에 적용하도록 요구한 다음, 안정성 보장 및 보안과 같은 통신 기능을 제공하기 위해 스택의 상위 채널을 사용하는 방식으로 이루어집니다.  
...
## <a name="channel-shapes"></a>채널 셰이프  
 각 채널에서는 채널 셰이프 인터페이스 또는 채널 셰이프라고 하는 인터페이스를 하나 이상 구현합니다. 이러한 채널 셰이프에서는 채널에서 구현되어 채널 사용자가 호출하는 보내기 및 받기 또는 요청 및 응답과 같은 통신 기반 메서드를 제공합니다. 채널 셰이프 하단에는 <xref:System.ServiceModel.Channels.IChannel> 인터페이스를 제공 하는 인터페이스를 `GetProperty` \<T > 스택에서 채널에 의해 노출 되는 임의의 기능에 액세스 하기 위한 계층화 된 메커니즘으로 사용 하는 메서드. <xref:System.ServiceModel.Channels.IChannel>을 확장하는 다섯 개의 채널 셰이프는 다음과 같습니다.  
...
- <xref:System.ServiceModel.Channels.IDuplexSessionChannel>  
 ...
 채널 셰이프는 기존 전송 프로토콜에서 지원하는 몇 가지 기본 메시지 교환 패턴에 따라 패턴화됩니다. 예를 들어 단방향 메시징에 <xref:System.ServiceModel.Channels.IInputChannel> / <xref:System.ServiceModel.Channels.IOutputChannel> 쌍에 해당 요청-회신 <xref:System.ServiceModel.Channels.IRequestChannel> / <xref:System.ServiceModel.Channels.IReplyChannel> 에 해당 하는 쌍 및 양방향 이중 통신 <xref:System.ServiceModel.Channels.IDuplexChannel> (모두 확장 <xref:System.ServiceModel.Channels.IInputChannel> 고 <xref:System.ServiceModel.Channels.IOutputChannel>).  
