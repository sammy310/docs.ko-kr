...
채널 팩터리가 채널을 만듭니다.  
 ...
 WCF는 이 프로세스에 대한 기본 클래스 도우미를 제공합니다. (이 항목에서 설명하는 채널 도우미 클래스다이어그램은 [채널 모델 개요를](channel-model-overview.md)참조하십시오.)  
  
- 클래스는 <xref:System.ServiceModel.Channels.CommunicationObject> 개발 <xref:System.ServiceModel.ICommunicationObject> [채널의](developing-channels.md)2단계에서 설명한 상태 머신을 구현하고 적용합니다.  
...
- <xref:System.ServiceModel.Channels.ChannelListenerBase> 클래스는 <xref:System.ServiceModel.Channels.IChannelListener>을 구현합니다. 이 클래스는 기본 상태 관리를 담당합니다.
 ...
 다음 설명은 [전송: UDP](../samples/transport-udp.md) 샘플을 기반으로 합니다.  
...
```  
 ...
 구현 `Send()` `BeginSend()` / `EndSend()`및 . 그러면 두 개의 기본 섹션으로 분할됩니다. 먼저 메시지를 바이트 배열로 serialize합니다.  
