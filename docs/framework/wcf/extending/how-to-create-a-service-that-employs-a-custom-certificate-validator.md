...
이 항목에서는 사용자 지정 인증서 유효성 검사기를 구현하는 방법과 클라이언트 또는 서비스 자격 증명을 구성하여 기본 인증서 유효성 검사 논리를 사용자 지정 인증서 유효성 검사기로 바꾸는 방법을 보여 줍니다.  
 ...
 X.509 인증서를 클라이언트 또는 서비스 인증을 사용 하는 경우 기본적으로 Windows Communication Foundation (WCF) 사용 하 여 Windows 인증서 저장소와 Crypto API 인증서의 유효성을 검사 하는 데 신뢰할 수 있는 것을 확인 합니다. 기본 제공 인증서 유효성 검사 기능으로 충분하지 않은 경우에는 변경해야 합니다. WCF는 사용자 지정 인증서 유효성 검사기를 추가 하는 사용자를 허용 하 여 유효성 검사 논리를 변경 하는 쉬운 방법을 제공 합니다. 사용자 지정 인증서 유효성 검사기를 지정 된 경우 WCF는 기본 제공 인증서 유효성 검사 논리를 사용 하지 않습니다 하지만 대신 사용자 지정 유효성 검사기에 의존 합니다.  
...
1. 추가 [ \<동작 >](../../../../docs/framework/configure-apps/file-schema/wcf/behaviors.md) 요소와 [ \<serviceBehaviors >](../../../../docs/framework/configure-apps/file-schema/wcf/servicebehaviors.md) 하는 [ \<system.serviceModel >](../../../../docs/framework/configure-apps/file-schema/wcf/system-servicemodel.md) 요소.  
  
2. 추가 [ \<동작 >](../../../../docs/framework/configure-apps/file-schema/wcf/behavior-of-endpointbehaviors.md) 설정 하 고는 `name` 특성을 적절 한 값으로.  
  
3. 추가 된 [ \<serviceCredentials >](../../../../docs/framework/configure-apps/file-schema/wcf/servicecredentials.md) 에 `<behavior>` 요소.  
...
2. 추가 된 [ \<endpointBehaviors >](../../../../docs/framework/configure-apps/file-schema/wcf/endpointbehaviors.md) 요소입니다.  
  
3. `<behavior>` 요소를 추가하고 `name` 특성을 적절한 값으로 설정합니다.  
  
4. 추가 된 [ \<clientCredentials >](../../../../docs/framework/configure-apps/file-schema/wcf/clientcredentials.md) 요소입니다.  
  
5. 추가 된 [ \<serviceCertificate >](../../../../docs/framework/configure-apps/file-schema/wcf/servicecertificate-of-clientcredentials-element.md)합니다.  
  
6. 추가 된 [ \<인증 >](../../../../docs/framework/configure-apps/file-schema/wcf/authentication-of-servicecertificate-element.md) 다음 예제와 같이 합니다.  
