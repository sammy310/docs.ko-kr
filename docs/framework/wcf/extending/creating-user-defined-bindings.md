---
description: '자세한 정보: User-Defined 바인딩 만들기'
...
ms.locfileid: "99735402"
---
# <a name="creating-user-defined-bindings"></a>사용자 정의 바인딩 만들기

시스템에서 제공하지 않는 바인딩은 다음과 같은 여러 가지 방법으로 만들 수 있습니다.  
  
- 바인딩 요소로 채워지는 컨테이너인 <xref:System.ServiceModel.Channels.CustomBinding> 클래스에 따라 사용자 지정 바인딩을 만듭니다. 그런 다음 사용자 지정 바인딩을 서비스 엔드포인트에 추가합니다. 프로그램 방식이나 애플리케이션 구성 파일을 통해 사용자 지정 바인딩을 만들 수 있습니다. 애플리케이션 구성 파일에서 바인딩 요소를 사용하려면 바인딩 요소가 <xref:System.ServiceModel.Configuration.BindingElementExtensionElement>를 확장해야 합니다. 사용자 지정 바인딩에 대 한 자세한 내용은 [사용자 지정 바인딩](custom-bindings.md) 및을 참조 하십시오 <xref:System.ServiceModel.Channels.CustomBinding> .  
...
## <a name="the-order-of-binding-elements"></a>바인딩 요소 순서  
 ...
 프로토콜 바인딩 요소 – 이 요소는 메시지에 대해 수행하는 높은 수준의 처리 단계를 나타냅니다. 이러한 바인딩 요소로 만들어진 채널 및 수신기는 메시지 내용을 추가, 제거 또는 수정할 수 있습니다. 제공된 바인딩에는 임의의 개수의 프로토콜 바인딩 요소가 포함될 수 있으며 각 요소는 <xref:System.ServiceModel.Channels.BindingElement>에서 상속됩니다. WCF (Windows Communication Foundation)에는 및를 비롯 한 여러 프로토콜 바인딩 요소가 포함 되어 있습니다 <xref:System.ServiceModel.Channels.ReliableSessionBindingElement> <xref:System.ServiceModel.Channels.SymmetricSecurityBindingElement> .  
 ...
 전송 바인딩 요소 – 이 요소는 전송 프로토콜에서 인코딩 메시지의 전송을 나타냅니다. 일반적인 WCF 바인딩에는에서 상속 되는 정확히 하나의 전송 바인딩 요소가 포함 되어 <xref:System.ServiceModel.Channels.TransportBindingElement> 있습니다. 전송 바인딩 요소의 예로는 <xref:System.ServiceModel.Channels.TcpTransportBindingElement>, <xref:System.ServiceModel.Channels.HttpTransportBindingElement> 및 <xref:System.ServiceModel.Channels.NamedPipeTransportBindingElement>가 있습니다.  
  
 새 바인딩을 만들 때 추가되는 바인딩 요소의 순서가 중요합니다. 항상 다음 순서대로 바인딩 요소를 추가합니다.  
  
|계층|옵션|필수|  
|-----------|-------------|--------------|  
|트랜잭션 흐름|<xref:System.ServiceModel.Channels.TransactionFlowBindingElement?displayProperty=nameWithType>|No|  
|안정성|<xref:System.ServiceModel.Channels.ReliableSessionBindingElement?displayProperty=nameWithType>|No|  
|보안|<xref:System.ServiceModel.Channels.SecurityBindingElement?displayProperty=nameWithType>|아니요|  
|복합 이중|<xref:System.ServiceModel.Channels.CompositeDuplexBindingElement?displayProperty=nameWithType>|No|  
...
## <a name="creating-a-new-binding-element"></a>새 바인딩 요소 만들기  
 ...
 WCF에서 제공 하는에서 파생 된 형식 외에도 <xref:System.ServiceModel.Channels.BindingElement> 고유한 바인딩 요소를 만들 수 있습니다. 따라서 바인딩 스택을 만드는 방법 및 스택에서 기타 시스템 제공 형식으로 구성할 수 있는 <xref:System.ServiceModel.Channels.BindingElement>를 만들어 이 스택에 삽입되는 구성 요소를 사용자 지정할 수 있습니다.  
...
```  
 ...
 새 바인딩 요소를 작성하는 방법은 정확한 기능에 따라 달라집니다. [전송: UDP](../samples/transport-udp.md)샘플 중 하나는 일종의 바인딩 요소를 구현 하는 방법에 대 한 자세한 설명을 제공 합니다.  
...
## <a name="creating-a-new-binding"></a>새 바인딩 만들기  
 ...
 <xref:System.ServiceModel.Channels.Binding.Scheme%2A> 속성은 바인딩에서 사용 중인 전송 프로토콜의 URI 스키마를 나타냅니다. 예를 들어 *WSHttpBinding* 및 *NetTcpBinding* 는 해당 속성에서 "http" 및 "net.tcp"를 반환 합니다 <xref:System.ServiceModel.Channels.Binding.Scheme%2A> .  
  
 사용자 정의 인코딩 바인딩에 대한 선택적 메서드 및 속성의 전체 목록은 <xref:System.ServiceModel.Channels.Binding>을 참조하세요.  
  
### <a name="example"></a>예제  

 이 예제에서는 `SampleProfileUdpBinding`에서 파생되는 <xref:System.ServiceModel.Channels.Binding>의 프로필 바인딩을 구현합니다. 에는 `SampleProfileUdpBinding` 최대 4 개의 바인딩 요소가 포함 됩니다. 하나는 사용자가 만든 `UdpTransportBindingElement` 이 고 3 개의 시스템 제공: `TextMessageEncodingBindingElement` , `CompositeDuplexBindingElement` 및 `ReliableSessionBindingElement` 입니다.  
...
### <a name="one-shot-security"></a>원샷 보안  
 ...
 구성 요소의 특성을로 설정 하 여 필요한 모든 보안 자격 증명이 단일 메시지로 전송 되는 "원 샷" 보안을 구현할 수 있습니다 `negotiateServiceCredential` \<message> `false` .  
