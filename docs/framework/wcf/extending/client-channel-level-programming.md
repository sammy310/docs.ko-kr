...
#### <a name="creating-a-binding"></a>바인딩 만들기  
 ...
 수신 케이스와 유사 하 게 ( [서비스 Channel-Level 프로그래밍](service-channel-level-programming.md)참조) 바인딩을 만들어 메시지를 전송 하기 시작 합니다. 이 예제에서는 새 <xref:System.ServiceModel.Channels.CustomBinding?displayProperty=nameWithType>을 만들고 <xref:System.ServiceModel.Channels.HttpTransportBindingElement?displayProperty=nameWithType>를 요소 컬렉션에 추가합니다.  
