...
4. 메시지를 처리하는 데 필요한 통신 인프라에 대한 범주별 문 (예: 통신을 위해 보안이 필요한지 여부 및 필요한 보안 양식 등에 대한 세부 사항)  
 ...
 관리 되는 형식으로 정의 되어 만들어진 계약은 수 *내보낸* 메타 데이터로, WSDL 및 XSD-클라이언트나 다른 서비스 구현자에서 필요한 경우. 이를 통해 모든 클라이언트 응용 프로그램에 대해 공용 메타데이터를 사용하여 기술할 수 있는 간단한 프로그래밍 모델이 만들어집니다. Wcf의 필요한 변환이 서비스 계약 형식 시스템에서 XML 형식 시스템에 자동으로 수행 하는 기본 SOAP 메시지, 전송 및 보안 관련 정보 및 등의 세부 정보를 유지할 수 있습니다.  
