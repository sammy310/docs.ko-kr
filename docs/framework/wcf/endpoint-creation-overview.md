...
- **주소**입니다. 주소는 엔드포인트를 고유하게 식별하고 잠재 고객에게 서비스가 있는 위치를 알려 줍니다. 이는 URI (Uniform Resource Identifier) 및 id를 포함 하는 주소 속성, 일부 WSDL (웹 서비스 기술 언어) 요소 및 선택적 헤더의 컬렉션을 포함 하는 <xref:System.ServiceModel.EndpointAddress> 주소로 WCF 개체 모델에 표시 됩니다. 선택적 헤더는 엔드포인트를 확인하거나 상호 작용하는 데 필요한 자세한 주소 지정 정보를 추가로 제공합니다. 자세한 내용은 [끝점 주소 지정](specifying-an-endpoint-address.md)을 참조 하세요.
...
- "Hello \<name >!" 응답과 함께 사용자의 이름과 에코를 수락 하는 서비스의 `IEcho` 형식에 대 한 계약을 정의 합니다.

- `Echo` 계약에 정의된 유형의 `IEcho` 서비스를 구현합니다.

- 서비스에 대 한 `http://localhost:8000/Echo`의 끝점 주소를 지정 합니다.
...
> [!NOTE]
> [ServiceModel Metadata 유틸리티 도구 (svcutil.exe)](servicemodel-metadata-utility-tool-svcutil-exe.md) 를 `/config:`*파일* 이름 `[,`*파일 이름* 으로 사용 하 `]` 스위치를 사용 하 여 구성 파일을 신속 하 게 만듭니다.
