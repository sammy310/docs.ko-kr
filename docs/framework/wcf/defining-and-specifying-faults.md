...
### <a name="defining-error-conditions-that-clients-should-know-about"></a>클라이언트가 알아야 하는 오류 조건 정의  
 SOAP 오류는 특정 작업에 대한 오류 정보를 전달하는 공개적으로 설명된 메시지입니다. WSDL로 다른 작업 메시지와 함께 설명되므로 클라이언트는 작업을 호출할 때 이러한 오류를 알고 처리합니다. 그러나 WCF 서비스는 관리 코드로 작성되므로 관리 코드의 오류 조건을 오류로 변환하고 클라이언트로 반환하여 공식적인 오류와 서비스의 오류 조건및 버그를 분리할 수 있습니다. 클라이언트와 대화를 나눌 수 있습니다.  
...
### <a name="define-the-content-of-error-conditions"></a>오류 조건의 콘텐츠 정의  
 ...
 데이터를 직렬화할 수 있도록 하는 방법에 대한 자세한 내용은 [서비스 계약의 데이터 전송 지정을](./feature-details/specifying-data-transfer-in-service-contracts.md)참조하십시오. 제공하는 직렬화 지원 목록은 데이터 [계약 Serializer에서 지원하는 형식을](./feature-details/types-supported-by-the-data-contract-serializer.md)참조하십시오. <xref:System.Runtime.Serialization.DataContractSerializer?displayProperty=nameWithType>  
...
### <a name="mark-operations-to-establish-the-fault-contract"></a>오류 계약을 설정하도록 작업 표시  
 ...
 다음 단계는 작업 구현에서 오류 계약을 구현하는 데 오류 보내기 및 수신 항목에 [설명되어 있습니다.](sending-and-receiving-faults.md)  
...
#### <a name="soap-wsdl-and-interoperability-considerations"></a>SOAP, WSDL 및 상호 운용성 고려 사항  
 ...
 오류를 생성하는 비서비스에 액세스하는 경우 특정 제한 사항이 있습니다. WCF는 스키마가 설명하고 데이터 계약과 호환되는 세부 형식의 오류만 지원합니다. 예를 들어 위에서 설명한 것처럼 WCF는 세부 유형에서 XML 특성을 사용하는 오류 또는 세부 정보 섹션에 두 개 이상의 최상위 요소가 있는 오류를 지원하지 않습니다.  
