...
다음 그림에서는 공용 보안되지 않은 WCF(Windows 통신 재단) 클라이언트 및 서비스의 예를 보여 주며 다음과 같은 예제를 보여 드립니다.  
...
|보안 모드|None|  
|전송|HTTP|  
|바인딩|<xref:System.ServiceModel.BasicHttpBinding>코드에서 또는 [ \<구성의 기본HttpBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md) 요소입니다.|  
...
### <a name="code"></a>코드  
 다음 코드는 보안되지 않은 끝점에 액세스하는 기본 WCF 클라이언트를 보여 주며, 이 코드는 보안되지 않은 끝점에 액세스합니다.  
