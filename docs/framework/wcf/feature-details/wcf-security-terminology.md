...
보안을 설명할 때 사용되는 일부 용어는 생소할 수 있습니다. 이 항목에서는 일부 보안 용어에 대해 간략하게 설명하며, 모든 항목을 포괄적으로 설명하지는 않습니다.  
 ...
 CA(인증 기관) 인증서  
 서버 및 클라이언트 인증 인증서를 해당 인증서를 요청하는 서버와 클라이언트에 발급하는 CA를 식별합니다. 디지털 서명에 사용 되는 공개 키를 포함 하기 때문에 *서명 인증서* 라고도 합니다. CA가 루트 인증 기관이 면 CA 인증서를 *루트 인증서* 라고 합니다. 또한 *사이트 인증서* 라고도 합니다.  
 ...
 hash  
 임의 크기의 데이터에 수학적 함수(해시 알고리즘 참조)를 적용하여 구하는 고정된 크기의 숫자 값입니다. 데이터에는 일반적으로 *nonce* 라고 하는 임의 데이터가 포함 됩니다. 서비스와 클라이언트는 모두 nonce를 교환하므로 결과가 더 복잡해집니다. 결과를 *메시지 다이제스트* 라고도 합니다. 해시 값을 보내는 것이 암호와 같은 중요한 데이터를 보내는 것보다 더 안전합니다. 이는 암호가 암호화되어 있는 경우에도 마찬가지입니다. 해시 발신자와 수신자가 수신된 해시를 확인할 수 있도록 해시 알고리즘과 nonce에 동의해야 합니다.  
 ...
 사용자 계정 이름 (사용자 *로그온 이름이* 라고도 함) 및 사용자 계정이 있는 도메인을 식별 하는 도메인 이름입니다. Windows 도메인에 로그온하는 표준 사용법입니다. 형식은 someone@example.com (전자 메일 주소)입니다.  
  
> [!NOTE]
> 표준 UPN 형식 외에도 WCF는 하위 수준의 Upn (예: cohowinery)을 허용 합니다. com\someone.  
