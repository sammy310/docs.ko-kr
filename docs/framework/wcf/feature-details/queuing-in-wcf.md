...
- `DeadLetterQueue`: 이 속성은 배달 못한 편지 큐가 필요한지 여부를 나타내는 열거형입니다. 요청 시 열거형에는 배달 못 한 편지 큐도 포함됩니다. 가능한 값은 `None`, `System` 및 `Custom`입니다. 이러한 속성의 해석에 대 한 자세한 내용은 [배달 못 한 편지 큐를 사용 하 여 메시지 전송 오류 처리](using-dead-letter-queues-to-handle-message-transfer-failures.md) 를 참조 하세요.  
