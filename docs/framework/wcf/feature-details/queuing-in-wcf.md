...
ms.locfileid: "79184558"
---
# <a name="queuing-in-wcf"></a>WCF의 큐
이 섹션에서는 WCF(Windows 통신 Foundation)에서 큐에 대기된 통신을 사용하는 방법에 대해 설명합니다.  
  
## <a name="queues-as-a-wcf-transport-binding"></a>큐를 WCF 전송 바인딩으로  
 WCF에서 계약은 교환되는 내용을 지정합니다. 계약은 비즈니스 종속 또는 애플리케이션별 메시지 교환입니다. 메시지를 교환하는 데 사용되는 메커니즘(또는 "방법")은 바인딩에 지정됩니다. WCF의 바인딩은 메시지 교환의 세부 정보를 캡슐화합니다. 또한 다양한 전송 모양과 바인딩이 나타내는 프로토콜을 제어할 수 있도록 구성 노브를 사용자에게 노출합니다. WCF에서 큐잉하는 것은 다른 전송 바인딩처럼 처리되며, 이는 많은 큐잉 응용 프로그램에 큰 이점입니다. 대부분의 큐 애플리케이션은 다른 RPC(원격 프로시저 호출) 스타일 분산 애플리케이션과 다르게 작성되어 사용 및 유지 관리가 어려울 수 있습니다. WCF를 사용하면 분산 응용 프로그램을 작성하는 스타일이 거의 동일하므로 따라및 유지 관리가 더 쉬워집니다. 교환 메커니즘을 비즈니스 논리와 분리하여 애플리케이션별 코드를 적용하지 않고도 전송을 변경하거나 쉽게 구성할 수 있습니다. 다음 그림에서는 MSMQ를 전송으로 사용하는 WCF 서비스 및 클라이언트의 구조를 보여 줍니다.  
  
 ![대기 중인 응용 프로그램 다이어그램](../../../../docs/framework/wcf/feature-details/media/distributed-queue-figure.jpg "분산 큐 그림")  
  
 이전 그림에서 볼 수 있듯이 클라이언트와 서비스는 애플리케이션 의미 즉, 계약과 구현만 정의해야 합니다. 서비스는 기본 설정을 사용하여 대기 중인 바인딩을 구성합니다. 클라이언트는 [ServiceModel 메타데이터 유틸리티 도구(Svcutil.exe)를](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) 사용하여 서비스에 WCF 클라이언트를 생성하고 서비스에 메시지를 보내는 데 사용할 바인딩을 설명하는 구성 파일을 생성합니다. 따라서 큐에 대기 중인 메시지를 보내려면 클라이언트가 WCF 클라이언트를 인스턴스화하고 클라이언트에 작업을 호출합니다. 그러면 메시지가 전송 큐로 보내진 다음 대상 큐로 전송됩니다. 대기 중인 통신의 모든 복잡한 과정이 메시지를 보내고 받는 애플리케이션에서 숨겨집니다.  
  
 WCF에서 큐에 대기된 바인딩에 대한 주의 사항은 다음과 같습니다.  
  
- WCF의 기본 큐에 대기된 바인딩은 큐를 사용하여 이중 통신을 지원하지 않으므로 모든 서비스 작업은 단방향이어야 합니다. 양방향 통신[샘플(양방향 통신)은](../../../../docs/framework/wcf/samples/two-way-communication.md)두 개의 단방향 계약을 사용하여 큐를 사용하여 이중 통신을 구현하는 방법을 보여 줍니다.  
  
- 메타데이터 교환을 사용하여 WCF 클라이언트를 생성하려면 WCF 클라이언트를 생성하고 큐에 대기된 통신을 적절하게 구성하기 위해 바인딩 정보를 얻기 위해 직접 쿼리할 수 있도록 서비스에 추가 HTTP 끝점이 필요합니다.  
  
- 큐에 대기된 바인딩에 따라 WCF 외부의 추가 구성이 필요합니다. 예를 들어 <xref:System.ServiceModel.NetMsmqBinding> WCF와 함께 제공되는 클래스에서는 바인딩을 구성하고 MSMQ(메시지 큐잉)를 최소한으로 구성해야 합니다.  
  
 다음 섹션에서는 MSMQ를 기반으로 하는 WCF와 함께 제공되는 특정 큐에 있는 바인딩에 대해 설명합니다.  
  
### <a name="msmq"></a>MSMQ  
 WCF에서 큐에 대기된 전송은 대기 대기 통신을 위해 MSMQ를 사용합니다.  
 ...
 MSMQ에 대한 자세한 내용은 [MSMQ(메시지 대기열 설치)를](../../../../docs/framework/wcf/samples/installing-message-queuing-msmq.md)참조하십시오.  
  
### <a name="netmsmqbinding"></a>NetMsmqBinding  
 netMsmqBinding>큐에 있는 바인딩 WCF는 MSMQ를 사용하여 통신하기 위해 두 개의 WCF 끝점을 제공합니다. [ \<](../../../../docs/framework/configure-apps/file-schema/wcf/netmsmqbinding.md) 따라서 바인딩은 MSMQ에 특정한 속성을 노출합니다. 그러나 모든 MSMQ 기능과 속성이 `NetMsmqBinding`에 노출되는 것은 아닙니다. 압축 `NetMsmqBinding`은 대부분의 고객에게 적합한 최적의 기능들로 디자인되었습니다.  
  
 `NetMsmqBinding`에서는 여기에서 지금까지 설명한 핵심 큐 개념을 바인딩에 대한 속성의 형태로 나타냅니다. 이러한 속성은 메시지를 전송 및 전달하는 방법을 MSMQ에 알려 줍니다. 속성 범주에 대한 자세한 내용은 다음 단원을 참조하세요. 자세한 내용은 특정 속성을 보다 완벽하게 설명하는 개념 항목을 참조하십시오.  
...
#### <a name="dead-letter-queue-properties"></a>배달 못한 편지 큐 속성  
 ...
 서로 다른 대상 큐에 메시지를 보내는 여러 클라이언트가 MSMQ 서비스를 공유하는 경우 클라이언트가 보내는 모든 메시지는 동일한 배달 못한 편지 큐로 이동됩니다. 이것이 항상 좋은 것은 아닙니다. 더 나은 격리를 위해 Windows Vista의 WCF 및 MSMQ는 사용자가 배달에 실패한 메시지를 저장하도록 지정할 수 있는 사용자 지정 배달 끊기 대기열(또는 응용 프로그램별 배달 끊기 대기열)을 제공합니다. 따라서 클라이언트 간에 동일한 배달 못한 편지 큐를 공유하지 않습니다.  
  
 바인딩에는 다음과 같은 두 가지 속성이 필요합니다.  
  
- `DeadLetterQueue`: 이 속성은 배달 못한 편지 큐가 필요한지 여부를 나타내는 열거형입니다. 요청 시 열거형에는 배달 못 한 편지 큐도 포함됩니다. 가능한 값은 `None`, `System` 및 `Custom`입니다. 이러한 속성의 해석에 대한 자세한 내용은 [배달 중단 자 큐를 사용하여 메시지 전송 실패를 처리합니다.](../../../../docs/framework/wcf/feature-details/using-dead-letter-queues-to-handle-message-transfer-failures.md)  
  
- `CustomDeadLetterQueue`: 이 속성은 애플리케이션별 배달 못한 편지 큐의 URI(Uniform Resource Identifier) 주소입니다. 이 경우 `DeadLetterQueue`이 필요합니다.`Custom` 선택됩니다.  
  
#### <a name="poison-message-handling-properties"></a>포이즌 메시지 처리 속성  
 서비스에서 트랜잭션의 대상 큐에 있는 메시지를 읽을 때 서비스에서 여러 가지 이유로 메시지를 처리하지 못할 수 있습니다. 그러면 메시지를 큐에 넣고 다시 읽습니다. 반복적으로 실패하는 메시지를 처리하려면 바인딩에서 포이즌 메시지 처리 속성을 구성할 수 있습니다. `ReceiveRetryCount`, `MaxRetryCycles`, `RetryCycleDelay` 및 `ReceiveErrorHandling`의 네 가지 속성이 있습니다. 이러한 속성에 대한 자세한 내용은 [포이즌 메시지 처리를](../../../../docs/framework/wcf/feature-details/poison-message-handling.md)참조하십시오.  
  
#### <a name="security-properties"></a>보안 속성  
 MSMQ는 큐의 ACL(액세스 제어 목록) 또는 인증된 메시지 보내기와 같은 고유한 보안 모델을 노출합니다. `NetMsmqBinding`은 이러한 보안 속성을 전송 보안 설정의 일부로 노출합니다. 전송 보안의 바인딩에는 `MsmqAuthenticationMode` 및 `MsmqProtectionLevel`의 두 가지 속성이 있습니다. 이러한 속성의 설정은 MSMQ가 구성되는 방법에 따라 다릅니다. 자세한 내용은 [전송 보안을 사용하는 메시지 보안을](../../../../docs/framework/wcf/feature-details/securing-messages-using-transport-security.md)참조하십시오.  
  
 전송 보안 이외에 메시지 보안을 사용하여 실제 SOAP 메시지를 보안할 수 있습니다. 자세한 내용은 [메시지 보안을 사용하는 메시지 보안](../../../../docs/framework/wcf/feature-details/securing-messages-using-message-security.md)을 참조하십시오.  
...
- `UseActiveDirectory`: 큐 주소 확인에 Active Directory를 사용해야 하는지 여부를 나타내는 부울 값입니다. 기본적으로 해제됩니다. 자세한 내용은 [서비스 끝점 및 큐 주소 지정](../../../../docs/framework/wcf/feature-details/service-endpoints-and-queue-addressing.md)을 참조하십시오.  
  
### <a name="msmqintegrationbinding"></a>MsmqIntegrationBinding  
 WCF `MsmqIntegrationBinding` 끝점이 C, C++, COM 또는 System.Messaging API로 작성된 기존 MSMQ 응용 프로그램과 통신하도록 할 때 사용됩니다.  
