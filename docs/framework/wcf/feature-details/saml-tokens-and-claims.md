...
3.  클라이언트의 복사본을 수신 합니다 *증명 키*합니다. 클라이언트 응용 프로그램에 SAML 토큰 제공 (합니다 *신뢰 당사자*) 하 고 해당 증명 키를 사용 하 여 메시지를 서명 합니다.  
...
## <a name="from-claims-to-samlattributes"></a>클레임에서 SamlAttribute로  
 Wcf에서 SAML 토큰의 문은으로 모델링 됩니다 <xref:System.IdentityModel.Tokens.SamlAttribute> 개체를 직접 채울 수 있습니다 <xref:System.IdentityModel.Claims.Claim> 제공 하는 개체를 <xref:System.IdentityModel.Claims.Claim> 개체에는 <xref:System.IdentityModel.Claims.Claim.Right%2A> 속성을 <xref:System.IdentityModel.Claims.Rights.PossessProperty%2A> 및 <xref:System.IdentityModel.Claims.Claim.Resource%2A> 속성은 형식 <xref:System.String>합니다. 예를 들어:  
