...
Windows 인증을 보안 메커니즘으로 사용하면 SSPI(보안 지원 공급자 인터페이스)에서 보안 프로세스를 처리합니다. SSPI 계층에 보안 오류가 발생 하면 Windows Communication Foundation (WCF)으로 표시 됩니다. 이 항목에서는 오류 진단에 도움이 되는 프레임워크 및 일련의 질문을 제공합니다.  
 ...
 Kerberos 프로토콜의 개요를 보려면 [Kerberos 설명](https://go.microsoft.com/fwlink/?LinkID=86946); SSPI의 개요를 참조 하십시오 [SSPI](https://go.microsoft.com/fwlink/?LinkId=88941)합니다.  
  
 Windows 인증을 위해 WCF 일반적으로 사용 합니다 *Negotiate* 보안 지원 공급자 (SSP) 클라이언트와 서비스 간의 Kerberos 상호 인증을 수행 하는 합니다. Kerberos 프로토콜을 사용할 수 없는 경우 기본적으로 WCF 대체 NT LAN Manager (NTLM)을 합니다. 그러나 Kerberos 프로토콜만 사용 (하는 데 Kerberos를 사용할 수 없는 경우 예외를 throw) WCF를 구성할 수 있습니다. 또한 제한 된 형식의 Kerberos 프로토콜을 사용 하는 WCF를 구성할 수 있습니다.  
...
#### <a name="spnupn-problems-with-the-kerberos-protocol"></a>Kerberos 프로토콜에 대한 SPN/UPN 문제  
 Windows 인증을 사용할 때 Kerberos 프로토콜을 사용하거나 SSPI 협상을 수행하는 경우, 클라이언트 엔드포인트에 사용되는 URL은 서비스 URL 내에 있는 서비스 호스트의 정규화된 도메인 이름을 포함해야 합니다. 이 서비스가 실행 되 고 있는 계정으로 서비스를 실행 하면 가장 일반적으로 Active Directory 도메인에 컴퓨터를 추가할 때 만들어진 시스템 (기본값) 서비스 사용자 이름 (SPN) 키에 대 한 액세스 권한이 있다고 가정 합니다 네트워크 서비스 계정입니다. 서비스에 시스템 SPN 키에 대한 액세스 권한이 없는 경우 클라이언트의 엔드포인트 ID로 서비스가 실행 중인 계정의 올바른 SPN 또는 UPN(User Principal Name)을 제공해야 합니다. SPN 및 UPN을 사용 하 여 WCF 작동 하는 방법에 대 한 자세한 내용은 참조 하세요. [서비스 Id 및 인증](../../../../docs/framework/wcf/feature-details/service-identity-and-authentication.md)합니다.  
  
 웹 팜 또는 웹 가든과 같은 부하 분산 시나리오에서는 각 응용 프로그램에 대해 고유한 계정을 정의하고, 해당 계정에 SPN을 할당하고, 응용 프로그램의 모든 서비스가 해당 계정으로 실행되도록 하는 것이 일반적입니다.  
  
 서비스 계정에 대한 SPN을 얻으려면 Active Directory 도메인 관리자여야 합니다. 자세한 내용은 [Kerberos 기술 보완에 대 한 Windows](https://go.microsoft.com/fwlink/?LinkID=88330)합니다.  
...
#### <a name="negotiate-ssp-falls-back-to-ntlm-but-ntlm-is-disabled"></a>협상 SSP가 NTLM으로 대체되어도 NTLM을 사용하지 않도록 설정  
 합니다 <xref:System.ServiceModel.Security.WindowsClientCredential.AllowNtlm%2A> 속성이 `false`, NTLM을 사용 하는 경우 예외를 throw 하는 최상의 노력을 하도록 Windows Communication Foundation (WCF) 하면 합니다. 이 속성을 `false`로 설정하면 유선을 통해 NTLM 자격 증명을 보낼 수 있습니다.  
...
#### <a name="anonymous-ntlm-logon-occurs-but-anonymous-logons-are-disabled-by-default"></a>익명 NTLM 로그온을 수행해도 기본적으로 익명 로그온을 사용하지 않도록 설정  
 ...
 가장에 대 한 자세한 내용은 참조 하세요. [위임 및 가장](../../../../docs/framework/wcf/feature-details/delegation-and-impersonation-with-wcf.md)합니다.  
