...
구성 기반 활성화는 이전에 .svc 파일에 있던 메타데이터를 가져와서 Web.config 파일에 저장합니다. 내에서 <`serviceHostingEnvironment`> 요소는 <`serviceActivations`> 요소입니다. 내에서 <`serviceActivations`> 요소는 하나 이상의 <`add`> 요소, 각 호스팅된 서비스에 대 한 합니다. <`add`> 요소는 서비스 및 서비스 유형 또는 서비스 호스트 팩터리를 상대 주소를 설정할 수 있는 특성을 포함 합니다. 다음 구성 예제 코드에서는 이 섹션을 사용하는 방법을 보여 줍니다.
...
> - 합니다 `relativeAddress` 와 같은 특성을 상대 주소로 설정 해야 합니다 "\<하위 디렉터리 > / service.svc" 또는 "~ /\<하위 directory/service.svc"입니다.
