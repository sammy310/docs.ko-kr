...
- <xref:System.ServiceModel.InstanceContextMode.Single>: 단일 <xref:System.ServiceModel.InstanceContext> (및 그에 따른 서비스 개체)는 애플리케이션의 수명에 대한 모든 클라이언트 요청을 처리합니다.  
...
- <xref:System.ServiceModel.ConcurrencyMode.Reentrant>: 각 서비스 인스턴스가 한 번에 하나의 메시지를 처리하지만 재진입 작업 호출을 허용합니다. 서비스는 WCF 클라이언트 개체를 통해 호출할 때만 이러한 호출을 허용합니다.  
...
> 둘 이상의 스레드를 안전하게 사용하는 코드를 이해하고 개발하기는 쉽지 않습니다. <xref:System.ServiceModel.ConcurrencyMode.Multiple> 또는 <xref:System.ServiceModel.ConcurrencyMode.Reentrant> 값을 사용하기 전에 이러한 모드에 대해 해당 서비스가 적절하게 디자인되었는지 확인합니다. 자세한 내용은 <xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A>을 참조하세요.  
 ...
 동시성 사용은 인스턴스 만들기 모드와 관련됩니다. instaning에서는 <xref:System.ServiceModel.InstanceContextMode.PerCall> 각 메시지가 새 <xref:System.ServiceModel.InstanceContext> 메시지에 의해 처리되므로 <xref:System.ServiceModel.InstanceContext>에서 하나 이상의 스레드가 활성화되지 않기 때문에 동시성은 관련이 없습니다.  
...
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|  
|PerCall|- 세션 채널과 행동 : <xref:System.ServiceModel.InstanceContext> 세션 및 각 통화에 대한.<br />- 세션없는 채널의 동작 : 예외가 throw됩니다.|- 세션 채널과 행동 : <xref:System.ServiceModel.InstanceContext> 세션 및 각 통화에 대한.<br />- 세션없는 채널행동 <xref:System.ServiceModel.InstanceContext> : 각 통화에 대한.|- 세션 채널이 있는 동작: 예외가 throw됩니다.<br />- 세션없는 채널행동 <xref:System.ServiceModel.InstanceContext> : 각 통화에 대한.|  
|PerSession|- 세션 채널과 행동 : <xref:System.ServiceModel.InstanceContext> 세션 및 각 채널에 대한.<br />- 세션없는 채널의 동작 : 예외가 throw됩니다.|- 세션 채널과 행동 : <xref:System.ServiceModel.InstanceContext> 세션 및 각 채널에 대한.<br />- 세션없는 채널행동 <xref:System.ServiceModel.InstanceContext> : 각 통화에 대한.|- 세션 채널이 있는 동작: 예외가 throw됩니다.<br />- 세션없는 채널행동 <xref:System.ServiceModel.InstanceContext> : 각 통화에 대한.|  
|Single|- 세션 채널과 행동 : <xref:System.ServiceModel.InstanceContext> 세션과 모든 통화에 대한 하나.<br />- 세션없는 채널의 동작 : 예외가 throw됩니다.|- 세션 채널의 동작 : <xref:System.ServiceModel.InstanceContext> 세션 및 생성 또는 사용자가 지정한 싱글 톤에 대한.<br />- 세션없는 채널과 <xref:System.ServiceModel.InstanceContext> 행동 : 생성 또는 사용자가 지정 한 싱글 톤에 대한.|- 세션 채널이 있는 동작: 예외가 throw됩니다.<br />- 세션없는 채널행동 <xref:System.ServiceModel.InstanceContext> : 생성된 각 싱글톤 또는 사용자 지정 싱글톤에 대한 동작입니다.|  
