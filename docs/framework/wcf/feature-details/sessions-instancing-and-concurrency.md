...
- WCF 세션과 연결 된 일반 데이터 저장소는 없습니다.  
 ...
 <xref:System.Web.SessionState.HttpSessionState?displayProperty=nameWithType>ASP.NET 응용 프로그램의 클래스와이 클래스에서 제공 하는 기능에 대해 잘 알고 있는 경우 해당 종류의 세션과 WCF 세션 간에 다음과 같은 차이점을 알 수 있습니다.  
...
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|  
|PerCall|-세션 channel을 사용한 동작: <xref:System.ServiceModel.InstanceContext> 각 호출에 대 한 세션 및입니다.<br />-세션 없는 채널이 있는 동작: 예외가 throw 됩니다.|-세션 channel을 사용한 동작: <xref:System.ServiceModel.InstanceContext> 각 호출에 대 한 세션 및입니다.<br />-세션 없는 채널이 있는 동작: <xref:System.ServiceModel.InstanceContext> 각 호출에 대 한입니다.|-세션 channel을 사용한 동작: 예외가 throw 됩니다.<br />-세션 없는 채널이 있는 동작: <xref:System.ServiceModel.InstanceContext> 각 호출에 대 한입니다.|  
|PerSession|-세션 channel을 사용한 동작: <xref:System.ServiceModel.InstanceContext> 각 채널에 대 한 세션 및<br />-세션 없는 채널이 있는 동작: 예외가 throw 됩니다.|-세션 channel을 사용한 동작: <xref:System.ServiceModel.InstanceContext> 각 채널에 대 한 세션 및<br />-세션 없는 채널이 있는 동작: <xref:System.ServiceModel.InstanceContext> 각 호출에 대 한입니다.|-세션 channel을 사용한 동작: 예외가 throw 됩니다.<br />-세션 없는 채널이 있는 동작: <xref:System.ServiceModel.InstanceContext> 각 호출에 대 한입니다.|  
|Single|-세션 channel을 사용한 동작: 세션 및 <xref:System.ServiceModel.InstanceContext> 모든 호출에 대 한 세션입니다.<br />-세션 없는 채널이 있는 동작: 예외가 throw 됩니다.|-세션 channel을 사용한 동작: <xref:System.ServiceModel.InstanceContext> 생성 되거나 사용자가 지정한 singleton의 세션 및입니다.<br />-세션 없는 채널이 있는 동작: <xref:System.ServiceModel.InstanceContext> 생성 되거나 사용자가 지정 하는 단일 항목에 대 한입니다.|-세션 channel을 사용한 동작: 예외가 throw 됩니다.<br />-세션 없는 채널이 있는 동작: <xref:System.ServiceModel.InstanceContext> 생성 된 각 singleton 또는 사용자 지정 singleton에 대 한입니다.|  
