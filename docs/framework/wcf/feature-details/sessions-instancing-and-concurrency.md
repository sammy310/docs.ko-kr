...
> 둘 이상의 스레드를 안전하게 사용하는 코드를 이해하고 개발하기는 쉽지 않습니다. <xref:System.ServiceModel.ConcurrencyMode.Multiple> 또는 <xref:System.ServiceModel.ConcurrencyMode.Reentrant> 값을 사용하기 전에 이러한 모드에 대해 해당 서비스가 적절하게 디자인되었는지 확인합니다. 자세한 내용은 <xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A>를 참조하세요.  
 ...
 동시성 사용은 인스턴스 만들기 모드와 관련됩니다. <xref:System.ServiceModel.InstanceContextMode.PerCall> 인스턴스는 각 메시지가 새 <xref:System.ServiceModel.InstanceContext>에 의해 처리 되므로 <xref:System.ServiceModel.InstanceContext>에서 두 개 이상의 스레드가 활성화 되지 않기 때문에 동시성이 관련 되지 않습니다.  
...
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|  
|PerCall|-세션 channel을 사용한 동작: 각 호출에 대 한 세션 및 <xref:System.ServiceModel.InstanceContext>입니다.<br />-세션 없는 채널이 있는 동작: 예외가 throw 됩니다.|-세션 channel을 사용한 동작: 각 호출에 대 한 세션 및 <xref:System.ServiceModel.InstanceContext>입니다.<br />-세션 없는 채널이 있는 동작: 각 호출에 대 한 <xref:System.ServiceModel.InstanceContext>입니다.|-세션 channel을 사용한 동작: 예외가 throw 됩니다.<br />-세션 없는 채널이 있는 동작: 각 호출에 대 한 <xref:System.ServiceModel.InstanceContext>입니다.|  
|PerSession|-세션 channel을 사용한 동작: 각 채널에 대 한 세션 및 <xref:System.ServiceModel.InstanceContext>입니다.<br />-세션 없는 채널이 있는 동작: 예외가 throw 됩니다.|-세션 channel을 사용한 동작: 각 채널에 대 한 세션 및 <xref:System.ServiceModel.InstanceContext>입니다.<br />-세션 없는 채널이 있는 동작: 각 호출에 대 한 <xref:System.ServiceModel.InstanceContext>입니다.|-세션 channel을 사용한 동작: 예외가 throw 됩니다.<br />-세션 없는 채널이 있는 동작: 각 호출에 대 한 <xref:System.ServiceModel.InstanceContext>입니다.|  
|Single|-세션 channel을 사용한 동작: 모든 호출에 대 한 세션 및 한 <xref:System.ServiceModel.InstanceContext>입니다.<br />-세션 없는 채널이 있는 동작: 예외가 throw 됩니다.|-세션 channel을 사용한 동작: 만들어지거나 사용자 지정 된 단일 항목에 대 한 세션 및 <xref:System.ServiceModel.InstanceContext>입니다.<br />-세션 없는 채널이 있는 동작: 만들어지거나 사용자가 지정한 단일 항목에 대 한 <xref:System.ServiceModel.InstanceContext>입니다.|-세션 channel을 사용한 동작: 예외가 throw 됩니다.<br />-세션 없는 채널이 있는 동작: 생성 된 각 singleton 또는 사용자 지정 singleton에 대 한 <xref:System.ServiceModel.InstanceContext>입니다.|  
