...
WCF 응용 프로그램 개발 일반적으로 복합 형식의 정의 사용 하 여 시작합니다. ASP.NET 웹 서비스와 동일한.NET Framework 형식을 사용 하도록 WCF 수 있습니다.

WCF<xref:System.Runtime.Serialization.DataContractAttribute> 고 <xref:System.Runtime.Serialization.DataMemberAttribute> 다음 샘플 코드에 표시 된 대로 특정 필드 또는 형식의 속성을 직렬화 하는 XML로 serialize 할 형식의 인스턴스가 되도록 하려면.NET Framework 형식에 추가할 수 있습니다.
...
합니다 <xref:System.ServiceModel.ServiceContractAttribute> 인터페이스는 WCF 서비스 계약을 정의 하도록 지정 하며 <xref:System.ServiceModel.OperationContractAttribute> , 있는 경우 인터페이스의 메서드 정의 하는 서비스 계약의 작업을 나타냅니다.
...
서비스 계약을 구현 하는 클래스를 WCF 서비스로 입력할 라고 합니다.
...
|NetTcpBinding|네트워크를 통해 WCF 소프트웨어 엔터티 간의 보안성, 안정성, 고성능 통신 합니다.|
|NetNamedPipeBinding|동일한 컴퓨터에 WCF 소프트웨어 엔터티 간의 보안, 안정성, 고성능 통신 합니다.|
|NetMsmqBinding|MSMQ를 사용 하 여 WCF 소프트웨어 엔터티 간의 통신 합니다.|
|MsmqIntegrationBinding|WCF 소프트웨어 엔터티 및 MSMQ를 사용 하 여 다른 소프트웨어 엔터티 간의 통신 합니다.|
...
WCF 응용 프로그램에 대 한 사용자 지정 바인딩은 WCF 개별 프로토콜을 구현 하는 데 사용 하는 바인딩 요소 클래스의 컬렉션으로 쉽게 정의 됩니다. 새 바인딩 요소를 작성하여 추가 프로토콜을 나타낼 수 있습니다.
...
WCF 서비스의 끝점에 대해 제공 된 주소는 끝점의 호스트의 기본 주소에 상대적인 주소가입니다. 호스트에는 통신 전송 프로토콜마다 기본 주소가 하나씩 있을 수 있습니다. 이전 구성 파일의 샘플 구성에서 엔드포인트에 대해 선택된 <xref:System.ServiceModel.BasicHttpBinding>은 HTTP를 전송 프로토콜로 사용하므로 `EchoService` 엔드포인트의 주소는 호스트의 HTTP 기본 주소에 상대적입니다. HTTP 기본 주소는 앞의 예제에서 호스트의 경우 `http://www.contoso.com:8000/`합니다. IIS 또는 WAS에서 호스트되는 서비스의 기본 주소는 해당 서비스에 대한 서비스 파일의 URL입니다.

IIS 또는 WAS 하도록 구성 된 http를 전송 프로토콜로 배타적으로에서 호스트 되는 서비스만 WCF ASP.NET 호환 모드 옵션을 사용할 수 있습니다. 이 옵션을 사용하려면 다음 단계를 수행해야 합니다.
...
2. 관리자는 ASP.NET 호환 모드를 사용하도록 응용 프로그램을 구성해야 합니다.
    ...
    또한.svc 대신 해당 서비스 파일에 대 한 확장명.asmx를 사용 하도록 WCF 응용 프로그램을 구성할 수 있습니다.
    ...
    이 옵션에서는 WCF를 사용 하는 서비스를 수정할 때.asmx 서비스 파일의 Url을 사용 하도록 구성 된 클라이언트를 수정 하지 않아도에서 저장할 수 있습니다.

## <a name="client-development"></a>클라이언트 개발

ASP.NET 웹 서비스용 클라이언트는 .asmx 파일의 URL을 입력으로 제공하는 WSDL.exe 명령줄 도구를 사용하여 생성됩니다. WCF에서 제공 되는 해당 도구는 [ServiceModel Metadata 유틸리티 도구 (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)합니다. 서비스 계약의 정 및 WCF 클라이언트 클래스의 정의 사용 하 여 코드 모듈을 생성합니다. 또한 서비스의 주소와 바인딩을 포함한 구성 파일도 생성합니다.

원격 서비스의 클라이언트를 프로그래밍할 때 일반적으로 비동기 패턴에 따라 프로그래밍하는 것이 좋습니다. WSDL.exe 도구로 생성되는 코드는 기본적으로 항상 동기 패턴과 비동기 패턴을 모두 제공합니다. 생성 한 코드를 [ServiceModel Metadata 유틸리티 도구 (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) 패턴 중 하나에 제공할 수 있습니다. 이 도구는 기본적으로 동기 패턴을 제공합니다. 이 도구를 `/async` 스위치와 함께 실행하면 생성된 코드에서 비동기 패턴을 제공합니다.

보장이 없습니다 ASP에서 생성 된 WCF 클라이언트 클래스의 이름입니다. NET의 WSDL.exe 도구에 기본적으로 Svcutil.exe 도구로 생성 된 WCF 클라이언트 클래스의 이름과 일치 합니다. 특히 <xref:System.Xml.Serialization.XmlSerializer>를 사용하여 serialize되어야 하는 클래스의 속성 이름에는 Svcutil.exe 도구로 생성되는 코드의 경우 기본적으로 Property 접미사가 지정되지만 WSDL.exe 도구로 생성되는 코드의 경우에는 Property 접미사가 지정되지 않습니다.
...
WCF 특성을 제공 <xref:System.ServiceModel.MessageContractAttribute>, <xref:System.ServiceModel.MessageHeaderAttribute>, 및 <xref:System.ServiceModel.MessageBodyMemberAttribute> 서비스에서 보내고 받는 SOAP 메시지의 구조를 설명 합니다.
...
이 구문은 메시지 구조를 명시적으로 표현하지만 메시지 구조는 ASP.NET 웹 서비스의 코드로 암시됩니다. 또한 ASP.NET 구문에서는 메시지 헤더 속성으로 표현 됩니다 서비스의 예는 `ProtocolHeader` 이전 예제의 속성 WCF 구문에서는 메시지 속성으로 더욱 정확 하 게 표시 되는 반면 합니다. 또한 WCF 메시지 헤더가 끝점의 구성에 추가할 수 있습니다.
...
WCF 서비스에서 처리 되지 않은 예외가 클라이언트에 중요 한 정보가 예외를 통해 실수로 노출 되지 않도록 하려면 SOAP 오류로 반환 되지 않습니다. 디버깅을 위해 처리되지 않은 예외가 클라이언트에게 반환되도록 하는 구성 설정이 제공됩니다.
...
여기, 서비스 유형 `TradingSystem`에 <xref:System.ServiceModel.ServiceBehaviorAttribute> 동일한 WCF 클라이언트 인스턴스의 모든 호출이 서비스 유형의 같은 인스턴스로 라우팅되도록 지정 하는 합니다.
...
상태 정보에는 ASP.NET을 통해 제어를 제공 하는 반면는 <xref:System.Web.HttpContext> 클래스는 실제로 저장 하 고, WCF, 적어도 초기 버전에서는 확장 가능한 개체가 저장 되는 위치 제어 하지 않습니다. WCF 서비스에 대 한 ASP.NET 호환 모드를 선택 하기 위한 최고의 이유를 구성 하는 합니다. 구성 가능한 상태 관리가 필수적인 경우 ASP.NET 호환 모드를 채택함으로써 <xref:System.Web.HttpContext> 클래스의 기능을 ASP.NET에서와 똑같은 방식으로 사용할 수 있으며 <xref:System.Web.HttpContext> 클래스를 사용하여 관리되는 상태 정보가 저장되는 위치를 구성할 수도 있습니다.
...
IIS는 익명 액세스 또는 다양 한 인증 모드를 선택할 수는 응용 프로그램에 대 한 액세스 제어 기능을 제공 합니다. Windows 인증, 다이제스트 인증, 기본 인증 및.NET Passport 인증 합니다. Windows 인증 옵션을 사용하면 ASP.NET 웹 서비스에 대한 액세스를 제어할 수 있습니다. 그러나 WCF 응용 프로그램을 IIS에서 호스팅되는 경우에 IIS 다양 한 다른 옵션과 함께 Windows 인증을 지 원하는 자체 WCF에서 해당 인증을 관리할 수 있도록 응용 프로그램에 익명 액세스를 허용 하도록 구성 되어야 합니다. 기본 제공되는 다른 옵션으로는 사용자 이름 토큰, X.509 인증서, SAML 토큰, CardSpace 카드 등이 있으며 사용자 지정 인증 메커니즘을 정의할 수도 있습니다.
...
WCF 구성 시스템 가장할 특정 사용자 지정 하기 위한 자체 id 요소를 제공 합니다. 또한 WCF 클라이언트 및 서비스 구성할 수 있습니다 독립적으로 가장 합니다. 클라이언트는 요청을 전송할 때 현재 사용자로 가장하도록 구성할 수 있습니다.
...
### <a name="security-authorization-using-access-control-lists"></a>보안: Access Control 목록을 사용 하 여 권한 부여

ACL(Access Control 목록)을 사용하면 .asmx 파일에 대한 액세스를 제한할 수 있습니다. 그러나 ASP.NET 호환 모드에서 WCF.svc 파일의 Acl은 제외 하 고 무시 됩니다.
...
실제로 역할 공급자 메커니즘은 WCF 응용 프로그램을 비롯 한 모든.NET 응용 프로그램에서 ASP.NET과 별도로 사용할 수 있습니다. WCF 응용 프로그램의 다음 샘플 구성 방법을 보여 줍니다 ASP.NET 역할 공급자를 사용 하 여 이용 하 여 선택한 옵션을 <xref:System.ServiceModel.Description.ServiceAuthorizationBehavior>입니다.
...
WCF의 가장 중요 한 혁신 중 하나는 클레임을 기반으로 하는 보호 된 리소스에 대 한 액세스 권한 부여를 위한 완벽 하 게 지원 합니다. 클레임은 형식, 권한 및 값으로 구성됩니다. 예를 들어, 운전 면허증을 생각해 보세요. 운전 면허증은 소지인에 대한 클레임 집합으로 구성되는데, 그 중 하나는 소지인의 생년월일입니다. 이 클레임의 형식은 생년월일이고, 값은 운전자의 생년월일입니다. 클레임이 소지인에게 부여하는 권한은 소지인이 클레임 값으로 수행할 수 있는 작업을 지정합니다. 운전자의 생년월일에 대한 클레임의 경우 권한은 단순한 소유입니다. 운전자는 해당 생년월일을 소유하고 있지만 변경하는 등의 작업은 할 수 없습니다. 역할은 단순히 클레임의 한 형식이기 때문에 클레임 기반 인증은 역할 기반 인증을 포함합니다.

클레임 기반 인증은 클레임 집합을 작업의 액세스 요구 사항과 비교하여 그 결과에 따라 작업에 대한 액세스 권한을 부여하거나 거부함으로써 수행됩니다. Wcf에서 사용 하 여 값을 할당 하 여 클레임 기반 권한 부여를 다시 한 번 실행 하는 클래스를 지정할 수 있습니다 합니다 `ServiceAuthorizationManager` 속성의 <xref:System.ServiceModel.Description.ServiceAuthorizationBehavior>합니다.
...
클레임 기반 권한 부여를 실행하는 데 사용되는 클래스는 <xref:System.ServiceModel.ServiceAuthorizationManager>에서 파생되어야 하며, 여기서 `AccessCheck()` 메서드만 재정의해야 합니다. WCF 서비스 작업 호출 되 고 제공 될 때마다 해당 메서드를 호출 하는 <xref:System.ServiceModel.OperationContext> 사용자에 대 한 클레임에 있는 개체에 해당 `ServiceSecurityContext.AuthorizationContext` 속성입니다. WCF는 보안 토큰 으로부터 사용자에 대 한 클레임을 조합 작업 상태로 유지 하는 인증을 위해 제공한 사용자는 이러한 클레임이 해당 작업에 대 한 충분 여부를 평가 하는 작업입니다.

WCF 보안의 모든 종류의 클레임을 자동으로 조합는 토큰은 매우 중요 한 혁신 인증 메커니즘 으로부터 완전히 독립 클레임 기반 권한 부여 코드입니다. 반면 ASP.NET에서 ACL 또는 역할을 사용하는 권한 부여는 Windows 인증과 밀접하게 연결됩니다.
