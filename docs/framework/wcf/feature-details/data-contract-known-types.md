...
- 보낸 데이터 계약은 필요한 데이터 계약에서 파생됩니다. 자세한 내용은 [데이터 계약 동등성](data-contract-equivalence.md)의 상속에 대 한 섹션을 참조 하세요. 이 경우 전송한 데이터에는 수신하는 엔드포인트에서 필요한 것과 동일한 데이터 계약이 없습니다.  
...
- 전송할 정보에 대해 선언된 형식은 <xref:System.Object>입니다. 모든 형식은 <xref:System.Object>에서 상속되며 실제로 보낸 형식을 미리 알 수 없으므로 수신하는 엔드포인트에서 전송된 데이터에 대한 데이터 계약을 미리 확인할 수 없습니다. 이것은 첫 번째 항목의 특별한 경우입니다. 모든 데이터 계약은 <xref:System.Object>에 대해 생성되는 빈 데이터 계약인 기본값에서 파생됩니다.  
