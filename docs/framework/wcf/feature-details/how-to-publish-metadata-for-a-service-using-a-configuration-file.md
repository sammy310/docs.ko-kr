...
> [!CAUTION]
> 이 항목에서는 보호되지 않은 방식으로 메타데이터를 게시하는 방법을 보여 줍니다. 즉, 모든 클라이언트가 서비스에서 메타데이터를 검색할 수 있습니다. 서비스에서 메타 데이터를 안전한 방식으로 게시 해야 하는 경우 [사용자 지정 보안 메타 데이터 끝점](../samples/custom-secure-metadata-endpoint.md)을 참조 하세요.  
  
 코드에 메타 데이터를 게시 하는 방법에 대 한 자세한 내용은 [방법: 코드를 사용 하 여 서비스의 메타 데이터 게시](how-to-publish-metadata-for-a-service-using-code.md)를 참조 하세요. 메타데이터를 게시하면 클라이언트에서 WS-Transfer GET 요청을 사용하는 메타데이터 또는 `?wsdl` 쿼리 문자열을 사용하는 HTTP/GET 요청을 검색할 수 있습니다. 코드가 작동 하는지 확인할 수 있도록 기본 WCF 서비스를 만듭니다. 편의상 다음 코드로 된 기본 자체 호스팅 서비스가 제공됩니다.  
...
10. Internet Explorer를 사용 하 여 서비스의 기본 주소 ( `http://localhost:8001/MetadataSample` 이 예제에서는)로 이동 하 여 메타 데이터 게시가 설정 되었는지 확인 합니다. 그렇지 않으면 "이 서비스에 대한 메타데이터 게시는 현재 사용할 수 없습니다."라는 메시지가 결과 페이지의 맨 위에 표시됩니다.  
