...
> [!CAUTION]
> Svcutil.exe를 실행 하 여 각각 같은 이름의 작업을 포함 하는 두 개의 서로 다른 서비스 계약을 노출 하는 서비스에 대 한 메타 데이터를 가져올 때 Svcutil.exe는 "에서 메타 데이터를 가져올 수 없습니다." 라는 오류를 표시 합니다. 예를 들어, 작업을 포함 하는 라는 서비스 계약을 노출 하는 서비스가 있고 `ICarService` `Get(Car c)` 동일한 서비스에서 `IBookService` 작업을 포함 하는 라는 서비스 계약을 노출 `Get(Book b)` 하는 경우 이 문제를 해결하려면 다음 중 하나를 수행합니다.
>
> - 작업 중 하나의 이름을 바꿉니다.
...
3. <`url`>인수는 메타 데이터를 제공 하는 서비스 끝점의 URL 또는 온라인으로 호스팅되는 메타 데이터 문서를 지정 합니다. <`epr`> 인수는 `EndpointAddress` Ws-ws-metadataexchange를 지 원하는 서비스 끝점에 대 한 ws-addressing을 포함 하는 XML 파일의 경로를 지정 합니다.  
  
 메타 데이터를 다운로드 하는 데이 도구를 사용 하는 방법에 대 한 자세한 옵션은 [ServiceModel Metadata 유틸리티 도구 (svcutil.exe)](../servicemodel-metadata-utility-tool-svcutil-exe.md)를 참조 하세요.  
