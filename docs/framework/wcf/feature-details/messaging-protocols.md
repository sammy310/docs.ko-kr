...
WCF(Windows 통신 재단) 채널 스택은 인코딩 및 전송 채널을 사용하여 내부 메시지 표현을 와이어 형식으로 변환하고 특정 전송을 사용하여 전송합니다. 웹 서비스 상호 운용성을 위해 사용되는 가장 일반적인 전송은 HTTP이고, 웹 서비스에 사용되는 가장 일반적인 인코딩은 XML 기반 SOAP 1.1, SOAP 1.2 및 MTOM(Message Transmission Optimization Mechanism)입니다.

이 항목에서는 에서 <xref:System.ServiceModel.Channels.HttpTransportBindingElement>사용하는 다음 프로토콜에 대한 WCF 구현 세부 정보를 다룹니다.

사양/문서:

- [HTTP 1.1](https://www.ietf.org/rfc/rfc2616.txt)
- [비누 1.1 HTTP 바인딩](https://www.w3.org/TR/2000/NOTE-SOAP-20000508), 섹션 7
- [비누 1.2 HTTP 바인딩](https://www.w3.org/TR/soap12-part2) 7편

이 항목에서는 <xref:System.ServiceModel.Channels.TextMessageEncodingBindingElement> 다음 프로토콜에 <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement> 대한 WCF 구현 세부 정보를 다루고 있습니다.
...
- [W3C 웹 서비스 주소 지정 1.0 - WSDL 바인딩](https://www.w3.org/TR/2006/CR-ws-addr-wsdl-20060529/)
...
이 항목에서는 <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement> 다음 프로토콜을 사용하는 WCF 구현 세부 정보를 다룹니다.
...
WCF는 기본 프로파일 1.1(BP11) 및 기본 프로파일 1.0(SSBP10)에 이어 SOAP 1.1 봉투 처리를 구현합니다. SOAP 1.2 봉투 처리는 SOAP12-Part1에 따라 구현됩니다.

이 섹션에서는 BP11 및 SOAP12-Part1과 관련하여 WCF에서 수행한 특정 구현 선택 사항을 설명합니다.

#### <a name="mandatory-header-processing"></a>필수 헤더 처리
WCF는 SOAP 1.1 `mustUnderstand` 및 SOAP 1.2 사양에 설명된 헤더 처리 규칙에 따라 다음과 같은 변형이 있습니다.

WCF 채널 스택에 들어가는 메시지는 텍스트 메시지 인코딩, 보안, 신뢰할 수 있는 메시징 및 트랜잭션과 같은 연결된 바인딩 요소로 구성된 개별 채널에서 처리됩니다. 각 채널은 연결된 네임스페이스에서 헤더를 인식한 후 인식된 것으로 표시합니다. 메시지가 디스패처에 전달되면 작업 포맷터는 해당 메시지/작업 계약에 필요한 헤더를 읽고 인식된 것으로 표시합니다. 그런 다음 디스패처는 나머지 헤더가 인식되지 않았지만 `mustUnderstand`로 표시되지 않았는지 확인하고 예외를 throw합니다. 받는 사람을 대상으로 하는 `mustUnderstand` 헤더가 포함된 메시지는 받는 사람 애플리케이션 코드로 처리되지 않습니다.

이러한 계층화된 처리를 사용하면 SOAP 노드의 애플리케이션 계층과 인프라 계층을 분리할 수 있습니다.

- B1111: 메시지가 WCF 인프라 채널 스택에 의해 처리된 후 응용 프로그램에서 처리되기 전에 이해되지 않는 헤더가 검색됩니다.
...
- B1112: WCF는 `mustUnderstand` SOAP 1.1 및 SOAP 봉투의 SOAP 1.2 버전 모두에 대해 값 0과 1을 방출합니다. `xs:boolean` WCF는 `mustUnderstand` 헤더의 전체 값 공간을 허용합니다(0, 1, `false`, ) `true`

#### <a name="soap-faults"></a>SOAP 오류
다음은 WCF 관련 SOAP 오류 구현 목록입니다.

- B2121: WCF는 다음 SOAP 1.1 `s11:mustUnderstand` `s11:Client`오류 `s11:Server`코드를 반환합니다.

- B2122: WCF는 다음 SOAP 1.2 `s12:MustUnderstand` `s12:Sender`오류 `s12:Receiver`코드를 반환합니다.

### <a name="http-binding"></a>HTTP 바인딩

#### <a name="soap-11-http-binding"></a>SOAP 1.1 HTTP 바인딩
WCF는 기본 프로필 1.1 사양 섹션 3.4에 따라 SOAP1.1 HTTP 바인딩을 구현하고 다음과 같은 설명을 수행합니다.
...
WCF는 다음과 같은 설명과 함께 SOAP 1.2 부 2(SOAP12Part2) 사양에 설명된 대로 SOAP 1.2 HTTP 바인딩을 구현합니다.
...
WCF가 구현하는 WS 주소 지정의 모든 버전은 끝점 참조를 사용하여 끝점을 설명합니다.

#### <a name="endpoint-references-and-ws-addressing-versions"></a>엔드포인트 참조 및 WS-Addressing 버전
WCF는 WS-주소 지정을 사용하는 여러 인프라 프로토콜을 구현하며, 특히 `EndpointReference` 요소 및 `W3C.WsAddressing.EndpointReferenceType` 클래스(예: WS-ReliableMessaging, WS-SecureConversation 및 WS-Trust)를 구현합니다. WCF는 다른 인프라 프로토콜과 함께 WS-주소 지정 의 두 버전 의 사용을 지원합니다. WCF 끝점은 엔드포인트당 WS 주소 지정의 한 버전을 지원합니다.

R3111의 `EndpointReference` 경우 WCF 끝점과 교환된 메시지에 사용되는 요소 또는 형식의 네임스페이스는 이 끝점에서 구현된 WS-Addressing 버전과 일치해야 합니다.

예를 들어 WCF 끝점이 WS-ReliableMessaging을 구현하는 `AcksTo` 경우 내부에 `CreateSequenceResponse` 있는 이러한 끝점에서 반환되는 헤더는 `EncodingBinding` 요소가 이 끝점에 대해 지정하는 WS-주소 지정 버전을 사용합니다.

#### <a name="endpoint-references-and-metadata"></a>엔드포인트 참조 및 메타데이터
대부분의 시나리오에서는 지정된 엔드포인트에 대해 메타데이터 또는 메타데이터 참조를 전달해야 합니다.

B3121: WCF는 WS-MetadataExchange(MEX) 사양 섹션 6에 설명된 메커니즘을 사용하여 값 또는 참조별로 끝점 참조에 대한 메타데이터를 포함합니다.

에서 토큰 발급자가 발급한 SAML(보안 어설션 태그 언어) 토큰을 사용하여 WCF 서비스에 인증이 필요한 시나리오를 생각해 `http://sts.fabrikam123.com`보십시오. WCF 끝점은 토큰 발급자를 가리키는 `sp:IssuedToken` 중첩 `sp:Issuer` 된 어설션을 사용하여 이 인증 요구 사항을 설명합니다. ph x=&quot;1&quot; /&amp;gt; 어설션에 액세스하는 클라이언트 애플리케이션은 토큰 발급자 엔드포인트와 통신하는 방법을 알고 있어야 합니다. 클라이언트는 토큰 발급자에 대한 메타데이터를 알고 있어야 합니다. WCF는 MEX에 정의된 끝점 참조 메타데이터 확장을 사용하여 토큰 발급자 메타데이터에 대한 참조를 제공합니다.
...
WS-주소 지정 버전 모두에서 `wsa:To`WCF는 사양에 의해 규정된 대로 `wsa:ReplyTo`다음 `wsa:Action` `wsa:MessageID`메시지 `wsa:RelatesTo`헤더를 사용합니다.

B3211: 모든 WS 주소 지정 버전에 대해 WCF는 명예를 제공하지만 즉시 생성되지는 않지만 `wsa:FaultTo` `wsa:From`WS-주소 지정 메시지 헤더 및 .

WCF 응용 프로그램과 상호 작용하는 응용 프로그램은 이러한 메시지 헤더를 추가할 수 있으며 WCF는 그에 따라 처리합니다.

#### <a name="reference-parameters-and-properties"></a>참조 매개 변수 및 속성

WCF는 각 사양에 따라 엔드포인트 참조 매개 변수 및 참조 속성의 처리를 구현합니다.

B3221: WS-주소 지정 2004/08을 사용하도록 구성된 경우 WCF 끝점은 처리 되는 참조 속성과 참조 매개 변수를 구분하지 않습니다.

### <a name="message-exchange-patterns"></a>메시지 교환 패턴
웹 서비스 작업 호출과 관련된 메시지 시퀀스를 *메시지 교환 패턴이라고*합니다. WCF는 단방향, 요청-회신 및 이중 메시지 교환 패턴을 지원합니다. 이 단원에서는 사용 중인 메시지 교환 패턴에 따른 메시지 처리에 대한 WS-Addressing 요구 사항에 대해 자세히 설명합니다.

이 단원에서 요청자는 첫 번째 메시지를 보내고 응답자는 첫 번째 메시지를 받습니다.

#### <a name="one-way-message"></a>단방향 메시지
WCF 끝점이 단방향 패턴을 따르도록 지정된 `Action` 메시지를 지원하도록 구성된 경우 WCF 끝점은 다음 동작 및 요구 사항을 따릅니다. 달리 명시되지 않는 한 WCF에서 지원되는 WS-주소 지정의 두 버전 모두에 대해 동작 및 규칙이 적용됩니다.
...
요청 응답 패턴을 따르도록 지정된 `Action` 메시지에 대해 WCF 끝점이 구성된 경우 WCF 끝점은 아래 동작 및 요구 사항을 따릅니다. 달리 명시되지 않는 한 WCF에서 지원되는 WS-주소 지정의 두 버전 모두에 대해 동작 및 규칙이 적용됩니다.

- R3321: 요청자는 끝점 참조에 `wsa:Action` `wsa:MessageID`의해 지정된 모든 참조 매개 변수 또는 참조 속성(또는 둘 다)에 대한 요청 `wsa:To`, 및 헤더에 포함해야 합니다.

- R3322: WS-Addressing 2004/08을 사용하는 경우 `ReplyTo`도 요청에 포함해야 합니다.

- R3323: WS-Addressing 1.0이 `ReplyTo` 사용되고 요청에 없는 경우 [주소] 속성이 있는 기본 끝점 `http://www.w3.org/2005/08/addressing/anonymous` 참조가 사용됩니다.

- R3324: 요청자는 요청의 `wsa:Action`끝점 참조에 의해 지정된 모든 참조 매개 변수 또는 참조 속성(또는 둘 다)에 대한 헤더뿐만 아니라 회신 메시지에 " 및 `wsa:RelatesTo` 헤더를 `ReplyTo` 포함해야 `wsa:To`합니다.

### <a name="web-services-addressing-faults"></a>Web Services Addressing 오류
R3411: WCF는 WS-주소 지정 2004/08에 의해 정의된 다음과 같은 오류를 생성합니다.
...
R3412: WCF는 WS 주소 지정 1.0에 의해 정의된 다음과 같은 오류를 생성합니다.

| 코드 | 원인 |
|----------|-----------|
| `wsa10:InvalidAddressingHeader` | 을 `wsa:To` `wsa:ReplyTo`복제합니다. `wsa:From` `wsa:MessageID` 동일한 `wsa:RelatesTo` `RelationshipType`. |
...
WCF는 정책 어설션을 사용하여 특정 WS 주소 지정 버전에 대한 끝점 지원을 나타냅니다.
...
WCF는 두 개의 기본 전송 채널이 하나의 채널이 입력 메시지에 사용되고 다른 채널은 출력 메시지에 사용되는 복합 듀플렉스 채널을 형성할 수 있는 기능을 제공합니다. HTTP 전송의 경우 복합 이중 채널은 두 개의 역방향 HTTP 연결을 제공합니다. 요청자가 한 연결을 사용하여 메시지를 응답자에게 보내고, 응답자는 다른 연결을 사용하여 메시지를 요청자에게 다시 보냅니다.
...
- R3514: 끝점으로 전송된 요청 메시지에는 끝점이 WSDL `http://www.w3.org/2005/08/addressing/anonymous` 1.1 SOAP 1.x HTTP 바인딩을 사용하고 `wsap10:UsingAddressing` `cdp:CompositeDuplex` 연결된 어설션과 `wsap:UsingAddressing` 결합된 정책 대안이 있는 경우와 같지 않은 `ReplyTo` `[address]` 속성이 있는 헤더가 있어야 합니다.

- R3515: 끝점에 보낸 요청 메시지에는 `ReplyTo` `[address]` WSDL 1.1 SOAP 1.x HTTP 바인딩을 `http://www.w3.org/2005/08/addressing/anonymous`사용하고 `ReplyTo` `wsap10:UsingAddressing` 어설션이 없고 어설션이 첨부된 `cdp:CompositeDuplex` 정책 대안이 있는 경우 속성이 같거나 헤더가 전혀 없어야 합니다.

- R3516: 끝점으로 전송된 요청 메시지에는 끝점이 WSDL 1.1 SOAP 1.x HTTP 바인딩을 사용하고 `ReplyTo` `[address]` `http://www.w3.org/2005/08/addressing/anonymous` `wsap:UsingAddressing` 어설션이 없고 `cdp:CompositeDuplex` 어설션이 연결되지 않은 정책 대안이 있는 경우와 동일한 속성이 있는 헤더가 있어야 합니다.
...
WCF 용어에서 동일하거나 `portType` 계약하지만 WS 주소 지정의 다른 버전을 사용하는 두 개의 끝점을 사용하는 것이 합리적입니다. 그러나 동작이 `portType`에 정의되어 있고 `portType`을 구현하는 엔드포인트를 통해 변경되지 않아야 할 경우 두 기본 동작 패턴을 모두 지원할 수 없습니다.

이 문제를 해결하기 위해 WCF는 `Action` 특성의 단일 버전을 지원합니다.

B3521: WCF는 `wsaw10:Action` WS-ADDR10-WSDL에 정의된 요소의 특성을 `wsdl:portType/wsdl:operation/[wsdl:input | wsdl:output | wsdl:fault]` 사용하여 끝점에서 사용되는 WS-주소 지정 버전에 관계없이 해당 메시지에 대한 `Action` URI를 결정합니다.

#### <a name="use-endpoint-reference-inside-wsdl-port"></a>WSDL 포트에서 엔드포인트 참조 사용
WS-ADDR10-WSDL 단원 4.1에서는 `wsdl:port` 자식 요소를 포함하도록 `<wsa10:EndpointReference…/>` 요소를 확장하여 WS-Addressing 용어로 엔드포인트를 설명합니다. WCF는 WS 주소 지정 2004/08에서 이 `<wsa:EndpointReference…/>` 유틸리티를 확장하여 `wsdl:port`의 자식 요소로 표시할 수 있습니다.

- R3531: 엔드포인트에 `<wsaw10:UsingAddressing/>` 정책 어설션과 연결된 정책 대안이 있는 경우 해당 `wsdl:port` 요소는 `<wsa10:EndpointReference …/>` 자식 요소를 포함할 수 있습니다.

- `wsdl:port` R3532: a가 자식 `<wsa10:EndpointReference …/>`요소를 `wsa10:EndpointReference/wsa10:Address` 포함하는 경우 자식 요소 `@address` 값은 `wsdl:port` / `wsdl:location` 형제 요소의 특성 값과 일치해야 합니다.

- R3533: 엔드포인트에 `<wsap:UsingAddressing/>``wsdl:port` 정책 어설션과 연결된 정책 대안이 있는 경우 해당 `<wsa:EndpointReference …/>` 요소는 자식 요소를 포함할 수 있습니다.

- `wsdl:port` R3534: a가 자식 `<wsa:EndpointReference …/>`요소를 `wsa:EndpointReference/wsa:Address` 포함하는 경우 자식 요소 `@address` 값은 `wsdl:port` / `wsdl:location` 형제 요소의 특성 값과 일치해야 합니다.
...
HTTP 헤더: POST의 대상이 요소의 `wsa10:To` URI와 일치합니다.
...
이 섹션에서는 HTTP SOAP MTOM에 대한 WCF 구현 세부 정보를 설명합니다. MTOM 기술은 기존의 텍스트/XML 인코딩 또는 WCF 바이너리 인코딩과 동일한 클래스의 SOAP 메시지 인코딩 메커니즘입니다. MTOM에는 다음과 같은 내용이 포함됩니다.
...
WCF에서 비 HTTP 전송과 MTOM을 사용할 수 있습니다. 그러나 이 항목에서는 HTTP에 중점을 두어 설명합니다.
...
1. 인코딩할 SOAP 봉투에 `[namespace name]` a `http://www.w3.org/2004/08/xop/include` 및 `[local name]` `Include`a가 있는 요소 정보 항목이 포함되어 있지 않은지 확인합니다.

2. 빈 MIME 패키지를 만듭니다.

3. 원본 XML Infoset에서 최적화할 요소 정보 항목을 식별합니다. 항목을 최적화하려면 요소 정보 항목을 구성하는 `[children]` 문자가 표준 `xs:base64Binary` 형식(XSD-2, 3.2.16 base64Binary 참조)이어야 하며 공백이 아닌 콘텐츠앞에, 인라인 또는 다음에 오는 공백 문자를 포함해서는 안 됩니다.
...
4. 원본 SOAP 봉투의 복사본인 XOP SOAP 봉투를 만듭니다. 단, 이전 단계에서 식별된 각 요소 정보 항목의 자식을 다음과 같이 구성된 `xop:Include` 요소 정보 항목으로 대체합니다.
    ...
    6. 4b 단계에서 생성된 Content-ID 헤더 값에서 파생된 cid: uri 값을 사용하여 `href` 특성을 `xop:Include` 요소에 추가합니다. \<"와 ">" 문자를 제거하고 나머지 문자열을 URL-이스케이프하고 접두사를 `cid:`추가합니다. RFC1738 및 RFC2396으로 이스케이프하려면 다음과 같은 최소 문자 집합이 필요합니다. 다른 문자를 이스케이프할 수 있습니다.
...
3. `xop:Include` 요소 정보 항목을 [children] 속성의 단독 멤버로 가진 구성된 SOAP 봉투의 각 요소 정보 항목의 경우 다음을 수행합니다.
    ...
    1. `cid:` 접두사를 제거하고 `@href` 요소의 `xop:Include` 특성 값에서 모든 URI 이스케이프 시퀀스(RFC 2396)를 이스케이프 해제합니다. 결과 문자열을 ",\<">"로 동봉합니다.
...
다음은 MTOM 사양 자체에 명시된 요구 사항에서 파생된 SOAP 1.x MTOM 인코딩 메시지의 HTTP 콘텐츠 유형 헤더 형식에 대한 WCF 설명 목록이며 MTOM 및 RFC 2387에서 파생됩니다.
...
RFC 2387에서 이중 따옴표를 사용해야 하는 요구 사항은 명시적이지 않지만 텍스트는 모든 다중 부분/관련 미디어 형식\@매개 변수에 "또는 "/"와 같은 예약된 문자가 포함될 가능성이 있으므로 이중 따옴표가 필요합니다.
...
">" 내에 동봉된\<이메일 주소입니다. `[CFWS]` 접두사와 접미사는 설명을 전달하기 위해 RFC 2822에 추가되었지만 상호 운용성을 유지하려면 사용하지 않아야 합니다.

R4143: Infoset MIME 부분의 Content-ID 헤더 값은 `msg-id` 접두사 및 접미사 부분을 생략한 상태의 RFC 2822의 `[CFWS]` 생성을 따라야 합니다.

많은 MIME 구현은 ">"\<내에 포함된 값에 대한 요구 사항을 완화하여 `absoluteURI` 전자 메일\<주소로 묶고 전자 메일 주소 외에 "" " " " >"로 동봉된 데 사용됩니다. 이 WCF 버전은 양식의 콘텐츠 ID MIME 헤더값을 사용합니다.
...
- R4148: SOAP1.1 정보 집합 부품에는 미디어 유형 응용 프로그램/xop+xml 및 매개 변수 유형="텍스트/xml" 및 charset이 있는 콘텐츠 유형 헤더가 포함되어야 합니다.
...
- R4149: SOAP 1.2 정보 집합 부분에는 미디어 유형 `application/xop+xml` 및 매개 변수`application/soap+xml`type=" 및 `charset`.
...
- B4152: WCF는 base64 인코딩된 데이터를 포함하고 길이가 1024바이트를 초과하는 요소 정보 항목을 최적화합니다.

MTOM을 사용하도록 구성된 WCF 끝점은 항상 MTOM 인코딩된 메시지를 보냅니다. 필수 조건을 충족하는 부분이 없더라도 MTOM 인코딩된 메시지를 보냅니다. 이 메시지는 단일 MIME 부분에서 SOAP 봉투를 포함하는 MIME 패키지로 serialize됩니다.
...
- B4212: MTOM 최적화를 사용하도록 구성된 경우 WCF 끝점은 해당 `wsdl:binding`에 연결된 정책에 MTOM 정책 어설션을 추가합니다.

### <a name="composition-with-ws-security"></a>WS-Security를 사용하여 구성
MTOM은 WCF 바이너리 `text/xml` XML과 유사한 인코딩 메커니즘입니다. MTOM은 WS-Security 및 기타 WS-* 프로토콜을 사용하여 자연스러운 구성을 제공합니다. WS-Security를 사용하여 보안된 메시지는 MTOM을 사용하여 최적화할 수 있습니다.
...
이 예제에서 메시지는 WS-Security를 사용하여 보호된 SOAP 1.2 및 MTOM을 사용하여 인코딩됩니다. 인코딩을 위해 식별되는 이진 부분은 `BinarySecurityToken`의 콘텐츠로서, 암호화된 서명 및 암호화된 본문에 해당하는 `CipherValue`의 `EncryptedData`입니다. `CipherValue` WCF의 `EncryptedKey` 최적화를 위해 의 길이가 1024 바이트 미만이기 때문에 의 최적화를 위해 식별되지 않았습니다.
