...
WCF에서 구현 하는 모든 버전의 WS-ADDRESSING은 끝점 참조를 사용 하 여 끝점을 설명 합니다.

#### <a name="endpoint-references-and-ws-addressing-versions"></a>엔드포인트 참조 및 WS-Addressing 버전
WCF는 ws-addressing 및 특히 `EndpointReference` 요소와 `W3C.WsAddressing.EndpointReferenceType` 클래스 (예: Ws-reliablemessaging, WS-SECURECONVERSATION 및 ws-trust)를 사용 하는 다양 한 인프라 프로토콜을 구현 합니다. WCF는 다른 인프라 프로토콜을 사용 하는 두 가지 버전의 WS-ADDRESSING 사용을 지원 합니다. WCF 끝점은 끝점 당 한 가지 버전의 WS-ADDRESSING을 지원 합니다.

R3111의 경우 `EndpointReference` WCF 끝점과 교환 되는 메시지에 사용 되는 요소 또는 형식에 대 한 네임 스페이스는이 끝점에서 구현 하는 ws-addressing의 버전과 일치 해야 합니다.

예를 들어 WCF 끝점이 WS-RELIABLEMESSAGING을 구현 하는 경우의 해당 `AcksTo` 끝점에서 반환 되는 헤더는 `CreateSequenceResponse` `EncodingBinding` 요소가이 끝점에 대해 지정 하는 ws-addressing 버전을 사용 합니다.

#### <a name="endpoint-references-and-metadata"></a>엔드포인트 참조 및 메타데이터
대부분의 시나리오에서는 지정된 엔드포인트에 대해 메타데이터 또는 메타데이터 참조를 전달해야 합니다.

B3121: WCF는 Ws-metadataexchange (MEX) 사양 섹션 6에 설명 된 메커니즘을 활용 하 여 값 또는 참조로 끝점 참조에 대 한 메타 데이터를 포함 합니다.
...
두 ws-addressing 버전에 대해 WCF는,,, 및 사양에 따라 지정 된 대로 다음 메시지 헤더를 사용 `wsa:To` `wsa:ReplyTo` `wsa:Action` `wsa:MessageID` `wsa:RelatesTo` 합니다.

B3211: 모든 WS-ADDRESSING 버전의 경우 WCF는이를 준수 하지만 기본적으로 ws-addressing 메시지 헤더 및를 생성 하지는 않습니다 `wsa:FaultTo` `wsa:From` .
...
B3221: WS-ADDRESSING 2004/08을 사용 하도록 구성 된 경우 WCF 끝점은 처리 참조 속성과 참조 매개 변수를 구분 하지 않습니다.
...
지정 된을 사용 하 여 메시지를 지원 하도록 WCF 끝점이 구성 된 경우 wcf 끝점 `Action` 은 다음과 같은 동작 및 요구 사항을 따릅니다. 별도로 지정 하지 않는 한 동작 및 규칙은 WCF에서 지원 되는 두 가지 버전의 WS-ADDRESSING에 적용 됩니다.
...
지정 된을 사용 하 여 메시지에 대 한 WCF 끝점이 `Action` 요청-회신 패턴을 따르도록 구성 된 경우 wcf 끝점은 아래 동작 및 요구 사항을 따릅니다. 달리 지정 되지 않은 경우, WCF에서 지원 되는 두 가지 버전의 WS-ADDRESSING에 대해 동작 및 규칙이 적용 됩니다.
...
- R3323: WS-ADDRESSING 1.0을 사용 하 고 `ReplyTo` 요청에 없는 경우 [address] 속성이 인 기본 끝점 참조가 `http://www.w3.org/2005/08/addressing/anonymous` 사용 됩니다.
...
R3411: WCF는 WS-ADDRESSING 2004/08에 의해 정의 된 다음과 같은 오류를 생성 합니다.
...
R3412: WCF는 WS-ADDRESSING 1.0에 의해 정의 된 다음과 같은 오류를 생성 합니다.
...
B3521: WCF는 `wsaw10:Action` `wsdl:portType/wsdl:operation/[wsdl:input | wsdl:output | wsdl:fault]` WS-ADDR10-WSDL-WSDL에 정의 된 요소에 대해 특성을 사용 하 여 `Action` 끝점에서 사용 하는 ws-addressing 버전에 관계 없이 해당 메시지에 대 한 URI를 확인 합니다.

#### <a name="use-endpoint-reference-inside-wsdl-port"></a>WSDL 포트에서 엔드포인트 참조 사용
WS-ADDR10-WSDL 단원 4.1에서는 `wsdl:port` 자식 요소를 포함하도록 `<wsa10:EndpointReference…/>` 요소를 확장하여 WS-Addressing 용어로 엔드포인트를 설명합니다. WCF는 WS-ADDRESSING 2004/08에서이 유틸리티를 확장 하 `<wsa:EndpointReference…/>` 여의 자식 요소로 표시 될 수 있도록 `wsdl:port` 합니다.
