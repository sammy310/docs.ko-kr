...
## <a name="definition-of-federated-security"></a>페더레이션 보안 정의  
 ...
 WCF는 페더레이션된 보안을 사용하는 분산 시스템을 구축하고 배포하는 데 대한 지원을 제공합니다.  
...
### <a name="example-scenario"></a>예제 시나리오  
 다음 그림에서는 페더레이션된 보안의 예를 보여 주며 다음과 같은 예가 보여 주어 있습니다.  
...
> [!NOTE]
> 이 섹션에서는 *리소스,* *서비스*및 *웹 서비스* 용어를 상호 교환적으로 사용합니다.  
...
## <a name="support-for-federated-security-in-wcf"></a>WCF의 페더레이션 보안을 위한 지원  
 WCF는 [ \<wsFederationHttpBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md)통해 페더레이션된 보안 아키텍처를 배포하기 위한 턴키 지원을 제공합니다.  
  
 wsFederationHttpBinding>요소는 HTTP를 요청 회신 통신 스타일에 대한 기본 전송 메커니즘으로 사용하도록 수반하는 안전하고 안정적이며 상호 운용 가능한 바인딩을 제공하며 텍스트와 XML을 인코딩용 와이어 형식으로 사용합니다. [ \<](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md)  
  
 페더레이션된 보안 시나리오에서 [ \<wsFederationHttpBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) 사용하는 것은 다음 섹션에 설명된 대로 논리적으로 독립적인 두 단계로 분리될 수 있습니다.  
  
### <a name="phase-1-design-phase"></a>1단계: 디자인 단계  
 설계 단계에서 클라이언트는 [ServiceModel 메타데이터 유틸리티 도구(Svcutil.exe)를](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) 사용하여 서비스 끝점이 노출하는 정책을 읽고 서비스의 인증 및 권한 부여 요구 사항을 수집합니다. 적절한 프록시는 클라이언트에서 다음 페더레이션 보안 통신 패턴을 만들기 위해 생성됩니다.  
...
### <a name="phase-2-run-time-phase"></a>2단계: 런타임 단계  
 런타임 단계에서 클라이언트는 WCF 클라이언트 클래스의 개체를 인스턴스화하고 WCF 클라이언트를 사용하여 호출합니다. WCF의 기본 프레임워크는 페더레이션된 보안 통신 패턴에서 앞에서 언급한 단계를 처리하고 클라이언트가 서비스를 원활하게 사용할 수 있도록 합니다.  
  
## <a name="sample-implementation-using-wcf"></a>WCF를 사용하여 샘플 구현  
 다음 그림에서는 WCF의 기본 지원을 사용하여 페더레이션된 보안 아키텍처에 대한 샘플 구현을 보여 주습니다.  
...
### <a name="example-myservice"></a>예제 MyService  
 ...
 서비스 끝점은 `MyServiceEndpoint` [ \<wsFederationHttpBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) 사용 하며 STS B에서 발급 하는 `accessAuthorized` 클레임을 사용 하는 유효한 보안 어설션 마크업 언어 (SAML) 토큰을 필요로 합니다. 이는 서비스 구성에 선언적으로 지정됩니다.  
...
> [!NOTE]
> `MyService`에서 필요로 하는 클레임에 대해 미세한 지점이 언급되어야 합니다. 두 번째 그림에서는 `MyService`에는 `accessAuthorized` 클레임과 함께 SAML 토큰이 필요함을 나타냅니다. 좀 더 구체적으로 설명하면 이는 `MyService`에서 필요로 하는 클레임 형식을 지정합니다. 이 클레임 유형의 정규화된 `http://tempuri.org:accessAuthorized` 이름은 서비스 구성 파일에 사용되는 연결된 네임스페이스와 함께 입니다. 이 클레임의 값은 이 클레임의 존재를 표시하고, STS B에 의해 `true`로 설정된다고 가정합니다.  
...
> [!NOTE]
> 다시 말하지만 `userAuthenticated` 클레임은 STS B에 필요한 클레임 유형입니다. 이 클레임 유형의 정규화된 `http://tempuri.org:userAuthenticated` 이름은 STS 구성 파일에 사용되는 관련 네임스페이스와 함께 입니다. 이 클레임의 값은 이 클레임의 존재를 표시하고 STS A에 의해 `true`로 설정된다고 가정합니다.  
...
#### <a name="sts-a"></a>STS A  
 ...
 STS B와 마찬가지로 STS A도 보안 토큰을 발급하고, 이를 위해 단일 엔드포인트를 노출하는 웹 서비스입니다. 그러나 다른 바인딩 ()`wsHttpBinding`사용 하 고 클레임과 함께 `emailAddress` 유효한 CardSpace를 제시 하는 사용자를 요구 합니다. 그 결과, STS A가 `userAuthenticated` 클레임과 함께 SAML 토큰을 발급합니다. 이는 서비스 구성에서 선언적으로 지정됩니다.  
