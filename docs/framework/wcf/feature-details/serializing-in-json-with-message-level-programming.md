...
## <a name="typed-message-programming"></a>형식화된 메시지 프로그래밍  
 서비스 작업에 <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> 또는 <xref:System.ServiceModel.Web.WebGetAttribute>가 적용되면 <xref:System.ServiceModel.Web.WebInvokeAttribute>를 사용합니다. 두 특성을 사용하여 `RequestFormat` 및 `ResponseFormat`을 지정할 수 있습니다. 요청 및 응답에 JSON을 사용 합니다. 모두 설정 `WebMessageFormat.Json`합니다.  JSON을 사용 하려면 사용 해야 합니다는 <xref:System.ServiceModel.WebHttpBinding>를 자동으로 구성 된 <xref:System.ServiceModel.Description.WebHttpBehavior>합니다. WCF 직렬화에 대 한 자세한 내용은 참조 하세요. [Serialization 및 Deserialization](../../../../docs/framework/wcf/feature-details/serialization-and-deserialization.md)합니다. JSON 및 WCF에 대 한 자세한 내용은 참조 하세요. [An Introduction RESTful 서비스와 WCF에 주유소](https://msdn.microsoft.com/magazine/dd315413.aspx)합니다.  
