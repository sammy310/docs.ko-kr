...
> [!NOTE]
>  실제 구현에서는 Match 메서드에 포함은이 메시지 필터가 반환 해야 하는 경우 확인 메시지를 검사 하는 논리 **true** 하거나 **false**합니다.  
...
## <a name="usage"></a>사용법  
 라우팅 서비스를 사용 하 여 사용자 지정 필터를 사용 하려면 추가 해야를 필터 테이블 형식의 새 필터 항목을 지정 하 여 "Custom" 메시지 필터의 정규화 된 형식 이름과 어셈블리의 이름입니다.  다른 MessageFilter와 마찬가지로 사용자 지정 필터의 생성자에게 전달될 filterData 문자열을 지정할 수 있습니다.  
