...
ASP.NET 웹 응용 프로그램 호스팅 서비스의 많은 상업적 공급자는 서버에서 실행 중인 응용 프로그램이 ASP.NET 2.0 Medium Trust 권한 집합에서 실행 되도록 합니다. <xref:System.ServiceModel.BasicHttpBinding>, <xref:System.ServiceModel.WebHttpBinding>또는 전송 수준 보안을 사용 하는 <xref:System.ServiceModel.WSHttpBinding>를 사용 하는 경우 WCF 서비스를 이러한 환경에서 실행할 수 있습니다.
...
WCF는 지원 되는 SOAP 바인딩 중 하나를 사용 하는 SOAP 메시징 외에도 부분적으로 신뢰할 수 있는 응용 프로그램에서 웹 스타일 서비스를 빌드하기 위한 <xref:System.ServiceModel.WebHttpBinding> 지원 합니다. Wcf의 wcf [웹 HTTP 프로그래밍 모델](wcf-web-http-programming-model.md), [Wcf 배포](wcf-syndication.md)및 [AJAX 통합 및 JSON 지원](ajax-integration-and-json-support.md) 기능은 모두 부분 신뢰에서 지원 됩니다.
...
- [ASP.NET 신뢰 수준 및 정책 파일](https://docs.microsoft.com/previous-versions/wyts434y(v=vs.140))
