...
## <a name="dynamicendpoint"></a>DynamicEndpoint  
 <xref:System.ServiceModel.Discovery.DynamicEndpoint>는 검색을 수행하고 일치하는 서비스를 자동으로 선택하는 표준 끝점(자세한 내용은 [표준 끝점](../../../../docs/framework/wcf/feature-details/standard-endpoints.md)참조)입니다. 검색할 계약과 사용할 바인딩을 전달하여 <xref:System.ServiceModel.Discovery.DynamicEndpoint>를 만들고 <xref:System.ServiceModel.Discovery.DynamicEndpoint> 인스턴스를 WCF 클라이언트에 전달합니다. 다음 예제에서는 <xref:System.ServiceModel.Discovery.DynamicEndpoint>를 만들고 사용하여 계산기 서비스를 호출하는 방법을 보여 줍니다. 검색은 클라이언트가 열릴 때마다 수행됩니다. 구성에 정의된 모든 끝점은 끝점 <xref:System.ServiceModel.Discovery.DynamicEndpoint> 구성 `kind ="dynamicEndpoint"` 요소에 특성을 추가하여 로 변환할 수도 있습니다.  
