...
## <a name="dynamicendpoint"></a>DynamicEndpoint  
 ...
 <xref:System.ServiceModel.Discovery.DynamicEndpoint> 검색을 수행 하 고 일치 하는 서비스를 자동으로 선택 하는 표준 끝점 (자세한 내용은 [표준](standard-endpoints.md)끝점 참조)입니다. 검색할 계약과 사용할 바인딩을 전달하여 <xref:System.ServiceModel.Discovery.DynamicEndpoint>를 만들고 <xref:System.ServiceModel.Discovery.DynamicEndpoint> 인스턴스를 WCF 클라이언트에 전달합니다. 다음 예제에서는 <xref:System.ServiceModel.Discovery.DynamicEndpoint>를 만들고 사용하여 계산기 서비스를 호출하는 방법을 보여 줍니다. 검색은 클라이언트가 열릴 때마다 수행됩니다. 구성에 정의 된 끝점은 <xref:System.ServiceModel.Discovery.DynamicEndpoint> `kind ="dynamicEndpoint"` 끝점 구성 요소에 특성을 추가 하 여로 설정할 수도 있습니다.  
