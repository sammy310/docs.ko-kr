...
- 추적과 같은 WCF 기능의 이점을 누릴 수 있습니다.
 ...
 AJAX 사용 끝점을 통해 WCF 서비스를 노출 하는 방법에 대 한 자세한 내용은 [방법: 구성을 사용 하 여 ASP.NET AJAX 끝점 추가](../../../../docs/framework/wcf/feature-details/how-to-use-configuration-to-add-an-aspnet-ajax-endpoint.md) 항목을 참조 하세요.
...
3. 프로젝트 이름을 `ASPHello` 하 고 **확인**을 클릭 합니다.
...
2. 서비스의 이름을 `WCFHello` 하 고 **추가**를 클릭 합니다.

3. WCFHello.svc.cs 파일을 엽니다.

4. Service1.asmx.cs에서 다음 `HelloWorld` 작업의 구현을 복사 합니다.
...
5. 다음 코드 대신 WCFHello.svc.cs 파일에 복사 된 `HelloWorld` 작업 구현을 붙여넣습니다.
...
6. <xref:System.ServiceModel.ServiceContractAttribute>에 대 한 `Namespace` 특성을 `WCFHello`로 지정 합니다.
...
7. `HelloWorld` 작업에 <xref:System.ServiceModel.Web.WebInvokeAttribute>을 추가 하 고 <xref:System.ServiceModel.Web.WebMessageFormat.Xml>를 반환 하도록 <xref:System.ServiceModel.Web.WebInvokeAttribute.ResponseFormat%2A> 속성을 설정 합니다. 이 속성을 설정하지 않은 경우 기본 반환 형식은 <xref:System.ServiceModel.Web.WebMessageFormat.Json>입니다.
...
10. 이제 서비스는 HTTP POST 요청에 응답 하는 `WCFHello.svc/HelloWorld`의 끝점을 노출 합니다. HTTP POST 요청은 브라우저에서 테스트할 수 없지만 엔드포인트는 다음과 같은 XML을 반환합니다.

    ```xml
    <string xmlns="http://schemas.microsoft.com/2003/10/Serialization/">Hello World</string>
    ```

11. `WCFHello.svc/HelloWorld` 및 `Service1.aspx/HelloWorld` 끝점은 이제 기능적으로 동일 합니다.
...
- WCF 서비스가 먼저 추가 되는 경우에는 프록시의 WCF 스타일 정의가 우선적으로 적용 되기 때문에 ASMX 웹 서비스에서 메서드를 호출 하면 <xref:System.Web.UI.ObjectConverter.ConvertValue%28System.Object%2CSystem.Type%2CSystem.String%29> 예외가 발생 합니다.

- ASMX 웹 서비스를 먼저 추가한 경우 WCF 서비스에서 메서드를 호출 하면 프록시에서의 웹 서비스 스타일 정의가 우선적으로 적용 되기 때문에 <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> 예외가 발생 합니다.
...
- ASP.NET AJAX <xref:System.Web.Script.Serialization.JavaScriptSerializer> {"one": 1, "two": 2}
...
|<xref:System.DateTime> UTC(협정 세계시)|는 "\\/날짜 (123456789U)\\/" 또는 "\\/날짜\\형식을 지원 하지 않습니다 (\d + (&#124;U (\\+\\-[\d{4}])).\\)\\\\/) ".|"\\/날짜 (123456789U)\\/" 및 "\\/날짜\\형식 (\d + (U&#124;(\\+\\-[\d{4}]))을 지원 하나요?\\)\\\\/) "를 DateTime 값으로|
