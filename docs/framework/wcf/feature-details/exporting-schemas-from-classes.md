...
## <a name="the-export-process"></a>내보내기 프로세스  
 ...
 `XmlSchemaSet` 일부인의.NET Framework의 개체 모델 SOM (스키마)는 XSD 스키마 문서 집합을 나타냅니다. `XmlSchemaSet`에서 XSD 문서를 만들려면 <xref:System.Xml.Schema.XmlSchemaSet.Schemas%2A> 클래스의 `XmlSchemaSet` 속성에서 스키마 컬렉션을 사용합니다. 그런 다음 <xref:System.Xml.Schema.XmlSchema> 를 사용하여 각 <xref:System.Xml.Serialization.XmlSerializer>개체를 serialize합니다.  
