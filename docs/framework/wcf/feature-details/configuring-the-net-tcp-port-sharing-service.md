...
> [!NOTE]
> UAC (사용자 계정 컨트롤)를 사용 하도록 설정 된 Windows Vista 시스템에서는 계정이 Administrators 그룹의 구성원 인 경우에도 로컬 사용자에 게 높은 권한이 필요 합니다. 이러한 사용자가 권한 상승 없이 포트 공유 서비스를 사용할 수 있게 하려면 사용자의 SID (또는 사용자가 구성원 인 그룹의 SID)가 Smsvchost.exe의 \<allowAccounts > 섹션에 명시적으로 추가 되어야 합니다.  
