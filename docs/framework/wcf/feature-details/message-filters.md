...
|ser|`http://schemas.microsoft.com/2003/10/Serialization`|  
 ...
 XPath 쿼리에서 특정 네임스페이스를 사용할 예정인 경우 이를 고유한 네임스페이스 접두사와 함께 네임스페이스 테이블에 추가하면 모든 XPath 쿼리에서 전체 네임스페이스 대신 이 접두사를 사용할 수 있습니다. 다음 예제에서는 네임 스페이스에 대 한 "custom" 접두사를 정의 `"http://my.custom.namespace"`, filterData에 포함 된 XPath 쿼리에서 사용할 합니다.  
...
```  
 ...
 앞의 예제에서 기본 끝점 "Destination"으로 송신에 실패 하면 라우팅 서비스는 시도 나열 된 시퀀스의 각 끝점에 보내는, 첫 번째 backupservicequeue 보내고 경우 그 다음 alternateServiceQueue로 보내기 이후에 backupServiceQueue로 보내기를 실패합니다. 모든 백업 엔드포인트가 실패할 경우 오류가 반환됩니다.
