...
>  데이터 계약 이름 및 네임스페이스와 데이터 멤버 이름은 대/소문자를 구분합니다.  
 ...
 데이터 멤버 이름 뿐만 아니라 데이터 계약 이름 및 네임 스페이스에 대 한 자세한 내용은 참조 하세요. [Data Contract Names](../../../../docs/framework/wcf/feature-details/data-contract-names.md)합니다.  
...
## <a name="inheritance-interfaces-and-data-contract-equivalence"></a>상속, 인터페이스 및 데이터 계약 동등성  
 ...
 클라이언트와 서비스 간에 매개 변수 및 반환 값을 전달할 때 수신하는 엔드포인트에 파생된 클래스의 데이터 계약이 필요한 경우 기본 클래스의 데이터 계약을 보낼 수 없습니다. 이는 개체 지향 프로그래밍 개념을 따릅니다. 이전 예제에서는 형식의 개체로 `Person` 보낼 수 없습니다는 `Employee` 예상 됩니다.  
 ...
 응용 프로그램 간에 매개 변수 및 반환 값을 전달할 때 필요한 형식이 인터페이스이면 <xref:System.Object> 형식의 예상 형식과 동등합니다. 모든 형식이 궁극적으로 <xref:System.Object>에서 파생되기 때문에 모든 데이터 계약은 궁극적으로 <xref:System.Object>에 대한 데이터 계약에서 파생됩니다. 따라서 인터페이스가 필요할 때 모든 데이터 계약 형식을 전달할 수 있습니다. 인터페이스를 사용 하 여 성공적으로 작동 하려면 추가 단계가 필요 자세한 내용은 [데이터 계약 알려진 형식을](../../../../docs/framework/wcf/feature-details/data-contract-known-types.md)합니다.  
