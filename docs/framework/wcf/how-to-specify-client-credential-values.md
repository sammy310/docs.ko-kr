...
3. **\<security>** **Mode** 특성을 사용 하 여 요소를 **\<security mode =**`MessageOrTransport`**>** 찾습니다 `MessageOrTransport` . 여기서은 보안 모드 중 하나로 설정 됩니다.

4. 모드 값과 일치하는 자식 요소를 찾습니다. 예를 들어 모드가 **Message**로 설정 된 경우 **\<message>** 요소에 포함 된 요소를 찾습니다 **\<security>** .
...
1. 요소 [\<behavior>](../configure-apps/file-schema/wcf/behavior-of-endpointbehaviors.md) 를 요소에 추가 [\<behaviors>](../configure-apps/file-schema/wcf/behaviors.md) 합니다.

2. 요소 [\<clientCredentials>](../configure-apps/file-schema/wcf/clientcredentials.md) 를 요소에 추가 [\<behaviors>](../configure-apps/file-schema/wcf/behaviors.md) 합니다. 필수 `name` 특성을 적절한 값으로 설정해야 합니다.

3. 요소 [\<clientCertificate>](../configure-apps/file-schema/wcf/clientcertificate-of-servicecredentials.md) 를 요소에 추가 [\<clientCredentials>](../configure-apps/file-schema/wcf/clientcredentials.md) 합니다.
...
5. 클라이언트를 구성할 때 다음 코드에서처럼 `behaviorConfiguration` 요소의 `<endpoint>` 특성을 설정하여 동작을 지정합니다. 끝점 요소는 요소의 자식입니다 [\<client>](../configure-apps/file-schema/wcf/client.md) . 또한 `bindingConfiguration` 특성을 클라이언트에 대한 바인딩으로 설정하여 바인딩 구성 이름을 지정합니다. 생성된 구성 파일을 사용하는 경우 바인딩의 이름이 자동으로 생성됩니다. 이 예제에서 이름은 `"tcpBindingWithCredential"`입니다.
...
- [인증서 사용](./feature-details/working-with-certificates.md)
