...
```  
 ...
 Configure 메서드는 개발자가 엔드포인트 및 동작을 추가할 수 있도록 해 주는 <xref:System.ServiceModel.ServiceConfiguration> 인스턴스를 사용합니다. 이 메서드는 서비스 호스트가 열리기 전에 WCF에 의해 호출 됩니다. 정의할 때 app.config 또는 web.config 파일에 지정된 서비스 구성 설정이 무시됩니다.  
...
```  
 ...
 설정에는 <`protocolMappings`> 섹션에는 응용 프로그램 끝점이 추가 되 면만 사용 됩니다는 <xref:System.ServiceModel.ServiceConfiguration> 프로그래밍 방식으로 합니다. 필요에 따라 서비스 구성에서는 기본 응용 프로그램 구성 파일에서 호출 하 여 로드할 수 있습니다 <xref:System.ServiceModel.ServiceConfiguration.LoadFromConfiguration%2A> 다음 설정을 변경 하 고 있습니다. <xref:System.ServiceModel.ServiceConfiguration.LoadFromConfiguration> 클래스를 사용하면 중앙 집중식 구성에서 구성을 로드할 수도 있습니다. 다음 코드에서는 이를 구현하는 방법을 보여 줍니다.  
...
> [!IMPORTANT]
>  <xref:System.ServiceModel.ServiceConfiguration.LoadFromConfiguration%2A> 무시 <`host`> 내에서 설정 된 <`service`>의 태그 <`system.serviceModel`>. 개념상 <`host`> 호스트 구성, 하지 서비스 구성 및 해당 하는 방법에 대 한은 메서드가 실행 되기 전에 로드 됩니다.  
