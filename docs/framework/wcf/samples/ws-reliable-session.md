...
```  
 ...
 엔드포인트에는 "Binding1"이라는 바인딩 구성을 참조하는 `bindingConfiguration` 특성이 포함됩니다. 바인딩 구성은 [\<reliableSession >](../../../../docs/framework/configure-apps/file-schema/wcf/reliablesession.md) 의 `enabled` 특성을 `true`로 설정 하 여 신뢰할 수 있는 세션을 사용 하도록 설정 합니다. ordered 특성을 `true` 또는 `false`로 설정하여 신뢰할 수 있는 순서가 지정된 세션에 대해 배달 보증을 제어합니다. 기본값은 `true`입니다.  
