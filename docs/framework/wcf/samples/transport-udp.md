...
UDP 전송 샘플에서는 UDP 유니캐스트 및 멀티캐스트를 사용자 지정 WCF(WCF) 전송으로 구현하는 방법을 보여 줍니다. 이 샘플에서는 채널 프레임워크를 사용하고 WCF 모범 사례를 따라 WCF에서 사용자 지정 전송을 만드는 데 권장되는 절차를 설명합니다. 사용자 지정 전송을 만드는 단계는 다음과 같습니다.  
  
1. 채널팩토리및ChannelListener가 지원하는 채널 메시지 교환 패턴(IOutputChannel, IInputChannel, IDuplexChannel, IRequest채널 또는 IReplyChannel)을 결정합니다. [Message Exchange Patterns](#MessageExchangePatterns) 그런 다음 이러한 인터페이스의 세션 변형을 지원할지 여부를 결정합니다.  
...
4. 채널 [ \<](../../configure-apps/file-schema/wcf/bindings.md) 스택에 사용자 지정 전송을 추가하는 바인딩>요소를 추가합니다. 자세한 내용은 [바인딩 요소 추가를](#AddingABindingElement)참조하십시오.  
...
### <a name="the-icommunicationobject-and-the-wcf-object-lifecycle"></a>ICommunicationObject 및 WCF 개체 수명 주기  
 WCF에는 와 <xref:System.ServiceModel.Channels.IChannel> <xref:System.ServiceModel.Channels.IChannelFactory>같은 개체의 수명 주기를 관리하는 데 사용되는 <xref:System.ServiceModel.Channels.IChannelListener> 공통 상태 컴퓨터가 있으며 통신에 사용됩니다. 이러한 통신 개체의 상태는 5가지로 나타낼 수 있습니다. 이러한 상태는 <xref:System.ServiceModel.CommunicationState> 열거형으로 나타내며 다음과 같습니다.  
...
- Faulted: Faulted 상태의 개체는 검사를 위해 액세스할 수 있지만 더 이상 사용할 수는 없습니다. 복구 불가능한 오류가 발생하면 해당 개체는 이 상태로 전환됩니다. 이 상태에서 유효한 유일한 전환은 `Closed` 상태입니다.  
...
## <a name="channel-factory-and-channel-listener"></a>채널 팩터리 및 채널 수신기  
 사용자 지정 전송을 작성하는 다음 단계는 클라이언트 채널을 위한 <xref:System.ServiceModel.Channels.IChannelFactory> 및 서비스 채널을 위한 <xref:System.ServiceModel.Channels.IChannelListener>의 구현을 만드는 것입니다. 채널 계층은 채널을 만들기 위해 팩토리 패턴을 사용합니다. WCF는 이 프로세스에 대한 기본 클래스 도우미를 제공합니다.  
...
- 클래스는 <xref:System.ServiceModel.Channels.ChannelFactoryBase> <xref:System.ServiceModel.Channels.ChannelManagerBase> 오버로드를 <xref:System.ServiceModel.Channels.IChannelFactory> `CreateChannel` 하나의 `OnCreateChannel` 추상 적 메서드로 구현하고 통합합니다.  
...
```  
 ...
 그런 다음 `Send()` `BeginSend()` / `EndSend()`구현하고 . 그러면 두 개의 기본 섹션으로 분할됩니다. 먼저 메시지를 바이트 배열로 serialize합니다.  
...
### <a name="the-udpchannellistener"></a>UdpChannelListener  
 샘플이 구현하는 `UdpChannelListener` 것은 <xref:System.ServiceModel.Channels.ChannelListenerBase> 클래스에서 파생됩니다. 단일 UDP 소켓을 사용하여 데이터그램을 받습니다. `OnOpen` 메서드는 비동기 루프에서 UDP 소켓을 사용하여 데이터를 받습니다. 그런 다음 데이터는 메시지 인코딩 프레임워크를 통해 메시지로 변환됩니다.  
  
```csharp
message = MessageEncoderFactory.Encoder.ReadMessage(new ArraySegment<byte>(buffer, 0, count), bufferManager);  
```  
  
 같은 데이터그램 채널은 여러 소스에서 도착한 메시지를 나타내므로 `UdpChannelListener`는 singleton 수신기입니다. 한 번에 이 수신기와 <xref:System.ServiceModel.Channels.IChannel> 연결된 활성 이 하나가 있습니다. 이 샘플에서는 `AcceptChannel` 메서드에서 반환되는 채널이 이후에 삭제되는 경우에만 새 채널을 생성합니다. 메시지를 받으면 이 singleton 채널의 큐에 삽입됩니다.  
...
## <a name="adding-a-binding-element"></a>바인딩 요소 추가  
 이제 팩터리와 채널이 빌드되었으므로 바인딩을 통해 ServiceModel 런타임에 팩터리와 채널을 노출해야 합니다. 바인딩은 서비스 주소와 연결된 통신 스택을 나타내는 바인딩 요소의 컬렉션입니다. 스택의 각 요소는 바인딩 [ \<>](../../configure-apps/file-schema/wcf/bindings.md) 요소로 표시됩니다.  
...
## <a name="adding-metadata-support-for-a-transport-binding-element"></a>전송 바인딩 요소에 대해 메타데이터 지원 추가  
 전송을 메타데이터 시스템에 통합하려면 정책 가져오기 및 내보내기를 모두 지원해야 합니다. 이를 통해 [ServiceModel 메타데이터 유틸리티 도구(Svcutil.exe)를](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)통해 바인딩 클라이언트를 생성할 수 있습니다.  
...
```  
 ...
 엔드포인트에서 SOAP 바인딩을 사용할 때 `UdpTransportBindingElement` 메서드의 `ExportEndpoint` 구현이 전송 URI도 내보냅니다.  
...
1. /SvcutilConfig를 사용하여 구성 파일에 대한 포인트 Svcutil.exe:\<파일>.  
...
#### <a name="policy-export"></a>정책 내보내기  
 형식은 `UdpTransportBindingElement` `IPolicyExportExtension` 내보내기 정책에 대한 지원을 추가하도록 구현합니다. 그 결과, `System.ServiceModel.MetadataExporter`는 이를 포함하는 모든 바인딩에 대한 정책 생성에 `UdpTransportBindingElement`를 포함합니다.  
...
- 해당 바인딩 요소가 포함된 시스템 제공 바인딩 사용 WCF는 에서와 `BasicHttpBinding` `NetTcpBinding`같은 시스템 정의 바인딩을 다수 `WsHttpBinding`제공합니다. 이러한 바인딩 각각은 제대로 정의된 프로필에 연결됩니다.  
...
## <a name="the-udp-test-service-and-client"></a>UDP 테스트 서비스 및 클라이언트  
 이 샘플 전송을 사용하기 위한 테스트 코드는 UdpTestService 및 UdpTestClient 디렉터리에 있습니다. 서비스 코드는 두 가지 테스트로 구성됩니다. 하나는 코드에서 바인딩 및 엔드포인트를 설정하고, 다른 하나는 구성을 통해 바인딩 및 엔드포인트를 설정합니다. 두 테스트 모두 두 개의 엔드포인트를 사용합니다. 한 끝점은 `SampleUdpProfileBinding` [ \<신뢰할 수 있는](https://docs.microsoft.com/previous-versions/ms731375(v=vs.90)) `true`세션>설정된 를 사용합니다. 다른 엔드포인트는 `UdpTransportBindingElement`가 포함된 사용자 지정 바인딩을 사용합니다. 이는 신뢰할 수 `SampleUdpProfileBinding` [ \<있는](https://docs.microsoft.com/previous-versions/ms731375(v=vs.90)) 세션>`false`설정된 사용과 동일합니다. 두 테스트 모두 서비스를 만들고 각 바인딩에 엔드포인트를 추가하며 서비스를 연 다음 서비스를 닫기 전에 사용자가 Enter 키를 누를 때까지 기다립니다.  
...
```  
 ...
 그런 다음 게시된 엔드포인트에 대해 테스트 클라이언트 애플리케이션을 실행할 수 있습니다. 클라이언트 테스트 애플리케이션은 각 엔드포인트에 대해 클라이언트를 만들고 각 엔드포인트에 5개의 메시지를 보냅니다. 클라이언트에 다음과 같이 출력됩니다.  
...
```  
 ...
 구성을 사용하여 게시된 엔드포인트에 대해 클라이언트 애플리케이션을 실행하려면 서비스에서 Enter 키를 누른 다음 테스트 클라이언트를 다시 실행합니다. 서비스에서 다음과 같이 출력되어야 합니다.  
...
```  
 ...
 Svcutil.exe를 사용하여 클라이언트 코드 및 구성을 다시 생성하려면 서비스 애플리케이션을 시작한 다음 샘플의 루트 디렉터리에서 다음 Svcutil.exe를 실행합니다.  
