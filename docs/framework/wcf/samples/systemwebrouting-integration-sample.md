...
이 예제에서는 WCF를 사용 하 여 `movies` 피드 및 `channels` 피드의 두 RSS 피드를 만듭니다. 서비스를 활성화 하는 Url에는 확장명이 포함 되어 있지 않으며 <xref:System.Web.HttpApplication> 클래스에서 파생 된 `Global` 클래스의 `Application_Start` 메서드에 등록 되어 있습니다.  
...
3. 주소 표시줄에서 `movies`를 URL에 추가 하 여 `http://localhost:[port]/movies`를 읽고 ENTER 키를 누릅니다.  
  
     movies 피드가 브라우저에 나타납니다.  
  
4. 주소 표시줄에서 URL에 `channels`을 추가 하 여가 `http://localhost:[port]/channels`를 읽고 ENTER 키를 누릅니다.  
...
3. IIS(인터넷 정보 서비스) 관리자에서 웹 애플리케이션을 만듭니다.  
    ...
    2. **별칭**에 `WebRoutingIntegration`을 입력 합니다.  
...
5. 주소 표시줄에서 URL에 `movies`을 추가 하 여가 `http://localhost:[port]/movies`를 읽고 ENTER 키를 누릅니다.  
  
     movies 피드가 브라우저에 나타납니다.  
  
6. 주소 표시줄에서 URL에 `channels`을 추가 하 여가 `http://localhost:[port]/channels`를 읽고 ENTER 키를 누릅니다.  
