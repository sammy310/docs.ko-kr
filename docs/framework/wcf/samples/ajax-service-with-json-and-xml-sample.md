...
```  
 ...
 기본 데이터 형식은 <xref:System.ServiceModel.Description.WebHttpEndpoint> 는 XML에 대 한 기본 데이터 형식으로 하는 동안 <xref:System.ServiceModel.Description.WebScriptEndpoint> JSON입니다. 자세한 내용은 [ASP.NET 없이 WCF AJAX 서비스 만들기](../../../../docs/framework/wcf/feature-details/creating-wcf-ajax-services-without-aspnet.md)합니다.  
  
 다음 샘플의 서비스는 두 작업을 사용 하 여 표준 WCF 서비스. 두 작업 모두 <xref:System.ServiceModel.Web.WebMessageBodyStyle.Wrapped> 또는 <xref:System.ServiceModel.Web.WebGetAttribute> 특성에서 <xref:System.ServiceModel.Web.WebInvokeAttribute> 본문 스타일을 필요로 합니다. 이는 `webHttp` 동작과 관련이 있으며 JSON/XML 데이터 형식 전환과는 관계가 없습니다.  
...
```
 ...
 작업에 대 한 응답 형식은 XML 기본값인로 지정 됩니다에 대 한 설정 합니다 [ \<webHttp >](../../../../docs/framework/configure-apps/file-schema/wcf/webhttp.md) 동작 합니다. 그러나 응답 형식을 명시적으로 지정하는 것이 좋습니다.  
...
```
 ...
 두 경우 모두 작업 반환 복합 형식을 `MathResult`는 표준 WCF 데이터 계약 형식입니다.  
  
 클라이언트 웹 페이지 XmlAjaxClientPage.htm에는 사용자가 클릭할 때 위의 두 작업 중 하나를 호출 하는 JavaScript 코드가 포함 되어는 **(return JSON) 계산을 수행** 또는 **계산 (return XML)를 수행 합니다.**  페이지의 단추입니다. 서비스를 호출하는 코드는 JSON 본문을 만든 다음 HTTP POST를 사용하여 전송합니다. 요청은 수동으로 만든 JavaScript에서와 달리 합니다 [기본 AJAX 서비스](../../../../docs/framework/wcf/samples/basic-ajax-service.md) 샘플 및 ASP.NET AJAX를 사용 하 여 다른 샘플입니다.  
