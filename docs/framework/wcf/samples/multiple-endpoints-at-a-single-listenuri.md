...
>  이 샘플의 설치 절차 및 빌드 지침은 이 항목의 끝부분에 나와 있습니다.  
 ...
 에 설명 된 대로 합니다 [여러 끝점](../../../../docs/framework/wcf/samples/multiple-endpoints.md) 샘플 서비스는 각각 다른 주소 및 또한 서로 다른 바인딩을 사용 하 여 여러 끝점을 호스팅할 수 있습니다. 이 샘플에서는 동일한 주소에서 여러 엔드포인트를 호스팅할 수 있다는 것을 보여 줍니다. 또한 이 샘플에서는 서비스 엔드포인트가 가지는 두 종류의 주소인 `EndpointAddress` 및 `ListenUri` 간의 차이점을 보여 줍니다.  
...
## <a name="service"></a>서비스  
 이 샘플의 서비스에는 두 개의 계약인 `ICalculator` 및 `IEcho`가 있습니다. 다음 코드에 표시된 것처럼 일반적인 `IMetadataExchange` 엔드포인트 외에도 세 개의 응용 프로그램 엔드포인트가 있습니다.  
