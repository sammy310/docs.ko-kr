...
```  
 ...
 인터넷 정보 서비스(IIS) 또는 WAS(Windows Process Activation Service)에 서비스가 호스팅된 경우 서비스의 기본 주소는 호스팅 환경에 의해 제공됩니다. 자체 호스팅의 경우에는 직접 기본 주소를 지정해야 합니다. 이렇게 사용 하 여 합니다 `add` 요소, 자식 [ \<baseAddresses >](../../../../docs/framework/configure-apps/file-schema/wcf/baseaddresses.md), 자식의 [ \<호스트 >](../../../../docs/framework/configure-apps/file-schema/wcf/host.md), 자식의 [ \<서비스 > ](../../../../docs/framework/configure-apps/file-schema/wcf/service.md) 다음 샘플 구성 에서처럼 합니다.  
