...
## <a name="creating-a-response-message"></a>응답 메시지 만들기  
 메시지 처리는 `MyServiceManager` 형식에서 구현됩니다. `HandleRequest` 메서드에서는 메시지의 `Action` 헤더를 먼저 검사하여 요청이 지원되는지 여부를 확인합니다. 미리 정의 된 SOAP 동작 "http://tempuri.org/HelloWorld/Hello"은 메시지 필터링을 제공 하도록 정의 됩니다. 이는 <xref:System.ServiceModel.ServiceHost>의 WCF 구현에서 서비스 계약 개념과 유사 합니다.  
