...
- 엔드포인트에서 지원되는 계약의 형식. 이는 위에서 생성 및 등록된 형식입니다. Visual Basic 스크립트가 강력한 형식의 COM 환경을 제공 하지 않기 때문에 계약에 대 한 식별자를 지정 해야 합니다. 이 GUID는 OLE/COM 개체 뷰어(OleView.exe)와 같은 COM 도구를 사용하여 볼 수 있는 CalcProxy.tlb에 있는 `interfaceID`입니다. Office VBA 또는 Visual Basic 6.0와 같은 강력한 형식의 환경에서는 계약 매개 변수 대신에 형식 라이브러리에 대 한 명시적 참조를 추가한 다음 프록시 개체의 형식을 선언 하는 데 사용할 수 있습니다. 또한 이렇게 하면 클라이언트 애플리케이션 개발 도중에 IntelliSense 지원이 제공됩니다.  
...
3. Visual Studio에 대 한 개발자 명령 프롬프트에서 언어 관련 폴더 아래의 \client\bin 폴더를 엽니다.  
    ...
    > [!NOTE]
    > Windows Vista, Windows Server 2008, Windows 7 또는 Windows Server 2008 r 2를 사용 하는 경우 관리자 권한으로 명령 프롬프트를 실행 해야 합니다.  
  
4. 을 입력 `tlbexp.exe client.dll /out:CalcProxy.tlb` 하 여 dll을 tlb 파일로 내보냅니다. "형식 라이브러리 내보내기 경고"가 발생할 수 있지만 제네릭 형식이 필요하지 않으므로 문제가 되지는 않습니다.  
  
5. 을 입력 `regasm.exe /tlb:CalcProxy.tlb client.dll` 하 여 형식을 COM에 등록 합니다. "형식 라이브러리 내보내기 경고"가 발생할 수 있지만 제네릭 형식이 필요하지 않으므로 문제가 되지는 않습니다.  
  
6. 을 입력 `gacutil.exe /i client.dll` 하 여 전역 어셈블리 캐시에 어셈블리를 추가 합니다.  
...
7. 명령 프롬프트에서 클라이언트 컴퓨터의 대상 디렉터리로 이동합니다. Windows Vista 또는 Windows Server 2008를 사용 하는 경우 관리자 권한으로 명령 프롬프트를 실행 해야 합니다.  
  
8. 을 입력 `tlbexp.exe client.dll /out:CalcProxy.tlb` 하 여 dll을 tlb 파일로 내보냅니다. "형식 라이브러리 내보내기 경고"가 발생할 수 있지만 제네릭 형식이 필요하지 않으므로 문제가 되지는 않습니다.  
  
9. 을 입력 `regasm.exe /tlb:CalcProxy.tlb client.dll` 하 여 형식을 COM에 등록 합니다. 명령을 실행 하기 전에가 포함 된 폴더로 경로가 설정 되었는지 확인 `regasm.exe` 합니다.  
  
10. 을 입력 `gacutil.exe /i client.dll` 하 여 전역 어셈블리 캐시에 어셈블리를 추가 합니다. 명령을 실행 하기 전에가 포함 된 폴더로 경로가 설정 되었는지 확인 `gacutil.exe` 합니다.  
