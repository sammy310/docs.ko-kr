...
이 샘플에서는 구성 파일이나 WCF(Windows Communication Foundation) 생성된 클라이언트를 사용하지 않고 사용자 지정 바인딩을 정의하고 사용하는 명령코드를 작성하는 방법을 보여 줍니다. 이 샘플에서는 HTTP 전송에서 제공하는 기능과 신뢰할 수 있는 세션 채널을 조합하여 신뢰할 수 있는 HTTP 기반 바인딩을 만듭니다. 이 샘플은 계산기 서비스를 구현하는 [시작 하기를](../../../../docs/framework/wcf/samples/getting-started-sample.md) 기반으로 합니다.  
...
- [사용자 지정 바인딩 샘플](custom-binding.md)
