...
3. 단일 컴퓨터 또는 다중 컴퓨터 구성에서 샘플을 실행 하려면 [Windows Communication Foundation 샘플 실행](running-the-samples.md)의 지침을 따르세요.
    ...
    > [!IMPORTANT]
    > 다중 컴퓨터 구성에서 클라이언트를 실행 하는 경우 `address` 다음에 표시 된 것 처럼 [ \<endpoint> \<client> 요소의](../../configure-apps/file-schema/wcf/endpoint-of-client.md) 특성 및 요소의 특성에서 "localhost"를 `clientBaseAddress` [\<binding>](../../configure-apps/file-schema/wcf/bindings.md) [\<wsDualHttpBinding>](../../configure-apps/file-schema/wcf/wsdualhttpbinding.md) 해당 컴퓨터의 이름으로 바꾸어야 합니다.
