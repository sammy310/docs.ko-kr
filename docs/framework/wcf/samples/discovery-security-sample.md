...
>  2008 검색 버전 프로토콜에는 `PrefixList`가 추가되었습니다.  
 ...
 서명을 컴퓨팅하기 위해 이 샘플에서는 확장된 서명 항목을 확인합니다. XML 서명(`SignedInfo`)은 WS-Discovery 사양에서 요구하는 대로 `ds` 네임스페이스 접두사를 사용하여 만듭니다. 검색 및 주소 지정 네임스페이스의 본문과 모든 헤더는 서명에서 참조되므로 변경할 수 없습니다. 참조 된 각 요소가 배타적 정형화를 사용 하 여 변환 됩니다 (http://www.w3.org/2001/10/xml-exc-c14n# ), 다음 sha-1 다이제스트 값을 계산 됩니다 (http://www.w3.org/2000/09/xmldsig#sha1 ). 참조 되는 모든 요소 및 해당 다이제스트 값에 따라 서명 값 RSA 알고리즘을 사용 하 여 계산 됩니다 (http://www.w3.org/2000/09/xmldsig#rsa-sha1 ).  
