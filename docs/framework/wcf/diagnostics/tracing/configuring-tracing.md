...
- 추적 기능 오작동 시 Windows 오류 이벤트. [이벤트 로깅](../../../../../docs/framework/wcf/diagnostics/event-logging/index.md)을 참조하십시오.  
 ...
 애플리케이션의 구성 파일(웹에 호스팅된 애플리케이션의 경우 Web.config, 자체 호스팅 애플리케이션의 경우 Appname.exe.config)을 편집하여 추적을 구성할 수 있습니다. 다음은 이러한 편집의 예제입니다. 이러한 설정에 대한 자세한 내용은 "추적 을 사용하도록 추적 리스너 구성" 섹션을 참조하십시오.  
...
## <a name="configuring-trace-listeners-to-consume-traces"></a>추적을 사용하도록 추적 수신기 구성  
 런타임시 WCF는 추적 데이터를 데이터를 처리하는 수신기에 공급합니다. WCF는 출력에 사용하는 <xref:System.Diagnostics>형식이 다른 에 대해 미리 정의된 여러 수신기를 제공합니다. 사용자 지정 수신기 형식을 추가할 수도 있습니다.  
...
|자세히|'긍정적인' 이벤트: 성공적인 이정표를 표시하는 이벤트입니다.|사용자 코드와 서비스에 대한 하위 수준 이벤트를 내보냅니다.|일반적으로 디버깅 또는 애플리케이션 최적화에 이 수준을 사용할 수 있습니다.<br /><br /> - 이해 메시지 헤더.|관리자<br /><br /> 애플리케이션 개발자<br /><br /> 제품 개발자|  
