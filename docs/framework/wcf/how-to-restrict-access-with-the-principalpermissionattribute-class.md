...
## <a name="using-a-certificate-to-control-access-to-a-method"></a>인증서를 사용하여 메서드에 대한 액세스 제어  
 ...
 해당 속성에 대 한 인증서를 검사 하려면 참조 [방법: MMC 스냅인을 사용 하 여 인증서 보기](../../../docs/framework/wcf/feature-details/how-to-view-certificates-with-the-mmc-snap-in.md)합니다. 지문 값을 찾으려면 참조 [방법: 인증서의 지문 검색](../../../docs/framework/wcf/feature-details/how-to-retrieve-the-thumbprint-of-a-certificate.md)합니다.  
...
4. 다음 구성 예제와 같이 <xref:System.ServiceModel.Description.ServiceAuthorizationBehavior.PrincipalPermissionMode%2A> 속성을 <xref:System.ServiceModel.Description.PrincipalPermissionMode.UseAspNetRoles>로 설정합니다.  
    ...
    ```  
     ...
     이 값을 `UseAspNetRoles`로 설정하면 `Name`의 `PrincipalPermissionAttribute` 속성이 문자열 비교를 수행하는 데 사용됨을 나타냅니다. 클라이언트 자격 증명으로 인증서를 사용 하면 기본적으로 WCF 연결 인증서 일반 이름과 지문을 클라이언트의 기본 id에 대 한 고유한 값을 세미콜론으로 구분 합니다. `UseAspNetRoles`를 서비스의 `PrincipalPermissionMode`로 설정한 경우 이 기본 ID 값을 `Name` 속성 값과 비교하여 사용자의 액세스 권한을 결정합니다.  
