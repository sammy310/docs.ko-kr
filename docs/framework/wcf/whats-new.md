...
## <a name="webservicehost-and-default-endpoints"></a>WebServiceHost 및 기본 엔드포인트
 Visual Studio 2010에서는 사용자가 엔드포인트를 명시적으로 지정했는지 여부에 상관 없이 WebServiceHost가 기본 엔드포인트를 자동으로 만들었습니다. Visual Studio 2012 이상 버전에서는 끝점이 명시적으로 추가 되 면 WebServiceHost에 기본 끝점을 만듭니다. 클라이언트에 기본 엔드포인트가 필요한 경우에는 엔드포인트를 명시적으로 추가하고 클라이언트를 이 엔드포인트에 연결할 수 있습니다. 또는 응용 프로그램의 구성 파일에 다음 설정을 추가하여 WCF가 이전 동작으로 되돌아가도록 지정할 수도 있습니다.
