...
## <a name="cause"></a>원인  
 CER에 대한 준비 패턴을 제대로 따르지 않았습니다.  이것은 오류 이벤트입니다. <xref:System.Runtime.CompilerServices.RuntimeHelpers.PrepareConstrainedRegions%2A> 메서드 호출은 `catch`/`finally`/`fault`/`filter` 문 바로 앞에 사용 해야 하는 CER을 소개 하는 예외 처리기를 표시 하는 데 사용 됩니다.`try`  
