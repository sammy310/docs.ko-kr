...
## <a name="overview"></a>개요  
 ...
 이 체계는 인터넷에 있는 다른 IP 주소(일반적으로 서버)로 요청을 전송하는 사설 인트라넷에서 실행 중인 클라이언트 애플리케이션에 적합합니다. NAT 디바이스 또는 서버에서 클라이언트 요청의 매핑을 유지할 수 있으므로 응답이 반환될 때 응답을 보낼 위치를 압니다. 하지만 이 체계를 사용하면 서비스를 제공하고 패킷을 수신 대기하며 응답하려는 NAT 장치 뒤에 있는 사설 인트라넷에서 실행 중인 애플리케이션에 문제가 제기될 수 있습니다. 이 피어 투 피어 애플리케이션의 경우 특히 해당됩니다.  
