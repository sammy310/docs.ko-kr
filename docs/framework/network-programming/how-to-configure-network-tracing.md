...
컴퓨터 구성 파일인 machine.config는 %windir%\Microsoft.NET\Framework 폴더에 저장됩니다. 컴퓨터에 설치된 각 .NET Framework 버전에 해당하는 %windir%\Microsoft.NET\Framework 아래의 폴더에 개별 machine.config 파일이 있습니다. 예:
