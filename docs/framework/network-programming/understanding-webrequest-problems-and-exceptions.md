...
## <a name="solutions"></a>솔루션  
 <xref:System.Net.WebException.Status%2A>의 <xref:System.Net.WebException> 속성을 검사하여 문제를 확인합니다. 다음 표에서는 여러 상태 값과 몇 가지 가능한 해결 방법을 보여 줍니다.  
...
|<xref:System.Net.WebExceptionStatus.ConnectFailure>|연결하지 못했습니다.|방화벽 또는 프록시가 연결을 차단합니다. 연결을 허용하도록 방화벽 또는 프록시를 수정합니다.<br /><br /> <xref:System.Net.WebProxy> 생성자(<xref:System.Net.WebProxy>)를 호출하여 클라이언트 애플리케이션에서 `WebServiceProxyClass.Proxy = new WebProxy("http://server:80", true)`를 명시적으로 지정합니다.<br /><br /> Filemon 또는 Regmon을 실행하여 작업자 프로세스 ID에 WSPWSP.dll, HKLM\System\CurrentControlSet\Services\DnsCache 또는 HKLM\System\CurrentControlSet\Services\WinSock2를 액세스하는 데 필요한 권한이 있는지 확인합니다.|  
...
|<xref:System.Net.WebExceptionStatus.ProxyNameResolutionFailure>|도메인 이름 서비스가 프록시 호스트 이름을 확인할 수 없습니다.|프록시를 올바르게 구성합니다. <https://support.microsoft.com/?id=318140>을 참조하세요.<br /><br /> <xref:System.Net.HttpWebRequest> 속성을 <xref:System.Net.HttpWebRequest.Proxy%2A>로 설정하여 `null`가 프록시를 사용하지 않도록 강제로 적용합니다.|  
|<xref:System.Net.WebExceptionStatus.ServerProtocolViolation>|서버 응답이 유효한 HTTP 응답이 아닙니다. 이 문제는 .NET Framework에서 서버 응답이 HTTP 1.1 RFC에 맞지 않는 것을 감지할 경우 발생합니다. 응답에 잘못된 헤더 또는 잘못된 헤더 구분 기호가 포함된 경우 이 문제가 발생할 수 있습니다. RFC 2616에서는 HTTP 1.1 및 서버 응답에 유효한 형식을 정의합니다. 자세한 내용은 [IETF(Internet Engineering Task Force)](https://tools.ietf.org/html/rfc2616) 웹 사이트에서 [RFC 2616 - Hypertext Transfer Protocol -- HTTP/1.1](https://www.ietf.org/)을 참조하세요.|트랜잭션의 네트워크 추적을 가져오고 응답의 헤더를 검사합니다.<br /><br /> 애플리케이션에 구문 분석 없이 서버 응답이 필요한 경우(보안 문제가 발생할 수 있음) 구성 파일에서 `useUnsafeHeaderParsing`을 `true`로 설정합니다. [\<httpWebRequest> 요소(네트워크 설정)](../configure-apps/file-schema/network/httpwebrequest-element-network-settings.md)를 참조하세요.|  
