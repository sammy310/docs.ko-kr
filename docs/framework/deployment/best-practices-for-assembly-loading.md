...
### <a name="consider-using-the-net-framework-add-in-model"></a>.NET Framework 추가 기능 모델 사용 고려  
 일반적으로 애플리케이션 기준 위치에 설치되지 않는 추가 기능을 구현하는 데 로드 소스 컨텍스트를 사용할 경우 .NET Framework 추가 기능 모델을 사용하세요. 이 모델은 애플리케이션 도메인 또는 프로세스 수준에서 격리되므로, 사용자가 애플리케이션 도메인을 직접 관리할 필요가 없습니다. 추가 기능 모델에 대한 자세한 내용은 [추가 기능 및 확장성](/previous-versions/dotnet/netframework-4.0/bb384200(v%3dvs.100))을 참조하세요.  
