...
\*\* 사용자 지정 설치로 웹 설치 관리자를 사용하는 경우 사용자의 MUI(다국어 사용자 인터페이스) 설정을 기준으로 기본 언어 설정을 사용하거나, 명령줄에서 `/LCID` 옵션을 사용하여 다른 언어 팩을 지정할 수 있습니다. 자세한 내용은 [기본 .NET Framework UI를 사용하여 연결](#chaining_default) 섹션을 참조하세요.
...
- [BITS(Background Intelligent Transfer Service) 오류 코드](/windows/win32/bits/bits-return-values)

- [URL 모니커 오류 코드](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/ms775145(v=vs.85))

- [WinHttp 오류 코드](/windows/win32/winhttp/error-messages)

#### <a name="other-error-codes"></a>기타 오류 코드

다음 콘텐츠를 참조하세요.

- [Windows Installer 오류 코드](/previous-versions/aa368542(v=vs.85))
