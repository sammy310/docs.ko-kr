...
|[GetSize 메서드](icordebugcode-getsize-method.md)|이 `ICorDebugCode`가 나타내는 이진 코드의 크기 (바이트)를 가져옵니다.|  
|[GetVersionNumber 메서드](icordebugcode-getversionnumber-method.md)|이 `ICorDebugCode` 가 나타내는 코드의 버전을 식별 하는 1부터 사용 하는 번호를 가져옵니다.|  
|[IsIL 메서드](icordebugcode-isil-method.md)|이이 `ICorDebugCode` MSIL로 컴파일 되었는지 여부를 나타내는 값을 가져옵니다.|  
