...
|[Terminate 메서드](icordebug-terminate-method.md)|`ICorDebug` 개체를 종료 합니다.|  
  
## <a name="remarks"></a>주의  
 `ICorDebug`는 디버거 프로세스에 대 한 이벤트 처리 루프를 나타냅니다. 디버거는이 인터페이스를 해제 하기 전에 디버깅 중인 모든 프로세스에서 [ICorDebugManagedCallback:: ExitProcess](icordebugmanagedcallback-exitprocess-method.md) 콜백이 대기 해야 합니다.  
