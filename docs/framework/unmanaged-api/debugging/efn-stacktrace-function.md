...
## <a name="parameters"></a>매개 변수  
 ...
 `Flags`  
 진행 각 `module!functionname` 줄 앞에 EBP 레지스터와 ESP (enter stack 포인터)를 표시 하려면 0 또는 SOS_STACKTRACE_SHOWADDRESSES (0x01)로 설정 합니다.  
...
- `wszTextOut`가 null이 아닌 경우 함수는 `puiTextLength`에 지정 된 위치까지 `wszTextOut` 텍스트를 저장 합니다. 버퍼에 충분 한 공간이 있으면 성공적으로 반환 되 고 버퍼가 충분 하지 않은 경우에는 E_OUTOFMEMORY을 반환 합니다.  
...
- `pTransitionContexts`가 null이 아닌 경우 함수는이를 길이가 `puiTransitionContextCount`구조체 배열로 처리 합니다. 구조 크기는 `uiSizeOfContext`로 제공 되며, 아키텍처의 경우 [Simplecontext](../../../../docs/framework/unmanaged-api/debugging/stacktrace-simplecontext-structure.md) 또는 `CONTEXT`의 크기 여야 합니다.  
...
- `Flags` 매개 변수는 각 `module!functionname` 줄 앞에 EBP 및 ESP를 표시 하는 0 또는 SOS_STACKTRACE_SHOWADDRESSES입니다. 기본적으로 0입니다.  
