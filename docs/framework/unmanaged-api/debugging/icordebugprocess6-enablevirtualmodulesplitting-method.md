...
>  이 메서드는 .NET 네이티브에서만 사용할 수 있습니다.  
 ...
 언제든지 이 메서드를 호출하여 `enableSplitting`의 값을 변경할 수 있습니다. 상태 저장 기능 변경 되지는지 않습니다는 [ICorDebug](../../../../docs/framework/unmanaged-api/debugging/icordebug-interface.md) 에 나와 있는 방법의 동작 변경 이외의 개체를 [가상 모듈 분할 및 관리 되지 않는 디버깅 Api](#APIs) 호출 시 섹션입니다. 가상 모듈을 사용하는 경우 해당 메서드를 호출할 때 성능이 저하됩니다. 또한 가상화 된 메타 데이터의 중요 한 메모리 내 캐시 해야 할 수 있습니다 올바르게 구현 하는 [IMetaDataImport](../../../../docs/framework/unmanaged-api/metadata/imetadataimport-interface.md) 가상 모듈 분할을 설정한 후에 Api 및 이러한 캐시를 유지 될 수 있습니다.  
...
## <a name="terminology"></a>용어  
 ...
 컨테이너 모듈과 하위 모듈 ICorDebugModule 인터페이스 개체로 표시 됩니다. 그러나 인터페이스의 동작은 약간씩 다르며 각각의 경우로는 \<xref 섹션 > 섹션에 설명 합니다.  
  
## <a name="modules-and-assemblies"></a>모듈 및 어셈블리  
 다중 모듈 어셈블리는 어셈블리 병합 시나리오에서 지원되지 않으므로 모듈과 어셈블리 간에는 일 대 일 관계가 적용됩니다. 컨테이너 모듈 또는 하위 모듈을 나타내는 여부에 관계 없이 각 ICorDebugModule 개체를는 해당 ICorDebugAssembly 개체가 있습니다. 합니다 [icordebugmodule:: Getassembly](../../../../docs/framework/unmanaged-api/debugging/icordebugmodule-getassembly-method.md) 어셈블리에 모듈에서 메서드를 변환 합니다. 다른 방향으로 매핑할 합니다 [icordebugassembly:: Enumeratemodules](../../../../docs/framework/unmanaged-api/debugging/icordebugassembly-enumeratemodules-method.md) 메서드 하나만 모듈을 열거 합니다. 이 경우 어셈블리와 모듈은 긴밀하게 결합된 쌍을 형성하므로 '어셈블리'와 '모듈'이라는 용어는 거의 동일한 의미로 사용됩니다.  
...
|[Icordebugcode:: Getcode](../../../../docs/framework/unmanaged-api/debugging/icordebugcode-getcode-method.md) (IL 코드에만 해당 참조) 하는 경우|병합 전 어셈블리 이미지에서 유효했던 IL을 반환합니다. 즉, 참조하는 형식이 IL을 포함하는 가상 모듈에 정의되어 있지 않으면 모든 인라인 메타데이터 토큰은 TypeRef 또는 MemberRef 토큰이 됩니다. 이러한 TypeRef 또는 MemberRef 토큰을 조회할 수 있습니다 합니다 [IMetaDataImport](../../../../docs/framework/unmanaged-api/metadata/imetadataimport-interface.md) 해당 가상 ICorDebugModule 개체에 대 한 개체입니다.|병합 후 어셈블리 이미지의 IL을 반환합니다.|  
