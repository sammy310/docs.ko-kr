...
이 ICorDebugReferenceValue가 null 값 인지 여부를 나타내는 값을 가져옵니다 .이 경우 `ICorDebugReferenceValue` 개체를 가리키지 않습니다.  
...
## <a name="parameters"></a>매개 변수  
 `pbNull`  
 제한이 이 `ICorDebugReferenceValue` 개체가 null 인 경우 `true` 되는 부울 값에 대 한 포인터입니다. 그렇지 않으면 `pbNull` `false`됩니다.  
