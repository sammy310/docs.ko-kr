...
## <a name="remarks"></a>주의  
 ...
 디버깅 이벤트가 큐에 대기 되는 경우 이미 발생 했으므로 디버거가 디버기 상태를 확인할 수 있도록 전체 큐를 드레이닝 해야 합니다. `ICorDebugController::Continue`를 호출 하 여 큐를 드레이닝 합니다. 예를 들어, 큐에 스레드 *x*에 대 한 두 개의 디버깅 이벤트가 포함 되어 있고 디버거가 첫 번째 디버깅 이벤트 후에 스레드 *x* 를 일시 중단 한 다음 `ICorDebugController::Continue`를 호출 하는 경우 스레드가 일시 중단 된 경우에도 스레드 *x* 에 대 한 두 번째 디버깅 이벤트가 디스패치 됩니다.  
