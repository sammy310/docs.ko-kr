...
## <a name="remarks"></a>설명  
 ...
 64 개를 초과 하는 레지스터가 없는 경우 x86 등의 플랫폼에 대 한 경우 처럼 `GetRegisters` 메서드는 실제로 `mask` 바이트 배열의 바이트를으로 변환한 `ULONG64` 다음 마스크를 사용 하는 [ICorDebugRegisterSet:: getregisters](icordebugregisterset-getregisters-method.md) 메서드를 호출 합니다 `ULONG64` .  
