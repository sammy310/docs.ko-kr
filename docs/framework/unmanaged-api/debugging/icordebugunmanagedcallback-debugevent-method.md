...
## <a name="parameters"></a>매개 변수  
 ...
 `fOutOfBand`  
 [in] `true`관리 되지 않는 이벤트가 발생 한 후 관리 되는 프로세스 상태와의 상호 작용을 할 수 없는 경우, 디버거가 [ICorDebugController:: Continue](icordebugcontroller-continue-method.md)를 호출할 때까지 불가능 합니다. 그렇지 않으면 `false`합니다.  
