...
## <a name="parameters"></a>매개 변수  
 `bIInspectableOnly`  
 진행 메서드가 RCW (런타임 호출 가능 래퍼)에 의해 캐시 된 인터페이스`IInspectable` (인터페이스) 또는 모든 COM 인터페이스만 Windows 런타임 반환 하는지 여부를 나타내는 값입니다.  
  
 `celt`  
 진행 주소를 검색할 개체의 수입니다.  
  
 `pceltFetched`  
 제한이 에 `CORDB_ADDRESS` `ptrs`실제로 반환 된 값 수에 대 한 포인터입니다.  
