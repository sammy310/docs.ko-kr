...
|`fAccurate`|매핑이 정확한 것으로 알려진 경우 `true` 합니다. 그렇지 않으면 `false`합니다.|  
  
## <a name="remarks"></a>주의  
 맵의 형식은 다음과 같습니다. 디버거는 `oldOffset` 수정 되지 않은 원래 MSIL 코드 내에서 MSIL 오프셋을 참조 한다고 가정 합니다. `newOffset` 매개 변수는 계측 된 새 코드 내에서 해당 MSIL 오프셋을 참조 합니다.  
