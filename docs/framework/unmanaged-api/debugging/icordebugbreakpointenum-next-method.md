...
## <a name="parameters"></a>매개 변수  
 ...
 `celt`  
 진행 `ICorDebugBreakpoint` 검색할 인스턴스 수입니다.  
  
 `breakpoints`  
 제한이 각각 중단점을 나타내는 개체를 가리키는 포인터의 배열입니다 `ICorDebugBreakpoint` .  
  
 `pceltFetched`  
 제한이 실제로 반환 된 인스턴스 수에 대 한 포인터 `ICorDebugBreakpoint` 입니다. 이 일 경우이 값은 null 일 수 있습니다 `celt` .  
