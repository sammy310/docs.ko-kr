...
생성자가 매개 변수를 사용 <xref:System.Type> 하거나 <xref:System.Type> 매개 변수를 사용할 수 있는 클래스 내에 중첩 될 수 있는 지정 된 ICorDebugFunction에 대 한 호출을 설정 합니다.  
...
## <a name="remarks"></a>설명  
 ...
 함수 실행은 제한 된 시나리오 에서만 수행할 수 있습니다. 또는 `CallParameterizedFunction` `ICorDebugEval::CallFunction` 이 실패할 경우 반환 되는 HRESULT는 가장 일반적인 실패 이유를 표시 합니다.  
