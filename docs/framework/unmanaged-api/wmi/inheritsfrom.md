...
`wszAncestor`  
진행 클래스의 이름입니다. `wszAncestor` 는 유효한을 가리켜야 합니다 `LPCWSTR` .
...
|상수  |값  |설명  |
|---------|---------|---------|
| `WBEM_S_NO_ERROR` | 0 | 현재 개체는에서 상속 `wszAncestor` 됩니다.  |
| `WBEM_S_FALSE` | 1 | 현재 `wszAncestor` 개체가에서 상속 되지 않는 경우 |
|`WBEM_E_INVALID_PARAMETER` | 0x80041008 | `wszAncestor`이(가) `null`인 경우. |
