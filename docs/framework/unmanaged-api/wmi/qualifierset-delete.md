...
한정자 전파 규칙으로 인해 특정 한정자는 다른 개체에서 상속 되 고 단순히 현재 클래스 또는 인스턴스에서 재정의 될 수 있습니다. 이 경우 `QualifierSet_Delete` 메서드는 한정자를 원래 상속 된 값으로 다시 설정 합니다. 이 경우 함수는 상태 코드를 반환 합니다 `WBEM_S_RESET_TO_DEFAULT` .
