...
| `WBEM_FLAG_IGNORE_CLASS` | 0x8 | 비교 되는 개체에 동일한 클래스의 인스턴스에 있다고 가정 합니다. 따라서이 플래그는 인스턴스와 관련 된 정보만을 비교합니다. 성능을 최적화 하려면이 플래그를 사용 합니다. 동일한 클래스의 개체가 없으면 결과가 정의 되지 않습니다. |
