...
|상수  |값  |설명  |
|---------|---------|---------|
| `WBEM_FLAG_USE_AMENDED_QUALIFIERS` | 0x20000 | 경우 설정, WMI는 수정 된 버전을 사용 하 여 모든 한정자를 저장 하지 않습니다. <br> 그렇지 않은 경우 집합 가정는이 개체에 지역화 되지 않으면 모든 한정자는이 인스턴스를 사용 하 여 저장 됩니다. |
...
| `WBEM_FLAG_UPDATE_FORCE_MODE` | 0x40 | 충돌 하는 자식 클래스는 존재할 때 클래스의 업데이트를 강제로 수행 합니다. 예를 들어,이 플래그는 업데이트 강제 클래스 한정자는 자식 클래스에서 정의 되 고 기본 클래스를 같은 한정자를 추가 하려고 합니다. 기존와 충돌 하는 합니다. 강제 모드에서 자식 클래스에 충돌 하는 한정자를 삭제 하 여이 충돌이 해결 됩니다. |
...
사용자 이름이 시작 또는 밑줄로 끝나야 하는 클래스를 만들 수 없습니다.
