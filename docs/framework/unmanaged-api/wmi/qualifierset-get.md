...
`pVal`   
제한이 성공 하면 한정자의 올바른 형식 및 값입니다. 함수가 실패 하면 `pVal`가 가리키는 `VARIANT` 수정 되지 않습니다. 이 매개 변수를 `null`하는 경우 매개 변수는 무시 됩니다.

`plFlavor`   
제한이 요청 된 한정자에 대 한 한정자 버전 비트를 받는 LONG에 대 한 포인터입니다. 버전 정보를 원하지 않는 경우이 매개 변수를 `null`수 있습니다. 
