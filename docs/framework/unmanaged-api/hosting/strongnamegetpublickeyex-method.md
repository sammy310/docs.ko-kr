...
## <a name="parameters"></a>매개 변수  
 `pwzKeyContainer`  
 진행 공개/개인 키 쌍을 포함 하는 키 컨테이너의 이름입니다. `pbKeyBlob`가 null 인 경우에는 CSP (암호화 서비스 공급자) 내에 올바른 컨테이너를 지정 해야 `szKeyContainer`. 이 경우 `StrongNameGetPublicKeyEx` 메서드는 컨테이너에 저장 된 키 쌍에서 공개 키를 추출 합니다.  
...
## <a name="remarks"></a>주의  
 다음 표에서는 `uHashAlgId` 매개 변수에 대해 허용 되는 값 집합을 보여 줍니다.  
