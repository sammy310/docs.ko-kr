...
현재 [IHostSemaphore](../../../../docs/framework/unmanaged-api/hosting/ihostsemaphore-interface.md) 인스턴스가 소유 될 때까지 또는 지정 된 시간이 경과할 때까지 대기 하도록 합니다.  
...
## <a name="parameters"></a>매개 변수  
 `dwMilliseconds`  
 진행 현재 `IHostSemaphore` 인스턴스를 소유 하지 않는 경우를 반환 하기 전에 대기 하는 시간 (밀리초)입니다.  
  
 `option`  
 진행 [WAIT_OPTION](../../../../docs/framework/unmanaged-api/hosting/wait-option-enumeration.md) 값 중 하나로,이 작업이 차단 되는 경우 호스트에서 수행할 동작을 지정 합니다.  
...
|HOST_E_DEADLOCK|호스트가 대기 간격 중에 교착 상태를 감지 하 여 교착 상태에서 현재 `IHostSemaphore` 인스턴스를 선택 했습니다.|  
