...
호스트에서 만든 힙의 할당 요청을 수행 하는 데 사용 되는 [IHostMAlloc](../../../../docs/framework/unmanaged-api/hosting/ihostmalloc-interface.md) 인스턴스에 대 한 인터페이스 포인터를 가져옵니다.  
...
## <a name="parameters"></a>매개 변수  
 `dwMallocType`  
 진행 할당 되는 메모리의 특성을 지정 하는 [MALLOC_TYPE](../../../../docs/framework/unmanaged-api/hosting/malloc-type-enumeration.md) 플래그의 조합입니다.  
  
 `ppMAlloc`  
 제한이 호스트에서 제공 하는 `IHostMAlloc` 인스턴스의 주소에 대 한 포인터입니다.  
...
## <a name="remarks"></a>주의  
 `CreateMAlloc`는 CLR이 표준 Win32 함수를 사용 하는 대신 호스트를 통해 할당 요청을 수행할 수 있도록 하는 개체를 반환 합니다.  
