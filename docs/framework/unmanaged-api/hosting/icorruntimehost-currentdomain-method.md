...
현재 스레드에 로드 된 도메인을 나타내는 <xref:System.AppDomain?displayProperty=nameWithType> 형식의 인터페이스 포인터를 가져옵니다.  
...
## <a name="parameters"></a>매개 변수  
 `pAppDomain`  
 제한이 스레드의 현재 응용 프로그램 도메인을 나타내는 <xref:System.AppDomain?displayProperty=nameWithType> 형식의 포인터입니다. 이 포인터는 `IUnknown`형식화 되어 있으므로 호출자가 <xref:System._AppDomain>형식의 포인터를 얻기 위해 일반적으로 `QueryInterface`를 호출 해야 합니다.  
