...
현재 실행 중인 <xref:System.AppDomain>의 숫자 식별자를 가져옵니다.  
...
## <a name="parameters"></a>매개 변수  
 `pdwAppDomainId`  
 제한이 현재 실행 중인 <xref:System.AppDomain>의 숫자 식별자입니다.  
...
## <a name="remarks"></a>주의  
 `pdwAppDomainId` 매개 변수는 현재 스레드가 실행 중인 <xref:System.AppDomain>의 <xref:System.AppDomain.Id%2A> 속성 값으로 설정 됩니다.  
