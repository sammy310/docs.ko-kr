...
## <a name="remarks"></a>주의  
 `SetMaxThreads`는 CLR에 i/o 포트에서 서비스 요청에 사용할 수 있는 최대 스레드 수를 설정할 수 있는 기회를 제공 합니다. 호스트는 구현, 성능 또는 확장성과 같은 이유로 스레드 풀의 크기를 독점적으로 제어 해야 할 수 있습니다. 따라서 호스트는 `SetMaxThreads`를 구현 하지 않아도 됩니다. 이 경우 호스트는이 메서드에서 E_NOTIMPL을 반환 해야 합니다.  
