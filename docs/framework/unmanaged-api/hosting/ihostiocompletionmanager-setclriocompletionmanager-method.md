...
호스트에 CLR (공용 언어 런타임)에 의해 구현 된 [Iclriocompletionmanager](../../../../docs/framework/unmanaged-api/hosting/iclriocompletionmanager-interface.md) 인스턴스에 대 한 인터페이스 포인터를 제공 합니다.  
...
## <a name="parameters"></a>매개 변수  
 `pManager`  
 진행 CLR에서 제공 하는 `ICLRIoCompletionManager` 인스턴스에 대 한 인터페이스 포인터입니다.  
...
## <a name="remarks"></a>주의  
 CLR `SetCLRIoCompletionManager`호출한 후 호스트는 [IclrioOnComplete manager::](../../../../docs/framework/unmanaged-api/hosting/iclriocompletionmanager-oncomplete-method.md) 를 호출 하 여 i/o 요청이 완료 되었을 때 clr에 알려야 합니다.  
