...
|E_NOTIMPL|호스트는 `SetMinThreads`구현을 제공 하지 않습니다.|  
  
## <a name="remarks"></a>주의  
 호스트는 구현, 성능 또는 확장성과 같은 이유로 i/o 요청을 처리 하기 위해 할당할 수 있는 스레드 수에 대 한 배타적 제어를 원할 수 있습니다. 따라서 호스트는 `SetMinThreads`를 구현 하지 않아도 됩니다. 이 경우 호스트는이 메서드에서 E_NOTIMPL을 반환 해야 합니다.  
