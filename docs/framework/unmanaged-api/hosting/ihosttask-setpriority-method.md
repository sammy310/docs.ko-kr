...
## <a name="parameters"></a>매개 변수  
 `newPriority`  
 진행 현재 `IHostTask` 인스턴스로 표시 되는 작업에 대해 요청 된 스레드 우선 순위 값을 나타내는 정수입니다.  
...
## <a name="remarks"></a>주의  
 스레드에는 스레드 우선 순위 수준에 따라 부분적으로 사용 되는 라운드 로빈 시스템을 사용 하 여 처리 시간이 부여 됩니다. `SetPriority`를 사용 하면 CLR에서 현재 작업에 대 한 스레드 우선 순위 수준을 설정할 수 있습니다. 지원 되는 `newPriority` 값은 다음과 같습니다.  
...
- THREAD_PRIORITY_TIME_CRITICAL  
 ...
 <xref:System.Threading.Thread.Priority%2A?displayProperty=nameWithType> 값이 사용자 코드에 의해 수정 되 면 CLR에서 `SetPriority`를 호출 합니다. 호스트는 스레드 우선 순위 할당에 대 한 자체 알고리즘을 정의할 수 있으며이 요청을 무시할 수 있습니다.  
