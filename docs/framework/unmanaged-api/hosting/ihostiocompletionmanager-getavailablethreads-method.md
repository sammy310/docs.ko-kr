...
현재 요청을 처리 하 고 있지 않은 호스트에서 관리 하는 총 스레드 수에 대 한 i/o 완료 스레드 수를 가져옵니다.  
...
## <a name="parameters"></a>매개 변수  
 ...
 제한이 현재 서비스 요청에 사용할 수 있는 호스트에서 관리 하는 i/o 완료 스레드 수에 대 한 포인터입니다.  
...
|E_NOTIMPL|호스트는 `GetAvailableThreads`구현을 제공 하지 않습니다.|  
  
## <a name="remarks"></a>주의  
 호스트는 구현, 성능 또는 확장성과 같은 이유로 i/o 완료 스레드 풀의 크기에 대 한 배타적 제어를 원할 수 있습니다. 따라서 호스트는 `GetAvailableThreads`을 구현 하는 데 필요 하지 않습니다. 이 경우 호스트는이 메서드에서 E_NOTIMPL을 반환 해야 합니다.  
