...
## <a name="remarks"></a>주의  
 <xref:System.Threading.Thread.CurrentUICulture%2A?displayProperty=nameWithType> 속성의 값이 관리 코드에 의해 변경 되 면 런타임에서 `SetUILocale`를 호출 합니다. 이 메서드는 호스트가 로캘 동기화에 대 한 메커니즘을 실행할 수 있는 기회를 제공 합니다. 호스트가 관리 코드에서 UI 로캘을 변경 하는 것을 허용 하지 않거나 로캘을 동기화 하는 메커니즘을 구현 하지 않는 경우이 메서드에서 E_NOTIMPL을 반환 해야 합니다.  
