...
## <a name="parameters"></a>매개 변수  
 ...
 `tk`  
 【인】 가능한 가장 넓은 범위를 검색하기 위해 검색 범위를 제한하는 메타데이터 토큰 또는 NULL입니다.  
  
 `dwActions`  
 【인】 `rPermission`에 포함할 <xref:System.Security.Permissions.SecurityAction> 값을 나타내는 플래그 또는 0을 사용하여 모든 작업을 반환합니다.  
  
 `rPermission`  
 【아웃】 권한 토큰을 저장하는 데 사용되는 배열입니다.  
  
 `cMax`  
 [in] `rPermission` 배열의 최대 크기입니다.  
  
 `pcTokens`  
 【아웃】 에서 반환되는 권한 토큰 `rPermission`수입니다.  
  
## <a name="return-value"></a>Return Value  
  
|HRESULT|Description|  
|-------------|-----------------|  
|`S_OK`|`EnumPermissionSets`성공적으로 반환됩니다.|  
|`S_FALSE`|등록할 토큰이 없습니다. 이 경우 `pcTokens` 0입니다.|  
