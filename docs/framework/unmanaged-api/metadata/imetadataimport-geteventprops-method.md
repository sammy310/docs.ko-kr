...
## <a name="parameters"></a>매개 변수  
 ...
 `szEvent`  
 제한이 `ev`에서 참조 하는 이벤트의 이름입니다.  
  
 `pchEvent`  
 진행 `szEvent`의 와이드 문자에서 요청 된 길이입니다.  
  
 `pdwEventFlags`  
 제한이 `szEvent`의 와이드 문자에서 반환 되는 길이입니다.  
  
 `ptkEventType`  
 제한이 이벤트의 <xref:System.Delegate> 형식을 나타내는 TypeRef 또는 TypeDef 메타 데이터 토큰에 대 한 포인터입니다.  
 ...
 `pcOtherMethod`  
 제한이 `rmdOtherMethod`에서 반환 된 토큰의 수입니다.  
