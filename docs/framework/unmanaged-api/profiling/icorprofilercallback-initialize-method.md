...
## <a name="remarks"></a>주의  
 `Initialize` 호출은 변경할 수 없는 콜백을 사용 하거나 사용 하지 않도록 설정 하는 유일한 기회입니다. `Initialize` 호출로 콜백을 사용 하도록 설정한 후에는 나중에 [ICorProfilerInfo:: SetEventMask](icorprofilerinfo-seteventmask-method.md)를 사용 하 여 콜백을 사용 하지 않도록 설정할 수 없습니다. [COR_PRF_MONITOR](cor-prf-monitor-enumeration.md) 열거형의 COR_PRF_MONITOR_IMMUTABLE 값은 변경할 수 없는 이벤트를 나타냅니다.  
