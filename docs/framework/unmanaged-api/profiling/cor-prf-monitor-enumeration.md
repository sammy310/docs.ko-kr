...
|`COR_PRF_MONITOR_APPDOMAIN_LOADS`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스의 `AppDomainCreation*` 및 `AppDomainShutdown*` 콜백을 제어 합니다.|  
|`COR_PRF_MONITOR_ASSEMBLY_LOADS`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스의 `AssemblyLoad*` 및 `AssemblyUnload*` 콜백을 제어 합니다.|  
|`COR_PRF_MONITOR_CACHE_SEARCHES`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스의 `JITCachedFunctionSearch*` 콜백을 제어 합니다.<br /><br /> 이 플래그의 동작은 .NET Framework 버전 2.0에서 변경되었습니다.|  
|`COR_PRF_MONITOR_CCW`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스의 `COMClassicVTable*` 콜백을 제어 합니다.|  
|`COR_PRF_MONITOR_CLASS_LOADS`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스의 `ClassLoad*` 및 `ClassUnload*` 콜백을 제어 합니다.|  
|`COR_PRF_MONITOR_CLR_EXCEPTIONS`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스의 `ExceptionCLRCatcher*` 콜백을 제어 합니다.|  
|`COR_PRF_MONITOR_CODE_TRANSITIONS`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스에서 [UnmanagedToManagedTransition](icorprofilercallback-unmanagedtomanagedtransition-method.md) 및 [managedtounmanagedtransition](icorprofilercallback-managedtounmanagedtransition-method.md) 콜백을 제어 합니다.|  
...
|`COR_PRF_MONITOR_EXCEPTIONS`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스에서 [exceptionthrown](icorprofilercallback-exceptionthrown-method.md) 된 콜백과 `ExceptionSearch*`, `ExceptionOSHandler*`, `ExceptionUnwind*`및 `ExceptionCatcher*` 콜백을 제어 합니다.|  
...
|`COR_PRF_MONITOR_REMOTING`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스의 `Remoting*` 콜백을 제어 합니다.|  
...
|`COR_PRF_MONITOR_SUSPENDS`|[ICorProfilerCallback](icorprofilercallback-interface.md) 인터페이스에서 `RuntimeSuspend*`, `RuntimeResume*`, [Runtimethreadsuspended](icorprofilercallback-runtimethreadsuspended-method.md)및 [runtimethreadsuspended](icorprofilercallback-runtimethreadresumed-method.md) 된 콜백을 제어 합니다.|  
...
|Member|설명|  
|------------|-----------------|  
|`COR_PRF_ENABLE_FRAME_INFO`|[FunctionEnter2](functionenter2-function.md) 콜백에서 반환 된 `COR_PRF_FRAME_INFO` 값을 사용 하 여 [GetFunctionInfo2](icorprofilerinfo2-getfunctioninfo2-method.md) 메서드를 호출 하 여 제네릭 함수에 대해 정확한 `ClassID`를 검색할 수 있도록 합니다.|  
...
|`COR_PRF_ENABLE_INPROC_DEBUGGING`|사용되지 않음.<br /><br /> 프로세스 내 디버깅은 지원되지 않습니다. 이 플래그는 아무런 영향을 주지 않습니다.|  
|`COR_PRF_ENABLE_JIT_MAPS`|사용되지 않음.<br /><br /> 프로파일러가 [GetILToNativeMapping](icorprofilerinfo-getiltonativemapping-method.md)를 사용 하 여 IL-네이티브 맵을 가져올 수 있도록 합니다. .NET Framework 2.0부터는 런타임이 항상 IL-네이티브 맵을 추적하므로 이 플래그는 항상 설정되어 있는 것으로 간주됩니다.|  
|`COR_PRF_ENABLE_OBJECT_ALLOCATED`|프로파일러가 개체 할당 알림을 받고자 할 수 있음을 런타임에 알립니다. 이 플래그는 초기화 중에 설정해야 합니다. 이를 통해 프로파일러는 나중에 `COR_PRF_MONITOR_OBJECT_ALLOCATED` 플래그를 사용 하 여 [Objectallocated](icorprofilercallback-objectallocated-method.md) 된 콜백을 받을 수 있습니다.|  
