...
또는 길이가 0인 `codeInfos` 버퍼로 `GetCodeInfo4`를 먼저 호출하여 올바른 버퍼 크기를 구합니다. 그런 다음 `codeInfos` 버퍼 크기를 `pcCodeInfos`에서 반환 된 값으로 설정 하 고 [COR_PRF_CODE_INFO](cor-prf-code-info-structure.md) 구조의 크기를 곱하여 `GetCodeInfo4`를 다시 호출할 수 있습니다.
