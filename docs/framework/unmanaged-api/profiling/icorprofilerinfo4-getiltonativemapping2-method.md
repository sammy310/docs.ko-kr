...
## <a name="remarks"></a>주의  
 `GetILToNativeMapping2`는 프로파일러가 이후 릴리스에서 다시 컴파일된 함수의 ID를 지정할 수 있도록 하는 것을 제외 하 고는 [ICorProfilerInfo:: GetILToNativeMapping](icorprofilerinfo-getiltonativemapping-method.md) 메서드와 비슷합니다.  
  
> [!NOTE]
> [ICorProfilerFunctionControl:: SetILInstrumentedCodeMap](icorprofilerfunctioncontrol-setilinstrumentedcodemap-method.md) 메서드는 .NET Framework 4.5에서 구현 되지 않으므로 JIT 다시 컴파일된 함수는 원래 컴파일된 함수와 다른 IL-네이티브 매핑을 포함할 수 없습니다. 따라서 `GetILToNativeMapping2` .NET Framework 4.5에서 0이 아닌 JIT 다시 컴파일된 ID를 사용 하 여 호출할 수 없습니다.  
