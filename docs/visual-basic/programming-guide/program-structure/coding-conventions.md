...
- 결과의 속성 이름이 모호하면 속성 이름을 바꿉니다. For example, if your query returns a customer name and an order ID, rename them instead of leaving them as `Name` and `ID` in the result:  
