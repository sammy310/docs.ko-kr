...
## <a name="char-type"></a>Char Type  
 The `Char` data type is a single two-byte (16-bit) Unicode character. If a variable always stores exactly one character, declare it as `Char`. 예를 들어 다음과 같은 가치를 제공해야 합니다.  
...
## <a name="string-type"></a>String Type  
 The `String` data type is a sequence of zero or more two-byte (16-bit) Unicode characters. If a variable can contain an indefinite number of characters, declare it as `String`. 예를 들어 다음과 같은 가치를 제공해야 합니다.  
