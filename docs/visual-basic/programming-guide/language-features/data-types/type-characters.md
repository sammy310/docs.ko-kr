...
|`$`|`String`|`Dim V$ = "Secret"`|  
 ...
 `Boolean`, `Byte`, `Char`, `Date`, `Object`, `SByte`, `Short`, `UInteger`, `ULong`, `UShort` 데이터 형식 또는 배열 또는 구조체와 같은 복합 데이터 형식에 대 한 식별자 형식 문자는 없습니다.

경우에 따라 `Left`대신 `Left$` 등의 Visual Basic 함수에 `$` 문자를 추가 하 여 `String`형식의 반환 값을 얻을 수 있습니다.
...
|숫자, 소수 부분이 없습니다. `Integer`에 비해 너무 큼|`Long`|`2147483648`|  
|숫자, 소수 부분|`Double`|`1.2`|  
|큰따옴표로 묶어서|`String`|`"A"`|  
...
리터럴은 변수, 상수 및 식으로 식별자 형식 문자 (`%`, `&`, `@`, `!`, `#`, `$`)를 사용할 수도 있습니다. 그러나 리터럴 형식 문자 (`S`, `I`, `L`, `D`, `F`, `R`, `C`)는 리터럴에만 사용할 수 있습니다.

모든 경우에서 리터럴 형식 문자는 리터럴 값 바로 뒤에와 야 합니다.

## <a name="hexadecimal-binary-and-octal-literals"></a>16 진수, 이진 및 8 진수 리터럴

컴파일러는 일반적으로 정수 리터럴을 10 진수 (밑수 10) 숫자 시스템에 있는 것으로 해석 합니다. 또한 정수 리터럴을 `&H` 접두사를 사용 하 여 16 진수 (기 수)로 정의 하 고, `&B` 접두사를 사용 하 여 이진 (밑수 2) 숫자로, `&O` 접두사가 있는 8 진수 (밑수 8) 숫자로 정의할 수 있습니다. 접두사 뒤의 숫자는 숫자 시스템에 적합 해야 합니다. 다음 표에서는이에 대해 설명 합니다.  
...
Visual Basic 2017부터 밑줄 문자 (`_`)를 그룹 구분 기호로 사용 하 여 정수 리터럴의 가독성을 향상 시킬 수 있습니다. 다음 예제에서는 `_` 문자를 사용 하 여 이진 리터럴을 8 비트 그룹으로 그룹화 합니다.

```vb
Dim number As Integer = &B00100010_11000101_11001111_11001101
```

리터럴 형식 문자를 사용 하 여 접두 리터럴을 따라갈 수 있습니다. 다음 예제에서는이를 보여 줍니다.
...
이전 예제에서 `counter`의 10 진수 값은-32768이 고 `flags`의 10 진수 값은 + 32768입니다.
