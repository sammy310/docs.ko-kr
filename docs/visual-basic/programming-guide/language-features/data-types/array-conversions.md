...
## <a name="conversion-to-an-object-array"></a>개체 배열로 변환  
 `Object` 배열을 초기화 하지 않고 선언 하는 경우 요소 형식은 초기화 되지 않은 상태로 유지 되는 동안 `Object` 됩니다. 특정 클래스의 배열로 설정 하면 해당 클래스의 형식을 사용 합니다. 그러나 해당 기본 형식은 여전히 `Object`되며 이후에는 관련 없는 클래스의 다른 배열로 설정할 수 있습니다. 모든 클래스는 `Object`에서 파생 되므로 모든 클래스에서 다른 클래스로 배열의 요소 형식을 변경할 수 있습니다.  
...
### <a name="underlying-type-of-an-array"></a>배열의 내부 형식  
 ...
 다음 예제에서 `students`은 `student` 배열입니다. `String`와 `student`간에 변환이 존재 하지 않으므로 마지막 문이 실패 합니다.  
