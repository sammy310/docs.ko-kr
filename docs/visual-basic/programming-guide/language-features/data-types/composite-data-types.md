...
- 배열의 요소 형식입니다.  
 ...
 특히 지정 된 차원의 길이는 인스턴스의 데이터 형식에 포함 되지 않습니다. 다음 예제에서는 이것을 보여 줍니다.  
...
```  
 ...
 앞의 예제에서 배열 변수 `arrayA` 및 `arrayB`은 다른 길이로 초기화 되더라도 `Byte()` 동일한 데이터 형식으로 간주 됩니다. 변수 `arrayB` 및 `arrayC`의 요소 형식이 다르기 때문에 동일한 형식이 아닙니다. 변수 `arrayC` 및 `arrayD`의 순위가 다르기 때문에 동일한 형식이 아닙니다. `arrayD`를 아직 초기화 하지 않은 경우에도 해당 순위와 요소 유형이 동일 하기 때문에 `arrayD` 및 `arrayE` 변수는 동일한 유형 (`Short(,)`)을 갖습니다.  
