 ...
 *축소 변환* 은 가능한 값의 일부를 저장하지 못할 수도 있는 데이터 형식으로 값을 변경합니다. For example, a fractional value is rounded when it is converted to an integral type, and a numeric type being converted to `Boolean` is reduced to either `True` or `False`.  
  
## <a name="widening-conversions"></a>확대 변환  
 The following table shows the standard widening conversions.  
  
|데이터 형식|Widens to data types <sup>1</sup>|  
|---|---|  
...
|모든 형식|Any interface it implements.|  
