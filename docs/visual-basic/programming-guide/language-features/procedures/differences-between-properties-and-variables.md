...
|Point of difference|변수|속성|  
|-------------------------|--------------|--------------|  
|선언|Single declaration statement|Series of statements in a code block|  
|구현|Single storage location|Executable code (property procedures)|  
|스토리지|Directly associated with variable's value|Typically has internal storage not available outside the property's containing class or module<br /><br /> Property's value might or might not exist as a stored element <sup>1</sup>|  
|Executable code|없음|Must have at least one procedure|  
