...
## <a name="backing-field"></a>지원 필드  
 Visual Basic 라는 숨겨진된 전용 필드가 자동으로 만듭니다 자동 구현 속성을 선언 합니다 *지원 필드* 속성 값을 포함 합니다. 지원 필드 이름은 밑줄(_) 다음에 자동 구현 속성 이름이 나오는 형태입니다. 예를 들어 `ID`라는 자동 구현 속성을 선언하는 경우 지원 필드의 이름은 `_ID`가 됩니다. 이름이 `_ID`인 클래스의 멤버를 포함하는 경우 이름 충돌이 발생하며 Visual Basic에서 컴파일러 오류가 보고됩니다.  
