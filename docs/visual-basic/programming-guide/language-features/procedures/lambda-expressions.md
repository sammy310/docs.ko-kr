...
> [!NOTE]
> `RemoveHandler` 문은 예외입니다. `RemoveHandler`의 대리자 매개 변수에는의 람다 식을 전달할 수 없습니다.
...
포함 범위의 지역 변수 및 매개 변수에 대 한 액세스는 해당 범위의 수명 이상으로 확장 될 수 있습니다. 람다 식을 참조 하는 대리자를 가비지 수집에 사용할 수 없는 경우 원래 환경의 변수에 대 한 액세스가 유지 됩니다. 다음 예제에서 변수 `target`는 `makeTheGame`에 대 한 로컬 이며, 람다 식 `playTheGame` 정의 된 메서드입니다. `Main`의 `takeAGuess`에 할당 된 반환 된 람다 식은 여전히 지역 변수 `target`에 액세스할 수 있습니다.
...
- `functionWithNestedLambda`지역 변수: `localVar`
