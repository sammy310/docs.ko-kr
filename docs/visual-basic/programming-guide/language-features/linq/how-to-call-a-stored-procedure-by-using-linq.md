...
1. **서버 탐색기**/**데이터베이스 탐색기**에서 Northwind 데이터베이스에 대 한 연결을 확장 합니다. **저장 프로시저** 폴더를 확장 합니다.  
...
2. Form1을 두 번 클릭 하 여 `Load` 이벤트에 코드를 추가 합니다.  
  
3. O/R 디자이너에 저장 프로시저를 추가 하는 경우 디자이너는 프로젝트에 대 한 <xref:System.Data.Linq.DataContext> 개체를 추가 했습니다. 이 개체에는 해당 프로시저에 액세스 하는 데 필요한 코드가 포함 되어 있습니다. 프로젝트에 대 한 <xref:System.Data.Linq.DataContext> 개체는 .dbml 파일의 이름을 기반으로 이름이 지정 됩니다. 이 프로젝트의 경우 <xref:System.Data.Linq.DataContext> 개체의 이름은 `northwindDataContext`입니다.  
  
     코드에서 <xref:System.Data.Linq.DataContext>의 인스턴스를 만들고 O/R 디자이너에 지정 된 저장 프로시저 메서드를 호출할 수 있습니다. <xref:System.Windows.Forms.DataGridView> 개체에 바인딩하려면 저장 프로시저의 결과에 대해 <xref:System.Linq.Enumerable.ToList%2A> 메서드를 호출 하 여 쿼리를 즉시 실행 해야 할 수 있습니다.  
  
     `Load` 이벤트에 다음 코드를 추가 하 여 데이터 컨텍스트에 대 한 메서드로 노출 되는 저장 프로시저 중 하나를 호출 합니다.  
