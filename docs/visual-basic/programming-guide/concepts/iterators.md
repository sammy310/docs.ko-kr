...
For Each ...를 사용 하 여 클라이언트 코드에서 반복기를 사용 합니다. [ 다음](../../language-reference/statements/for-each-next-statement.md) 문 또는 LINQ 쿼리 사용.
...
Visual Basic 반복기 함수 또는 `get` 접근자 선언에는 [반복기](../../language-reference/modifiers/iterator.md) 한정자가 포함 되어 있습니다.
...
## <a name="creating-a-collection-class"></a><a name="BKMK_CollectionClass"></a> 컬렉션 클래스 만들기
...
`GetEnumerator`메서드는 문을 사용 하 여 각 문자열을 한 번에 하나씩 반환 `Yield` 하 고, `Iterator` 한정자는 함수 선언에 있습니다.
...
다음 예제에서는 `Try` `Catch` 반복기 함수에, 및 블록을 포함 합니다 `Finally` . `Finally`반복기 함수의 블록은 반복이 완료 되기 전에 실행 `For Each` 됩니다.
...
유효성 검사가 반복기 함수 내에 있는 경우 본문의 첫 번째 반복이 시작 될 때까지 유효성 검사를 수행할 수 없습니다 `For Each` .

## <a name="using-iterators-with-a-generic-list"></a><a name="BKMK_GenericList"></a> 제네릭 목록과 함께 반복기 사용
...
`BottomToTop`속성 선언에는 키워드가 포함 됩니다 `Iterator` .
...
## <a name="syntax-information"></a><a name="BKMK_SyntaxInformation"></a> 구문 정보
...
Visual Basic 반복기 메서드에는 매개 변수를 사용할 수 없습니다 `ByRef` .
...
[클래스](../../language-reference/statements/class-statement.md) 또는 [구조체](../../language-reference/statements/structure-statement.md)에 대 한 반복기를 만들 때 전체 인터페이스를 구현할 필요가 없습니다 <xref:System.Collections.IEnumerator> . 컴파일러는 반복기를 검색할 경우 <xref:System.Collections.IEnumerator> 또는 <xref:System.Collections.Generic.IEnumerator%601> 인터페이스의 `Current`, `MoveNext` 및 `Dispose` 메서드를 자동으로 생성합니다.

`For Each…Next` 루프를 연속 반복하거나 `IEnumerator.MoveNext`를 직접 호출하면 다음 반복기 코드 본문이 이전 `Yield` 문 다음에 다시 시작됩니다. 그런 다음 `Yield` 반복기 본문의 끝에 도달 하거나 `Exit Function` 또는 문이 발생할 때까지 다음 문으로 계속 합니다 `Return` .

반복기는 메서드를 지원 하지 않습니다 <xref:System.Collections.IEnumerator.Reset%2A?displayProperty=nameWithType> . 처음부터 다시 반복하려면 새 반복기를 가져와야 합니다.
