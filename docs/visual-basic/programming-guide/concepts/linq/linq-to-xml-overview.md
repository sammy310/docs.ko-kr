...
## <a name="what-is-linq-to-xml"></a>LINQ to XML이란?  
 ...
 [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]의 가장 중요한 이점은 [!INCLUDE[vbteclinqext](~/includes/vbteclinqext-md.md)]와 통합되었다는 점입니다. 이 통합을 통해 메모리 내 XML 문서에 대한 쿼리를 작성하여 요소와 특성의 컬렉션을 검색할 수 있습니다. [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]의 쿼리 기능은 기능 면에서(구문 면에서는 아니지만) XPath 및 XQuery와 유사합니다. Visual Basic에서 [!INCLUDE[vbteclinq](~/includes/vbteclinq-md.md)]를 통합 하면 더 강력한 형식 지정, 컴파일 시간 검사, 향상 된 디버거 지원 기능을 제공 합니다.  
