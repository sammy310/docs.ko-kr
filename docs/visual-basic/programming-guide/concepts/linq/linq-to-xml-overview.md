...
## <a name="what-is-linq-to-xml"></a>LINQ to XML이란?  
 ...
 [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]은 XML 문서를 메모리로 가져온다는 점에서 DOM(문서 개체 모델)과 같습니다. 문서를 쿼리하고 수정할 수 있으며 문서를 수정한 후 파일에 저장하거나 serialize하고 네트워크를 통해 보낼 수 있습니다. 그러나 [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] DOM과 다를 수 있습니다 .이는 더 가벼운이 고 작업 하기 쉬우며 Visual Basic의 언어 기능을 활용 하는 새로운 개체 모델을 제공 합니다.  
  
 [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]의 가장 중요 한 이점은 LINQ (통합 언어 쿼리)와의 통합입니다. 이 통합을 통해 메모리 내 XML 문서에 대한 쿼리를 작성하여 요소와 특성의 컬렉션을 검색할 수 있습니다. [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]의 쿼리 기능은 기능 면에서(구문 면에서는 아니지만) XPath 및 XQuery와 유사합니다. Visual Basic LINQ의 통합은 더 강력한 형식 지정, 컴파일 시간 검사 및 향상 된 디버거 지원 기능을 제공 합니다.  
...
```  
 ...
 이러한 LINQ 기능 외에도 [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]는 향상 된 XML 프로그래밍 인터페이스를 제공 합니다. [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]을 사용하면 다음과 같은 작업을 수행할 수 있습니다.  
