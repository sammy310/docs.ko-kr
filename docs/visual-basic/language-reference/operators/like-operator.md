...
## <a name="parts"></a>요소  
 `result`  
 필수 요소. Any `Boolean` variable. The result is a `Boolean` value indicating whether or not the `string` satisfies the `pattern`.  
  
 `string`  
 필수 요소. 임의의 `String` 식입니다.  
  
 `pattern`  
 필수 요소. Any `String` expression conforming to the pattern-matching conventions described in "Remarks."  
