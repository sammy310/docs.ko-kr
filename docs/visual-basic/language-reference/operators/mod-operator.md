...
모든 숫자 형식. This includes the unsigned and floating-point types and `Decimal`.
...
The result of a `Mod` operation retains the sign of the dividend, `number1`, and so it may be positive or negative. The result is always in the range (-`number2`, `number2`), exclusive. 예를 들어 다음과 같은 가치를 제공해야 합니다.
