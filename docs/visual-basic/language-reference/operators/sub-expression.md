...
|용어|정의|  
|---|---|  
|`parameterlist`|(선택 사항) A list of local variable names that represent the parameters of the procedure. The parentheses must be present even when the list is empty. 자세한 내용은 [Parameter List](../../../visual-basic/language-reference/statements/parameter-list.md)을 참조하세요.|  
|`statement`|필수 요소. A single statement.|  
|`statements`|필수 요소. A list of statements.|  
