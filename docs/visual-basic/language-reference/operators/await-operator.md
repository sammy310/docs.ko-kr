...
`Await` 사용 되는 메서드에는 [비동기](../../../visual-basic/language-reference/modifiers/async.md) 한정자가 있어야 합니다. `Async` 한정자를 사용하여 정의하고 일반적으로 하나 이상의 `Await` 식을 포함하는 이러한 메서드를 *비동기 메서드*라고 합니다.
