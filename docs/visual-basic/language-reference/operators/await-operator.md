...
The method in which `Await` is used must have an [Async](../../../visual-basic/language-reference/modifiers/async.md) modifier. `Async` 한정자를 사용하여 정의하고 일반적으로 하나 이상의 `Await` 식을 포함하는 이러한 메서드를 *비동기 메서드*라고 합니다.
