...
이러한 연산자는 null 검사를 처리 하는 코드를 작성 하는 데 도움이 됩니다. 특히 데이터 구조체로 내림차순으로 사용할 수 있습니다. 예를 들면,
...
@No__t_0 호출 목록에 요소가 없으면 `SendNews` 대리자는 <xref:System.NullReferenceException>을 throw 합니다. Null 조건부 연산자 앞에 다음과 같은 코드는 대리자 호출 목록이 `Nothing` 되지 않도록 합니다.
