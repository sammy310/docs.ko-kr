...
|<xref:Microsoft.VisualBasic.Strings.Filter%2A>|하위 집합을 포함 하는 0부터 시작 배열을 반환을 `String` 배열의 지정 된 필터 조건에 따라 합니다.|  
|<xref:Microsoft.VisualBasic.Strings.Format%2A>|반환 형식에 포함 된 지침에 따라 서식이 지정 문자열로 `String` 식입니다.|  
