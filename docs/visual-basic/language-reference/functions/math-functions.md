...
다음 표에서는 <xref:System.Math?displayProperty=nameWithType> 클래스의 메서드를 보여 줍니다. 이러한 작업은 Visual Basic 프로그램에서 사용할 수 있습니다.
...
다음 표에서는 .NET Framework에 없지만 .NET Standard 또는 .NET Core에 추가 된 <xref:System.Math?displayProperty=nameWithType> 클래스의 메서드를 보여 줍니다.
...
|<xref:System.Math.ScaleB%2A>|는 x \* 2 ^ n을 효율적으로 계산 합니다.|.NET Core 3.0부터|
...
> [!NOTE]
> <xref:System.Math?displayProperty=nameWithType> 클래스는 <xref:System.Math.PI?displayProperty=nameWithType> 상수 필드를 포함 합니다. 이를 계산 하는 대신 사용할 수 있습니다.
