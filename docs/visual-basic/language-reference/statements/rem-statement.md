...
## <a name="parts"></a>요소  
 `comment`  
 (선택 사항) 포함 하려는 주석의 텍스트입니다. @No__t_0 키워드와 `comment` 사이에 공백이 필요 합니다.  
  
## <a name="remarks"></a>주의  
 @No__t_0 문을 한 줄에 단독으로 넣거나 다른 문 다음의 줄에 삽입할 수 있습니다. @No__t_0 문은 줄의 마지막 문 이어야 합니다. 다른 문을 따르는 경우에는 `REM`를 해당 문에서 공백으로 구분 해야 합니다.  
  
 @No__t_1 대신 작은따옴표 (`'`)를 사용할 수 있습니다. 이는 주석이 동일한 줄에서 다른 문을 팔 로우 하거나 한 줄에만 적용 되는지 여부에 해당 합니다.  
  
> [!NOTE]
> @No__t_1 (줄 연속 시퀀스)를 사용 하 여 `REM` 문을 계속할 수 없습니다. 주석이 시작 되 면 컴파일러는 특수 한 의미에 대 한 문자를 검사 하지 않습니다. 여러 줄로 된 주석을 보려면 각 줄에서 다른 `REM` 문이나 주석 기호 (`'`)를 사용 합니다.  
