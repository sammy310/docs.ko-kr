...
## <a name="parts"></a>요소  
 `On`  
 (선택 사항) `Option Explicit` 검사를 사용 하도록 설정 합니다. `On` 또는 `Off`을 지정 하지 않으면 기본값은 `On`입니다.  
  
 `Off`  
 (선택 사항) `Option Explicit` 검사를 사용 하지 않습니다.  
  
## <a name="remarks"></a>주의  
 `Option Explicit On` 또는 `Option Explicit` 파일에 표시 되는 경우 `Dim` 또는 `ReDim` 문을 사용 하 여 모든 변수를 명시적으로 선언 해야 합니다. 선언 되지 않은 변수 이름을 사용 하려고 하면 컴파일 타임에 오류가 발생 합니다. `Option Explicit Off` 문은 변수를 암시적으로 선언할 수 있습니다.  
