...
|용어|정의|  
|---|---|  
|`condition`|필수 요소. `Boolean` 식입니다. If `condition` is `Nothing`, Visual Basic treats it as `False`.|  
|`statements`|(선택 사항) One or more statements following `While`, which run every time `condition` is `True`.|  
|`Continue While`|(선택 사항) Transfers control to the next iteration of the `While` block.|  
|`Exit While`|(선택 사항) Transfers control out of the `While` block.|  
