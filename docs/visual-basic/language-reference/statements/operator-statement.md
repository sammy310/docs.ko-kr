...
`Public`  
필수 요소. Indicates that this operator procedure has [Public](../../../visual-basic/language-reference/modifiers/public.md) access.

`Overloads`  
(선택 사항) See [Overloads](../../../visual-basic/language-reference/modifiers/overloads.md).

`Shared`  
필수 요소. Indicates that this operator procedure is a [Shared](../../../visual-basic/language-reference/modifiers/shared.md) procedure.
...
`operatorsymbol`  
필수 요소. The symbol or identifier of the operator that this operator procedure defines.

`operand1`  
필수 요소. The name and type of the single operand of a unary operator (including a conversion operator) or the left operand of a binary operator.
...
|`operandname`|필수 요소. Name of the variable representing this operand. [Declared Element Names](../../../visual-basic/programming-guide/language-features/declared-elements/declared-element-names.md)을 참조하세요.|
...
`statements`  
(선택 사항) Block of statements that the operator procedure runs.

`returnvalue`  
필수 요소. The value that the operator procedure returns to the calling code.

`End` `Operator`  
필수 요소. Terminates the definition of this operator procedure.

## <a name="remarks"></a>주의

You can use `Operator` only in a class or structure. This means the *declaration context* for an operator cannot be a source file, namespace, module, interface, procedure, or block. 자세한 내용은 [선언 컨텍스트 및 기본 액세스 수준](../../../visual-basic/language-reference/statements/declaration-contexts-and-default-access-levels.md)을 참조하세요.
...
Defining an operator in this way is called *operator overloading*, whether or not you use the `Overloads` keyword. 다음 표에는 정의할 수 있는 연산자가 나와 있습니다.
...
A *widening conversion* always succeeds at run time, while a *narrowing conversion* can fail at run time. 자세한 내용은 [Widening and Narrowing Conversions](../../../visual-basic/programming-guide/language-features/data-types/widening-and-narrowing-conversions.md)을 참조하세요.

If you declare a conversion procedure to be `Widening`, your procedure code must not generate any failures. 이것은 다음을 의미합니다.
