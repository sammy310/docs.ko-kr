...
|`constraintlist`|(선택 사항) @No__t_0에 대해 제공할 수 있는 데이터 형식을 제한 하는 요구 사항 목록입니다. 여러 제약 조건이 있는 경우 중괄호 (`{ }`)로 묶고 쉼표로 구분 합니다. 제약 조건 목록에 [As](../../../visual-basic/language-reference/statements/as-clause.md) 키워드를 사용 해야 합니다. 목록의 시작 부분에 `As` 한 번만 사용 합니다.|
...
- **적용.** 형식 매개 변수에 대 한 *제약 조건* 목록에는 다음과 같은 항목이 조합 되어 포함 될 수 있습니다.
  ...
  - @No__t_0 키워드 또는 `Structure` 키워드입니다. @No__t_0 키워드는 제네릭 형식 매개 변수에 전달 된 형식 인수가 문자열, 배열 또는 대리자와 같은 참조 형식 이거나 클래스에서 만든 개체를 참조 형식으로 제한 하도록 제한 합니다. @No__t_0 키워드는 제네릭 형식 매개 변수에 전달 된 형식 인수가 구조체, 열거형 또는 기본 데이터 형식 등의 값 형식이 되도록 제한 합니다. 동일한 `constraintlist`에 `Class`와 `Structure`를 모두 포함할 수는 없습니다.
...
@No__t_0은 일반적 이므로이를 사용 하는 코드는 동일한 기능을 갖지만 다른 데이터 형식에서 작동 하는 다양 한 개체를 만들 수 있습니다. 다음 예제에서는 `String` 항목과 `Integer` 키를 사용 하 여 `dictionary` 개체를 만드는 코드 줄을 보여 줍니다.
