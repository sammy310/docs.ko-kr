...
Specifies the attributes to be applied to a declared programming element. 여러 특성은 쉼표로 구분합니다. Following is the syntax for one attribute.  
...
|`attributearguments`|(선택 사항) List of positional arguments for this attribute. Multiple arguments are separated by commas.|
|`attributeinitializer`|(선택 사항) List of variable or property initializers for this attribute. Multiple initializers are separated by commas.|
