...
|용어|정의|  
|---|---|  
|`Do`|필수 요소. Starts the definition of the `Do` loop.|  
|`While`|`Until`를 사용하는 경우를 제외하고는 필수입니다. Repeat the loop until `condition` is `False`.|  
|`Until`|`While`를 사용하는 경우를 제외하고는 필수입니다. Repeat the loop until `condition` is `True`.|  
|`condition`|(선택 사항) `Boolean` 식입니다. If `condition` is `Nothing`, Visual Basic treats it as `False`.|  
|`statements`|(선택 사항) One or more statements that are repeated while, or until, `condition` is `True`.|  
|`Continue Do`|(선택 사항) Transfers control to the next iteration of the `Do` loop.|  
|`Exit Do`|(선택 사항) Transfers control out of the `Do` loop.|  
|`Loop`|필수 요소. Terminates the definition of the `Do` loop.|  
