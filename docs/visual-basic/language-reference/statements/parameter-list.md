...
`parametertype`  
`Option Strict` `On`경우 필수입니다. 매개 변수를 나타내는 지역 변수의 데이터 형식입니다.

`defaultvalue`  
`Optional` 매개 변수에 필요 합니다. 매개 변수의 데이터 형식으로 계산 되는 상수 또는 상수 식입니다. 형식이 `Object`이거나 클래스, 인터페이스, 배열 또는 구조 이면 기본값은 `Nothing`만 가능 합니다.

## <a name="remarks"></a>주의

매개 변수는 괄호로 묶고 쉼표로 구분 됩니다. 모든 데이터 형식을 사용 하 여 매개 변수를 선언할 수 있습니다. `parametertype`지정 하지 않으면 기본적으로 `Object`로 설정 됩니다.
...
- **매개 변수 배열.** `ParamArray` 매개 변수에 대 한 `ByVal` 지정 해야 합니다.
