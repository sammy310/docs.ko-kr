...
|용어|정의|
|---|---|
|`attributelist`|(선택 사항) [특성 목록](attribute-list.md)을 참조 하십시오.|
|`accessmodifier`|(선택 사항) 다음 중 하나일 수 있습니다.<br /><br /> [공용](../modifiers/public.md) -   <br />-   [보호](../modifiers/protected.md) 됨<br />-   [Friend](../modifiers/friend.md)<br />-   [개인](../modifiers/private.md)<br />- [보호 된 Friend](../modifiers/protected-friend.md)<br/>- [개인 보호](../modifiers/private-protected.md) 됨 <br /><br /> [Access levels in Visual Basic](../../programming-guide/language-features/declared-elements/access-levels.md)을 참조하세요.|
|`Shadows`|(선택 사항) [그림자](../modifiers/shadows.md)를 참조 하세요.|
|`Partial`|(선택 사항) 구조체의 부분 정의를 나타냅니다. [부분](../modifiers/partial.md)을 참조 하세요.|
|`name`|필수 요소. 구조체의 이름입니다. [Declared Element Names](../../programming-guide/language-features/declared-elements/declared-element-names.md)을 참조하세요.|
...
|`Implements`|(선택 사항) 이 구조체는 하나 이상의 인터페이스의 멤버를 구현 함을 나타냅니다. [Implements 문](implements-statement.md)을 참조 하세요.|
|`interfacenames`|`Implements` 문을 사용 하는 경우 필요 합니다. 이 구조체에서 구현 하는 인터페이스의 이름입니다.|
...
|`End Structure`|필수 요소. `Structure` 정의를 종료 합니다.|
...
`Structure`는 네임 스페이스 또는 모듈 수준 에서만 사용할 수 있습니다. 즉, 구조체의 *선언 컨텍스트* 는 소스 파일, 네임 스페이스, 클래스, 구조체, 모듈 또는 인터페이스 여야 하며 프로시저 또는 블록일 수 없습니다. 자세한 내용은 [선언 컨텍스트 및 기본 액세스 수준](declaration-contexts-and-default-access-levels.md)을 참조하세요.
...
- **멤버 선언입니다.** 구조체의 모든 멤버를 선언 해야 합니다. 구조체 멤버는 구조체에서 상속할 수 없으므로 구조체 멤버를 [보호](../modifiers/protected.md) 하거나 `Protected Friend` 수 없습니다. 그러나 구조체 자체는 `Protected` 또는 `Protected Friend` 수 있습니다.
...
- **상속.** 구조체는 모든 구조체가 상속 하는 <xref:System.ValueType> 이외의 형식에서 상속할 수 없습니다. 특히 한 구조체는 다른 구조체에서 상속할 수 없습니다.
     ...
     구조체 정의에서는 [Inherits 문을](inherits-statement.md) 사용 하 여 <xref:System.ValueType> 지정할 수 없습니다.

- **구현이.** 구조체에서 [Implements 문을](implements-statement.md)사용 하는 경우 `interfacenames`에서 지정 하는 모든 인터페이스에서 정의 된 모든 멤버를 구현 해야 합니다.
...
다음 예에서는 `Structure` 문을 사용 하 여 직원에 대 한 관련 데이터 집합을 정의 합니다. `Public`, `Friend`및 `Private` 멤버를 사용 하 여 데이터 항목의 민감도를 반영 하는 방법을 보여 줍니다. 또한 프로시저, 속성 및 이벤트 멤버도 보여 줍니다.

[!code-vb[VbVbalrStatements#57](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrStatements/VB/Class1.vb#57)]

`Structure`s를 사용 하는 방법에 대 한 자세한 내용은 [구조체 변수](../../programming-guide/language-features/data-types/structure-variables.md)를 참조 하세요.
