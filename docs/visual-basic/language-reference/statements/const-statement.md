...
|`datatype`|@No__t_0 `On` 경우 필수입니다. 상수의 데이터 형식입니다.|
...
- **데이터 형식.** @No__t_0 문은 변수의 데이터 형식을 선언할 수 있습니다. 모든 데이터 형식 또는 열거형의 이름을 지정할 수 있습니다.

- **기본 형식입니다.** @No__t_0 지정 하지 않으면 상수는 `initializer` 데이터 형식을 사용 합니다. @No__t_0와 `initializer`를 모두 지정 하는 경우 `initializer`의 데이터 형식을 `datatype`로 변환할 수 있어야 합니다. @No__t_0 또는 `initializer` 모두 없는 경우에는 데이터 형식이 기본적으로 `Object` 됩니다.
...
- **초기.** @No__t_0의 모든 상수 값을 초기화 해야 합니다. @No__t_0 사용 하 여 상수에 할당할 식을 제공 합니다. 식에는 리터럴, 이미 정의 된 다른 상수 및 이미 정의 된 열거형 멤버가 모두 조합 될 수 있습니다. 산술 연산자와 논리 연산자를 사용 하 여 이러한 요소를 결합할 수 있습니다.
...
@No__t_0 데이터 형식을 사용 하 여 상수를 정의 하는 경우 Visual Basic 컴파일러는 `Object` 대신 `initializer` 형식을 제공 합니다. 다음 예제에서는 상수 `naturalLogBase`에 `Decimal` 런타임 형식이 있습니다.
