...
- **Declaration Context.** `Shared`는 모듈 수준에서만 사용할 수 있습니다. This means the declaration context for a `Shared` element must be a class or structure, and cannot be a source file, namespace, or procedure.  
