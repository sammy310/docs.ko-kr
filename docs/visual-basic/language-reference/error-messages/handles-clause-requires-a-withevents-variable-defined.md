...
제공 하지 않은 경우는 `WithEvents` 변수에 프로그램 `Handles` 절. 합니다 `Handles` 프로시저 선언의 끝에서 키워드를 사용 하 여 선언 된 개체 변수에 의해 발생 한 이벤트를 처리 하면는 `WithEvents` 키워드입니다.  
  
 **오류 ID:** BC30506  
  
## <a name="to-correct-this-error"></a>이 오류를 해결하려면  
  
-   필요한 제공 `WithEvents` 변수입니다.  
