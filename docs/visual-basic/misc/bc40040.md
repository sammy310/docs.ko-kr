...
제네릭 형식이 `<CLSCompliant(True)>`로 표시되었지만 해당 형식 매개 변수 중 하나의 제약 조건에서 `<CLSCompliant(False)>`으로 표시되었거나, 표시되지 않았거나, 비규격 형식이므로 사용할 수 없는 형식을 지정합니다.  
 ...
 형식이 [언어 독립성 및 언어 독립적 구성 요소](../../standard/language-independence-and-language-independent-components.md) (cls)와 호환 되려면 cls 규격 형식만 사용 해야 합니다. 이는 제네릭 형식의 형식 매개 변수에 대한 제약 조건에 적용됩니다.  
