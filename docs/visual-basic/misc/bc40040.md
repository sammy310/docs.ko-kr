...
---
# <a name="generic-parameter-constraint-type-typename-is-not-cls-compliant"></a>제네릭 매개 변수 제약 조건 형식이 \<typename > CLS 규격이 아님
제네릭 형식이 `<CLSCompliant(True)>`로 표시되었지만 해당 형식 매개 변수 중 하나의 제약 조건에서 `<CLSCompliant(False)>`으로 표시되었거나, 표시되지 않았거나, 비규격 형식이므로 사용할 수 없는 형식을 지정합니다.  
 ...
 유형이 호환 되어야 하는 데는 [Language Independence and Language-independent Components](../../standard/language-independence-and-language-independent-components.md) (CLS) CLS 규격 형식만 사용 해야 합니다. 이는 제네릭 형식의 형식 매개 변수에 대한 제약 조건에 적용됩니다.  
