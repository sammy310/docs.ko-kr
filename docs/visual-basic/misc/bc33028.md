...
변환 연산자가 [Object Data Type](../../visual-basic/language-reference/data-types/object-data-type.md)의 반환 형식으로 선언되었습니다.  
 ...
 컴파일 타임에 Visual Basic은 미리 정의 된 변환이 있는 임의 참조 형식에서, 해당 상속 계층 구조의 모든 형식에 형식에서 파생 된 또는 파생 된 간주 합니다. `Object` .NET framework에서 유니버설 데이터 형식 이므로 모든 형식에서 파생 `Object`합니다.  
