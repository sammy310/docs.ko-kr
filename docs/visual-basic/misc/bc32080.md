...
제네릭 `Sub` 프로시저의 선언에 [Handles](../../visual-basic/language-reference/statements/handles-clause.md) 절이 포함되어 있습니다.  
 ...
 `Handles` 절은 `Sub` 프로시저가 처리하는 이벤트 목록을 지정합니다. 이벤트 처리기로 사용하려면 처리할 각 이벤트와 동일한 서명이 `Sub` 프로시저에 있어야 합니다. 제네릭 프로시저는 컴파일 시간에 예측할 수 없는 Visual Basic 서명을 사용 하 여 두 번 이상 만들 수 있습니다. 따라서 Visual Basic는 `Handles` 절의 이벤트와 일치 하는 서명을 보장할 수 없습니다.  
