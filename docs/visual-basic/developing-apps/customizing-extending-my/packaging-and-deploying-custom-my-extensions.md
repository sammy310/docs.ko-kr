...
|특성|설명|
|---|---|
|`ID`|필수 요소. A unique identifier for the extension. If the extension that has this ID has already been added to the project, the user will not be prompted to add it again.|
|`Version`|필수 요소. A version number for the item template.|
|`AssemblyFullName`|(선택 사항) 어셈블리 이름입니다. When a reference to this assembly is added to the project, the user will be prompted to add the `My` extension from this item template.|
