...
`filepath`  
참조 어셈블리의 경로 및 파일 이름입니다. 일반적으로 주 어셈블리의 하위 폴더에 있어야 합니다. 권장되는 규칙(MSBuild에서 사용됨)은 주 어셈블리에 상대적으로 “ref/” sub-폴더에 참조 어셈블리를 배치하는 것입니다. `filepath`의 모든 폴더가 존재 해야 합니다. 컴파일러는 생성 하지 않습니다.
