...
## <a name="arguments"></a>인수  
 `response_file`  
 필수 요소. 컴파일할 소스 코드 파일이 나 컴파일러 옵션을 나열 하는 파일입니다. 공백을 포함 하는 경우 파일 이름을 따옴표 ("")로 묶습니다.  
...
```  
 ...
 Visual Basic는 vbc.exe 파일과 동일한 디렉터리에 있는 Vbc.exe 파일을 제공 합니다. 이 `-noconfig` 옵션을 사용 하지 않는 한 vbc.exe 파일은 기본적으로 포함 됩니다. 자세한 내용은 [-noconfig](../../../visual-basic/reference/command-line-compiler/noconfig.md)를 참조 하세요.  
  
> [!NOTE]
> 이 `@` 옵션은 Visual Studio 개발 환경에서 사용할 수 없습니다. 명령줄에서 컴파일하는 경우에만 사용할 수 있습니다.  
...
## <a name="example"></a>예제  
 다음 예제에서는 라는 `@` `File1.rsp`지시 파일에 옵션을 사용 하는 방법을 보여 줍니다.  
