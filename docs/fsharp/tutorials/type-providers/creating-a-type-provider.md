...
- <span data-ttu-id="c04e5-129">스키마가 얼마나 변경 되나요?</span><span class="sxs-lookup"><span data-stu-id="c04e5-129">How much will your schema change?</span></span>
...
### <a name="implementation-of-the-type-provider"></a><span data-ttu-id="c04e5-154">형식 공급자 구현</span><span class="sxs-lookup"><span data-stu-id="c04e5-154">Implementation of the Type Provider</span></span>
...
<span data-ttu-id="c04e5-174">다음 사항에 유의 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-174">You should note the following points:</span></span>
...
- <span data-ttu-id="c04e5-235">제공 된 지워진 클래스 형식을 삭제 하는 것은 항상 형식의 상속 체인에서 삭제 되지 않은 첫 번째 기본 형식입니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-235">The erasure of a provided erased class type is always the first non-erased base type in the inheritance chain of the type.</span></span>
...
### <a name="type-checked-regex-provider"></a><span data-ttu-id="c04e5-259">선택 된 Regex 공급자 유형</span><span class="sxs-lookup"><span data-stu-id="c04e5-259">Type Checked Regex Provider</span></span>
...
- <span data-ttu-id="c04e5-270">`RegexTyped`생성자는 Regex 생성자를 호출 하 여 패턴에 대 한 정적 형식 인수를 전달 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-270">The `RegexTyped` constructor results in a call to the Regex constructor, passing in the static type argument for the pattern.</span></span>
...
- <span data-ttu-id="c04e5-329">헤더 이름은 단위 이거나 "이름 (단위)" 형식이 며 쉼표를 포함 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-329">Header names are either unit-less or have the form "Name (unit)" and don't contain commas.</span></span>
...
<span data-ttu-id="c04e5-336">이 경우 컴파일러는 이러한 호출을 다음 예제와 같은 항목으로 변환 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-336">In this case, the compiler should convert these calls into something like the following example:</span></span>
...
<span data-ttu-id="c04e5-337">최적의 변환에서는 형식 공급자가 형식 `CsvFile` 공급자의 어셈블리에서 실제 형식을 정의 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-337">The optimal translation will require the type provider to define a real `CsvFile` type in the type provider's assembly.</span></span> <span data-ttu-id="c04e5-338">형식 공급자는 종종 몇 가지 도우미 형식 및 메서드를 사용 하 여 중요 한 논리를 래핑합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-338">Type providers often rely on a few helper types and methods to wrap important logic.</span></span> <span data-ttu-id="c04e5-339">측정값은 런타임에 지워집니다 `float[]` .를 행의 지워진 형식으로 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-339">Because measures are erased at runtime, you can use a `float[]` as the erased type for a row.</span></span> <span data-ttu-id="c04e5-340">컴파일러는 서로 다른 측정값 형식이 있는 것으로 다른 열을 처리 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-340">The compiler will treat different columns as having different measure types.</span></span> <span data-ttu-id="c04e5-341">예를 들어이 예제의 첫 번째 열에는 형식이 `float<meter>` 있고, 두 번째 열에는가 `float<second>` 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-341">For example, the first column in our example has type `float<meter>`, and the second has `float<second>`.</span></span> <span data-ttu-id="c04e5-342">그러나 지워진 표현은 매우 간단 하 게 유지할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-342">However, the erased representation can remain quite simple.</span></span>

<span data-ttu-id="c04e5-343">다음 코드는 구현의 핵심을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-343">The following code shows the core of the implementation.</span></span>
...
<span data-ttu-id="c04e5-344">구현에 대 한 다음 사항에 유의 하세요.</span><span class="sxs-lookup"><span data-stu-id="c04e5-344">Note the following points about the implementation:</span></span>

- <span data-ttu-id="c04e5-345">오버 로드 된 생성자는 원본 파일 또는 동일한 스키마를 가진 파일을 읽을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-345">Overloaded constructors allow either the original file or one that has an identical schema to be read.</span></span> <span data-ttu-id="c04e5-346">이 패턴은 로컬 또는 원격 데이터 원본에 대 한 형식 공급자를 작성할 때 일반적으로 사용 되며,이 패턴을 사용 하면 로컬 파일을 원격 데이터의 템플릿으로 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-346">This pattern is common when you write a type provider for local or remote data sources, and this pattern allows a local file to be used as the template for remote data.</span></span>

- <span data-ttu-id="c04e5-347">형식 공급자 생성자에 전달 된 [Typeproviderconfig](https://fsharp.github.io/fsharp-core-docs/reference/fsharp-core-compilerservices-typeproviderconfig.html) 값을 사용 하 여 상대 파일 이름을 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-347">You can use the [TypeProviderConfig](https://fsharp.github.io/fsharp-core-docs/reference/fsharp-core-compilerservices-typeproviderconfig.html) value that’s passed in to the type provider constructor to resolve relative file names.</span></span>

- <span data-ttu-id="c04e5-348">메서드를 사용 하 여 `AddDefinitionLocation` 제공 된 속성의 위치를 정의할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-348">You can use the `AddDefinitionLocation` method to define the location of the provided properties.</span></span> <span data-ttu-id="c04e5-349">따라서 제공 된 속성에서를 사용 하는 경우 `Go To Definition` CSV 파일은 Visual Studio에서 열립니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-349">Therefore, if you use `Go To Definition` on a provided property, the CSV file will open in Visual Studio.</span></span>

- <span data-ttu-id="c04e5-350">형식을 사용 하 여 `ProvidedMeasureBuilder` SI 단위를 조회 하 고 관련 형식을 생성할 수 있습니다 `float<_>` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-350">You can use the `ProvidedMeasureBuilder` type to look up the SI units and to generate the relevant `float<_>` types.</span></span>

### <a name="key-lessons"></a><span data-ttu-id="c04e5-351">주요 단원</span><span class="sxs-lookup"><span data-stu-id="c04e5-351">Key Lessons</span></span>

<span data-ttu-id="c04e5-352">이 섹션에서는 데이터 원본 자체에 포함 된 간단한 스키마를 사용 하 여 로컬 데이터 원본에 대 한 형식 공급자를 만드는 방법에 대해 설명 했습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-352">This section explained how to create a type provider for a local data source with a simple schema that's contained in the data source itself.</span></span>

## <a name="going-further"></a><span data-ttu-id="c04e5-353">더 나아가기</span><span class="sxs-lookup"><span data-stu-id="c04e5-353">Going Further</span></span>

<span data-ttu-id="c04e5-354">다음 섹션에는 추가 연구에 대 한 제안이 포함 되어 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-354">The following sections include suggestions for further study.</span></span>

### <a name="a-look-at-the-compiled-code-for-erased-types"></a><span data-ttu-id="c04e5-355">컴파일된 형식에 대 한 컴파일된 코드 살펴보기</span><span class="sxs-lookup"><span data-stu-id="c04e5-355">A Look at the Compiled Code for Erased Types</span></span>

<span data-ttu-id="c04e5-356">형식 공급자를 사용 하는 방법에 대 한 자세한 내용은 `HelloWorldTypeProvider` 이 항목의 앞부분에서 사용 된를 사용 하 여 다음 함수를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="c04e5-356">To give you some idea of how the use of the type provider corresponds to the code that's emitted, look at the following function by using the `HelloWorldTypeProvider` that's used earlier in this topic.</span></span>
...
<span data-ttu-id="c04e5-357">ildasm.exe를 사용 하 여 결과 코드 디컴파일된 이미지는 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-357">Here’s an image of the resulting code decompiled by using ildasm.exe:</span></span>
...
<span data-ttu-id="c04e5-358">예제에 나와 있는 것 처럼 형식 및 속성의 모든 멘 션이 `Type1` `InstanceProperty` 삭제 되어 관련 된 런타임 형식에 대 한 작업만 남게 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-358">As the example shows, all mentions of the type `Type1` and the `InstanceProperty` property have been erased, leaving only operations on the runtime types involved.</span></span>

### <a name="design-and-naming-conventions-for-type-providers"></a><span data-ttu-id="c04e5-359">형식 공급자의 디자인 및 명명 규칙</span><span class="sxs-lookup"><span data-stu-id="c04e5-359">Design and Naming Conventions for Type Providers</span></span>

<span data-ttu-id="c04e5-360">형식 공급자를 제작할 때 다음 규칙을 준수 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-360">Observe the following conventions when authoring type providers.</span></span>

<span data-ttu-id="c04e5-361">**연결 프로토콜에 대 한 공급자** 일반적으로 데이터 및 서비스 연결 프로토콜에 대 한 대부분의 공급자 Dll 이름 (예: OData 또는 SQL 연결)은 또는로 끝나야 합니다 `TypeProvider` `TypeProviders` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-361">**Providers for Connectivity Protocols** In general, names of most provider DLLs for data and service connectivity protocols, such as OData or SQL connections, should end in `TypeProvider` or `TypeProviders`.</span></span> <span data-ttu-id="c04e5-362">예를 들어 다음 문자열과 유사한 DLL 이름을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-362">For example, use a DLL name that resembles the following string:</span></span>

`Fabrikam.Management.BasicTypeProviders.dll`

<span data-ttu-id="c04e5-363">제공 된 형식이 해당 네임 스페이스의 멤버 인지 확인 하 고 구현 된 연결 프로토콜을 표시 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-363">Ensure that your provided types are members of the corresponding namespace, and indicate the connectivity protocol that you implemented:</span></span>
...
<span data-ttu-id="c04e5-364">**일반적인 코딩을 위한 유틸리티 공급자** 입니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-364">**Utility Providers for General Coding**.</span></span>  <span data-ttu-id="c04e5-365">정규식의 경우와 같은 유틸리티 유형 공급자의 경우 다음 예제와 같이 형식 공급자가 기본 라이브러리의 일부일 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-365">For a utility type provider such as that for regular expressions, the type provider may be part of a base library, as the following example shows:</span></span>

```fsharp
#r "Fabrikam.Core.Text.Utilities.dll"
```

<span data-ttu-id="c04e5-366">이 경우 제공 된 형식이 일반적인 .NET 디자인 규칙에 따라 적절 한 지점에 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-366">In this case, the provided type would appear at an appropriate point according to normal .NET design conventions:</span></span>
...
<span data-ttu-id="c04e5-367">**Singleton 데이터 원본** 입니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-367">**Singleton Data Sources**.</span></span> <span data-ttu-id="c04e5-368">일부 형식 공급자는 단일 전용 데이터 원본에 연결 하 고 데이터만 제공 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-368">Some type providers connect to a single dedicated data source and provide only data.</span></span> <span data-ttu-id="c04e5-369">이 경우 접미사를 삭제 `TypeProvider` 하 고 .net 명명에 대 한 일반 규칙을 사용 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-369">In this case, you should drop the `TypeProvider` suffix and use normal conventions for .NET naming:</span></span>
...
<span data-ttu-id="c04e5-370">자세한 내용은 `GetConnection` 이 항목의 뒷부분에서 설명 하는 디자인 규칙을 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="c04e5-370">For more information, see the `GetConnection` design convention that's described later in this topic.</span></span>

### <a name="design-patterns-for-type-providers"></a><span data-ttu-id="c04e5-371">형식 공급자의 디자인 패턴</span><span class="sxs-lookup"><span data-stu-id="c04e5-371">Design Patterns for Type Providers</span></span>

<span data-ttu-id="c04e5-372">다음 섹션에서는 형식 공급자를 제작할 때 사용할 수 있는 디자인 패턴에 대해 설명 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-372">The following sections describe design patterns you can use when authoring type providers.</span></span>

#### <a name="the-getconnection-design-pattern"></a><span data-ttu-id="c04e5-373">GetConnection 디자인 패턴</span><span class="sxs-lookup"><span data-stu-id="c04e5-373">The GetConnection Design Pattern</span></span>

<span data-ttu-id="c04e5-374">`GetConnection`다음 예제와 같이 FSharp.Data.TypeProviders.dll의 형식 공급자가 사용 하는 패턴을 사용 하도록 대부분의 형식 공급자를 작성 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-374">Most type providers should be written to use the `GetConnection` pattern that's used by the type providers in FSharp.Data.TypeProviders.dll, as the following example shows:</span></span>
...
#### <a name="type-providers-backed-by-remote-data-and-services"></a><span data-ttu-id="c04e5-375">원격 데이터 및 서비스에서 지원 되는 형식 공급자</span><span class="sxs-lookup"><span data-stu-id="c04e5-375">Type Providers Backed By Remote Data and Services</span></span>

<span data-ttu-id="c04e5-376">원격 데이터와 서비스에서 지원 되는 형식 공급자를 만들기 전에 연결 된 프로그래밍에 내재 된 다양 한 문제를 고려해 야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-376">Before you create a type provider that's backed by remote data and services, you must consider a range of issues that are inherent in connected programming.</span></span> <span data-ttu-id="c04e5-377">이러한 문제에는 다음 사항이 포함 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-377">These issues include the following considerations:</span></span>

- <span data-ttu-id="c04e5-378">스키마 매핑</span><span class="sxs-lookup"><span data-stu-id="c04e5-378">schema mapping</span></span>

- <span data-ttu-id="c04e5-379">스키마가 변경 된 상태에서 선거의 및 무효화</span><span class="sxs-lookup"><span data-stu-id="c04e5-379">liveness and invalidation in the presence of schema change</span></span>

- <span data-ttu-id="c04e5-380">스키마 캐싱</span><span class="sxs-lookup"><span data-stu-id="c04e5-380">schema caching</span></span>

- <span data-ttu-id="c04e5-381">데이터 액세스 작업의 비동기 구현</span><span class="sxs-lookup"><span data-stu-id="c04e5-381">asynchronous implementations of data access operations</span></span>

- <span data-ttu-id="c04e5-382">LINQ 쿼리를 비롯 한 지원 쿼리</span><span class="sxs-lookup"><span data-stu-id="c04e5-382">supporting queries, including LINQ queries</span></span>

- <span data-ttu-id="c04e5-383">자격 증명 및 인증</span><span class="sxs-lookup"><span data-stu-id="c04e5-383">credentials and authentication</span></span>

<span data-ttu-id="c04e5-384">이 항목에서는 이러한 문제를 더 자세히 설명 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-384">This topic doesn't explore these issues further.</span></span>

### <a name="additional-authoring-techniques"></a><span data-ttu-id="c04e5-385">추가 제작 기술</span><span class="sxs-lookup"><span data-stu-id="c04e5-385">Additional Authoring Techniques</span></span>

<span data-ttu-id="c04e5-386">사용자 고유의 형식 공급자를 작성 하는 경우 다음과 같은 추가 기술을 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-386">When you write your own type providers, you might want to use the following additional techniques.</span></span>

### <a name="creating-types-and-members-on-demand"></a><span data-ttu-id="c04e5-387">요청 시 형식 및 멤버 만들기</span><span class="sxs-lookup"><span data-stu-id="c04e5-387">Creating Types and Members On-Demand</span></span>

<span data-ttu-id="c04e5-388">프로 비전 된 유형 API는 지연 된 버전의 AddMember를 포함 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-388">The ProvidedType API has delayed versions of AddMember.</span></span>
...
<span data-ttu-id="c04e5-389">이러한 버전은 형식의 주문형 공간을 만드는 데 사용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-389">These versions are used to create on-demand spaces of types.</span></span>

### <a name="providing-array-types-and-generic-type-instantiations"></a><span data-ttu-id="c04e5-390">배열 형식 및 제네릭 형식 인스턴스화 제공</span><span class="sxs-lookup"><span data-stu-id="c04e5-390">Providing Array types and Generic Type Instantiations</span></span>

<span data-ttu-id="c04e5-391">을 포함 하 여 `MakeArrayType` `MakePointerType` `MakeGenericType` 의 모든 인스턴스에서 일반, 및를 <xref:System.Type> 사용 하 여 제공 된 멤버 (시그니처에 배열 형식, byref 형식 및 제네릭 형식의 인스턴스화 포함)를 만듭니다 `ProvidedTypeDefinitions` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-391">You make provided members (whose signatures include array types, byref types, and instantiations of generic types) by using the normal `MakeArrayType`, `MakePointerType`, and `MakeGenericType` on any instance of <xref:System.Type>, including `ProvidedTypeDefinitions`.</span></span>

> [!NOTE]
> <span data-ttu-id="c04e5-392">에서 도우미를 사용 해야 하는 경우도 있습니다 `ProvidedTypeBuilder.MakeGenericType` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-392">In some cases you may have to use the helper in `ProvidedTypeBuilder.MakeGenericType`.</span></span>  <span data-ttu-id="c04e5-393">자세한 내용은 [형식 공급자 SDK 설명서](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) 를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="c04e5-393">See the [Type Provider SDK documentation](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) for more details.</span></span>

### <a name="providing-unit-of-measure-annotations"></a><span data-ttu-id="c04e5-394">측정 단위 주석 제공</span><span class="sxs-lookup"><span data-stu-id="c04e5-394">Providing Unit of Measure Annotations</span></span>

<span data-ttu-id="c04e5-395">프로 비전 된 형식 API는 측정값 주석을 제공 하는 도우미를 제공 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-395">The ProvidedTypes API provides helpers for providing measure annotations.</span></span> <span data-ttu-id="c04e5-396">예를 들어 형식을 제공 하려면 `float<kg>` 다음 코드를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-396">For example, to provide the type `float<kg>`, use the following code:</span></span>
...
```
  ...
  <span data-ttu-id="c04e5-397">형식을 제공 하려면 `Nullable<decimal<kg/m^2>>` 다음 코드를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-397">To provide the type `Nullable<decimal<kg/m^2>>`, use the following code:</span></span>
...
### <a name="accessing-project-local-or-script-local-resources"></a><span data-ttu-id="c04e5-398">Project-Local 또는 Script-Local 리소스에 액세스</span><span class="sxs-lookup"><span data-stu-id="c04e5-398">Accessing Project-Local or Script-Local Resources</span></span>

<span data-ttu-id="c04e5-399">형식 공급자의 각 인스턴스에는 생성 중에 값을 지정할 수 있습니다 `TypeProviderConfig` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-399">Each instance of a type provider can be given a `TypeProviderConfig` value during construction.</span></span> <span data-ttu-id="c04e5-400">이 값에는 공급자에 대 한 "해결 폴더" (즉, 컴파일에 대 한 프로젝트 폴더 또는 스크립트가 포함 된 디렉터리), 참조 된 어셈블리 목록 및 기타 정보가 포함 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-400">This value contains the "resolution folder" for the provider (that is, the project folder for the compilation or the directory that contains a script), the list of referenced assemblies, and other information.</span></span>

### <a name="invalidation"></a><span data-ttu-id="c04e5-401">무효화</span><span class="sxs-lookup"><span data-stu-id="c04e5-401">Invalidation</span></span>

<span data-ttu-id="c04e5-402">공급자는 무효화 신호를 발생 시켜 스키마 가정이 변경 되었을 수 있음을 F # 언어 서비스에 알릴 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-402">Providers can raise invalidation signals to notify the F# language service that the schema assumptions may have changed.</span></span> <span data-ttu-id="c04e5-403">무효화가 발생 하면 Visual Studio에서 공급자가 호스트 되는 경우 typecheck가 다시 실행 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-403">When invalidation occurs, a typecheck is redone if the provider is being hosted in Visual Studio.</span></span> <span data-ttu-id="c04e5-404">공급자가 F# 대화형 또는 F # 컴파일러 (fsc.exe)에서 호스트 되는 경우이 신호는 무시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-404">This signal will be ignored when the provider is hosted in F# Interactive or by the F# Compiler (fsc.exe).</span></span>

### <a name="caching-schema-information"></a><span data-ttu-id="c04e5-405">스키마 정보 캐싱</span><span class="sxs-lookup"><span data-stu-id="c04e5-405">Caching Schema Information</span></span>

<span data-ttu-id="c04e5-406">공급자는 종종 스키마 정보에 대 한 액세스를 캐시 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-406">Providers must often cache access to schema information.</span></span> <span data-ttu-id="c04e5-407">정적 매개 변수 또는 사용자 데이터로 지정 된 파일 이름을 사용 하 여 캐시 된 데이터를 저장 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-407">The cached data should be stored by using a file name that's given as a static parameter or as user data.</span></span> <span data-ttu-id="c04e5-408">스키마 캐싱의 예는 `LocalSchemaFile` 어셈블리의 형식 공급자에 있는 매개 변수입니다 `FSharp.Data.TypeProviders` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-408">An example of schema caching is the `LocalSchemaFile` parameter in the type providers in the `FSharp.Data.TypeProviders` assembly.</span></span> <span data-ttu-id="c04e5-409">이러한 공급자의 구현에서이 정적 매개 변수는 네트워크를 통해 데이터 원본에 액세스 하는 대신 지정 된 로컬 파일의 스키마 정보를 사용 하도록 형식 공급자에 게 지시 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-409">In the implementation of these providers, this static parameter directs the type provider to use the schema information in the specified local file instead of accessing the data source over the network.</span></span> <span data-ttu-id="c04e5-410">캐시 된 스키마 정보를 사용 하려면 정적 매개 변수도로 설정 해야 `ForceUpdate` 합니다 `false` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-410">To use cached schema information, you must also set the static parameter `ForceUpdate` to `false`.</span></span> <span data-ttu-id="c04e5-411">유사한 기술을 사용 하 여 온라인 및 오프 라인 데이터 액세스를 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-411">You could use a similar technique to enable online and offline data access.</span></span>

### <a name="backing-assembly"></a><span data-ttu-id="c04e5-412">어셈블리 지원</span><span class="sxs-lookup"><span data-stu-id="c04e5-412">Backing Assembly</span></span>

<span data-ttu-id="c04e5-413">또는 파일을 컴파일할 `.dll` 때 `.exe` 생성 된 형식에 대 한 지원 .dll 파일은 결과 어셈블리에 정적으로 링크 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-413">When you compile a `.dll` or `.exe` file, the backing .dll file for generated types is statically linked into the resulting assembly.</span></span> <span data-ttu-id="c04e5-414">이 링크는 IL (중간 언어) 형식 정의와 지원 어셈블리에서 관리 되는 모든 리소스를 최종 어셈블리에 복사 하 여 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-414">This link is created by copying the Intermediate Language (IL) type definitions and any managed resources from the backing assembly into the final assembly.</span></span> <span data-ttu-id="c04e5-415">F# 대화형 사용 하는 경우에는 지원 .dll 파일이 복사 되지 않고 대신 F# 대화형 프로세스로 직접 로드 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-415">When you use F# Interactive, the backing .dll file isn't copied and is instead loaded directly into the F# Interactive process.</span></span>

### <a name="exceptions-and-diagnostics-from-type-providers"></a><span data-ttu-id="c04e5-416">형식 공급자의 예외 및 진단</span><span class="sxs-lookup"><span data-stu-id="c04e5-416">Exceptions and Diagnostics from Type Providers</span></span>

<span data-ttu-id="c04e5-417">제공 된 형식의 모든 멤버를 사용 하면 예외가 throw 될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-417">All uses of all members from provided types may throw exceptions.</span></span> <span data-ttu-id="c04e5-418">모든 경우에서 형식 공급자가 예외를 throw 하는 경우 호스트 컴파일러는 오류를 특정 형식 공급자에 게 특성 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-418">In all cases, if a type provider throws an exception, the host compiler attributes the error to a specific type provider.</span></span>

- <span data-ttu-id="c04e5-419">형식 공급자 예외는 내부 컴파일러 오류가 발생 해서는 안 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-419">Type provider exceptions should never result in internal compiler errors.</span></span>

- <span data-ttu-id="c04e5-420">형식 공급자는 경고를 보고할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-420">Type providers can't report warnings.</span></span>

- <span data-ttu-id="c04e5-421">형식 공급자가 F # 컴파일러, F # 개발 환경 또는 F# 대화형에서 호스트 되는 경우 해당 공급자의 모든 예외가 catch 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-421">When a type provider is hosted in the F# compiler, an F# development environment, or F# Interactive, all exceptions from that provider are caught.</span></span> <span data-ttu-id="c04e5-422">메시지 속성은 항상 오류 텍스트 이며 스택 추적이 표시 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-422">The Message property is always the error text, and no stack trace appears.</span></span> <span data-ttu-id="c04e5-423">예외를 throw 할 경우 `System.NotSupportedException` , `System.IO.IOException` , `System.Exception` 와 같은 예를 throw 할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-423">If you’re going to throw an exception, you can throw the following examples: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span></span>

#### <a name="providing-generated-types"></a><span data-ttu-id="c04e5-424">생성 된 형식 제공</span><span class="sxs-lookup"><span data-stu-id="c04e5-424">Providing Generated Types</span></span>

<span data-ttu-id="c04e5-425">지금까지이 문서에서는 지워진 형식을 제공 하는 방법에 대해 설명 했습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-425">So far, this document has explained how to provide erased types.</span></span> <span data-ttu-id="c04e5-426">F #에서 형식 공급자 메커니즘을 사용 하 여 생성 된 형식을 제공할 수도 있습니다 .이 형식은 사용자의 프로그램에 실제 .NET 형식 정의로 추가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-426">You can also use the type provider mechanism in F# to provide generated types, which are added as real .NET type definitions into the users' program.</span></span> <span data-ttu-id="c04e5-427">형식 정의를 사용 하 여 생성 된 제공 형식을 참조 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-427">You must refer to generated provided types by using a type definition.</span></span>
...
<span data-ttu-id="c04e5-428">F # 3.0 릴리스에 포함 된 프로 비전 된 유형-0.2 도우미 코드는 생성 된 유형 제공을 제한적 으로만 지원 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-428">The ProvidedTypes-0.2 helper code that is part of the F# 3.0 release has only limited support for providing generated types.</span></span> <span data-ttu-id="c04e5-429">다음 문은 생성 된 형식 정의에 대해 true 여야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-429">The following statements must be true for a generated type definition:</span></span>

- <span data-ttu-id="c04e5-430">`isErased`은 `false`로 설정해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-430">`isErased` must be set to `false`.</span></span>

- <span data-ttu-id="c04e5-431">생성 된 형식은 `ProvidedAssembly()` 생성 된 코드 조각에 대 한 컨테이너를 나타내는 새로 생성 된에 추가 되어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-431">The generated type must be added to a newly constructed `ProvidedAssembly()`, which represents a container for generated code fragments.</span></span>

- <span data-ttu-id="c04e5-432">공급자에는 디스크에 일치 하는 .dll 파일과 함께 실제 지원 .NET .dll 파일이 있는 어셈블리가 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-432">The provider must have an assembly that has an actual backing .NET .dll file with a matching .dll file on disk.</span></span>

## <a name="rules-and-limitations"></a><span data-ttu-id="c04e5-433">규칙 및 제한 사항</span><span class="sxs-lookup"><span data-stu-id="c04e5-433">Rules and Limitations</span></span>

<span data-ttu-id="c04e5-434">형식 공급자를 작성 하는 경우에는 다음 규칙 및 제한 사항을 염두에 두어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-434">When you write type providers, keep the following rules and limitations in mind.</span></span>

### <a name="provided-types-must-be-reachable"></a><span data-ttu-id="c04e5-435">제공 된 형식에 연결할 수 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-435">Provided types must be reachable</span></span>

<span data-ttu-id="c04e5-436">제공 된 모든 형식은 중첩 되지 않은 형식에서 연결할 수 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-436">All provided types should be reachable from the non-nested types.</span></span> <span data-ttu-id="c04e5-437">중첩 되지 않은 형식은 생성자에 대 한 호출 `TypeProviderForNamespaces` 또는에 대 한 호출에서 제공 됩니다 `AddNamespace` .</span><span class="sxs-lookup"><span data-stu-id="c04e5-437">The non-nested types are given in the call to the `TypeProviderForNamespaces` constructor or a call to `AddNamespace`.</span></span> <span data-ttu-id="c04e5-438">예를 들어 공급자가 형식을 제공 하는 경우 `StaticClass.P : T` T가 중첩 되지 않은 형식 이거나 중첩 된 형식 인지 확인 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-438">For example, if the provider provides a type `StaticClass.P : T`, you must ensure that T is either a non-nested type or nested under one.</span></span>

<span data-ttu-id="c04e5-439">예를 들어, 일부 공급자는 `DataTypes` 이러한 형식을 포함 하는와 같은 정적 클래스를 포함 `T1, T2, T3, ...` 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-439">For example, some providers have a static class such as `DataTypes` that contain these `T1, T2, T3, ...` types.</span></span> <span data-ttu-id="c04e5-440">그렇지 않으면 어셈블리 A의 T 형식에 대 한 참조를 찾았지만 해당 어셈블리에서 형식을 찾을 수 없다는 오류가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-440">Otherwise, the error says that a reference to type T in assembly A was found, but the type couldn't be found in that assembly.</span></span> <span data-ttu-id="c04e5-441">이 오류가 표시 되 면 공급자 형식에서 모든 하위 형식에 연결할 수 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-441">If this error appears, verify that all your subtypes can be reached from the provider types.</span></span> <span data-ttu-id="c04e5-442">참고: 이러한 `T1, T2, T3...` 형식은 *즉석* 형식 이라고 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-442">Note: These `T1, T2, T3...` types are referred to as the *on-the-fly* types.</span></span> <span data-ttu-id="c04e5-443">이러한 항목은 액세스 가능한 네임 스페이스 또는 부모 형식에 저장 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-443">Remember to put them in an accessible namespace or a parent type.</span></span>

### <a name="limitations-of-the-type-provider-mechanism"></a><span data-ttu-id="c04e5-444">유형 공급자 메커니즘의 제한 사항</span><span class="sxs-lookup"><span data-stu-id="c04e5-444">Limitations of the Type Provider Mechanism</span></span>

<span data-ttu-id="c04e5-445">F #의 형식 공급자 메커니즘에는 다음과 같은 제한 사항이 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-445">The type provider mechanism in F# has the following limitations:</span></span>

- <span data-ttu-id="c04e5-446">F #에서 형식 공급자의 기본 인프라는 제공 된 제네릭 형식 또는 제공 된 제네릭 메서드를 지원 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-446">The underlying infrastructure for type providers in F# doesn't support provided generic types or provided generic methods.</span></span>

- <span data-ttu-id="c04e5-447">메커니즘은 정적 매개 변수를 포함 하는 중첩 형식을 지원 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-447">The mechanism doesn't support nested types with static parameters.</span></span>

## <a name="development-tips"></a><span data-ttu-id="c04e5-448">개발 팁</span><span class="sxs-lookup"><span data-stu-id="c04e5-448">Development Tips</span></span>

<span data-ttu-id="c04e5-449">개발 프로세스 중에 다음 팁을 유용 하 게 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-449">You might find the following tips helpful during the development process:</span></span>

### <a name="run-two-instances-of-visual-studio"></a><span data-ttu-id="c04e5-450">Visual Studio의 두 인스턴스를 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-450">Run two instances of Visual Studio</span></span>

<span data-ttu-id="c04e5-451">테스트 IDE는 형식 공급자가 다시 작성 되지 않도록 방지 하는 .dll 파일에 대 한 잠금을 사용 하기 때문에 한 인스턴스에서 형식 공급자를 개발 하 고 다른 공급자에서 공급자를 테스트할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-451">You can develop the type provider in one instance and test the provider in the other because the test IDE will take a lock on the .dll file that prevents the type provider from being rebuilt.</span></span> <span data-ttu-id="c04e5-452">따라서 공급자가 첫 번째 인스턴스에서 빌드되는 동안 Visual Studio의 두 번째 인스턴스를 닫은 다음 공급자가 작성 된 후에 두 번째 인스턴스를 다시 열어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-452">Thus, you must close the second instance of Visual Studio while the provider is built in the first instance, and then you must reopen the second instance after the provider is built.</span></span>

### <a name="debug-type-providers-by-using-invocations-of-fscexe"></a><span data-ttu-id="c04e5-453">fsc.exe 호출을 사용 하 여 형식 공급자 디버그</span><span class="sxs-lookup"><span data-stu-id="c04e5-453">Debug type providers by using invocations of fsc.exe</span></span>

<span data-ttu-id="c04e5-454">다음 도구를 사용 하 여 형식 공급자를 호출할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-454">You can invoke type providers by using the following tools:</span></span>

- <span data-ttu-id="c04e5-455">fsc.exe (F # 명령줄 컴파일러)</span><span class="sxs-lookup"><span data-stu-id="c04e5-455">fsc.exe (The F# command line compiler)</span></span>

- <span data-ttu-id="c04e5-456">fsi.exe (F# 대화형 컴파일러)</span><span class="sxs-lookup"><span data-stu-id="c04e5-456">fsi.exe (The F# Interactive compiler)</span></span>

- <span data-ttu-id="c04e5-457">devenv.exe (Visual Studio)</span><span class="sxs-lookup"><span data-stu-id="c04e5-457">devenv.exe (Visual Studio)</span></span>

<span data-ttu-id="c04e5-458">테스트 스크립트 파일 (예: .fsx)에서 fsc.exe를 사용 하면 형식 공급자를 가장 쉽게 디버그할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-458">You can often debug type providers most easily by using fsc.exe on a test script file (for example, script.fsx).</span></span> <span data-ttu-id="c04e5-459">명령 프롬프트에서 디버거를 시작할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-459">You can launch a debugger from a command prompt.</span></span>

```console
devenv /debugexe fsc.exe script.fsx
```

  <span data-ttu-id="c04e5-460">인쇄 stdout 로깅을 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c04e5-460">You can use print-to-stdout logging.</span></span>

## <a name="see-also"></a><span data-ttu-id="c04e5-461">추가 정보</span><span class="sxs-lookup"><span data-stu-id="c04e5-461">See also</span></span>

- [<span data-ttu-id="c04e5-462">형식 공급자</span><span class="sxs-lookup"><span data-stu-id="c04e5-462">Type Providers</span></span>](index.md)
- [<span data-ttu-id="c04e5-463">형식 공급자 SDK</span><span class="sxs-lookup"><span data-stu-id="c04e5-463">The Type Provider SDK</span></span>](https://github.com/fsprojects/FSharp.TypeProviders.SDK)
