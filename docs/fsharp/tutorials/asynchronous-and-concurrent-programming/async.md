...
1. @No__t_1를 사용 하 여 명령줄 인수를 `Async<unit>` 계산으로 변환 합니다.
2. @No__t_1 계산이 실행 될 때 병렬 실행을 예약 하 고 실행 하는 `Async<'T[]>`을 만듭니다.
3. 병렬 계산을 실행 하 고 결과를 무시 하는 `Async<unit>`을 만듭니다.
4. @No__t_0를 사용 하 여 마지막 계산을 명시적으로 실행 하 고 완료 될 때까지 차단 합니다.
...
@No__t_0은 이미 실행 중인 태스크가 아니라 작업 사양 이므로 더 복잡 한 변환을 쉽게 수행할 수 있습니다. 다음은 비동기 계산 집합을 시퀀스 하 여 한 번 실행 하는 예제입니다.
...
- @No__t_0를 사용 하 여 여러 계산을 시작 하는 것은 병렬로 일정을 예약 하는 것과 다릅니다. 계산을 병렬로 예약 하려는 경우 `Async.Parallel`를 사용 합니다.
...
병렬로 실행 되는 비동기 계산의 시퀀스를 예약 합니다. @No__t_0 매개 변수를 지정 하 여 병렬 처리 수준을 선택적으로 조정/제한 할 수 있습니다.
...
- @No__t_0 또는 `Async<unit>`를 필요로 하는 다른 함수를 사용 하려고 하므로이를 사용 해야 하는 경우 결과를 삭제 하는 것이 좋습니다. 형식 시그니처에 맞게 결과를 삭제 하는 것은 일반적으로 수행 되지 않아야 합니다.
...
- @No__t_0를 호출 하면 실행이 완료 될 때까지 호출 스레드가 차단 됩니다.

### <a name="asyncstart"></a>Async. 시작

@No__t_0를 반환 하는 스레드 풀에서 비동기 계산을 시작 합니다. 는 결과를 기다리지 않습니다. @No__t_0에서 시작 된 중첩 계산은 해당 계산을 호출한 부모 계산과 완전히 독립적으로 시작 됩니다. 수명은 부모 계산과 연결 되지 않습니다. 부모 계산이 취소 되 면 자식 계산이 취소 되지 않습니다.
...
- @No__t_0에서 시작 된 계산에 의해 발생 한 예외는 호출자에 게 전파 되지 않습니다. 호출 스택이 완전히 해제 됩니다.
- @No__t_1로 시작 되는 effectful 작업 (예: `printfn`)은 프로그램 실행의 주 스레드에서 효과가 발생 하지 않습니다.
...
@No__t_0를 사용 하는 .NET 비동기 라이브러리 및 코드 베이스 (즉, 반환 값이 있는 비동기 계산)는 간단 하며를 F#기본적으로 지원 합니다.

@No__t_0 함수를 사용 하 여 .NET 비동기 계산을 대기할 수 있습니다.
...
@No__t_0 함수를 사용 하 여 .NET 호출자에 비동기 계산을 전달할 수 있습니다.
...
@No__t_0 (즉, 값을 반환 하지 않는 .NET 비동기 계산)를 사용 하는 Api를 사용 하려면 `Async<'T>`를 <xref:System.Threading.Tasks.Task> 변환 하는 추가 함수를 추가 해야 할 수 있습니다.
...
@No__t_1를 입력으로 허용 하는 `Async.AwaitTask` 이미 있습니다. 이 함수와 이전에 정의 된 `startTaskFromAsyncUnit` 함수를 사용 하 여 F# 비동기 계산에서 <xref:System.Threading.Tasks.Task> 형식을 시작 하 고 기다릴 수 있습니다.
...
- 및 F#의 C# [Async: C# @No__t_1 비동기 알려진 문제
