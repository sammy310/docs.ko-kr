...
이 항목을 검토 하 여 확인할 수 있습니다 F# 컬렉션 형식 최상의 특정 요구에 적합 합니다. 이러한 컬렉션 형식에서 같은.NET framework에서 컬렉션 형식에서 다른 합니다 `System.Collections.Generic` 네임 스페이스에는 F# 개체 지향 보다는 기능을 프로그래밍 관점에서 설계 된 컬렉션 형식 큐브 뷰입니다. 보다 구체적으로, 배열 컬렉션만 요소가 변경할 수 있습니다. 따라서 컬렉션을 수정 하면 원래 컬렉션을 변경 하는 대신 수정 된 컬렉션의 인스턴스를 만듭니다.
...
|[Map](https://msdn.microsoft.com/library/975316ea-55e3-4987-9994-90897ad45664)|요소는 변경할 수 없는 사전입니다. 키로는 요소에 대 한 액세스입니다.|[Map 모듈](https://msdn.microsoft.com/library/bfe61ead-f16c-416f-af98-56dbcbe23e4f)|
|[Set](https://msdn.microsoft.com/library/50cebdce-0cd7-4c5c-8ebc-f3a9e90b38d8)|비교는 이진 트리를 기반으로 하는 변경할 수 없는 집합을는 F# 잠재적으로의 구현을 사용 하는 구조적 비교 함수는 `System.IComparable` 키 값에 대 한 인터페이스입니다.|[모듈 설정](https://msdn.microsoft.com/library/61efa732-d55d-4c32-993f-628e2f98e6a0)|

### <a name="table-of-functions"></a>함수는 테이블

이 섹션에서는 비교 함수에서 사용할 수 있는 F# 컬렉션 형식입니다. 여기서 N은 첫 번째 컬렉션의 크기 및 M 크기가 두 번째 컬렉션의 경우 모든 함수의 계산 복잡성이 제공 됩니다. 대시 (-)는이 함수는 컬렉션에 사용할 수 없습니다 나타냅니다. 시퀀스 지연 평가 되므로 Seq.distinct 등의 함수 될 수 O(1) 즉시 반환 하기 때문에 있지만 시퀀스를 열거 하는 경우의 성능에 영향을 계속 합니다.

|함수|배열|목록|Sequence|맵|Set|설명|
|--------|-----|----|--------|---|---|-----------|
|append|O(M)|O (N)|O (N)|-|-|두 번째 컬렉션의 요소 뒤에 첫 번째 컬렉션의 요소를 포함 하는 새 컬렉션을 반환 합니다.|
|add|-|-|-|O (로그 N)|O (로그 N)|추가 된 요소를 사용 하 여 새 컬렉션을 반환 합니다.|
