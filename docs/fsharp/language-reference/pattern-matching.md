...
패턴은 입력 데이터를 변환 하는 규칙입니다. 이는 데이터를 논리적 F# 구조와 비교 하거나 데이터를 구성 부분으로 분해 하거나 다양 한 방법으로 데이터에서 정보를 추출 하기 위해 언어 전체에서 사용 됩니다.

## <a name="remarks"></a>주의

패턴은 `match` 식과 같은 다양 한 언어 구문에서 사용 됩니다. `let` 바인딩, 람다 식 및 `try...with` 식과 연결 된 예외 처리기에서 함수에 대 한 인수를 처리할 때 사용 됩니다. 자세한 내용은 [일치 식](match-expressions.md), [let 바인딩](./functions/let-bindings.md), [람다 식: `fun` 키워드](./functions/lambda-expressions-the-fun-keyword.md)및 [예외: `try...with` 식](./exception-handling/the-try-with-expression.md)을 참조 하세요.
...
각 패턴은 특정 방식으로 입력을 변환 하는 규칙 역할을 합니다. `match` 식에서 각 패턴을 검사 하 여 입력 데이터가 패턴과 호환 되는지 확인 합니다. 일치 하는 항목이 있으면 결과 식이 실행 됩니다. 일치 항목을 찾을 수 없는 경우 다음 패턴 규칙을 테스트 합니다. 선택적 when *조건* 부분은 [일치 식](match-expressions.md)에 설명 되어 있습니다.
...
상수 패턴은 숫자, 문자 및 문자열 리터럴, 열거형 상수 (열거형 형식 이름 포함)입니다. 상수 패턴만 있는 `match` 식을 다른 언어의 case 문과 비교할 수 있습니다. 입력을 리터럴 값과 비교 하 고 값이 같으면 패턴이 일치 합니다. 리터럴의 형식은 입력의 형식과 호환 되어야 합니다.
...
`option` 형식은 `Some` 및 `None`의 두 가지 사례가 있는 구별 된 공용 구조체입니다. 한 case (`Some`)에는 값이 있지만 다른 하나 (`None`)는 명명 된 경우 일 뿐입니다. 따라서 `Some`는 `Some` 사례와 연결 된 값에 대 한 변수를 포함 해야 하지만 `None`은 단독으로 나타나야 합니다. 다음 코드에서 `var1` 변수에는 `Some` 대/소문자와 일치 하 여 얻은 값이 제공 됩니다.

[!code-fsharp[Main](~/samples/snippets/fsharp/lang-ref-2/snippet4803.fs)]

다음 예제에서 구별 된 공용 구조체 `PersonName`에는 가능한 형식의 이름을 나타내는 문자열과 문자의 혼합이 포함 되어 있습니다. 구분 된 공용 구조체의 사례는 `FirstOnly`, `LastOnly`및 `FirstLast`입니다.
...
명명 된 필드의 사용은 선택 사항 이므로 앞의 예제에서 `Circle(r)`와 `Circle(radius = r)`는 모두 동일한 결과를 가집니다.
...
`as` 패턴은 `as` 절이 추가 된 패턴입니다. `as` 절은 일치 하는 값을 `match` 된 식의 실행 식에 사용할 수 있는 이름에 바인딩하거나,이 패턴이 `let` 바인딩에서 사용 되는 경우에는 이름이 로컬 범위에 바인딩으로 추가 됩니다.

다음 예제에서는 `as` 패턴을 사용 합니다.
...
다음 예제는이 항목의 뒷부분에 나오는 [튜플 패턴](https://msdn.microsoft.com/library/#tuple) 섹션에 표시 된 `detectZeroTuple`와 유사 하지만 `var1` 및 `var2`는 및 패턴을 사용 하 여 값으로 가져옵니다.
...
와일드 카드 패턴은 밑줄 (`_`) 문자로 표시 되 고 변수 패턴과 마찬가지로 입력은 변수에 할당 되지 않고 무시 된다는 점을 제외 하 고 모든 입력과 일치 합니다. 와일드 카드 패턴은 `->` 기호 오른쪽의 식에 필요 하지 않은 값에 대 한 자리 표시자로 다른 패턴 내에서 자주 사용 됩니다. 또한 와일드 카드 패턴은 일치 하지 않는 입력을 일치 시키기 위해 패턴 목록의 끝에 자주 사용 됩니다. 와일드 카드 패턴은이 항목의 여러 코드 예제에서 보여 줍니다. 한 가지 예는 위의 코드를 참조 하세요.
...
식별자가 특정 파생 형식 인지만 확인 하는 경우 다음 예제와 같이 패턴의 `as identifier` 부분이 필요 하지 않습니다.
...
Null 패턴은 null 값을 허용 하는 형식으로 작업할 때 나타날 수 있는 null 값과 일치 합니다. Null 패턴은 .NET Framework 코드와 상호 작용할 때 자주 사용 됩니다. 예를 들어 .NET API의 반환 값은 `match` 식에 대 한 입력이 될 수 있습니다. 반환 값이 null 인지 여부 및 반환 된 값의 다른 특성에 따라 프로그램 흐름을 제어할 수 있습니다. Null 패턴을 사용 하 여 null 값이 프로그램의 나머지 부분에 전파 되지 않도록 할 수 있습니다.
