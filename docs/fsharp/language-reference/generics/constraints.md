...
|제약 조건|구문|설명|
|----------|------|-----------|
|형식 제약 조건|*type-parameter* :&gt; *type*|제공된 된 형식 같거나 파생, 지정 된 형식에서 이거나, 형식이 인터페이스 이면 제공 된 형식이 인터페이스를 구현 해야 합니다.|
|Null 제약 조건|*type-parameter* : null|제공된 된 형식에는 null 리터럴을 지원 해야 합니다. 모든.NET 개체 유형이 여기에 있지만 F# 목록, 튜플, 함수, 클래스, 레코드 또는 공용 구조체 형식입니다.|
...
|생성자 제약 조건|*type-parameter* : ( new : unit -&gt; 'a )|제공된 된 형식 기본 생성자가 있어야 합니다.|
|값 형식 제약 조건|: 구조체|제공된 된 형식에는.NET 값 형식 이어야 합니다.|
|참조 형식 제약 조건|: 하지 구조체|제공된 된 형식에는.NET 참조 형식 이어야 합니다.|
|열거형 형식 제약 조건|: enum&lt;*underlying-type*&gt;|제공된 된 형식에 지정된 된 기본 형식을 하는 열거 형식 이어야 합니다. 일반적인 용도로 적합 하지 합니다.|
|대리자 제약 조건|: delegate&lt;*tuple-parameter-type*, *return-type*&gt;|제공된 된 형식 인수가 지정 하는 대리자 형식 이어야 하며 값을 반환 합니다. 일반적인 용도로 적합 하지 합니다.|
...
사용 하는 가장 일반적인 제약 조건을 F# 코드는 기본 클래스 또는 인터페이스를 지정 하는 형식 제약 조건입니다. 다른 제약 조건에서 사용 하는 중 하나는 F# 산술 연산자의 오버 로드 된 연산자를 구현 하는 데 사용 됩니다 또는 기능이 있기 때문에 제공 되는 명시적 구성원 제약 조건 등의 특정 기능을 구현 하는 라이브러리 F# 공용 언어 런타임에서 지원 되는 제약 조건의 전체 집합을 지원 합니다.
