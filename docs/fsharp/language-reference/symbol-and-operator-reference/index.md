...
다음 표에서는 F# 언어에서 사용되는 기호에 대해 설명하고, 추가 정보를 제공하는 일부 기호와 링크의 사용 방식을 간략하게 설명합니다. 기호는 ASCII 문자 집합 순서에 따라 정렬되어 있습니다.
...
|`!=`||<ul><li>F#에서 사용되지 않습니다. 다름 연산에서는 `<>`를 사용합니다.<br /></li></ul>|
...
|`$`||<ul><li>특정 컴파일러 생성 변수 및 함수 이름에 내부적으로 사용됩니다.<br /></li></ul>|
...
|`%?`|[null 허용 연산자](nullable-operators.md)|<ul><li>우변이 nullable 형식이면 정수 나머지를 계산합니다.<br /></li></ul>|
...
|<code>&#96;&#96;...&#96;&#96;</code>||<ul><li>일반적으로는 유효한 식별자가 아닌 언어 키워드 등의 식별자를 구분합니다.<br /></li></ul>|
...
|`.`|[멤버](../members/index.md)<br /><br />[기본 유형](../basic-types.md)|<ul><li>멤버에 액세스하고 개별 이름을 정규화된 이름으로 구분합니다.<br /></li><li>부동 소수점 수로 소수점을 지정합니다.<br /></li></ul>|
...
|`:?`|[일치 식](../match-expressions.md)|<ul><li>값이 지정한 형식(하위 형식인 경우 포함)과 일치하면 `true`를 반환하고 그렇지 않으면 `false`를 반환합니다(형식 테스트 연산자).<br /></li></ul>|
...
|`==`||<ul><li>F#에서 사용되지 않습니다. 같음 연산에는 `=`를 사용합니다.<br /></li></ul>|
...
|`>=`|[산술 연산자](arithmetic-operators.md)|<ul><li>좌변이 우변 이상이면 `true`를 반환하고 작으면 `false`를 반환합니다.<br /></li></ul>|
...
|`? ... <- ...`||<ul><li>동적 속성 설정 위한 연산자로 사용됩니다. 고유한 구현을 제공해야 합니다.<br /></li></ul>|
...
|`&`, `&&`|왼쪽|
|`:>`, `:?>`|오른쪽|
|`<`‘연산자’, `>`‘연산자’, `=`, <code>&#124;</code>‘연산자’, `&`‘연산자’, `&`   <br /><br />(`<<<`, `>>>`, <code>&#124;&#124;&#124;</code>, `&&&` 포함)|왼쪽|
|`^`*연산자*<br /><br />(`^^^` 포함)|오른쪽|
|`::`|오른쪽|
|`:?`|결합성 없음|
