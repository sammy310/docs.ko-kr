...
아래 예제에서는 공유에 대한 현재 사용량을 확인하고 공유에 대해 할당량을 설정하는 방법을 보여 줍니다. 공유의 `Properties`을 채우기 위해 `FetchAttributes`를 호출 하 고, 로컬 변경 내용을 Azure File storage에 전파 하려면 `SetProperties` 합니다.
...
- [Linux에서 Azure File Storage 사용 방법](/azure/storage/storage-how-to-use-files-linux)
