...
> [!NOTE]
> 이 자습서에서는 ML.NET 이미지 분류 API의 미리 보기 버전을 사용합니다.
...
1. **Microsoft.ML 1.4.0-preview2** NuGet 패키지를 설치합니다.
    ...
    1. **Microsoft.ML.Dnn 0.16.0-preview2** 및 **Microsoft.ML.ImageAnalytics 1.4.0-preview2**에 대해 이 단계를 반복합니다.
...
1. `mapLabelEstimator` 및 `ImageClassification` 변환으로 구성된 학습 [`EstimatorChain`](xref:Microsoft.ML.Data.EstimatorChain%601) 파이프라인을 정의합니다.
    ...
    `ImageClassification` 예측은 여러 가지 매개 변수를 사용합니다.

    - `featuresColumnName`은 모델의 입력으로 사용되는 열입니다.
    - `labelColumnName`은 예측할 값의 열입니다.
    - `arch`는 사용할 사전 학습 모델 아키텍처를 정의합니다. 이 자습서에서는 ResNetv2 모델의 101 계층 변형을 사용합니다.
    - `epoch`는 학습 프로세스 전반에 걸쳐 전체 데이터 세트의 최대 반복 횟수를 지정합니다. 숫자가 클수록 모델의 학습 기간이 길어지고 생성되는 모델이 향상됩니다.
    - `batchSize`는 학습을 위해 한 번에 사용할 샘플 수입니다. 단일 Epoch에서 batchSize와 동일한 여러 개의 일괄 처리가 모델을 학습하고 업데이트하는 데 사용됩니다. 숫자가 작을수록 각 일괄 처리에 필요한 메모리가 줄어듭니다.
    ...
    - `disableEarlyStopping`은 ImageClassification 예측에 조기 중지 전략을 사용할지 여부를 지시합니다. 학습 중에 모델이 정확한 예측에 도움이 되는 최적 값을 검색함에 따라 성능이 향상되거나 감소할 수 있습니다. 궁극적으로, 모델이 마지막 Epoch에 도달할 때 학습에서 배운 패턴이 최적이 아닌 경우도 있습니다. 조기 중지는 학습에서 이러한 성능 저하를 모니터링하고 학습 프로세스를 중지하여 모델의 최적 버전을 유지합니다.
