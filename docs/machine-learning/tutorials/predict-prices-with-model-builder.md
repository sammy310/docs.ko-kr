...
1. 솔루션 탐색기에서 *TaxiFarePrediction* 프로젝트를 마우스 오른쪽 단추로 클릭합니다. 그런 다음, 선택 **추가 > 기존 항목**을 선택합니다. 파일 유형 드롭다운의 경우 `All Files`를 선택하고 *TaxiFarePredictionML.Model* 프로젝트 디렉터리로 이동하여 `MLModel.zip` 파일을 선택합니다. 그런 다음, 최근에 추가한 `MLModel.zip` 파일을 마우스 오른쪽 단추로 클릭하고 *속성*을 선택합니다. 출력 디렉터리에 복사 옵션의 경우 드롭다운에서 *변경된 내용만 복사*를 선택합니다.
