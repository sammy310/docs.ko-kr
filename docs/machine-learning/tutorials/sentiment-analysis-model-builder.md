...
1. **예측할 레이블 또는 열** 드롭다운에서 **감정**을 선택합니다.
...
1. 모델 작성기 도구의 *코드* 단계에서 **프로젝트 추가**를 선택하여 자동 생성된 프로젝트를 솔루션에 추가합니다.
    ...
    **솔루션 탐색기**에 다음 프로젝트가 표시될 것입니다.
    ...
    - *SentimentRazorML.Model*: 입력 및 출력 모델 데이터의 스키마를 정의하는 데이터 모델뿐만 아니라 학습 중 가장 성능이 뛰어난 모델의 지속 버전을 포함하는 .NET Standard 클래스 라이브러리입니다.
...
1. 학습된 모델을 Razor Pages 애플리케이션에서 사용하려면 *SentimentRazorML.Model* 프로젝트에 대한 참조를 추가합니다.

    1. **SentimentRazor** 프로젝트를 마우스 오른쪽 단추로 클릭합니다. 
    1. **추가 > 참조**를 선택합니다. 
    1. **프로젝트 > 솔루션** 노드를 선택하고 목록에서 **SentimentRazorML.Model** 프로젝트를 선택합니다.
    1. **확인**을 선택합니다.
...
> [!WARNING]
> [`PredictionEngine`](xref:Microsoft.ML.PredictionEngine%602)은 스레드로부터 안전하지 않습니다. 성능과 스레드 안전 향상을 위해 애플리케이션용으로 `PredictionEngine` 개체의 [`ObjectPool`](xref:Microsoft.Extensions.ObjectPool.ObjectPool%601)을 만드는 `PredictionEnginePool` 서비스를 사용합니다. [ASP.NET Core에서 `PredictionEngine`개체 풀 만들기 및 사용](https://devblogs.microsoft.com/cesardelatorre/how-to-optimize-and-run-ml-net-models-on-scalable-asp-net-core-webapis-or-web-apps/)에 대한 자세한 정보는 다음 블로그 게시물을 참조하세요. 
