...
`Test` 메서드에 `Animal` 매개 변수가 있으므로 인수 `a`를 `Reptile`로 명시적으로 캐스팅하면 위험한 가정이 생성됩니다. 가정을 생성하지 않고 형식을 확인하는 것이 더 안전합니다. C#에서는 실제로 캐스트를 수행하기 전에 호환성을 테스트할 수 있도록 [is](../../language-reference/operators/type-testing-and-cast.md#is-operator) 연산자를 제공합니다. 자세한 내용은 [패턴 일치, as 및 is 연산자를 사용하여 안전하게 캐스트하는 방법](../../how-to/safely-cast-using-pattern-matching-is-and-as-operators.md)을 참조하세요.
...
- [일반화된 형식 변환](/previous-versions/visualstudio/visual-studio-2013/yy580hbd(v=vs.120))
