...
## <a name="passing-value-types-by-reference"></a>참조로 값 형식 전달  
 ...
 이 예제에서는 `n`의 값이 아니라 `n`에 대한 참조가 전달됩니다. `x` 매개 변수는 [int](../../../csharp/language-reference/builtin-types/integral-numeric-types.md)가 아니며 `int`에 대한 참조(이 예제에서는 `n`에 대한 참조)입니다. 그러므로 메서드 내에서 `x`를 제곱할 때 실제로는 `x`가 참조하는 `n`을 제곱하게 됩니다.  
