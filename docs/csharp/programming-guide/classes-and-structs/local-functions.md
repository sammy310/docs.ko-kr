...
다음 예제에서는 `AppendPathSeparator` 메서드에 대해 private인 로컬 함수 `GetText`를 정의합니다.
...
다음 예제에서는 `GetMultipleAsync`라는 비동기 메서드를 사용하여 지정된 시간(초) 동안 일시 중지하고 해당 시간(초)의 임의 배수인 값을 반환합니다. 최대 지연 시간은 5초입니다. 값이 5보다 크면 <xref:System.ArgumentOutOfRangeException>이 발생합니다. 다음 예제와 같이 값 6이 `GetMultipleAsync` 메서드에 전달될 때 throw되는 예외는 <xref:System.AggregateException> 메서드 실행이 시작된 후에 `GetMultipleAsync`에 래핑됩니다.
