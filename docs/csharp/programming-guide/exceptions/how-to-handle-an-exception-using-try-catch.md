...
## <a name="example"></a>예제  
 이 예제에서 <xref:System.IndexOutOfRangeException>은 가장 적합한 예외가 아닙니다. 호출자가 전달한 <xref:System.ArgumentOutOfRangeException> 인수로 인해 오류가 발생하기 때문에 `index`이 메서드에 더 적합합니다.  
  
 [!code-csharp[csProgGuideExceptions#5](~/samples/snippets/csharp/VS_Snippets_VBCSharp/csProgGuideExceptions/CS/Exceptions.cs#5)]  
  
## <a name="comments"></a>설명  
 예외가 발생하는 코드는 `try` 블록으로 묶여 있습니다. `catch`이 발생할 경우 처리하기 위해 `IndexOutOfRangeException` 문이 바로 뒤에 추가됩니다. `catch` 블록은 `IndexOutOfRangeException`을 처리하고 더 적합한 `ArgumentOutOfRangeException` 예외를 대신 throw합니다. 호출자에게 최대한 많은 정보를 제공하기 위해 원래 예외를 새 예외의 <xref:System.Exception.InnerException%2A>으로 지정하는 것이 좋습니다. <xref:System.Exception.InnerException%2A> 속성은 [readonly](../../language-reference/keywords/readonly.md)이기 때문에 새 예외의 생성자에 할당해야 합니다.  
