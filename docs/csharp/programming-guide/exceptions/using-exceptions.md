...
`try` 문은 `catch` 블록을 둘 이상 포함할 수 있습니다. 예외를 처리할 수 있는 첫 번째 `catch` 문이 실행됩니다. 그 뒤의 `catch` 문은 호환되더라도 무시됩니다. catch 블록은 항상 가장 구체적인 것(또는 가장 많이 파생된 것)에서 가장 덜 구체적인 것 순으로 정렬해야 합니다. 예를 들어:
