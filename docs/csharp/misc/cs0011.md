 ...
 **/reference**를 사용하여 파일에서 가져온 클래스가 클래스에서 파생되거나 찾을 수 없는 인터페이스를 구현합니다. **/reference**를 사용한 컴파일에 필요한 DLL이 포함되지 않은 경우에도 발생할 수 있습니다.  
 ...
 자세한 내용은 [참조 대화 상자 및](/visualstudio/ide/how-to-add-or-remove-references-by-using-the-reference-manager) [-참조(C# 컴파일러 옵션)를](../language-reference/compiler-options/reference-compiler-option.md)참조하십시오.  
...
- [-참조(C# 컴파일러 옵션)](../language-reference/compiler-options/reference-compiler-option.md)
