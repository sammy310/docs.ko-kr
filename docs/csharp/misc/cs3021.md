...
어셈블리에 CLSCompliant 특성이 없으므로 'type'에 CLSCompliant 특성이 필요하지 않습니다.  
 ...
 이 경고는 `[CLSCompliant(false)]` 어셈블리 수준의 CLSCompliant 특성이 true (즉, 줄)로 설정 되지 않은 어셈블리의 클래스에 표시 되는 경우에 발생 `[assembly: CLSCompliant(true)]` 합니다. 어셈블리가 자체적으로 CLS 규격으로 선언하지 않아 비규격으로 간주되므로 어셈블리 내에서 별도로 비규격으로 선언할 필요가 없습니다. CLS 규격에 대 한 자세한 내용은 [언어 독립성 및 언어 독립적 구성 요소](../../standard/language-independence.md)를 참조 하세요.
