...
‘T’: Using 문에 사용된 형식은 암시적으로 ‘System.IDisposable’로 변환 가능해야 합니다.  
 ...
 [Using 문](../keywords/using-statement.md)은 `using` 블록의 끝에서 개체를 삭제하는 데 사용하기 위한 용도이므로 삭제 가능한 형식만 이러한 문에 사용할 수 있습니다. 예를 들어, 값 형식은 삭제 가능하지 않으며 클래스로 제한되지 않는 형식 매개 변수는 삭제 가능한 것으로 간주되지 않을 수 있습니다.  
