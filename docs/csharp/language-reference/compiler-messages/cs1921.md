...
‘method’에 가장 일치하는 오버로드된 메서드에 이니셜라이저 요소의 잘못된 시그니처가 있습니다. 초기화 가능한 Add는 액세스 가능한 인스턴스 메서드여야 합니다.  
 ...
 이 오류는 공용 비정적 `Add` 메서드가 없는 클래스와 함께 컬렉션 이니셜라이저를 사용하려고 할 때 발생합니다. 보호 수준(`Add`, `private`, `protected`) 때문에 `internal` 메서드에 액세스할 수 없는 경우에는 CS0122 오류가 발생합니다. 따라서 이 오류는 메서드가 `static`으로 정의되어 있음을 의미할 수 있습니다.  
