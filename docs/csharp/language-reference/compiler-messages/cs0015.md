---
title: 컴파일러 오류 CS0015
ms.date: 07/20/2015
f1_keywords:
- CS0015
helpviewer_keywords:
- CS0015
ms.assetid: e9d3272b-2b80-435a-98ef-79bff158ae7b
ms.openlocfilehash: c5e08b57ba97df2a723ca020dd9fa5ee44b4fb27
ms.sourcegitcommit: 7588136e355e10cbc2582f389c90c127363c02a5
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 03/14/2020
ms.locfileid: "71332100"
---
# <a name="compiler-error-cs0015"></a><span data-ttu-id="63d50-102">컴파일러 오류 CS0015</span><span class="sxs-lookup"><span data-stu-id="63d50-102">Compiler Error CS0015</span></span>

<span data-ttu-id="63d50-103">'type' 형식의 이름이 너무 깁니다.</span><span class="sxs-lookup"><span data-stu-id="63d50-103">The name of type 'type' is too long</span></span>

 <span data-ttu-id="63d50-104">사용자 정의 형식의 정규화된 이름은 마침표를 포함하여 1024자 미만이어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="63d50-104">The fully qualified name of a user-defined type must have fewer than 1024 characters, including the periods.</span></span>

 <span data-ttu-id="63d50-105">다음 샘플에서는 CS0015를 생성합니다.</span><span class="sxs-lookup"><span data-stu-id="63d50-105">The following sample generates CS0015:</span></span>

```csharp
// CS0015.cs

// Remove a C from one of the namespace names or the class name to resolve the error by
// reducing the number of characters in the qualified class name to fewer than 1024.

namespace CC
{
    // The following namespace name has 510 characters.
    namespace 
    {
        // The following class name has 510 characters. The qualified class name,
        // CC.CCC510chars…C.CCCC510chars…C (namespace.namespace.class), has 1024
        // characters, which causes compiler error CS0015.
        public class 
        {
            public static void Main()
            {
            }
        }
    }
}
```
