...
CS0173을 해결하려면 변환 방향과 관계없이, 그리고 변환이 있는 클래스와 관계없이 `class1` 및 `class2` 간에 하나의 암시적 변환만 있는지 확인합니다. 자세한 내용은 [암시적 숫자 변환 표](../keywords/implicit-numeric-conversions-table.md) 및 [사용자 정의 변환 연산자](../operators/user-defined-conversion-operators.md)를 참조하세요.
