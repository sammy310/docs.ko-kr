...
`uint`에서 [long](long.md), [ulong](ulong.md), [float](float.md), [double](double.md) 또는 [decimal](decimal.md)로 미리 정의된 암시적 변환이 있습니다. 예:

```csharp
float myFloat = 4294967290;   // OK: implicit conversion to float
```

[byte](byte.md), [ushort](ushort.md) 또는 [char](char.md)에서 `uint`로 미리 정의된 암시적 변환이 있습니다. 그렇지 않으면 캐스트를 사용해야 합니다. 예를 들어 다음 대입문은 캐스트 없이 컴파일 오류를 생성합니다.
