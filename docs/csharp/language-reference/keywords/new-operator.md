...
`new` 연산자는 값 형식에 대한 매개 변수 없는 생성자를 호출하는 데도 사용됩니다. 예:
...
모든 값 형식에 암시적으로 공용 매개 변수 없는 생성자가 있기 때문에 [struct](struct.md)에 대한 매개 변수 없는 생성자를 선언하면 오류가 발생합니다. 구조체 형식에서 매개 변수가 있는 생성자를 선언하여 해당 초기 값을 설정할 수 있지만, 이 작업은 기본값이 아닌 값이 필요한 경우에만 요구됩니다.
