...
처리된 예외 내에서는 연결된 `finally` 블록이 항상 실행됩니다. 그러나 예외가 처리되지 않은 경우 `finally` 블록의 실행 여부는 예외 해제 작업의 트리거 방법에 따라 달라집니다. 트리거 방법은 다시 사용자 컴퓨터의 설정 방법에 따라 달라집니다. `finally` 절이 실행되지 않는 유일한 경우는 프로그램이 즉시 중지될 때입니다. 이러한 예로는 IL 문이 손상되어 <xref:System.InvalidProgramException>이 throw되는 경우를 들 수 있습니다. 대부분의 운영 체제에서 적절한 리소스 정리는 프로세스를 중지하고 언로드하는 과정에서 발생합니다.
