...
- <xref:System.Reflection.MetadataLoadContext> 클래스를 사용하여 검사할 어셈블리만 로드할 수 있습니다.
...
어셈블리는 콘텐츠, 버전 관리 및 종속성에 대한 정보를 포함하므로 이를 사용하는 애플리케이션은 제대로 작동하기 위해 Windows 레지스트리 값을 사용할 필요가 없습니다. 어셈블리는 .dll 충돌을 줄이고 더 안정적이고 배포하기 쉬운 애플리케이션을 구현합니다. 많은 경우에 해당 파일을 대상 컴퓨터에 복사하는 것만으로 .NET 기반 애플리케이션을 설치할 수 있습니다. 자세한 내용은 [어셈블리 매니페스트](../../framework/app-domains/assembly-manifest.md)를 참조하세요.
