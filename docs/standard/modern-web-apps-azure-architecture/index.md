...
.NET core와 ASP.NET Core는 기존의.NET 개발 방식에 비해 여러 가지 이점을 제공합니다. 다음 항목 중 일부 또는 전부가 애플리케이션의 성공에 중요한 요소인 경우 서버 애플리케이션에 .NET Core를 사용해야 합니다.
...
Azure에서 호스팅되는 ASP.NET Core를 사용하여 웹 애플리케이션을 만들면 기존 방법보다 더 많은 경쟁 우위의 방법을 제공할 수 있습니다. ASP.NET Core는 최신 웹 애플리케이션 개발 방법과 클라우드 호스팅 시나리오에 최적화되어 있습니다. 이 가이드에서는 이러한 기능을 최대한 활용할 수 있도록 ASP.NET Core 애플리케이션을 설계하는 방법을 알아봅니다.

## <a name="purpose"></a>용도

이 가이드에서는 ASP.NET Core와 Azure를 사용하는 *모놀리식* 웹 애플리케이션을 만드는 방법에 대한 포괄적인 지침을 제공합니다. 여기서 "모놀리식"이란 상호 작용하는 여러 서비스와 애플리케이션의 컬렉션이 아니라, 하나의 단위로 배포되는 애플리케이션을 가리킵니다.

이 가이드는 엔터프라이즈 애플리케이션을 호스팅하는 Docker, 마이크로 서비스 및 컨테이너의 배포에 중점을 둔 [".NET 마이크로 서비스. 컨테이너화된 .NET 애플리케이션을 위한 아키텍처"](../microservices-architecture/index.md)를 보완합니다. 
...
이 가이드의 주요 대상은 클라우드에서 Microsoft 기술과 서비스를 사용하여 최신 웹 애플리케이션을 구축하는 데 관심 있는 개발자와 개발 책임자, 설계자입니다.

그 다음 대상자는 이미 ASP.NET과 Azure에 익숙하고 신규 또는 기존 프로젝트를 ASP.NET Core로 업그레이드하는 것이 적절한지에 관한 정보를 찾고 있는 기술 의사 결정권자입니다.

## <a name="how-you-can-use-this-guide"></a>이 가이드를 사용하는 방법

이 가이드는 최신 .NET 기술과 Windows Azure로 웹 애플리케이션을 구축하는 방법에 중점을 둔, 비교적 적은 양의 요약 문서입니다. 따라서 전제적으로 읽어보면 이러한 애플리케이션과 기술적인 고려 사항들을 이해하기 위한 기반을 마련할 수 있습니다. 이 가이드와 애플리케이션 예제는 출발점이나 참고 자료로 사용할 수도 할 수 있습니다. 관련 애플리케이션 예제를 자신의 애플리케이션 템플릿으로 사용하거나 애플리케이션의 구성 요소를 어떻게 구성하는지 알아보는 용도로 사용하세요. 자신의 애플리케이션에서 사용할 옵션을 가늠할 때는 가이드의 원칙과 아키텍처 및 기술 옵션, 의사 결정을 위한 고려 사항을 참조하세요.

이 가이드를 팀에서 자유롭게 공유하고 이러한 여러가지 고려 사항과 기회에 대한 이해를 도모하세요. 모든 사람이 공통적인 용어와 기본 원칙으로 작업하도록 하면 아키텍처 패턴및 사례를 일관성 있게 적용하는데 도움이 됩니다.
