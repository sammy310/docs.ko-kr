...
.NET core 및 ASP.NET Core는 기존의.NET 개발에 비해 여러 가지 이점을 제공합니다. 다음 중 일부 또는 전부가 애플리케이션의 성공에 중요한 경우 서버 애플리케이션에 .NET Core를 사용해야 합니다.
...
기존 .NET 애플리케이션은 이러한 많은 요구 사항을 지원할 수 있지만 ASP.NET Core 및 .NET Core는 위의 시나리오에 대한 향상된 지원을 제공하도록 최적화되었습니다.

점점 더 많은 조직에서 Microsoft Azure 같은 서비스를 사용하는 클라우드에 웹 애플리케이션을 호스팅하기로 선택하고 있습니다. 애플리케이션 또는 조직에 다음 사항이 중요한 경우 클라우드에서 애플리케이션을 호스팅하는 것이 좋습니다.
...
이 가이드에서는 ASP.NET Core 및 Azure를 사용하는 *모놀리식* 웹 애플리케이션을 빌드하는 방법에 대한 포괄적인 지침을 제공합니다. 이 컨텍스트에서 "모놀리식"은 이러한 애플리케이션이 상호 작용하는 서비스와 애플리케이션의 컬렉션이 아니라 하나의 단위로 배포되는 사실을 가리킵니다.

이 가이드는 ["_.NET 마이크로 서비스를 보완합니다. 컨테이너화된 .NET 애플리케이션_"](../microservices-architecture/index.md)의 아키텍처는 엔터프라이즈 애플리케이션을 호스팅하는 Docker, 마이크로 서비스 및 컨테이너의 배포에 중점을 둡니다.
