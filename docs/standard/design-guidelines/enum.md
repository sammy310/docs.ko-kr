...
❌ 개방형 집합 (예: 운영 체제 버전, 친구 이름 등)에 대해 열거형을 사용 하지 않습니다.

❌는 나중에 사용 하기 위해 예약 된 열거형 값을 제공 하지 않습니다.
...
❌ 한 값만 포함 된 열거형을 공개적으로 노출 하지 않도록 합니다.
...
다음 중 하나에 해당 하지 않는 한 열거형의 기본 형식으로 <xref:System.Int32> (대부분의 프로그래밍 언어의 기본값)를 사용 하는 것이 좋습니다. ✔️

- 열거형이 플래그 열거형 이며 32 개 이상의 플래그를 포함 하거나 앞으로 더 많은 것이 필요 합니다.

- 다른 크기의 열거를 기대 하는 비관리 코드와의 상호 운용성을 위해 기본 형식은 <xref:System.Int32>과 달라 야 합니다.
...
메모리 내 사용의 경우 관리 되는 개체는 항상 `DWORD`맞춰져 있으므로 전체 인스턴스 크기는 항상 `DWORD`으로 반올림 되기 때문에 더 작은 열거형을 압축 하려면 인스턴스의 여러 열거형 또는 다른 작은 구조를 사용 해야 합니다.

✔️ DO name은 복수 명사 또는 명사구를 사용 하는 열거형과 단일 명사 또는 명사구를 사용 하는 단순 열거형을 플래그 지정 합니다.

❌ <xref:System.Enum?displayProperty=nameWithType>를 직접 확장 하지 않습니다.

<xref:System.Enum?displayProperty=nameWithType>은 CLR에서 사용자 정의 열거형을 만드는 데 사용 하는 특수 한 형식입니다. 대부분의 프로그래밍 언어는이 기능에 대 한 액세스를 제공 하는 프로그래밍 요소를 제공 합니다. 예를 들어 C# `enum` 키워드는 열거형을 정의 하는 데 사용 됩니다.

<a name="design"></a>

### <a name="designing-flag-enums"></a>플래그 열거형 디자인

<xref:System.FlagsAttribute?displayProperty=nameWithType>을 플래그 열거형에 적용 ✔️ 합니다. 단순 열거형에는이 특성을 적용 하지 마십시오.

✔️는 플래그 열거형 값으로 2의 거듭제곱을 사용 하므로 비트 OR 연산을 사용 하 여 자유롭게 결합할 수 있습니다.

일반적으로 사용 되는 플래그 조합에 특수 한 열거형 값을 제공 하는 것이 좋습니다 ✔️.

비트 연산은 고급 개념 이므로 간단한 작업에는 필요 하지 않습니다. <xref:System.IO.FileAccess.ReadWrite>은 이러한 특수 값의 예입니다.

값의 특정 조합이 유효 하지 않은 경우 플래그 열거형을 만들지 않도록 ❌ 합니다.

값이 "모든 플래그가 지워졌습니다."를 나타내고 다음 지침에서 설명한 대로 이름을 적절 하 게 지정 하지 않으면 플래그 열거형 값을 0으로 사용 하지 않습니다. ❌

✔️ 플래그 열거형의 0 값 `None`합니다. 플래그 열거형의 경우 값은 항상 "모든 플래그가 지워졌습니다."을 의미 해야 합니다.
