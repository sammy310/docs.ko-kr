...
일반적으로 당사자(당사자 A)는 공개 키와 개인 키를 둘 다 생성하고 메모리 또는 암호화 키 컨테이너에 키를 저장합니다. 당사자 A가 다른 당사자(당사자 B)에게 공개 키를 보냅니다. 공개 키를 사용 하 여 당사자 B 데이터를 암호화 및 파티 A로 다시 데이터를 보내 파티 A는 데이터를 받은 후 해당 하는 개인 키를 사용 하 여 해독 합니다. 암호 해독은 당사자 A가 당사자 B에서 데이터를 암호화하는 데 사용한 공개 키에 해당하는 개인 키를 사용하는 경우에만 성공합니다.
