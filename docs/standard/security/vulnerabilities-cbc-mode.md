...
Microsoft는 매우 특정 한 경우를 제외 하 고, 암호 해독의 무결성을 먼저 확인 하지 않고 확인 가능한 패딩이 적용 되었을 때 대칭 암호화의 CBC (암호 블록 체인) 모드를 사용 하 여 암호화 된 데이터를 더 이상 안전 하지 않은 것으로 간주 합니다. 이 judgement는 현재 알려진 암호화 연구를 기반으로 합니다.

## <a name="introduction"></a>소개

패딩 oracle 공격은 암호화 된 데이터에 대 한 공격 유형으로, 공격자가 키를 몰라도 데이터의 콘텐츠를 해독할 수 있습니다.

Oracle은 실행 중인 작업이 정확한 지 여부에 대 한 공격자 정보를 제공 하는 "알림"을 참조 합니다. 자녀가 있는 보드 또는 카드 게임을 재생 한다고 가정 합니다. 얼굴을 큰 웃는 얼굴을 사용 하는 경우에는 oracle 이라고 생각 하 게 됩니다. 이 oracle을 사용 하 여 다음 이동을 적절 하 게 계획할 수 있습니다.
...
- Dampen에 대 한 암호 해독 호출의 평가를 게이트가 다음과 같이 타이밍 신호를 만듭니다.
  ...
  - 시간 계산은를 사용 하는 것이 아니라 [Acquiring high-resolution time stamps](/windows/desktop/sysinfo/acquiring-high-resolution-time-stamps) <xref:System.Environment.TickCount?displayProperty=nameWithType> (롤아웃/오버플로 적용)를 사용 하거나 두 개의 시스템 타임 스탬프를 빼서 (NTP 조정 오류 발생), 고해상도 타임 스탬프를 획득 하는 지침에 따라 수행 해야 합니다.
  - 시간 계산은 끝에 패딩 되는 것이 아니라 관리 되거나 c + + 응용 프로그램의 모든 잠재적 예외를 포함 하 여 암호 해독 작업에 포함 되어야 합니다.
...
- 암호 해독 호출은 플래그를 지정 하 여 [BCryptDecrypt](/windows/desktop/api/bcrypt/nf-bcrypt-bcryptdecrypt)입니다 `BCRYPT_BLOCK_PADDING` .
- 키 핸들이로 설정 된 [BCRYPT_CHAINING_MODE](/windows/desktop/SecCNG/cng-property-identifiers#BCRYPT_CHAINING_MODE) 로 [BCryptSetProperty](/windows/desktop/api/bcrypt/nf-bcrypt-bcryptsetproperty) 를 호출 하 여 초기화 되었습니다 `BCRYPT_CHAIN_MODE_CBC` .
  - `BCRYPT_CHAIN_MODE_CBC`가 기본값 이므로 영향을 받는 코드에는에 대 한 값이 할당 되지 않을 수 있습니다 `BCRYPT_CHAINING_MODE` .

이전 Windows 암호화 API에 대해 빌드된 프로그램의 경우:

- 암호 해독 호출은를 사용 하 여 [Cryptdecrypt](/windows/desktop/api/wincrypt/nf-wincrypt-cryptdecrypt) 합니다 `Final=TRUE` .
- 키 핸들이로 설정 된 [KP_MODE](/windows/desktop/api/wincrypt/nf-wincrypt-cryptgetkeyparam) 로 [CryptSetKeyParam](/windows/desktop/api/wincrypt/nf-wincrypt-cryptsetkeyparam) 를 호출 하 여 초기화 되었습니다 `CRYPT_MODE_CBC` .
  - `CRYPT_MODE_CBC`가 기본값 이므로 영향을 받는 코드에는에 대 한 값이 할당 되지 않을 수 있습니다 `KP_MODE` .

## <a name="finding-vulnerable-code---managed-applications"></a>취약 한 코드 관리 응용 프로그램 찾기

- 해독 호출은 <xref:System.Security.Cryptography.SymmetricAlgorithm.CreateDecryptor> 의 또는 메서드에 대 한 호출입니다 <xref:System.Security.Cryptography.SymmetricAlgorithm.CreateDecryptor(System.Byte[],System.Byte[])> <xref:System.Security.Cryptography.SymmetricAlgorithm?displayProperty=nameWithType> .
...
- <xref:System.Security.Cryptography.SymmetricAlgorithm.Padding?displayProperty=nameWithType>속성이 <xref:System.Security.Cryptography.PaddingMode.PKCS7?displayProperty=nameWithType> , <xref:System.Security.Cryptography.PaddingMode.ANSIX923?displayProperty=nameWithType> 또는로 설정 된 경우 <xref:System.Security.Cryptography.PaddingMode.ISO10126?displayProperty=nameWithType>
  - <xref:System.Security.Cryptography.PaddingMode.PKCS7?displayProperty=nameWithType>가 기본값 이므로 영향을 받는 코드는 속성을 할당 하지 못할 수 있습니다 <xref:System.Security.Cryptography.SymmetricAlgorithm.Padding?displayProperty=nameWithType> .
- <xref:System.Security.Cryptography.SymmetricAlgorithm.Mode?displayProperty=nameWithType>속성이로 설정 된 경우<xref:System.Security.Cryptography.CipherMode.CBC?displayProperty=nameWithType>
  - <xref:System.Security.Cryptography.CipherMode.CBC?displayProperty=nameWithType>가 기본값 이므로 영향을 받는 코드는 속성을 할당 하지 못할 수 있습니다 <xref:System.Security.Cryptography.SymmetricAlgorithm.Mode?displayProperty=nameWithType> .

## <a name="finding-vulnerable-code---cryptographic-message-syntax"></a>취약 한 코드 찾기-암호화 메시지 구문

암호화 된 콘텐츠가 AES (2.16.840.1.101.3.4.1.2, 2.16.840.1.101.3.4.1.22, 2.16.840.1.101.3.4.1.42), DES (1.3.14.3.2.7), 3DES (1.2.840.113549.3.7) 또는 RC2 (1.2.840.113549.3.2)의 CBC 모드를 사용 하는 인증 되지 않은 CMS EnvelopedData 메시지는 물론 CBC 모드에서 다른 블록 암호화 알고리즘을 사용 하는 메시지도 노출 됩니다.

스트림 암호화는 이러한 특정 취약점에 취약 하지 않지만 Content라는 알고리즘 값을 검사 하는 동안 항상 데이터를 인증 하는 것이 좋습니다.

관리 되는 응용 프로그램의 경우에 전달 되는 모든 값으로 CMS EnvelopedData blob을 검색할 수 있습니다 <xref:System.Security.Cryptography.Pkcs.EnvelopedCms.Decode(System.Byte[])?displayProperty=fullName> .

네이티브 응용 프로그램의 경우, cms EnvelopedData blob는 결과 [CMSG_TYPE_PARAM](/windows/desktop/api/wincrypt/nf-wincrypt-cryptmsggetparam) 가 및/또는 cms 핸들이 나중에 [CryptMsgUpdate](/windows/desktop/api/wincrypt/nf-wincrypt-cryptmsgupdate) `CMSG_ENVELOPED` `CMSG_CTRL_DECRYPT` [Cryptmsgupdate](/windows/desktop/api/wincrypt/nf-wincrypt-cryptmsgcontrol)을 통해 명령을 전송 하는 cryptmsgupdate를 통해 cms 핸들에 제공 된 값으로 검색 될 수 있습니다.
...
이 샘플에서는 <xref:System.IO.Stream> 암호화 또는 암호 해독에 대해를 허용 하지 않습니다. 현재 데이터 형식을 사용 하는 경우에는 `hmac_tag` 값이 암호 텍스트 앞에 있기 때문에 한 단계 암호화가 어려워집니다. 그러나이 형식은 파서를 간소화 하기 위해 시작 부분에 모든 고정 크기 요소를 유지 하기 때문에 선택 되었습니다. 이 데이터 형식을 사용 하는 경우 cautioned는 GetHashAndReset를 호출 하 고 TransformFinalBlock를 호출 하기 전에 결과를 확인 하는 것이 좋습니다. 단, 단일 패스 암호 해독을 사용할 수 있습니다. 스트리밍 암호화가 중요 한 경우 다른 AE 모드가 필요할 수 있습니다.
