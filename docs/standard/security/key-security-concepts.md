...
## <a name="type-safety-and-security"></a>형식 안전성 및 보안  
 형식 안전 코드는 액세스 권한이 부여된 메모리 위치에만 액세스합니다. (이 설명의 경우 형식 안전성은 특히 메모리 형식 안전을 나타내며 더 넓은 측면에서 형식 안전과 혼동해서는 안 됩니다.) 예를 들어 형식 안전 코드는 다른 개체의 개인 필드에서 값을 읽을 수 없습니다. 잘 정의된 허용되는 방식으로만 형식에 액세스합니다.  
 ...
 자세한 내용은 [Code Access Security Basics](../../../docs/framework/misc/code-access-security-basics.md)을 참조하세요.  
