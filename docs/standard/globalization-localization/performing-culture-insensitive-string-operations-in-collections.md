...
`CaseInsensitiveHashCodeProvider` 및 `CaseInsensitiveComparer`에 대한 매개 변수 없는 생성자는 `Thread.CurrentCulture`를 사용하여 클래스의 새 인스턴스를 초기화함으로써 문화권 구분 동작을 발생합니다. 다음 코드 예제에서는 `Hashtable` 및 `CaseInsensitiveHashCodeProvider`에 대해 매개 변수 없는 생성자를 사용하므로 문화권을 구분하는 `CaseInsensitiveComparer`에 대한 생성자를 보여 줍니다.
...
`Hashtable` 및 `CaseInsensitiveComparer` 클래스를 사용하여 문화권을 구분하지 않는 `CaseInsensitiveHashCodeProvider`을 만들려는 경우 `culture` 매개 변수를 수락하는 생성자를 사용하여 이러한 클래스의 새 인스턴스를 초기화합니다. `culture` 매개 변수의 경우 <xref:System.Globalization.CultureInfo.InvariantCulture%2A?displayProperty=nameWithType>를 지정합니다. 다음 코드 예제에서는 문화권을 구분하지 않는 `Hashtable`에 대한 생성자를 보여 줍니다.
...
`SortedList`는 키를 기준으로 정렬되고 키와 인덱스로 액세스할 수 있는 키/값 쌍의 컬렉션을 나타냅니다. 문자열이 키인 경우에 `SortedList`를 사용하면 정렬 및 조회가 `Thread.CurrentCulture` 속성의 영향을 받을 수 있습니다. `SortedList`에서 문화권을 구분하지 않는 동작을 가져오려면 `SortedList` 매개 변수를 수락하는 생성자 중 하나를 사용하여 `comparer`를 만듭니다. `comparer` 매개 변수는 키를 비교할 때 사용할 <xref:System.Collections.IComparer> 구현을 지정합니다. 매개 변수의 경우 `CultureInfo.InvariantCulture`를 사용하는 사용자 지정 비교자 클래스를 지정하여 키를 비교합니다. 다음 예제에서는 `comparer` 생성자에 대한 `SortedList` 매개 변수로 지정할 수 있는 사용자 지정 문화권 비구분 비교자 클래스를 보여 줍니다.
