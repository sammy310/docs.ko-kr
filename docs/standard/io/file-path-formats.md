...
- 경로를 DOS 디바이스 경로로 식별하는 디바이스 경로 지정자(`\\.\` 또는 `\\?\`).
   ...
   > [!NOTE]
   > `\\?\`는 모든 버전의 .NET Core 및 .NET 5 이상과 버전 4.6.2부터 .NET Framework에서 지원됩니다.
...
> [!NOTE]
> .NET Core 및 .NET 5 이상은 암시적으로 긴 경로를 처리하며 `MAX_PATH` 검사를 수행하지 않습니다. `MAX_PATH` 검사는 .NET Framework에만 적용됩니다.
