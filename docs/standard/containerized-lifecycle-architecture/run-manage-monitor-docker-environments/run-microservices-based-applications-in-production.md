...
이 전자책의 앞부분에 나오는 *클러스터* 하 고 *스케줄러* 소프트웨어 아키텍처 및 개발에 대 한 논의의 일부로 도입 되었습니다. Kubernetes 및 Service Fabric은 Docker 클러스터의 예입니다. 이러한 오 케 스트레이이 터의 두 가지 모두 Microsoft Azure Kubernetes Service에서 제공 하는 인프라의 일부로 실행할 수 있습니다.
...
- **오 케 스트레이 터**합니다. 호스트 클러스터에 배포 하는 복잡 하 고 다중 컨테이너 워크 로드에 수명 주기 관리 기능을 확장 하는 플랫폼입니다. 호스트 인프라를 추상화 하 여 오케스트레이션 도구 전체 클러스터는 단일 배포 대상으로 처리 하는 방법을 사용자에 게를 제공 합니다.
...
개발 하 고 처음부터 새로 만드는 복잡 한 오 케 스트레이 터 및 스케줄러에서 제공 하는 기능을 따라서 일반적으로 하려는 공급 업체에서 제공 하는 오케스트레이션 솔루션을 사용 합니다.
