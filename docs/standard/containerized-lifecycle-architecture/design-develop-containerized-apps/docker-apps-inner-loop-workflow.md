...
Microsoft는 Mac, Windows 및 Linux에서 지원되는 경량급 코드 편집기인 Visual Studio Code를 제공하며, 
...
- Azure App Service에 Docker Hub 및 Azure 컨테이너 레지스트리의 이미지 배포
...
> [!NOTE]
>
> "외부 루프 DevOps 워크플로"를 고려하는 경우 소스 코드를 Git 리포지토리에 푸시할 때마다(연속 통합) 자동화된 빌드 프로세스에 의해 이미지가 생성되므로 해당 이미지는 소스 코드에서 해당 전역 환경에 생성됩니다.
>
