...
특정 앱에서 값 형식의 경우 오류 표시기로 Nullable<T> 또는 기본값 중 무엇을 사용할지 고려해야 합니다. `Nullable<Guid>`를 사용하면 `default`는 `Guid.Empty` 대신 `null`이 됩니다. 값이 있거나 없는 경우 `Nullable<T>`을 추가하여 더 명확하게 만들 수도 있습니다. 또는 `Nullable<T>`을 추가하여 불필요한지를 확인하는 추가 사례를 만들고 오류의 잠재적 원인을 만드는 역할만 할 수도 있습니다. 
