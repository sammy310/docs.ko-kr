...
## <a name="canceling-an-asynchronous-operation"></a>비동기 작업 취소
 <xref:System.Threading.CancellationToken>부터는 취소를 지원하는 TAP 메서드가 취소 토큰( 개체)을 수락하는 오버로드를 하나 이상 제공합니다.
