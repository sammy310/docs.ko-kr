...
- EAP(이벤트 기반 비동기 패턴) - *및*와 같이 *OperationName*Async 및 <xref:System.Net.WebClient.DownloadStringAsync%2A?displayProperty=nameWithType>OperationName<xref:System.Net.WebClient.DownloadStringCompleted?displayProperty=nameWithType>Completed라는 메서드/이벤트 쌍으로 비동기 작업을 표현합니다. EAP는 .NET Framework 버전 2.0에 도입되었습니다.  
...
- `Begin_`와 <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAll%2A> 메서드를 사용하거나 <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAny%2A> 메서드 또는 <xref:System.Threading.Tasks.Task.WaitAll%2A> 메서드를 사용하여 <xref:System.Threading.Tasks.Task.WaitAny%2A> 메서드에 대한 응답으로 실행되는 여러 작업을 조정합니다.  
  
- I/O에 바인딩된 비동기 작업과 컴퓨팅에 바인딩된 비동기 작업을 같은 작업 개체로 캡슐화합니다.  
...
## <a name="wrapping-apm-operations-in-a-task"></a>작업으로 APM 작업 래핑  
 <xref:System.Threading.Tasks.TaskFactory?displayProperty=nameWithType> 및 <xref:System.Threading.Tasks.TaskFactory%601?displayProperty=nameWithType> 클래스는 둘 다 APM Begin/End 메서드 쌍을 <xref:System.Threading.Tasks.TaskFactory.FromAsync%2A?displayProperty=nameWithType> 또는 <xref:System.Threading.Tasks.TaskFactory%601.FromAsync%2A?displayProperty=nameWithType> 인스턴스 하나로 캡슐화하도록 하는 <xref:System.Threading.Tasks.Task> 및 <xref:System.Threading.Tasks.Task%601> 메서드의 여러 가지 오버로드를 제공합니다. 다양한 오버로드가 입력 매개 변수 수가 0~3개인 Begin/End 메서드 쌍을 수용합니다.  
  
 값(Visual Basic의 `End`)을 반환하는 `Function` 메서드가 있는 쌍의 경우 <xref:System.Threading.Tasks.TaskFactory%601>에서 <xref:System.Threading.Tasks.Task%601>을 만드는 메서드를 사용합니다. void(Visual Basic의 `End`)를 반환하는 `Sub` 메서드의 경우 <xref:System.Threading.Tasks.TaskFactory>에서 <xref:System.Threading.Tasks.Task>를 만드는 메서드를 사용합니다.  
  
 `Begin` 메서드에 매개 변수가 4개 이상 포함되거나 `ref` 또는 `out` 매개 변수가 포함되는 경우에는 `FromAsync` 메서드만 캡슐화하는 추가 `End` 오버로드가 제공됩니다.  
  
 다음 예제에서는 `FromAsync` 및 <xref:System.IO.FileStream.BeginRead%2A?displayProperty=nameWithType> 메서드와 일치하는 <xref:System.IO.FileStream.EndRead%2A?displayProperty=nameWithType> 오버로드에 대한 서명을 보여 줍니다. 이 오버로드는 다음과 같이 입력 매개 변수 세 개를 사용합니다.  
  
 [!code-csharp[FromAsync#01](../../../samples/snippets/csharp/VS_Snippets_Misc/fromasync/cs/fromasync.cs#01)]
 [!code-vb[FromAsync#01](../../../samples/snippets/visualbasic/VS_Snippets_Misc/fromasync/vb/module1.vb#01)]  
  
 첫 번째 매개 변수는 <xref:System.Func%606> 메서드의 서명과 일치하는 <xref:System.IO.FileStream.BeginRead%2A?displayProperty=nameWithType> 대리자입니다. 두 번째 매개 변수는 <xref:System.Func%602>를 사용하고 <xref:System.IAsyncResult>를 반환하는 `TResult` 대리자입니다. <xref:System.IO.FileStream.EndRead%2A>는 정수를 반환하므로 컴파일러에서는 `TResult` 형식을 <xref:System.Int32>로 유추하고 작업 형식을 <xref:System.Threading.Tasks.Task>로 유추합니다. 마지막 매개 변수 4개는 <xref:System.IO.FileStream.BeginRead%2A?displayProperty=nameWithType> 메서드의 매개 변수와 동일합니다.  
...
### <a name="using-continuewith-for-the-callback-functionality"></a>콜백 기능에 ContinueWith 사용  
 단순한 바이트 수와는 달리 파일의 데이터에 대한 액세스 권한이 필요할 경우에는 <xref:System.Threading.Tasks.TaskFactory.FromAsync%2A> 메서드로는 충분하지 않습니다. 대신에 파일 데이터가 들어 있는 <xref:System.Threading.Tasks.Task> 속성이 포함된 `Result`를 사용하세요. 이 작업을 하려면 원래 작업에 연속을 추가합니다. 연속은 일반적으로 <xref:System.AsyncCallback> 대리자에서 수행하는 작업을 수행합니다. 연속은 선행이 완료되고 데이터 버퍼가 채워졌을 때 호출됩니다. 돌아가기 전에 <xref:System.IO.FileStream> 개체를 닫아야 합니다.  
  
 다음 예제에서는 <xref:System.Threading.Tasks.Task> 클래스의 BeginRead/EndRead 쌍을 캡슐화하는 <xref:System.IO.FileStream>를 반환하는 방법을 보여 줍니다.  
...
### <a name="fromasync-tasks-for-only-the-end-method"></a>End 메서드 전용 FromAsync 작업  
 `Begin` 메서드에 입력 매개 변수가 4개 이상 필요하거나 `ref` 또는 `out` 매개 변수가 있는 경우 `FromAsync` 메서드만 표현하는 <xref:System.Threading.Tasks.TaskFactory%601.FromAsync%28System.IAsyncResult%2CSystem.Func%7BSystem.IAsyncResult%2C%600%7D%29?displayProperty=nameWithType> 오버로드(예: `End`)를 사용할 수 있습니다. <xref:System.IAsyncResult>를 전달하고 이를 작업으로 캡슐화하려는 시나리오에서 이들 메서드를 사용할 수도 있습니다.  
...
## <a name="exposing-complex-eap-operations-as-tasks"></a>복잡한 EAP 작업을 작업으로 노출  
 TPL은 메서드의 `FromAsync` 패밀리가 <xref:System.IAsyncResult> 패턴을 래핑하는 것과 같은 방법으로 이벤트 기반 비동기 작업을 캡슐화하도록 특수 디자인된 메서드를 제공하지 않습니다. 그러나 TPL은 임의 작업 집합을 <xref:System.Threading.Tasks.TaskCompletionSource%601?displayProperty=nameWithType>로 표현하는 데 사용할 수 있는 <xref:System.Threading.Tasks.Task%601> 클래스를 제공합니다. 작업은 동기 또는 비동기 작업일 수 있고 I/O에 바인딩되거나 컴퓨팅에 바인딩된 작업이거나 두 작업에 모두 해당할 수 있습니다.  
