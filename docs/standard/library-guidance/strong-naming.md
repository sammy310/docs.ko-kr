...
강력한 이름 지정의 단점은 어셈블리에 강력한 이름이 지정되고 나면 Windows의 .NET Framework에서 어셈블리를 엄격하게 로드한다는 것입니다. 강력한 이름의 어셈블리 참조는 로드된 어셈블리 버전과 정확히 일치해야 하므로 개발자는 어셈블리를 사용할 때 [바인딩 리디렉션을 구성](../../framework/configure-apps/redirect-assembly-versions.md)해야 합니다.
...
3. 다른 어셈블리 버전과 나란히 어셈블리를 로드할 수 있습니다. 병렬 어셈블리 로드는 일반적으로 플러그 인 아키텍처를 사용하는 애플리케이션에 필요합니다.
...
> [!NOTE]
> 이 지침은 NuGet.org에 게시된 .NET 라이브러리와 같은 공개적으로 배포된 .NET 라이브러리에만 해당합니다. 대부분의 .NET 애플리케이션에는 강력한 이름 지정이 필요하지 않으므로 기본적으로 수행해서는 안 됩니다.

✔️ 라이브러리 어셈블리에 강력한 이름을 지정하는 것을 고려하세요.

✔️ 소스 제어 시스템에 강력한 이름 지정 키를 추가하는 것을 고려하세요.
...
✔️ 사용자가 바인딩 리디렉션 및 업데이트 빈도를 줄일 수 있도록 주 버전 변경 시에만 어셈블리 버전을 높이는 것을 고려하세요.
...
❌ 강력한 이름 및 강력하지 않은 이름이 지정된 버전의 라이브러리를 게시하지 마세요. 예를 들어 `Contoso.Api` 및 `Contoso.Api.StrongNamed`를 지정합니다.
