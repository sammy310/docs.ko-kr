...
✔️ NuGet 패키지 버전을 관리하는 데 [SemVer 2.0.0](https://semver.org/)을 사용하는 것을 고려하세요.
...
✔️ AssemblyVersion의 주 버전 번호와 NuGet 패키지 버전을 동기화 상태로 유지하는 것을 고려하세요.
...
✔️ 연속 통합 빌드 번호를 AssemblyFileVersion 수정으로 포함하는 것을 고려하세요.

> 예를 들어 프로젝트 버전 1.0.0을 빌드 중이며 연속 통합 빌드 번호가 99인 경우 AssemblyFileVersion은 1.0.0.99입니다.

✔️ 파일 버전에 대해 `Major.Minor.Build.Revision` 형식을 사용합니다.
