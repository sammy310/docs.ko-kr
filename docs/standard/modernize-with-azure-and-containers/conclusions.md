...
- 컨테이너 기반 솔루션을 통해 궁극적으로 비용 절감 혜택을 얻을 수 있습니다. 컨테이너는 상용 환경에서 종속성이 존재하지 않음으로써 발생하는 문제를 제거해주기 때문에 배포 문제가 해결됩니다. 이러한 이슈가 제거됨으로써 개발과 테스트, DevOps, 상용 작업의 성능이 크게 개선 됩니다.

- Docker 컨테이너는 서버 기반 응용 프로그램이나 서비스 배포의 표준 단위가 되고 있습니다.

- 상용 환경에서는 확장성이 뛰어난 Windows 컨테이너 기반 응용 프로그램(예: Service Fabric이나 Kubernetes)을 게시하기 위해 오케스트레이터를 사용해야 합니다.
...
- 상용 환경에서 컨테이너화된 응용 프로그램을 배치하는 경우는 항상 Azure DevOps Services나 Jenkins와 같은 CI/CD 파이프라인에 대한 DevOps 도구와 DevOps문화를 만들거나 채택 합니다.
