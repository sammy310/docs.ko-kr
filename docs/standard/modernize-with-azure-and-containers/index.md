...
가장 적합한 장기 유연성 및 조직에 대한 값을 사용하여 최신 응용 프로그램의 경우 어떤 이점이 있는지에 투자 *클라우드 네이티브* 응용 프로그램 아키텍처입니다. 그러나 기존 응용 프로그램 또는 기존 자산에 대 한 키는로 상당한 이점을 실현 하려면 클라우드로 이동 하는 최소한의 시간과 비용 (재설계 나 코드 변경 없음)를 사용해 보세요.
