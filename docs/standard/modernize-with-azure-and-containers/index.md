...
웹 응용 프로그램 또는 서비스를 최신으로 변경하고 클라우드로 이동하기위해 앱 아키텍처를 완전히 변경해야 하는 것은 아닙니다. 제한된 시간과 비용 때문에 마이크로 서비스 같은 향상된 방법으로 응용 프로그램을 재설계하는 것을 항상 선택 할 수는 없습니다. 응용 프로그램의 유형에 따라 앱의 아키텍처를 변경하지 않을 수도 있습니다. 조직의 클라우드 마이그레이션 전략을 비용 효과적으로 최적화하려면 비즈니스와 앱의 요구 사항을 고려해야 합니다. 다음 사항을 파악해야 합니다. 
...
이 가이드에서는 클라우드로 앱을 옮기는 것의 이득과 Windows 컨테이너와 Windows 컨테이너를 지원하는 Azure의 관련된 계산 플랫폼과 같은 새로운 기술과 방법을 사용하여 앱을 부분적으로 최신화하는 것에 대한 잇점을 강조 합니다.  

## <a name="path-to-the-cloud-for-existing-net-applications"></a>기존.NET 응용 프로그램을 클라우드로 옮기는 방법

조직에서는 일반적으로 응용 프로그램이 민첩성과 속도를 갖게 하기 위해 클라우드로 옮기는 것을 선택합니다. 클라우드에서는 몇 분 안에 수천 대의 서버(VM)를 설치할 수 있는데 비해 직접 구축한 서버에서는 몇 주의 시간이 걸리는 것이 일반적입니다.

한번에 완벽하게 클라우드로 응용 프로그램을 마이그레이션하는 방법은 존재하지 않습니다. 귀하에게 적합한 마이그레이션 전략은 귀사의 요구 사항과 우선 순위, 마이그레이션하는 응용 프로그램의 종류에 따라 달라집니다. 모든 응용 프로그램을 서비스 형태의 플랫폼([PaaS](https://azure.microsoft.com/overview/what-is-paas/)) 모델로 전환하거나 [클라우드 전용](https://www.gartner.com/doc/3181919/architect-design-cloudnative-applications) 응용 프로그램 모델로 개발할 수 있는 것은 아닙니다. 대다수의 경우 비즈니스 요구 사항에 따라 단계식 또는 증분식 방법을 사용하여 클라우드로 자산을 전환할 수 있습니다.

조직의 기민성과 가치를 아주 오래 가져갈 수 있는 최신 응용 프로그램의 경우 *클라우드 네이티브* 응용 프로그램 아키텍처에 투자하는 것이 좋습니다. 그러나 기존 응용 프로그램이나 기존 자산의 경우 상당한 이득을 얻기 위해서는 최소한의 시간과 비용(재설계나 코드 변경을 하지 않고)을 들여 클라우드로 이동하는 것이 중요합니다.
...
각 마이그레이션 방법마다 사용해야 할 이유와 이점이 다릅니다. 클라우드에 앱을 마이그레이션하는 경우 하나의 방법을 선택하거나 여러 방법 중에서 특정 구성 요소를 선택할 수 있습니다. 개별 응용 프로그램이 단일 접근법이나 완성 상태로 제한되는 것은 아닙니다. 예를 들어, 일반적인 하이브리드 접근법은 특정 온-프레미스 구성 요소와 다른 클라우드 구성 요소를 사용합니다.
