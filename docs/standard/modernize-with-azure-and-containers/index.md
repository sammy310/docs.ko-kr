...
|---|---|---|
| **응용 프로그램의 계산 대상** |
| Azure의 VM에 배포된 응용 프로그램 | 컨테이너 또는 AKS (Azure Kubernetes Service)를 사용 하 여 Azure App Service, Azure 컨테이너 인스턴스 (ACI), Vm에 배포 된 모놀리식 또는 N 계층 앱 | Azure Kubernetes Service (AKS) 및/또는 Azure 함수를 기반으로 하는 서버 리스 마이크로 서비스에 컨테이너 화 된 마이크로 서비스. |
| **데이터 대상** |
...
이 가이드는 특정 부분에 중점을 둡니다. 코드를 변경하거나 다시 아키텍처를 변경하지 않고 기존 .NET 응용 프로그램을 리프트 앤 시프트할 수 있는 방법을 보여줍니다. 궁극적으로는 응용 프로그램을 Cloud-Optimized하게 만드는 방법으로 보여줍니다.

이 가이드에서는 마이크로 서비스 아키텍처로 진화하는 방법과 같이 클라우드 기본 응용 프로그램을 만드는 방법을 보여주지 않습니다. 응용 프로그램을 재구성하거나 마이크로 서비스를 기반으로 하는 새로운 응용 프로그램을 만들려면 [.NET Microservices: 컨테이너화된 .NET 응용 프로그램용 아키텍처](https://aka.ms/microservicesebook) 전자 문서를 참조합니다.
