...
이 가이드는 특정 부분에 중점을 둡니다. 코드를 변경이나 다시 아키텍처를 변경하지 않고 기존 .NET 응용 프로그램을 리프트 앤 시프트 할 수 있는 방법을 보여줍니다. 궁극적으로 응용 프로그램의 Cloud-Optimized 방법으로 보여줍니다.

이 가이드에서는 마이크로 서비스 아키텍처로 진화하는 방법과 같이 클라우드 기본 응용 프로그램을 만드는 방법을 보여주지 않습니다. 응용 프로그램을 재구성하거나 마이크로 서비스를 기반으로하는 아주 새로운 응용 프로그램을 만들려면 [.NET Microservices : 컨테이너 .NET 응용 프로그램용 아키텍처](https://aka.ms/microservicesebook)전자 문서를 참조하십시오.
