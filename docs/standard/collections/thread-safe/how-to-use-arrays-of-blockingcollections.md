...
다음 예제에서는 <xref:System.Collections.Concurrent.BlockingCollection%601?displayProperty=nameWithType> 및 <xref:System.Collections.Concurrent.BlockingCollection%601.TryAddToAny%2A>와 같은 정적 메서드와 함께 <xref:System.Collections.Concurrent.BlockingCollection%601.TryTakeFromAny%2A> 개체의 배열을 사용하여 구성 요소 간에 빠르고 유연한 데이터 전송을 구현하는 방법을 보여줍니다.  
