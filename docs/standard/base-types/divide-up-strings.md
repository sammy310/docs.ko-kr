...
<span data-ttu-id="e2610-124">정규식 패턴 `(\d+)\s+([-+*/])\s+(\d+)`는 다음과 같이 정의됩니다.</span><span class="sxs-lookup"><span data-stu-id="e2610-124">The regular expression pattern `(\d+)\s+([-+*/])\s+(\d+)` is defined like this:</span></span>
