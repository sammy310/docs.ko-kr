...
|문자 클래스|설명|무늬|일치 항목|  
|---------------------|-----------------|-------------|-------------|  
|`[` *character_group* `]`|*character_group*에서 단일 문자를 찾습니다. 기본적으로 일치 항목 찾기에서는 대/소문자를 구분합니다.|`[ae]`|`"a"`의 `"gray"`<br /><br /> `"lane"`의 `"a"`, `"e"`|  
...
|`.`|와일드카드: \n 이외의 모든 단일 문자를 찾습니다.<br /><br /> 리터럴 마침표 문자(. 또는 `\u002E`)를 찾으려면 마침표 문자 앞에 이스케이프 문자를 추가해야 합니다(`\.`).|`a.e`|`"ave"`의 `"nave"`<br /><br /> `"ate"`의 `"water"`|  
...
|`\s`|공백 문자를 나타냅니다.|`\w\s`|`"D "`의 `"ID A1.3"`|  
|`\S`|공백 문자가 아닌 문자를 찾습니다.|`\s\S`|`" _"`의 `"int __ctr"`|  
|`\d`|10진수를 찾습니다.|`\d`|`"4"`의 `"4 = IV"`|  
...
|어설션|설명|무늬|일치 항목|  
|---------------|-----------------|-------------|-------------|  
|`^`|기본적으로 일치 항목은 문자열의 시작 부분에서 시작되어야 합니다. 다중 선에서는 줄의 시작 부분에서 시작되어야 합니다.|`^\d{3}`|`"901"`의 `"901-333-"`|  
|`$`|기본적으로 일치 항목은 문자열의 끝부분 또는 문자열의 끝부분 `\n` 앞에서 발생해야 합니다. 다중 선에서는 줄의 끝부분 또는 줄의 끝 `\n` 앞에서 발생해야 합니다.|`-\d{3}$`|`"-333"`의 `"-901-333"`|  
|`\A`|일치 항목이 문자열의 시작 부분에 있어야 합니다.|`\A\d{3}`|`"901"`의 `"901-333-"`|  
|`\Z`|일치 항목이 문자열의 끝이나 문자열의 끝에 있는 `\n` 앞에 있어야 합니다.|`-\d{3}\Z`|`"-333"`의 `"-901-333"`|  
|`\z`|일치 항목이 문자열의 끝에 있어야 합니다.|`-\d{3}\z`|`"-333"`의 `"-901-333"`|  
...
|그룹화 구문|설명|무늬|일치 항목|  
|------------------------|-----------------|-------------|-------------|  
|`(` *subexpression* `)`|일치하는 하위 식을 캡처하고 서수(1부터 시작)를 할당합니다.|`(\w)\1`|`"ee"`의 `"deep"`|  
|`(?<` *name* `>` *subexpression* `)`|일치하는 하위 식을 명령된 그룹에 캡처합니다.|`(?<double>\w)\k<double>`|`"ee"`의 `"deep"`|  
|`(?<` *이름1* `-` *이름2* `>` *subexpression* `)`|균형 조정 그룹 정의를 정의합니다. 자세한 내용은 [그룹화 구문](grouping-constructs-in-regular-expressions.md)의 "균형 조정 그룹 정의" 섹션을 참조하세요.|`(((?'Open'\()[^\(\)]*)+((?'Close-Open'\))[^\(\)]*)+)*(?(Open)(?!))$`|`"((1-3)*(3-1))"`의 `"3+2^((1-3)*(3-1))"`|  
|`(?:` *subexpression* `)`|비캡처 그룹을 정의합니다.|`Write(?:Line)?`|`"WriteLine"`의 `"Console.WriteLine()"`<br /><br /> `"Write"`의 `"Console.Write(value)"`|  
...
|`{` *n* `}`|이전 요소를 정확히 *n* 회 찾습니다.|`",\d{3}"`|`"1,043.6"`의 `",043"`, `"9,876,543,210"`의 `",876"`, `",543"` 및 `",210"`|  
|`{` *n* `,}`|이전 요소를 최소한 *n* 회 찾습니다.|`"\d{2,}"`|`"166"`, `"29"`, `"1930"`|  
|`{` *n* `,` *분* `}`|이전 요소를 *n* 회 이상 *m* 회 이하로 찾습니다.|`"\d{3,5}"`|`"166"`, `"17668"`<br /><br /> `"19302"`의 `"193024"`|  
...
|역참조 구문|설명|무늬|일치 항목|  
|-----------------------------|-----------------|-------------|-------------|  
|`\` *number*|역참조입니다. 번호가 매겨진 하위 식의 값을 찾습니다.|`(\w)\1`|`"ee"`의 `"seek"`|  
|`\k<` *name* `>`|명명된 역참조입니다. 명명된 식의 값을 찾습니다.|`(?<char>\w)\k<char>`|`"ee"`의 `"seek"`|  
...
|구문|정의|예제|  
|---------------|----------------|-------------|  
|`(?imnsx-imnsx)`|패턴 중간에 대/소문자 구분하지 않음과 같은 옵션을 설정하거나 해제합니다. 자세한 내용은 [정규식 옵션](regular-expression-options.md)을 참조하세요.|`\bA(?i)b\w+\b`는 `"ABA Able Act"`의 `"ABA"`, `"Able"`와 일치합니다.|  
