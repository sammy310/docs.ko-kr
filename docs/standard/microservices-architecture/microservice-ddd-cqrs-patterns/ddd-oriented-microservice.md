...
**애플리케이션 계층:** 소프트웨어가 수행할 작업을 정의하고 표현적 도메인 개체가 문제를 해결하도록 지시합니다. 이 계층은이 담당하는 작업은 비즈니스에 유의미하거나, 다른 시스템의 애플리케이션 계층과의 상호 작용에 필요합니다. 이 계층은 씬으로 유지됩니다. 비즈니스 규칙이나 지식은 포함하지 않으며 작업을 조정하고 다음 하위 계층에서 도메인 계층의 협력을 위해 업무를 위임하기만 합니다. 비비즈니스 상환을 반영하는 상태를 갖지 않지만 사용자 또는 프로그램에 대해 작업의 진행 상황을 반영하는 상태는 가질 수 있습니다.
