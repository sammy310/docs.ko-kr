...
- 요소 사이에 있는 공백이 변경됩니다. 유효 공백은 유지되고 무효 공백은 유지되지 않습니다. 그러나 문서를 저장할 때 기본적으로 <xref:System.Xml.XmlTextWriter> **들여쓰기** 모드를 사용 하 여 출력을 보다 쉽게 읽을 수 있도록 출력을 깔끔하게 인쇄 합니다.  
...
```  
 ...
 <xref:System.Xml.XmlNode.OuterXml%2A>, <xref:System.Xml.XmlDeclaration.WriteTo%2A>및 <xref:System.Xml.XmlNode.InnerXml%2A>멤버 <xref:System.Xml.XmlDeclaration> <xref:System.Xml.XmlDeclaration.Encoding%2A> 속성이 설정 되지 않은 경우 인코딩은 쓰여지지 않습니다. 그렇지 않으면 XML 선언에서 작성 된 인코딩이 <xref:System.Xml.XmlDeclaration.Encoding%2A> 속성에 있는 인코딩과 동일 합니다.  
