...
*.pubxml* 파일(.NET Core 3.1 SDK부터 제공)을 참조하여 게시 관련 속성을 설정할 수도 있습니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.
...
- **`-p:PublishReadyToRun=true`**
  ...
  런타임 오류를 발생시킬 수 있는 누락된 종속성에 대한 경고를 보려면 `-p:PublishReadyToRunShowWarnings=true`를 사용합니다.
