...
그런 다음, 인터넷 기술은 개발 전 세계에 shocking, 손쉬운 배포 및 간소화 된 배포 프로세스와 같은 이점을 제공 하는 더 많은 엔지니어에 게 제공 되었습니다. 프로덕션 환경에 웹 응용 프로그램을 배포한 후에는 모든 사용자가 자동 업데이트를 수행 하 여 소프트웨어 민첩성에 큰 영향을 줄 수 있습니다.

그러나 HTTP 및 HTML과 같은 인터넷 인프라, 기본 프로토콜 및 표준은 복잡 한 응용 프로그램을 빌드하기 위해 설계 되지 않았습니다. 실제로 개발 작업을 수행 하는 데는 빠른 데이터 입력 및 상태 관리와 같이 데스크톱 응용 프로그램에 포함 된 기능과 동일한 기능을 웹 응용 프로그램에 제공 하는 것을 목표로 합니다.
...
- Ui (사용자 인터페이스) 디자인을 위한 학습 곡선이 부족 합니다. WinForms의 경우 Windows Forms 디자이너의 끌어서 놓기 환경에서 편리 하 게 사용할 수 있습니다.
...
.NET Core 3.0을 시작한 후에는 여러 버전의 .NET을 함께 배포 하는 새로운 방법을 활용 하 고 각 응용 프로그램에서 대상으로 지정 해야 하는 .NET 버전을 지정할 수 있습니다. 이러한 방식으로 한 응용 프로그램에서 최신 기능을 사용할 수 있으며,이를 통해 다른 응용 프로그램을 중단 하지 않을 수 있습니다.

### <a name="installation"></a>설치

데스크톱 응용 프로그램은 사용자가 사용을 시작 하기 전에 항상 일종의 설치 프로세스를 사용 합니다. 이번에는 MSI 및 ClickOnce에서 사용자 지정 설치 관리자 또는 XCOPY 배포로 일련의 기술을 게임에 가져왔습니다. 이러한 방법 중 하나는 응용 프로그램에서 컴퓨터의 공유 리소스에 액세스 하는 방법이 필요 하기 때문에 정교한 문제를 처리 합니다. 경우에 따라 새 키 값을 삽입 하거나 업데이트 하기 위해 레지스트리에 액세스 해야 합니다. 경우에 따라 주 응용 프로그램에서 참조 하는 공유 Dll도 업데이트 해야 합니다. 이 동작으로 인해 사용자에 게 연속 골칫거리 발생 하며,이로 인해 일부 응용 프로그램을 설치한 후에는 나중에 제거 하더라도 컴퓨터가 동일 하지는 않습니다.
