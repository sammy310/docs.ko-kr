...
클라우드 네이티브는 *속도* 와 *민첩성* 에 대 한 것입니다. 비즈니스 시스템은 비즈니스 기능을 사용 하 여 비즈니스 속도와 성장을 가속화 하는 전략적 변환의 무기로 발전 하 고 있습니다. 시장에 즉시 아이디어를 얻는 것이 필수적입니다.
...
### <a name="the-twelve-factor-application"></a>Twelve-Factor 응용 프로그램
...
웹 기반 응용 프로그램에 적용할 수 있지만 많은 전문가는 클라우드 네이티브 앱을 빌드하기 위한 견고한 기반으로 Twelve-Factor을 고려 합니다. 이러한 원칙을 바탕으로 구축 된 시스템은 신속 하 게 배포 하 고 확장할 수 있으며 시장 변화에 신속 하 게 대응할 수 있는 기능을 추가할

다음 표에서는 Twelve-Factor 방법을 중점적으로 설명 합니다.
...
[Twelve-Factor 앱 외](https://content.pivotal.io/blog/beyond-the-twelve-factor-app)의 작성자 Kevin hoffman은는 각각의 원래 12 가지 요인 (2011으로 작성)을 자세히 설명 합니다. 또한 오늘날의 최신 클라우드 응용 프로그램 디자인을 반영 하는 세 가지 추가 요소에 대해 설명 합니다.
...
| 15 | 인증/권한 부여  | 시작에서 id를 구현 합니다. 공용 클라우드에서 사용할 수 있는 [RBAC (역할 기반 액세스 제어)](/azure/role-based-access-control/overview) 기능을 고려 합니다.  |
...
Microsoft .NET 플랫폼은 탁월한 선택입니다. 무료 오픈 소스는 마이크로 서비스 개발을 간소화 하는 여러 가지 기본 제공 기능을 제공 합니다. .NET은 플랫폼 간입니다. Windows, macOS 및 대부분의 Linux에서 응용 프로그램을 빌드하고 실행할 수 있습니다.

.NET은 성능이 뛰어나고 Node.js 및 기타 경쟁 플랫폼과 비교 하 여 잘 점수가 매겨집니다. 흥미롭게도 [TechEmpower](https://www.techempower.com/) 는 여러 웹 응용 프로그램 플랫폼 및 프레임 워크에서 광범위 한 [성능 벤치 마크](https://www.techempower.com/benchmarks/#section=data-r17&hw=ph&test=plaintext) 를 수행 했습니다. 상위 10 개에서 점수가 매겨진 .NET은 Node.js 및 기타 경쟁 플랫폼에서 점수가 매겨집니다.

.NET은 Microsoft 및 GitHub의 .NET 커뮤니티에서 유지 관리 됩니다.
...
| 크기 조정 | 필요에 맞게 컨테이너 인스턴스를 자동으로 추가 또는 제거 합니다.|
...
Azure Kubernetes 서비스에 대 한 자세한 내용은 2 장, *Cloud-Native 응용 프로그램 크기 조정* 을 참조 하세요.
...
널리 승인 된 인프라 사례를 [코드](/azure/devops/learn/what-is-infrastructure-as-code)또는 IaC로 입력 합니다.
...
[Azure Resource Manager](/azure/azure-resource-manager/management/overview), terraform 및 [Azure CLI](/cli/azure/)와 같은 도구를 사용 하 여 필요한 클라우드 인프라를 선언적으로 스크립팅할 수 있습니다. 리소스 이름, 위치, 용량 및 암호는 매개 변수화 된 및 동적입니다. 이 스크립트는 프로젝트의 아티팩트로 버전이 지정 되 고 소스 제어에 체크 인 됩니다. 스크립트를 호출 하 여 QA, 스테이징 및 프로덕션과 같은 시스템 환경에서 일관 되 고 반복 가능한 인프라를 프로 비전 합니다.
