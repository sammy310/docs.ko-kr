...
이전 이미지에 표시 된 디자인의 위험을 분명히 생각해 볼 수 있습니다. 3 단계 \#에 실패 하는 경우 어떻게 되나요? 또는 8 \#단계가 실패 하는 경우 어떻게 복구 하나요? 기본 서비스가 사용 \#중 이므로 6 단계가 느려지는 경우 어떻게 하나요? 계속 하 시겠습니까? 모든 것이 제대로 작동 하는 경우에도이 호출이 발생 하는 대기 시간을 고려해 야 합니다 .이는 각 단계의 대기 시간 합계입니다.
