...
첫 번째 <xref:Microsoft.EntityFrameworkCore.DbContext>는 `_catalogContext`이고, 두 번째 `DbContext`는 `_catalogIntegrationEventService` 개체 내에 있습니다. 커밋 작업은 EF 실행 전략을 사용하여 모든 `DbContext` 개체에서 수행됩니다.
