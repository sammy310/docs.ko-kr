...
- 파일 저장소는 표준 SMB 프로토콜을 사용하여 레거시 애플리케이션에 대한 공유 저장소를 제공합니다. Azure 가상 머신 및 클라우드 서비스는 탑재된 공유를 통해 애플리케이션 구성 요소 간에 파일 데이터를 공유할 수 있습니다. 온-프레미스 애플리케이션은 파일 서비스 REST API를 통해 공유된 파일 데이터에 액세스할 수 있습니다.
