...
엔터프라이즈 애플리케이션은 복잡할 수 있으며 단일 서비스 기반 애플리케이션 대신 여러 서비스로 구성된 경우가 많습니다. 이러한 경우 마이크로 서비스, 특정 DDD(도메인 기반 디자인) 패턴 및 컨테이너 오케스트레이션 개념과 같은 추가 아키텍처 접근 방법을 이해해야 합니다. 이 장에서는 컨테이너에서의 마이크로 서비스뿐만 아니라 컨테이너화된 애플리케이션에 대해서도 설명합니다.
