...
WCF에서 동적 속성 형식 (즉, `object`형식의 속성)을 처리 하는 것은 복잡 합니다. Serializer를 지정 하 고 [Knowntype](xref:System.Runtime.Serialization.KnownTypeAttribute) 특성을 제공 해야 합니다.
...
생성 된 각 형식의 `Descriptor` 정적 필드는 Protobuf의 내부 리플렉션 코드에서 `Any` 필드 형식을 확인 하는 데 사용 됩니다. `TryUnpack<T>` 메서드도 있지만 `T`의 초기화 되지 않은 인스턴스를 만든 후에도 `Is` 메서드를 위에 표시 된 대로 사용 하는 것이 좋습니다.

## <a name="oneof"></a>중

중 필드는 언어 기능입니다. `oneof` 키워드는 메시지 클래스를 생성할 때 컴파일러에서 처리 됩니다. `oneof`를 사용 하 여 `ChangeNotification` 메시지를 지정 하는 것은 다음과 같습니다.
...
`oneof` 집합 내의 필드는 전체 메시지 선언 내에 고유한 필드 번호를 포함 해야 합니다.
