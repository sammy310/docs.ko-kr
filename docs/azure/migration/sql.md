...
* [Azure 가상 머신의 SQL Server에 대한 고가용성 및 재해 복구](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-high-availability-dr)
...
* [Azure SQL 데이터베이스 리소스 제한](https://docs.microsoft.com/azure/sql-database/sql-database-resource-limits)

## <a name="choosing-iaas-or-paas"></a>IaaS 또는 PaaS 선택

데이터베이스를 마이그레이션할 위치를 평가할 때 IaaS 또는 PaaS가 더 적합한지 확인합니다.

**다음과 같은 경우 Azure VM에서 SQL 서버를 선택합니다.**
...
| 데이터베이스 크기 관리 | SQL Server 인스턴스당 최대 64TB의 저장소를 지원합니다. | 수평 파티션이 필요하기 전에 4TB의 스토리지를 지원합니다. |
...
* **PaaS를 가고 싶지만 데이터베이스가 호환되는지 확실하지 않습니다. 도움이 되는 도구가 있습니까?**

    예. [Data Migration Assistant](https://www.microsoft.com/download/details.aspx?id=53595)는 Azure SQL Database로 마이그레이션 시 사용되는 도구입니다. [Azure 데이터베이스 마이그레이션 서비스는](https://azure.microsoft.com/campaigns/database-migration/) IaaS 또는 PaaS에 사용할 수 있는 미리 보기 서비스입니다.
