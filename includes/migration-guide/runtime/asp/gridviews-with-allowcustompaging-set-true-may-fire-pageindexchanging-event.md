...
|   |   |
|---|---|
|세부 정보|.NET Framework 4.5의 버그로 인해 <xref:System.Web.UI.WebControls.GridView.PageIndexChanging?displayProperty=name>이 활성화된 <xref:System.Web.UI.WebControls.GridView?displayProperty=name>에 대해 <xref:System.Web.UI.WebControls.GridView.AllowCustomPaging?displayProperty=name>이 발생하지 않는 경우가 있습니다.|
