...
TaskFactory.FromAsync를 호출할 때 <xref:System.IAsyncResult.CompletedSynchronously> 속성이 올바르게 구현되어야 결과 작업을 완료할 수 있습니다. 즉, 이 속성은 구현이 동기적으로 완료된 경우에만 true를 반환해야 합니다. 이전에는 이 속성이 선택되어 있지 않았습니다.

#### <a name="suggestion"></a>제안 해결 방법

<xref:System.IAsyncResult?displayProperty=fullName> 구현이 작업이 동기적으로 완료된 경우에만 <xref:System.IAsyncResult.CompletedSynchronously?displayProperty=fullName> 속성에 대해 true를 반환한다면, 중단이 관찰되지 않습니다. 사용자는 소유하고 있는 <xref:System.IAsyncResult?displayProperty=fullName> 구현(소유한 경우)을 검토하여 작업의 동기적 완료 여부를 올바르게 평가하는지 확인해야 합니다.
