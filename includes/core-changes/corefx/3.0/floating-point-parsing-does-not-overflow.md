...
부동 소수점 구문 분석 메서드는 더 이상 <xref:System.OverflowException>을 throw하지 않거나, 숫자 값이 `false` 또는 <xref:System.Single> 부동 소수점 형식의 범위를 벗어난 문자열을 구문 분석할 때 <xref:System.Double>를 반환합니다.
...
- 코드는 오버플로가 발생할 때 실행되는 <xref:System.OverflowException>에 대한 처리기에 따라 다릅니다. 이 경우 `catch` 문을 제거하고 `If` 또는 <xref:System.Double.IsInfinity%2A?displayProperty=nameWithType>가 <xref:System.Single.IsInfinity%2A?displayProperty=nameWithType>인지 여부를 테스트하는 `true` 문에 필요한 코드를 넣어야 합니다.
